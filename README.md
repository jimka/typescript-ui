...
* **Fix Table construction live-DOM overhead** — when `Window.show()` appends the window to the document and immediately calls `doLayout()`, the `Body` row pool is built inside the now-live DOM. Each `new Row()` / `new Cell()` constructor inserts a CSS rule via `CSS.createComponentRule()` (`sheet.insertRule(...)`), and each `element.appendChild(rowEl)` call happens on an element already attached to the document. Chrome DevTools observes every one of these mutations (Elements panel for DOM appends, Styles panel for CSS rule insertions), which is the primary reason construction is extremely slow with F12 open. Two targeted fixes:
  1. **Reorder `show()`** — call `doLayout()` *before* `document.documentElement.appendChild(...)`. The body element exists in a detached subtree at that point, so pool growth and CSS rule insertions happen off-document where DevTools is not watching. Only a single attach (the final `appendChild`) reaches the live tree.
  2. **Batch CSS rule insertion** — `CSS.createRule()` calls `sheet.insertRule()` once per component, each time at index 0 which shifts all existing rule indices. Consider appending to the end of the sheet instead (pass `sheet.cssRules.length` as the second argument), or collecting all new rules into one `insertRule` pass after the pool is fully built. Alternatively, per-component positioning (x, y, width, height) could be moved entirely to inline `element.style` and the shared CSS rule reserved only for static properties (overflow, cursor, etc.), reducing the total number of rules that need to be created.

* **Throttle resize layout with `requestAnimationFrame`** — `Window.onResize()` calls `this.doLayout()` synchronously on every `mousemove` event. Browsers fire `mousemove` far faster than the display can refresh (often hundreds of events per second), so most of those layout passes produce frames the user never sees. The `doLayout()` path touches every visible row and cell in the table (40 rows × 5 cells = ~200 style assignments per call), so the redundant work adds up quickly. Consider wrapping the resize handler so it schedules a single `requestAnimationFrame` callback if one is not already pending, and cancels the pending frame if a newer event arrives before it fires. This caps layout work at ~60 passes per second regardless of mouse speed, and aligns DOM writes with the browser's own paint cycle. A further option is to show only a lightweight resize outline (a semi-transparent overlay `<div>`) during the drag and defer the full `doLayout()` until `mouseup`, which keeps the drag interaction entirely in the compositor thread.
