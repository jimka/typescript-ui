...
| `required` | When `true`, the header shows a trailing asterisk and every empty cell in this column renders with a tint sourced from `--ts-ui-table-cell-required-empty-bg`. |
| `requiredPredicate` | Optional predicate `(record) => boolean`. When it returns `true` for a record, this column's cell on that record's row tints when empty. Composes with `required` via OR for the tint; does NOT drive the header asterisk (the header has no bound record). |
...
A column is required when its `required` flag is `true`, OR its `requiredPredicate(record)` returns `true`; the tint applies only while the bound value is empty (`null`, `undefined`, or `''` — `0` and `false` are legit values, not empty). The header asterisk reflects the static `required` flag alone, since the header has no bound record to evaluate a per-record predicate against. A read-only cell never shows the required tint — it can't be filled, so nagging it would be misleading. This is a visual affordance only: it does not block commits or integrate with store-level validation.
