...
- **`retain`** — for nodes the framework *creates* and owns (clip frames, glyph sprites, overlays). A strong registry entry, **released** at the element's dispose site (`Component.destructor`, `disposeFrame`, the drained grow-fragment). A missed release would pin a detached element, so each created-element site has a paired `release`.
