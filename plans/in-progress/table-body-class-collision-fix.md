...
**`## Expected Behaviour` row 7 ("No consumer-visible behaviour changes") and `## Documentation Impact` ("No doc-source changes needed") turned out to be wrong** — the actual mechanism produces a real, breaking DOM-class-list change that both sections failed to anticipate. Hoisting `TableBody` onto the hierarchy-aware class tier (`## Architecture Decisions`'s second subsection) means `getStyleClassChain` widens its rendered element's class list to include its ancestor's own name (`VirtualRowView`) once `TableBody` participates — and, because the class actually rendered is no longer named `Body`, the element also stops carrying the old bare `Body` name it used to get from the pre-fix `ctor.name` lookup. Net effect: a table's row-viewport body goes from `.Body` to `.VirtualRowView.TableBody`; a tree table's body goes from `.TreeBody` to `.TreeBody.VirtualRowView.TableBody`. A consumer stylesheet selector targeting `.Body` on either no longer matches.

This is disclosed to consumers via a new "Breaking changes" entry in `packages/lib/docs/reference/changelog/next.md` and an `ARCHITECTURE.md` update (both already committed, listing `TableBody` among the class tier's participating middle classes) — the audit that found this gap confirmed both are accurate and complete. What was missing was recording the deviation here, per `~/.claude/skills/implement/worker.md`'s "Deviating from the plan" section: the plan predicted no consumer impact, the actual change has one, and that gap needed a note in the plan file itself, not just in the two doc surfaces.
