...
Five deviations from the plan text, discovered during implementation and manual verification. None change the public API or the plan's architecture decisions; each is a correction to an assumption the plan text made about the surrounding codebase or test harness.
...
**2. `resolveApiLink`'s relative-link fragment stripping was left unchanged, so one `Verification` grep line doesn't come back clean.** Step 7 and the `## Expected Behaviour` bullets scope the fragment-carrying change to `resolveDocLink`'s route branch only; `resolveApiLink`'s relative `.md` link branch (used only when browsing an already-rendered API page) keeps its own `href.split('#')[0]`, and its own pre-existing tests (stripping a fragment from `BaseObject.md#constructor` etc.) were left untouched, per the plan's explicit scope. The `## Verification` section's `grep -rn "split('#')" packages/docs/src/` line is written more broadly than the Ordered Steps and therefore does not come back empty — it still finds the one call inside `resolveApiLink`. Left as-is rather than widened to match the broader grep, because the corpus guard in step 15 only validates the authored 7-section glob (which never contains `/api/...`), so a relative in-page-API-doc cross-link's fragment is outside what step 15 (or any other step) verifies; extending `resolveApiLink` would be unreviewed, untested scope creep beyond what the plan specified.
