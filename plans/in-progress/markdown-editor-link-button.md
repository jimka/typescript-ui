...
- **Step 14's claim that "every existing `.toEqual(NEUTRAL)`/`{ ...NEUTRAL, ... }` assertion keeps passing unchanged" was wrong for one pre-existing test.** `markdown-editor.test.ts`'s `'reports bold: true with the selection inside a bold run, and true again for a caret collapsed at its very start'` test (originally lines 2021-2039) asserts `{ ...NEUTRAL, bold: true }` at two collapsed-caret positions inside a bold run. `$computeWordExpansion`'s own documented `format !== 0` branch returns a non-null expansion for *any* collapsed offset inside a formatted run, regardless of position — so `hasSelectedText` is correctly `true` at both positions, not the `NEUTRAL` default of `false`. Verified empirically (ran the test red before this fix, green after) rather than assumed. Fixed by adding `hasSelectedText: true` to both of that test's expected objects; no source-code change was needed, since `$readSelectionState`'s formula is exactly what the plan specified.
- **The `hasSelectedText: false` "no word to expand into" test case uses `'a  b'` (two spaces), not the `'hello world'` string the plan's Expected Behaviour section named for that case.** A single space between two words has no offset where the caret is simultaneously non-adjacent to a word character on both sides (per `$computeWordExpansion`'s left/right scan), so `'hello world'` cannot actually produce `hasSelectedText: false` at any collapsed offset — every offset expands into one of the two words instead. `'a  b'` at offset 2 (between the two spaces) is the existing, already-proven precedent for this exact case (`$selectEnclosingWordIfCollapsed`'s own `'is a no-op when neither adjacent character is a word character'` test). Used in both the `MarkdownEditor.getSelectionState()` test and the `MarkdownDocumentPanel` Link-button-disabled test that needed the same setup.
