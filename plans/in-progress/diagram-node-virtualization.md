...
**Three pre-existing cases in `DiagramView.test.ts` needed a one-line edit each**, despite the Verification section's "every existing case must pass unmodified" instruction. All three live in the `'DiagramView — incoming nodes mount only once placed'` describe block and never call `view.setSize(...)`: *"mounts and reveals every node component together once ELK has placed them"*, *"leaves the shown graph mounted while a re-layout is pending, mounting only once it lands"*, and *"leaves the first graph mounted when a re-layout fails"*. Each asserts that every node component lands in `_contentHost.getComponents()` once its layout lands.

This is a direct, provable consequence of this plan's own §B behaviour ("An unsized view mounts nothing") rather than an implementation slip: `viewportGraphRect()` returns `null` whenever `getWidth()`/`getHeight()` are `NaN` (a view that was never sized), which makes `updateNodeResidency()` a no-op, which leaves `_residentIds` empty — so nothing is ever added to `_contentHost` for a never-sized view. Before this plan, `promoteIncomingNodes` mounted every node unconditionally regardless of size; §B deliberately changes that. The three tests above test exactly the old, now-superseded guarantee on an unsized view, so they could not pass unedited under the behaviour this plan requires — the plan's "must pass unmodified" instruction and its own §B are mutually unsatisfiable for this one describe block.

The fix applied is the minimal one: each test now calls `view.setSize({ width: 1280, height: 800 })` right after constructing the view, before the assertions that check content-host membership — the same pattern nearly every other test in the file already uses. With `fixedResult()`'s nodes both well inside a 1280×800 residency rect, every original assertion still holds; only the missing precondition was added. No test's *expected outcome* changed, only what state it exercises that outcome under.

**§D's manual verification was carried out** against sqladmin's real Tables-mode 325-table diagram, via `npm run build:lib` consumed through the symlink override (see `## Verification`). Every §D bullet was addressed, and the mounted-node reduction was confirmed directly against the live DOM — 12 of 325 node components attached at a working zoom inside the `hub` chain (150 of 325 one zoom step out), against the plan's own "roughly an order of magnitude" estimate. The node-emphasis bullet could not be exercised inside the Tables-mode panel itself — `DatabaseDiagramPanel` never calls `setNodeEmphasis` in this app — so it was checked instead in `RelationDiagramPanel` (the FK-explorer diagram, the app's only `setNodeEmphasis` call site), which is the same `DiagramView`/library code path. The `destructor` fix (Ordered Implementation Step 10) was also exercised live — closing the Tables-mode diagram tab with 150 nodes mounted dropped the page's stylesheet rule count from 242 to 220, at or below the pre-diagram baseline rather than above it — though that specific metric barely moves for Tables mode's default (unconfigured) node renderer, which carries no per-instance style rule to begin with; see the LIBRARY_NOTES.md entry for why the number is still consistent with the fix without being a strong stress test of it. Full findings, including the 6×-throttled `ForcedReflow`/`DOMSize` trace results and a note on why INP specifically could not be captured from a scripted interaction, are recorded in `/home/jika/typescript/sqladmin/LIBRARY_NOTES.md` (uncommitted in that repo — sqladmin commits are made by the user by hand, per that repo's own convention).
