...
| Anywhere in a table cell | `table-cell` | `true` (every table cell has an enclosing table) | Yes — targets the whole enclosing table, not just the clicked cell |
...
- **This footnote's "reconstructs as an empty paragraph on re-import" claim does not hold.** Verified empirically during implementation: `getValue()` on a document with a still-empty inserted paragraph (whether it is the last child or sits between two other blocks) emits *zero* Markdown text for that paragraph — not even a blank-line placeholder — because Markdown has no literal representation of "an empty paragraph" (a blank line is separator syntax between blocks, not content). Re-importing therefore loses the paragraph entirely; this is a pre-existing property of the whole editor's markdown export, equally true of `$handleSeparatorShortcut`'s own identical `insertAfter($createParagraphNode())` for `TableNode`/`CodeNode`, which had never previously been exercised through a `getValue()`/`setValue()` round-trip. The `"empty-line"` classification mechanism this footnote points to depends only on an empty paragraph existing in the *live* document tree, never on it surviving a Markdown round-trip, so the two are not actually the same guarantee. Expected Behaviour item 9's round-trip test was adjusted accordingly: it types placeholder text into the new paragraph before taking `getValue()`, since that (not a still-empty scratch line) is what a round-trip actually needs to preserve.
- **Step 8's footnote undercounted the pre-existing `toEqual` sites.** Alongside the two cited (`markdown-editor.test.ts:1005-1007` and `:1024-1026`), a third pre-existing test — "classifying a collapsed caret inside a word does not mutate the selection…" in the `'MarkdownEditor context-menu paste target'` describe block — also asserts `$classifyContextMenuTarget`'s exact return value via `toEqual` on a plain-paragraph caret, and needed the same `hasEnclosingBlock: false` addition. The line `1458` the footnote listed among the *unaffected* hand-built fixtures is actually this real classifier call, not a hand-built literal; the other seven listed sites were correctly hand-built fixtures and needed no change.
- **`childTypes`/`caretIsInAParagraph` were hoisted from the Alt+Enter describe block to module scope** so the new `'MarkdownEditor insertParagraphBeforeBlock / insertParagraphAfterBlock'` describe block could reuse the identical helpers the plan's Expected Behaviour item 3 asked for ("same `childTypes`/`caretIsInAParagraph` helpers as the Alt+Enter describe block"), rather than duplicating them.
- **Reversed post-implementation: table cells now get Insert line before/after block too, targeting the whole enclosing table.** The original Non-Goals bullet excluding table cells was based on a misreading of the user's intent, corrected in a follow-up debugging session. `$findEnclosingInsertableBlock` now falls back to the same table-cell resolution `$findEnclosingSeparatorTarget` already uses (`$getTableCellNodeFromLexicalNode` + `$getTableNodeFromLexicalNodeOrThrow`) when the clicked node sits in a table cell and none of code/quote/list matches first — giving a table cell the same "whole thing, not the clicked piece" treatment already given to a list. `ContextMenuTarget`'s `"table-cell"` variant, `$classifyContextMenuTarget`, and `buildTableCellContextMenuItems` were widened correspondingly (mirroring the existing `"text"` context's `hasEnclosingBlock` plumbing), and `insertParagraphBeforeBlock`/`insertParagraphAfterBlock`'s JSDoc, the Expected Behaviour table above, and `docs/components/MarkdownEditor.md` were updated to match. This supersedes the Problem Statement's and Expected Behaviour's "Behaviours to pin" item 7 / manual-verify bullets' original claim that a table cell never shows these items — a table cell is no longer a no-op case, only a plain paragraph and a heading still are.
