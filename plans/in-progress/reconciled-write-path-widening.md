...
**`Markdown` needed an unplanned fix: an `applyStyle` override so its own `maxWidth` write survives the widened render-phase reconciliation.** `Markdown.setMaxMeasure()` (called from the constructor for the unset default too) writes a real value straight to the `maxWidth` CSS key via `setElementCSSRule`, entirely independent of the size-*constraint* system. `Component.getMaxSizeConstraint()` is never actually `null` — `ComponentDefaults.ts`'s `BASE_DEFAULTS.maxSize` gives every class an unconditional `{UNBOUNDED, UNBOUNDED}` floor — so `applySizeConstraintStyles()`'s `maxWidth`/`maxHeight` writes always fire, and for a class with no `maxSize` default (Markdown included) they resolve to `"none"`, matching the framework tier. Before this plan, that render-phase write was skip-based and never touched the dirty map on a match, so Markdown's constructor-queued real value survived untouched. After migrating it to `reconcileRuleDeclaration` (this plan's own change, needed for the setter-reconciliation case), a match now queues a *removal* onto the same key in the same render pass, silently clobbering Markdown's value on every `applyStyle()` call (first render, `setId`, `sync`) — a real regression: every default-configured `Markdown` would have rendered with no prose-width cap at all. Fixed with a `Markdown.applyStyle()` override that calls `super.applyStyle()` then re-asserts `setElementCSSRule("maxWidth", ...)` last, so Markdown's own value always wins. Caught by the two pre-existing `Markdown.test.ts` `setMaxMeasure`/`getMaxMeasure` tests, which failed before the fix. No other property this plan touches has a similar independent CSS-key collision (confirmed by the full test suite passing).

**Several pre-existing tests pinned the old unconditional/skip behaviour and needed updating to the new removal semantics** — not full rewrites, just replacing a `toBeUndefined()`/literal-value assertion with `toBeNull()` (or, for `Component.test.ts` cases 2-3 and `ClassStyleRules.test.ts` cases 8-9, a value that reflects a removal instead of a restated default): `tests/core/ClassStyleRules.test.ts` cases 8-9 (`setMinSize`/`setOverflowY` restoring a framework-matching value), `tests/component/Component.test.ts` cases 2-3 (`setMinSize`/`setMaxSize`'s UNBOUNDED half), `tests/component/input/Checkbox.test.ts` and `RadioButton.test.ts`'s delegate static-style-hoisting rows (the size/color keys that match `CheckboxBox`/`RadioButtonRing`/`CheckboxCheckGlyph`/`RadioButtonDot`'s own class defaults), and `tests/component/table/CellTextSelection.test.ts`'s `userSelect` assertion. Each failure was for the exact reason predicted by this plan's own Architecture Decisions (a match now surfaces as an explicit removal instead of silence, once something else in the same render batch already forces `#id` to materialise) — none indicated a defect in the migration itself.

**Two more pre-existing tests (`tests/core/Panel.styleRuleDisposal.test.ts` B1-2 and `tests/component/container/VirtualScroller.styleRuleDisposal.test.ts` B1-3) needed a different fix: switching their per-instance-rule-exists sanity check from the `Scrollbar` root to its `_thumb` child.** `Scrollbar`'s constructor calls `setUserSelect("none")`, which — once reconciled — now also dedupes against the framework tier for a fresh, otherwise non-deviating `Scrollbar` (nothing else about it deviates from the class/framework baseline once `backgroundColor` is already deduped by the earlier chrome-hoisting plan). Its own `#id` rule can therefore legitimately never materialise at all, which is the correct, intended outcome of this plan — not a defect — but it left B1-2/B1-3's "the fresh/original instance has its own rule" assertions checking something no longer guaranteed. Re-pointed both at the scrollbar's `_thumb` (a bare `Component` whose real, always-deviating `backgroundColor`/`cursor` guarantee it a rule, and which is only reachable through the scrollbar's own destructor recursion — the actual mechanism Bug 1 regression-tests).

**The plan's own Architecture Decision "The motivating example fully collapses for `maxSize`, only partly for `minSize`" undersells the actual result — confirmed live (see `## Verification`), both axes fully collapse for `TabButton`.** The footnote `[^resolveDeclarations-asymmetry]` reasoned from `resolveDeclarations()`'s `minWidth: minSize ? ... : "auto"` ternary, assuming an undefaulted class's `defaults.minSize` is `null`/`undefined`. In practice `ComponentDefaults.ts`'s `BASE_DEFAULTS.minSize` (`{width:0, height:0}`) gives *every* class — `TabButton` included — a non-null `minSize` unless a subclass explicitly overrides it to something else (as `TabButton` does not), so `resolveDeclarations().minWidth` resolves to `"0px"`, which already equals `FRAMEWORK_DECLARATIONS.minWidth`. The live Style Audit Panel (`#/style-audit`) confirms this: the `TabButton`/`{ min-width: 0px; min-height: 0px; max-width: none; max-height: none; }` duplicate row (33 instances, 2.22 KB) is gone entirely after this plan, not merely shrunk to a min-only body. This is a stronger result than promised, not a shortfall, and needs no code change — noted so a future reader doesn't mistake the shipped footnote for the actual behaviour.

**The plan's `setReconciledCSSRules(` grep-count checks (step 3 and `## Verification`) predict 11; the actual count after every step is 16.** The predicted baseline of "9 before this step" omitted step 2's five single-key setter additions (each also matches `setReconciledCSSRules(`), so the running count the checks were written against was never updated past step 2. All other grep invariants — the zero-match old-pattern checks, `writeRuleDeclaration(` = 12, `reconcileRuleDeclaration(` = 14, and the `hoisted chrome` zero-match check — hold exactly as predicted, and manual line-by-line review confirms every intended call site was migrated and no unintended one was. A stale hand-written count, not a defect (see the project's own recorded lesson on this class of plan-authoring mistake).
