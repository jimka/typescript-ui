...
### `register` links a new layer under the layer it was opened from, not the last-registered one

[`register`](packages/lib/src/typescript/lib/core/LayerManager.ts#L211) currently treats the last entry in `_stack` as "the layer this one was opened from", and a nested layer inherits that layer's band. With one window band that always produced a correct result, because a later registration always drew a higher counter. A second root band breaks that match: a pinned window can sit visually in front of a window that registered after it, and a menu opened inside the pinned window would inherit the *other* window's band and paint behind its own opener. `register` therefore resolves the new layer's actual opener — via the anchor element a dropdown, popover, or rebuild-mode menu already tracks for its own outside-click exclusion (`getAnchorElement`), matched against whichever registered layer's DOM subtree physically contains it — falling back to the last-registered layer, the original rule, for a layer type with no anchor to resolve.[^frontmost]

### Lock freezes both window drags and is not routed through `resizable`

`setLocked(true)` vetoes `startMoveFrom` and `onResize`, hides the eight resize strips, and disarms the snap-resize detector; `setLocked(false)` restores whatever `resizable` allows. It deliberately does **not** call `setResizable`, because `resizable` is the master switch for the minimize and maximize affordances ([`isMinimizable`](packages/lib/src/typescript/lib/overlay/AbstractWindow.ts#L1381)) — routing lock through it would strip the header buttons and the menu's own Minimize and Maximize rows every time the user locked a window.[^lock-not-resizable] Maximize (and un-maximizing) is itself a resize, though, so a separate `canMaximize()` (`isMaximizable() && !isLocked()`) independently disables — never hides — every user-facing maximize control while locked: the header/tool button, the header/bar double-click, and the menu's own Maximize/Restore row. The gate is on the action, not on which label is currently showing, so an already-maximized window that is then locked has its Restore control disabled too. Minimize and Close stay untouched by lock in every case.

Hiding the strips (rather than leaving them visible and vetoing the drag) is the precedent [`setResizable`](packages/lib/src/typescript/lib/overlay/AbstractWindow.ts#L1426) sets: a hidden strip takes no cursor and no hit test, so a frozen edge shows the ordinary pointer instead of a resize cursor that silently does nothing. Disabling (rather than hiding) the maximize control while locked follows the same precedent [`setCloseable`](packages/lib/src/typescript/lib/component/container/WindowHeader.ts#L361) already sets for Close: `locked` is a runtime-togglable state, not a construction-time capability, so the still-visible control communicates "usable right now" rather than disappearing and reappearing as the user locks and unlocks.

| `resizable` | `locked` | Resize strips | Border resize | Drag-to-move | Maximize available |
| --- | --- | --- | --- | --- | --- |
| `true` | `false` | shown | yes | yes | yes |
| `true` | `true` | hidden | no | no | no |
| `false` | `false` | hidden | no | yes | no |
| `false` | `true` | hidden | no | no | no |
...
/** Finds the registered layer whose own element contains `target`. */
...
/** Resolves the layer a new (non-root) registration was opened from. */
...
`register` then reads `const parent = layer.isLayerRoot?.() ? null : resolveParent(layer);` in place of its current `_stack[_stack.length - 1]` lookup. A layer that tracks its own opener via `getAnchorElement` (a dropdown, popover, or rebuild-mode menu already does, for their own outside-click exclusion) links under whichever registered layer's DOM subtree physically contains that anchor — the layer it was actually opened from — falling back to the last-registered layer, the original rule, for an anchor-less layer or one with no anchor yet inside any registered layer. **Post-implementation correction:** an earlier version of this section proposed a `frontmostNode()` helper (parent = whichever registered layer currently paints in front, by z-index) instead. That was implemented, audited, and found wrong: "paints in front" identifies no relationship between a new layer and the layer it was actually opened from, and a `Drawer` or a pinned window could paint in front of the real opener and wrongly absorb a layer opened elsewhere — see `[^frontmost]`. The anchor-based `resolveParent`/`nodeOwning` pair above is what shipped. `getTopLayer` and `topmostInputLayer` keep their stack-order meaning either way — they answer a different question (which layer owns Escape and keyboard routing).
...
2. **`packages/lib/src/typescript/lib/core/LayerManager.ts`** — add the band and the move primitive.
   ...
   - Add the private `nodeOwning()` / `resolveParent()` helpers and use `resolveParent` for `parent` inside `register` — see `[^frontmost]` for why an anchor-based lookup, not a frontmost-by-z one, is the correct rule.
   - Update `bandFor`'s and `DismissableLayer.getBand`'s JSDoc where they say a nested layer inherits the band of "the current topmost layer" — it now inherits the band of whichever registered layer owns its anchor, falling back to the last-registered layer when it has none.
...
- **Moving or resizing a locked window programmatically.** `setX` / `setWidth` / `toggleMaximize` / `setWindowState` keep working while locked — the lock gates user-facing gestures and controls (the two drag gestures, plus every maximize affordance: the header/tool button, the header/bar double-click, and the menu's Maximize/Restore row), not the API itself. Persisting the lock through layout serialization is likewise out.
...
**`tests/component/container/WindowHeader.test.ts` needed a follow-up fix outside the plan's file table.** Step 15's `glyph.setCursor("pointer")` is the title glyph's first-ever real per-instance style write; queuing it into `_pendingStyleKeys` is what finally flushes the still-pending, always-null `minWidth`/`minHeight`/`maxWidth`/`maxHeight` baseline keys `NO_OWN_SIZE_DEFAULT` queued at construction (see `Component.flushStyleBag`'s class-default-only comprehensive write) — before this change nothing else ever gave that instance a real declaration to flush alongside. The existing style-hoisting test asserted `declarations.minWidth` etc. were `undefined` (no write at all); with the cursor write they now flush as explicit `null` removals in the same batch — behaviourally identical (no real min/max size value is ever declared), but a different shape in `sink.writes`. The test's four assertions were changed from `toBeUndefined()` to `(?? null).toBeNull()`, which accepts either shape while still failing on a real (non-null) override. No plan-file table update: this is a pre-existing test's assertion catching up to an intended, correct side effect of step 15, not a new file this feature owns.

**`packages/lib/docs/concepts/layering.md` needed updating outside the plan's `## Documentation Impact` list.** That section named `AbstractWindow.md`, `Window.md`, `TabWindow.md`, and the changelog as the docs this feature invalidates, but missed `layering.md` — the conceptual doc that separately describes `LayerManager`'s "opened-from edge", `register`'s parent-selection rule, and the z-index band table. Step 2's new `PinnedWindow` band and `register` change directly invalidate that page's band table and its description of how a nested layer's parent and band are chosen (and the later audit-round correction of `register`'s parent-resolution rule, from frontmost-by-z to anchor-based, invalidated it again in the same way). No plan-file table update: this is an existing doc page whose description of already-documented `LayerManager` behaviour changed underneath it, not a new file this feature owns.

**The "Always on top" row's text was visibly clipped on its right side — root cause was a pre-existing `Menu.ts` bug, not `CheckboxMenuRow` or `Checkbox`.** `Checkbox.getPreferredSize()` already folds its label's full measured width in via the HBox layout manager it shares its box and label through — that hypothesis, floated before this fix, does not hold up: `CheckboxMenuRow.getContentWidth()` reads a correct, complete number. The real defect is in `Menu.layOutColumns()`: it sizes the panel from each row's content-box requirement (`MenuItem.RIGHT_PAD`'s own doc calls it padding "at the panel's inner edge"), but `showAnchored` and `buildPersistentItems` both applied that content-width value directly to `setWidth()`, which sets the panel's *outer*, border-box width — silently starving every row's actual content box by the panel's own left+right border (2px total, from the `"1px solid ..."` both `applyRebuildChrome` and `applyPersistentChrome` set). A `MenuItem`'s title absorbs this invisibly behind its `overflow:hidden`/`text-overflow:ellipsis`; a `CheckboxMenuRow`'s plain, unellipsized label has neither, so the 2px shortfall showed as a visible hard clip. Confirmed offline (no live browser needed) by constructing a real `Menu` with a `CheckboxMenuRow` long enough to drive the panel's natural width past `Menu`'s `MIN_MENU_WIDTH` floor, and comparing the checkbox's committed width against its own preferred width. Fixed by adding the panel's own border back onto `layOutColumns()`'s returned value — which already matches that method's own "returns the panel width" doc comment, so no contract change — leaving `setMenuWidth()`'s explicit-override path untouched (it still pins an exact outer width, as documented). Three pre-existing `Menu.test.ts` assertions that hardcoded the old (content-only, pre-border) width had to grow by the panel's border width to match; a new regression test pins the fixed behaviour. No plan-file table update: `Menu.ts` is a file this feature doesn't own, and the bug predates this branch, reachable by every existing `CheckboxMenuRow` consumer (`Table.buildColumnMenuItems`, `Split.openGutterMenu`), not just the new window menu.

**Locking a window needed to also disable Maximize, not just gate resize strips and drag-to-move — a correction to this plan's own Architecture Decision and truth table, found via manual testing after the branch was first marked implemented.** Confirmed with the user directly: minimize must stay available while locked (it never changes stored position/size — it collapses to a rail/taskbar form and restores to the exact same geometry), but maximize must not, since maximizing (and un-maximizing) is itself a resize. The plan's original reasoning for not routing `locked` through `setResizable` was correct as far as it went — it keeps minimize (and the header buttons/menu rows) from disappearing every time a window is locked — but it stopped there and left every maximize control fully usable while locked. Fixed with a new `canMaximize()` (`isMaximizable() && !isLocked()`), consulted everywhere maximize-availability is decided: the header's maximize button (`WindowHeader.setMaximizeButtonEnabled`, disabling rather than hiding, mirroring `setCloseable`), `TabWindow`'s maximize tool, the header/bar double-click-to-maximize gesture (`Window.onHeaderDoubleClick` / `TabWindow.onBarDoubleClick`), and the window menu's Maximize/Restore row's `enabled` field. The gate is on the action, not on which label is currently showing, so an already-maximized-then-locked window's Restore control is disabled too. `toggleMaximize()` itself is deliberately left ungated, preserving the plan's Non-Goal that `locked` gates user drag/click gestures, not the programmatic API (e.g. layout restore). The plan's "Lock freezes both window drags..." Architecture Decision section, its truth table, and the `[^lock-not-resizable]` footnote were updated in place to record this. No plan-file table update beyond that: the correction lives inside an existing section, not a new file.

**Minimize/Maximize/Close menu rows gained glyph icons matching their header/tool button's icon in every state — an addition discussed directly with the user, not in the original plan.** `MenuItemConfig.glyph` (a registry glyph name rendered in `MenuItem`'s dedicated leading-icon zone) already existed and needed no changes. `buildWindowMenuItems()` now sets it per row: `window-minimize`/`window-restore` for Minimize, `window-maximize`/`window-restore` for Maximize (each mirroring the icon swap `WindowHeader.setMinimizeButtonGlyph`/`Window.reflectMaximizeState` already perform on the header button — so the menu row and the header button always agree on `Window`, though not on `TabWindow`, whose trailing tool buttons never swap since its own `reflectMaximizeState` is a documented no-op), and `xmark` for Close. The "Always on top" and "Lock position" rows deliberately carry none — an explicit scope decision made with the user to keep them as `CheckboxMenuRow` factories (a real `Checkbox` widget) rather than switching to icon-capable plain `MenuItem`s (`checked` + `glyph`), which would have discarded the already-implemented, already-audited Checkbox-based mechanism this plan built. No plan-file table update: `MenuItemConfig` is pre-existing public API, not a new file or symbol this feature owns.

**Locking a window from inside its own open menu left that menu's Maximize row stale — a real bug found during audit, needing a new `Menu`/`MenuItem` capability the original plan never anticipated.** The menu is rebuild-mode: `buildWindowMenuItems()` computes the Maximize row's `enabled: this.canMaximize()` once, at build time. The "Lock position" row's own action calls `setLocked(true)` and deliberately leaves the panel open (so both switches can be flipped in one visit — see the row's own doc), but nothing then pushed a live update to the sibling Maximize row already sitting in that still-open panel: open the menu, click "Lock position", click "Maximize" in the same panel, and the window maximized despite being locked. No existing `Menu` consumer needed to refresh a rebuild-mode panel's row while keeping it open (`Table.buildColumnMenuItems` and `Split.openGutterMenu` are right-click menus, reopened fresh via `show()` each time), so there was no precedent to reuse. Fixed with a new public `Menu.setItemEnabled(index, enabled)` (rebuild-mode only; a no-op on a separator, a custom `row()` factory, or an out-of-range index) and a new public `MenuItem.setEnabled(value)` it delegates to, mirroring the dim/aria/cursor handling `MenuItemConfig.enabled` already applies at construction. Wiring the fix up surfaced a second, independent latent bug in `MenuItem` itself: its `_onMouseOver`/`_onClick` closures gated on a `const enabled` captured once at construction, never re-read afterward, so `setEnabled()` alone would have updated `isEnabled()`'s answer and the row's dimmed appearance while leaving the row still clickable — both closures now read `this.isEnabled()` live instead. `AbstractWindow.applyMaximizeAvailability()` — the single existing chokepoint every lock/resizable/maximizable change already funnels through — gained one more line, `refreshWindowMenuMaximizeAvailability()`, which computes the Maximize row's index the same way `buildWindowMenuItems` does (Minimize, when present, precedes it) and calls `setItemEnabled` on the lazily-created `_windowMenu` if one exists. No plan-file table update: `Menu.ts`/`MenuItem.ts` are files this feature doesn't own the way it owns `AbstractWindow.ts`, and the new methods are additive public API, not a new file.

**Step 10's literal instruction — delete `button.setPointerEvents("none")` from `createWindowLeadGlyphButton` — would have left the button non-interactive; the shipped code instead keeps the call and flips its value to `"auto"`.** The step's own goal (make the leading glyph a real, clickable button) is correct, but its literal text conflated "the glyph's `pointer-events: none` was wrong" with "the call itself should go away." `Button` unconditionally sets its own `_content` row to `pointer-events: none` so a face click falls through to the native `<button>` element underneath, and this button additionally sits inside `TabBar._leadGroup`'s own `pointer-events: none` wrapper — the pointer-transparency this plan's own Architecture Decisions deliberately leave untouched (see "The window icon becomes interactive; the surrounding chrome stays pass-through"). Deleting the call outright would have left the button inheriting `none` from both ancestors with nothing of its own to override it, permanently un-hit-testable — the opposite of the step's own purpose. `SplitButton`'s chevron button sits inside an equivalent pointer-transparent wrapper and already solves this the same way (`this._chevron.setPointerEvents("auto")`); `createWindowLeadGlyphButton` mirrors that precedent instead of deleting the call, and its own code comment already explains why. No plan-file table update: `windowControls.ts` is already in the plan's file table; this corrects the step's own instruction text, not a new file.
...
[^frontmost]: A window-local "re-raise on every other window's `bringToFront`" workaround was rejected in favour of a real band — it would have to hook every raise of every peer, and two pinned windows would then fight over the top slot. Given the band, the `register` change is not optional polish: with one root band, "the layer registered last" and "the layer painting in front" were the same layer, and the band-inheritance rule silently depended on that. With two root bands they diverge, and a nested layer would inherit a band that paints behind its own opener. **Post-implementation correction:** the fix first shipped here — linking a new layer under whichever registered layer currently paints frontmost (the highest z-index) — was itself wrong, not merely incomplete. "Paints in front" identifies no relationship between a new layer and the layer it was actually opened from: an unrelated, differently-banded peer (a pinned window, or a `Drawer`, which registers as its own root with no band of its own and so defaults to the dropdown band) can paint in front of the real opener and wrongly absorb a layer opened from somewhere else entirely — a regression from master's single-band behaviour, not just a gap in new-feature coverage. The corrected rule resolves the opener directly: a dropdown, popover, or rebuild-mode menu already tracks its own anchor/opener element via `getAnchorElement` (for its own outside-click exclusion), and `register` links the new layer under whichever registered layer's DOM subtree physically contains that anchor, falling back to the last-registered layer — the original single-band rule — only when there is no anchor to resolve. `getTopLayer` and `topmostInputLayer` keep their stack-order meaning because they answer a different question (which layer owns Escape and keyboard routing), and changing that would alter dismissal behaviour well outside this feature.

[^lock-not-resizable]: The name is "Lock position" but the behaviour is "freeze this window" — both the title-bar drag-to-move and the resize-border drag. This is a deliberate product decision, not an oversimplification of a position-only lock. Naming a menu row for the user's mental model while the underlying API says something more precise is the existing house style: `Split`'s gutter menu shows "Lock gutter" over `SplitGutter.setMovable(false)`. The option is called `locked` rather than `movable` because it governs two gestures, not one. It is a separate flag from `resizable` because `resizable` doubles as the master switch for the minimize and maximize affordances, so folding lock into it would strip the header buttons — and the menu's own Minimize and Maximize rows — every time a window was locked. **Post-implementation correction:** that reasoning was necessary but stopped one step short — it correctly protects minimize, but it was read (both in this plan's prose and in the shipped code) as meaning `locked` leaves maximize alone too, when a user testing the feature found they could still maximize a locked window from the header button, the header/bar double-click, and the menu's own Maximize row. Maximizing (and un-maximizing) is itself a resize, so it needed its own gate — just not `resizable`'s, which would have re-broken minimize. The fix is `canMaximize()` (`isMaximizable() && !isLocked()`), consulted everywhere maximize-availability is decided, disabling rather than hiding each control for the same reason `setCloseable` already disables rather than hides Close.
