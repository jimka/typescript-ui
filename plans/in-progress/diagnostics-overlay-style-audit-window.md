...
- **Every titled `Button` this plan adds needed an explicit tooltip-detach on dispose.** `Button` unconditionally attaches a hover tooltip showing its own title (`_rebuildTooltip()`) but never detaches it itself — nothing in the plan's design anticipated this, since neither `DiagnosticsOverlay` nor the new `StyleAuditView` previously had a titled `Button` in its tree. Two sites needed the fix: `DiagnosticsOverlay.teardown()` now calls `Tooltip.detach(this._styleAuditButton)` alongside stopping the sampler (surfaced by the pre-existing `DiagnosticsOverlay.rowTooltips.test.ts`, whose live-open "24 attachments" count became 25 — its own disposal case, case 9, is the one that would have caught the leak on `close()`), and `StyleAuditView`'s new `destructor()` calls `Tooltip.detach(this._refreshButton)` (caught by an audit round, since neither `StyleAuditView.test.ts` nor `StyleAuditOverlay.test.ts` originally asserted on `Tooltip.attachments`, only on `_ruleCacheKeys()`; a case 12b was added to close that gap, verified to fail without the fix). Both fixes are scoped to the one new call site each — `Button.ts`'s general disposal contract is unchanged, and a pre-existing, unrelated leak of the same shape in `Table`'s own "Column options" button was left alone (out of scope; not introduced by this plan). The precedent named in the original note (`LabeledGrid.destructor` detaching the tooltips *it* attached to other components) is not quite parallel — this shape is a component detaching a self-attached tooltip on a `Button` it merely holds a field to — but the same "explicit detach in the owner's destructor" mechanism applies.
- **Case 1 (`styleRuleEntries()` on an "empty cache") is driven differently than its literal wording**, for a reason already established elsewhere in the codebase: `_ruleCache` is module state that survives every test in `StyleTarget.test.ts` (already true of the adjacent `styleRuleCounts` tests in the same file), so no test in that file can observe a literally empty cache. The case is instead covered as a before/after invariant (`styleRuleEntries().length` always equals `_ruleCacheKeys().length`), which is what "empty returns `[]`" reduces to at the zero case, matching that file's own established pattern for the same constraint.
- **Case 19's test lives in its own file, `DiagnosticsOverlay.styleAuditButton.test.ts`, not in `DiagnosticsOverlay.test.ts`** as the plan's step 11 and *Files to Create/Modify/Delete* table say. `DiagnosticsOverlay.test.ts` carries only a pointer comment to it. Reason: proving the button's `on("action", …)` wiring actually fires requires a real `.click()`, not calling the bound handler field directly — but `Event`'s window-level base listener is armed once per event type and never re-armed across a later `installTestDOM()` swap, so a real click only works reliably as the first (and only) native-click test in its file, the same constraint `DiagnosticsOverlay.rowTooltips.test.ts` was already split out for. This was found and fixed during the branch's first audit round but the deviation itself was never recorded; a second audit round caught the omission.
- **The embedded `StyleAuditView` was missing a layout weight in both hosts it's added to**, found by the second audit round: `StyleAuditOverlay.ts`'s body `Panel` and `StyleAuditPanel.ts`'s own `VBox` both added `new StyleAuditView()` with no `{ weight: 1 }`, so the view — and the `Table` inside it, despite the `Table`'s own `weight: 1` — never grew past its preferred height (~194px of a 520px-tall window; ~100px of table). The plan's *Internal Structure* trees for both hosts specified the same omission, so the code faithfully matched a plan that itself under-specified the layout. Fixed by adding `{ weight: 1 }` at both call sites (`StyleAuditOverlay.ts` and `StyleAuditPanel.ts`), pinned by a new geometry regression test (`StyleAuditOverlay.test.ts` case 22-partial: the `Table` descendant's committed height after `flushLayout()` exceeds 300px — verified to fail at ~100px before the fix), and confirmed live: `npm run dev`, both windows opened back to back (manual cases 20-22) — the table now fills the available height in both the demo tab and the overlay window, and both report identical summary figures (363 rules, 85.06 KB, 294 per-instance, 59 unique bodies) before either is refreshed.
- **`packages/lib/scripts/llms/generate.mjs`'s `TOKEN_BUDGET` was nudged from 6350 to 6440**, a file not in the plan's *Files to Create/Modify/Delete* table. This is the same mechanical step every prior catalog-adding branch has taken (the constant's own doc comment already records the history: `…Raised again from 6260: the DiagnosticsOverlay catalog entry…`, `…Raised again from 6350: the StyleAuditOverlay catalog entry…`), needed because `npm run docs:llms`'s own verification step (this plan's *Verification* list) fails once the new `StyleAuditOverlay` catalog row pushes the generated site variant over the previous budget. A third audit round flagged the omission from this notes section; recorded here rather than added to the Files table after the fact, since the table describes what the plan called for and this wasn't planned, it was a consequence of running the plan's own verification step.
