...
- **`MenuButton<MenuButtonOptions>` explicit type argument needed on every construction call.** The plan's Internal Structure code constructs each dropdown trigger as `new MenuButton({ glyph, text, showText, menuItems })`, matching `Button`'s own options-only call shape. `Button`'s options-only overload pins its parameter to the concrete `ButtonOptions` type specifically to prevent generic narrowing (documented in its own constructor JSDoc), but `MenuButton`'s matching overload (`constructor(options: TOptions)`) does not carry that same protection — it types the parameter as the generic `TOptions` directly. An inline object literal therefore narrows `TOptions` to that literal's exact shape, producing e.g. `MenuButton<{ glyph: string; text: string; showText: false; menuItems: MenuItemConfig[] }>`, which then fails `tsc` when passed to `ToolBar.addComponents(...)` (which expects `Component<ComponentOptions>`) — "Types of property '_options' are incompatible." Fixed by adding an explicit `<MenuButtonOptions>` type argument to each of the five `new MenuButton<MenuButtonOptions>({...})` calls in `buildToolbar()`, which pins `TOptions` to the class default and avoids the narrowing. No change was made to `MenuButton.ts` itself — that file has no precedent of this call shape being used elsewhere in the codebase (a pre-existing gap in its own overload set, out of this plan's scope to fix).

- **`llms.txt` token budget needed raising, plus a coupled test-fixture fix.** Adding the `MarkdownDocumentPanel` catalog entry (Step 6) pushed the site variant of the generated manifest to ~7023 tokens, over the existing 6940-token `TOKEN_BUDGET` ceiling in `packages/lib/scripts/llms/generate.mjs` — the same self-catching failure mode the file's own comment history documents for every previous catalog addition (each one raised the ceiling by the minimum needed, rather than trimming existing wording). Raised `TOKEN_BUDGET` from 6940 to 7030 following that established pattern, with a matching comment entry. This pushed the budget past 7000 for the first time, which broke an assumption in `packages/lib/tests/unit/llms-generate.test.ts`'s `'passes a within-budget document and throws past the ceiling'` test: its over-budget fixture was sized at `'a'.repeat(28000)` (~7000 estimated tokens) on the stated assumption that 7000 tokens is "over any ceiling the constant has carried." Bumped the fixture to `'a'.repeat(40000)` (~10000 estimated tokens) so the test's over-budget case stays clearly over budget regardless of near-future ceiling raises.
