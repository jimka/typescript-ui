...
`border` and `outline` are stored in private fields (`_border`, `_outline`), not in `_options`, and `applyStyle` reads the field directly. `outline` has **no `applyStyle` handling at all** today — only `setOutline` materializes it — so a render block must be added, the getter folds in `_defaultOptions.outline`, and `applyOptions` dispatches raw `options.outline`.

> **Implementation drift — border stays dispatched (decided during implementation).** `border` is NOT converted to the getter-fallback pattern. Unlike outline, border is read directly as `this._border` by the layout border-width path [`getBorderSize()`](../src/typescript/lib/core/Component.ts#L2406), and `setBorder` registers a theme-change listener that invalidates the cached border widths. If the default stopped flowing through `setBorder`, default-bordered components (inputs, buttons) would report a **zero border width** to layout and lose theme re-measure. Because `border`'s state lives in `_border` (private) — never in `_options` — dispatching its default does **not** violate the clean-bag invariant. So border keeps being dispatched, via an explicit per-field default read in `applyChromeOptions`: `const border = options.border ?? this._defaultOptions.border; if (border !== undefined) this.setBorder(border);`. `getBorder()` and the `applyStyle` `this._border` read are left untouched.

> **Implementation drift — padding keeps `?? null` (decided during implementation).** `getPadding()` is NOT given a `_defaultOptions` fallback. Its default is `(0,0,0,0)`, indistinguishable from "no padding" for rendering (CSS padding initial is 0), and 8 call sites branch on the `null` return ([getContentInsets](../src/typescript/lib/core/Component.ts#L1531), input subclasses). `applyStyle` reads `this.getPadding()` (null for the default → no CSS write, harmless) and `applyOptions` dispatches raw `options.padding`; no getter change.
