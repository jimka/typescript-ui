...
### Single-line-box height routes through a new `Util.singleLineBoxHeight` helper (authored here)

The `chrome = insets + padding + border; h = Util.lineHeightPx() + chrome` idiom is copy-pasted at [TextField.ts:61](src/typescript/lib/component/input/TextField.ts#L61), [PasswordField.ts:55](src/typescript/lib/component/input/PasswordField.ts#L55), [ComboBox.ts:741](src/typescript/lib/component/input/ComboBox.ts#L741), [AbstractPickerField.ts:251](src/typescript/lib/component/input/AbstractPickerField.ts#L251), and [NumberSpinner.ts:208](src/typescript/lib/component/input/NumberSpinner.ts#L208) (`TextArea` uses a fixed `preferredSize` and has no such method).

**Drift correction (2026-07-05):** the original plan assumed `shared-clamp-timer-size-sentinel-utils` would land a single-line-height `Util` helper it could consume. That plan shipped `Util.clamp`/`lineHeightPx` etc. but **no** such helper — `grep -n "singleLine\|boxHeight" src/typescript/lib/core/Util.ts` returns nothing. Per the user's decision, **this plan now authors the helper itself** in `core/Util.ts`:

```typescript
// core/Util.ts, inside the Util namespace
/**
 * Height in px of a single-line input box: the theme line-box height plus the
 * component's own vertical chrome (insets + optional padding + border).
 */
...
Each `updateHeight` replaces its four-line `chrome`/`h` computation with one call, e.g. `const h = Util.singleLineBoxHeight(this.getInsets(), this.getPadding(), this.getBorderSize());`, then keeps its own per-class `setPreferredSize(width, h)` / `setMaxSize` and `ThemeManager.onThemeChange` subscription (the width differs per class, and `NumberSpinner` uniquely reads the *inner input's* padding, not its own — [NumberSpinner.ts:211](src/typescript/lib/component/input/NumberSpinner.ts#L211), so it passes `this._input.getPadding()`). Match the helper's parameter types to whatever `getInsets()` / `getPadding()` / `getBorderSize()` actually return at the call sites; the body above is the canonical computation. Adding a new export to `Util.ts` does not conflict with the phase-1 siblings (overlay only imports `Util.clamp`; data-view does not touch `Util.ts`). Write the helper **test-first** — a unit test over `core/Util.ts` pinning `singleLineBoxHeight` for representative insets/padding/border before the call sites are routed through it.
...
### `ComboBox` — collapse the self-consumed synthetic `change` event

`ComboBox`'s programmatic and user selection both funnel through `setSelectedIndex(idx, true)`, which dispatches a **synthetic DOM `change`** via `Event.fireEvent(this, "change")` ([ComboBox.ts:1062](src/typescript/lib/component/input/ComboBox.ts#L1062)) that the combo then immediately catches on *itself* — the constructor bridge `Event.addListener(this, "change", () => this.notifyChange(this.getValue()))` ([ComboBox.ts:662](src/typescript/lib/component/input/ComboBox.ts#L662)) → `AbstractInput.notifyChange` → `emit("change")` / `emit("binding")`, and `on("action")` wired as a *raw DOM `change` listener* ([ComboBox.ts:979](src/typescript/lib/component/input/ComboBox.ts#L979) / off at [:998](src/typescript/lib/component/input/ComboBox.ts#L998)). There is no native `change` source (it is a custom component, not a `<select>`), `Event.fireEvent` routes through `dispatchCustomEvent`, and nothing in the codebase delegates or bubbles a combo `change` at a container level — so the DOM dispatch is pure indirection that lands right back on the same object.

Collapse it: `setSelectedIndex(idx, true)` calls `this.notifyChange(this.getValue())` directly (replacing the `Event.fireEvent` at [:1062]); delete the constructor bridge listener ([:662]); and re-route `on("action")` / `off("action")` through the framework bag by treating `action` as the alias of `change` — `super.on(event === "action" ? "change" : event, listener)` (and the same in `off`), replacing the `Event.addListener(this, "change", …)` / `removeListener` calls. This is **behaviour-preserving**: `notifyChange` still fires both `change` and `binding` on every committed selection, and the user path (`onRowSelected` → `setSelectedIndex(index, true)`, [ComboBox.ts:930](src/typescript/lib/component/input/ComboBox.ts#L930)) is unchanged. It also removes the materialisation-order fragility of the DOM bridge, so the offline test can assert a real `on("change")` bag listener instead of spying `Event.fireEvent`. `notifyChange` is `protected` on `AbstractInput`, so `ComboBox` may call it. The same synthetic-`change` + DOM-`action`-listener pattern also lives in `ToggleButton` ([ToggleButton.ts:243](src/typescript/lib/component/button/ToggleButton.ts#L243)) and `AbstractListComponent` ([AbstractListComponent.ts:172](src/typescript/lib/component/list/AbstractListComponent.ts#L172)); those sit in `button/` and `list/`, outside this input-only plan's scope — unify them in their owning areas (see Non-Goals).
...
`ComboBox` (event wiring only — public signatures unchanged): `setSelectedIndex` notifies through `AbstractInput.notifyChange` instead of dispatching a DOM `change`; `on("action")` / `off("action")` route through the framework bag as an alias of `change`.
...
7. **Item 4 — single-line-box height.** First author `Util.singleLineBoxHeight` in `core/Util.ts` (see Architecture Decisions), test-first with a unit test over the pure computation. Then route the `updateHeight` bodies in `TextField`, `PasswordField`, `ComboBox`, `AbstractPickerField`, `NumberSpinner` through it (passing `this._input.getPadding()` for `NumberSpinner`, `this.getPadding()` elsewhere); keep each `setPreferredSize` / `setMaxSize` / theme subscription. → verify: typecheck; the new `Util.singleLineBoxHeight` test passes; `default-options-fallback` test still reports `TextField padding` `[3,3,3,3]`.
...
9. **ComboBox change dispatch.** In `setSelectedIndex`, replace `Event.fireEvent(this, "change")` ([ComboBox.ts:1062](src/typescript/lib/component/input/ComboBox.ts#L1062)) with `this.notifyChange(this.getValue())`; delete the constructor `Event.addListener(this, "change", …notifyChange)` bridge ([:662](src/typescript/lib/component/input/ComboBox.ts#L662)); re-route `on("action")` / `off("action")` ([:979](src/typescript/lib/component/input/ComboBox.ts#L979)/[:998](src/typescript/lib/component/input/ComboBox.ts#L998)) through the bag (`super.on(event === "action" ? "change" : event, listener)`, same in `off`). → verify: `grep -rn 'fireEvent(this, "change")' src/typescript/lib/component/input/ComboBox.ts` prints nothing; a programmatic `setSelectedIndex(i)` fires an `on("change")` / `on("action")` bag listener.
10. **Types.** Ensure every touched method carries an explicit return type (`: void` on the base `onInput`, etc.).
11. **Full sweep.** `npm run build:lib`, `npm test`, `npm run docs:build` (zero warnings).
...
| Modify | `src/typescript/lib/core/Util.ts` (author `singleLineBoxHeight`) |
| Modify | `tests/unit/core/Util.test.ts` (new `singleLineBoxHeight` case) |
...
| Modify | `src/typescript/lib/component/input/ComboBox.ts` (route `updateHeight`; collapse synthetic-`change` dispatch, bag-route `on("action")`) |
...
- **ComboBox change (now bag-delivered offline):** a programmatic `setSelectedIndex(i)` (default `fireEvent`) invokes an `on("change", fn)` **and** an `on("action", fn)` bag listener with the new value, and `setSelectedIndex(i, false)` invokes neither — assertable with a real listener, since notification no longer round-trips the DOM (no `Event.fireEvent` spy needed, unlike the current `tests/component/input/ComboBox.test.ts` change cases). `getValue()` / `getSelectedRecord()` reflect the new selection.
...
- **No `Util.clamp` / `AutoRepeat` / sentinel authoring** — those live in `shared-clamp-timer-size-sentinel-utils`. *(Exception per the 2026-07-05 drift correction: `Util.singleLineBoxHeight` **is** authored by this plan, because the dependency never shipped it — see Architecture Decisions.)*
...
- **No `ToggleButton` / `AbstractListComponent` change-event rework** — the same self-consumed synthetic-`change` + DOM-`action`-listener pattern lives in `ToggleButton` (`button/`) and `AbstractListComponent` (`list/`), outside this input-only plan's scope; unify them in their owning areas for consistency. Only `ComboBox`'s instance is collapsed here.
