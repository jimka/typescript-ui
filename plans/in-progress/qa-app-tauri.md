...
- **The unknown-host message separates the names with a comma.** The plan asks for `unknown host "<host>" (supported: minibrowser tauri)` "through `HOSTS`, with no other change". The runner `qa-app` built already prints the list as `${HOSTS// /, }`, which with one host read `minibrowser` and now reads `minibrowser, tauri`. The runner's message line is unchanged, as the plan's "no other change" requires, so the printed message is `unknown host "bogus" (supported: minibrowser, tauri)`.
- **`qa-host` reads its arguments with `args_os`.** The plan's `main` passes `std::env::args()` to a `run_url` over `String`s, and `args()` panics on an argument that is not valid UTF-8, exiting 101 instead of the contract's 2. `run_url` takes `OsString`s instead and refuses such an argument as `not a URL: "<arg>"`, the invalid bytes shown as U+FFFD, even when the rest of it would parse; a fifth unit test, beside T1–T4, pins that on Unix.
- **The runner's build hint names the manifest by an absolute path.** The plan's message says `cargo build --manifest-path packages/qa/src-tauri/Cargo.toml`, which is right only from the repository root. It now names `$QA/src-tauri/Cargo.toml`, derived from the script's own directory as the rest of the runner's paths are, so the hint works wherever the runner is started. The README's build command still reads from the repository root.
- **`src/main.rs` is `rustfmt`-formatted.** `cargo fmt` rewraps the plan's sketch where a line passes 100 columns (the `http`/`https` error arm and three test assertions); the code is the plan's otherwise. `cargo clippy --all-targets` reports nothing.
- **The README says slightly more than step 8 lists.** Besides the *Tauri host* section, the extended same-host rule and the per-host validation lines, the package's path table gains a `src-tauri/` row and the *Running* section's `--host` paragraph names `tauri`, so the two places that list the hosts and the package's files stay complete. The *Tauri host* section also says what Tauri still injects into the page (checked against `tauri` 2.11.5: `window.isTauri`, `window.__TAURI_INTERNALS__`, wry's `window.ipc`, a drag-region `mousedown` listener and, in a debug build, a devtools `keydown` listener, with every command refused because the dev-server page is remote content and no capability exists), and that each worktree needs its own 2.9 GB build unless `QA_TAURI_BIN` points at an existing binary. The cross-host rule adds that census counts compare only at the same `before.viewport`, the caveat the plan's *Potential Challenges* gives for one host.
- **The first build was quick here.** The crates were already in the local Cargo registry from Loom's builds, so the first `cargo test` compiled everything in about 40 seconds of wall time; a machine without them still downloads and compiles about 430 crates. `Cargo.lock` resolves `tauri` 2.11.5, `wry` 0.55.1 and `webkit2gtk` 2.0.2, the versions the plan's notes name.

**Verification run.** `cargo test` passes T1–T4 and the non-UTF-8 case (each red first, then green); `cargo fmt --check` and `cargo clippy --all-targets` are clean; `cargo build` produces `target/debug/qa-host`, and `git status` lists no `target/` or `gen/` path; `ldd` shows `qa-host` and MiniBrowser both linking `/lib/x86_64-linux-gnu/libwebkit2gtk-4.1.so.0`; `npm -w packages/qa run typecheck` and `npm -w packages/qa run test` (69 tests, all under `tests/`, none under `src-tauri/`) pass with the built shell in place; `bash -n runqa.sh` passes; `runqa.sh --host bogus x main` exits 2 with the message above, and `QA_TAURI_BIN=/nonexistent runqa.sh --host tauri x main` exits 2 with the build hint naming `/nonexistent`, and afterwards nothing listens on port 5190; `git check-ignore packages/qa/src-tauri/target packages/qa/src-tauri/gen` prints both.

**Still to do: the manual sweep (M1–M4).** Neither `qa-host` nor `runqa.sh --host tauri` has been started: each opens a full-screen window, and only the user authorises that. This includes `qa-host`'s bad-argument exits, which open no window but were not run either; `run_url`'s tests cover the messages, and `main`'s exit code 2 is covered only by reading it. The README's `chart-line` row reads "not yet" for both hosts until the first authorised sweep fills in each host's date, library commit, census and frame time.
