...
- **`Menu.test.ts`'s new `dispose()` case drains stray `LayerManager` registrations before asserting.** The plan's exact assertion (`LayerManager.getTopLayer()` is `null` after `dispose()`) is only reliable if the module-level layer stack is empty beforehand. Three pre-existing tests in the file's `Menu rebuild-mode submenus` block open a rebuild-mode menu via `show()` and hover a submenu without ever calling `hide()` / `close()` / `dispose()`, so when the whole file runs in declaration order they leave stray entries on `LayerManager`'s stack — a `toBeNull()` assertion run after them failed even though the fix itself was correct. Rather than editing those three unrelated, already-passing tests (out of this plan's scope), the new test drains any stray top layer via a small loop before its own `show()` / `dispose()`, so its assertion reflects only its own menu regardless of what ran earlier in the file. This is a pre-existing test-isolation gap this plan did not introduce and did not fix at the source.
- **`dispose-full-teardown.test.ts`'s new `MenuButton` row needs an explicit generic argument.** `new MenuButton('Export', { menuItems: [...] })`, assigned into the registry's `make: () => Component` slot, made the compiler infer `TOptions` from the literal options bag rather than defaulting to `MenuButtonOptions`, producing a type TypeScript then rejected as incompatible with `Component`. Fixed with `new MenuButton<MenuButtonOptions>(...)`; `SplitButton`, `ToolBar`, `Table`, and `MenuBar` are all non-generic classes and needed no equivalent change.
- **The registry file's re-derived `protected destructor(` count was already stale before this plan touched it** — it read "20 hits" but a plain grep returns 30 on this branch's base, because several later, unrelated plans (`Dock`, `Notification`, `DropZoneOverlay`, `Dialog`, `Drawer`, `Rail`, `DiagramView`, `VirtualRowView`, `table/cell/Header`, plus the abstract `AnimatedDropdown` and the singleton `Tooltip`) added `destructor()` overrides without a matching registry row. This plan's own count update (to 35, after its five new overrides) documents that gap rather than closing it — auditing and back-filling those rows is unrelated, out-of-scope work.
- **The manual demo-app verification (`## Verification`, the numbered browser-check list) used per-cycle `#`-prefixed-selector counts on a repeatedly-added-and-closed "Async" panel, not the single-cycle raw `document.styleSheets` total the plan describes.** Driving the literal steps first surfaced a confound: the demo's "Add async panel" / close cycle leaks ~6 stylesheet rules per cycle *with no `Menu` involved at all* — reproduced identically with the temporary `ToolBar`/`MenuButton` content reverted back to a plain `Text` node. That leak is real but unrelated to this plan (nothing under `plans/in-progress/table-tab-close-residual-leak.md`'s scope touches whatever the async-tab/TabBar path is leaking), so counting it into the sanity check would have misread a pass as a failure. Narrowing to `#`-prefixed (per-instance) selector rules and diffing four repeated open-menu/close-menu/close-tab cycles against that same ~6-rule baseline showed **zero incremental growth from opening and closing the `MenuButton` dropdown** — the baseline noise stayed flat at +6 whether or not the menu was ever opened. This is the live-browser confirmation the plan's manual step asked for; the unrelated ~6-rule-per-cycle async-tab leak is a new, separate finding, not something this plan fixes.
