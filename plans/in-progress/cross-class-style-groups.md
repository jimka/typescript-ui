...
Four deviations from the plan's exact text, found while implementing and verifying against the test harness / `docs:api`:

- **`ensureTraitStyleRule`'s cache must be keyed on the `StyleTrait` object, not on the trait's `name` string.** The plan's `## Internal Structure` snippet keys `_traitBags` by `className` (derived from `trait.name`) and checks `existing !== undefined` before ever consulting `_owners`. With a single shared `name`-keyed cache, the *first* trait object to resolve for a given name permanently satisfies that cache-hit check for every later call — including one passing a genuinely *different* `StyleTrait` object with the same `name` — so the `_owners` collision check that follows becomes unreachable dead code and Expected Behaviour row 11's "a name collision self-corrects" never actually fires (verified by a failing test: `StyleTraitRules.test.ts`'s row 11 case). Fixed by keying `_traitBags: Map<StyleTrait, StyleLayer | null>` on the trait object's own identity instead — each distinct object gets its own fresh evaluation, so the second, differently-identified trait genuinely reaches the `_owners.get(className) !== trait` check and is correctly rejected. `_owners` itself is unchanged (still keyed by the string name, exactly as the plan specifies) — only `_traitBags`'s key type changed.
- **The instance-level trait's DOM token needs its own tracked field, `_instanceTraitToken`, to clear correctly.** The plan's `applyStyle` seeding snippet only calls `DOM.sink.apply` inside `if (instanceTrait) { … }`, so clearing the trait (`setStyleTrait(null)`) skips that block entirely and never removes the token added by an earlier render — Expected Behaviour row 10 ("clearing an instance-level trait and re-rendering removes the token") would silently fail to remove it. Added a private `_instanceTraitToken: string | null` field that records the token most recently applied; `applyStyle` now diffs the previous token against the next one (derived from the *current* `getStyleTrait()`) and issues `addClass`/`removeClass` only for what actually changed — a `null`-to-token transition adds, a token-to-`null` transition removes, and a same-token re-render write no-ops, matching the write-skip discipline every other tier in this codebase already follows.
- **`TextInputClassTier.test.ts` needed three more assertions updated, not only row 3.** The plan's step 18 called out row 3 (the `.TextInput` class rule losing border/borderRadius) but not the knock-on effect the plan's own Architecture Decisions section predicts for `PickerInput` (the "worked example"): once the trait layer outranks the class tier, `PickerInput`'s own `border: "none"` — dispatched as an authored instance value — genuinely deviates from the inherited trait and writes for real to `#id`, where it previously matched `PickerInput`'s own class-tier rule and was skipped. The same mechanism affects every other subclass with the same shape: `NumberSpinnerField` and `AutoCompleteTextField` (both declare their own class-tier `border: "none"`/`borderRadius: "0"`, dispatched the same way). Updated row 4 (`DateField`'s inner `PickerInput`), row 5 (`NumberSpinner`'s inner field), and the `AutoCompleteTextField` test to expect the now-real `borderTop`/`borderRight`/`borderBottom`/`borderLeft`/`borderRadius` declarations on `#id`, rather than `undefined`. `outline` is unaffected (never dispatched to an instance setter, and not a trait property), so those assertions are unchanged.
- **`core/index.ts` now exports `StyleTrait`, `StyleBag`, and `TextStyleBag`, contradicting the plan's `## Public API` claim that "Nothing in `core/ClassStyleRules.ts`... is re-exported."** `npm run docs:api` (a `## Verification` requirement: "must finish with zero warnings") reported `StyleTrait` — referenced by the now-public `ComponentOptions.styleTrait` field — as "referenced... but not included in the documentation." Exporting it resolved that warning but surfaced the same problem one hop deeper for `StyleTrait.declarations`'s type (`StyleBag`), then for `StyleBag.font`'s type (`TextStyleBag`); all three are now exported from `core/index.ts`, matching the established precedent `ComponentStyleRuleSpec` (a `ComponentOptions` field's type) already sets. Nothing else from `ClassStyleRules.ts` is exported — `ensureTraitStyleRule`, `resolveStyleTraits`, `resolveTraitStyleDefaults`, `traitClassName`, and `traitTopStateConflictKeys` all stay internal, as the plan specifies.
