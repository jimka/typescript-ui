...
- **`DocumentSelectionRange` does need a barrel export, despite `## Documentation Impact`'s claim otherwise.** That section states "No barrel or sidebar change — `DOM`, `DOMSource`, and `Handle` are already exported and documented; `DocumentSelectionRange` and `getDocumentSelection` are picked up automatically by TypeDoc from their JSDoc." Running `npm run docs:api` after implementation produced a warning instead: `DocumentSelectionRange, defined in @jimka/typescript-ui/src/typescript/lib/core/DOM.ts, is referenced by core.ProductionDOMSource.getDocumentSelection but not included in the documentation`. `getActiveElement`'s return type is `Handle | null` — a type already exported — so it never needed a matching barrel entry, but `getDocumentSelection` returns a type unique to it, and TypeDoc only documents a referenced type if it is itself reachable from an entry point. Fixed by adding `DocumentSelectionRange` alongside `Rect` / `ScrollMetrics` / `OffsetSize` / `MediaState` / `MediaQueryResult` in the `export type { ... } from '~/core/DOM.js';` line in [`packages/lib/src/typescript/lib/core/index.ts`](packages/lib/src/typescript/lib/core/index.ts). `npm run docs:api` afterward reports 0 errors and only the pre-existing, unrelated `DiagramEdgeLayer.setEdges` warning (confirmed present on this branch's start point too).

- **The `copy` listener must be a subtree listener, not an exact-target listener — `## Architecture Decisions`' "no subtree walk" reasoning is wrong.** That section, and footnote `[^copy-target]`, argue that a click-then-select gesture always leaves `document.activeElement` as `Body`'s own element by the time `Ctrl+C` fires, so `Event.addListener(this, "copy", this.onCopy)` (exact-target routing) would be sufficient. A live-browser audit check disproved this: Chrome's `copy` event targets wherever the *selection* actually starts (a cell's own element or its `Text` descendant) — Blink's `Editor::FindEventTargetFromSelection` — not `document.activeElement`. An exact-target listener on `Body` therefore never fires for a real drag-selection. Fixed by registering it as `Event.addSubtreeListener(this, "copy", this.onCopy)` instead, the same routing `onSubtreeClick` already uses for the identical class of problem (a native event landing on a cell descendant, not on `Body`'s own element) — see [`Body.ts`](packages/lib/src/typescript/lib/component/table/Body.ts)'s `init()`. `onCopy` itself needed no change: it already reads the selection from `DOM.source.getDocumentSelection()` rather than from `e.target`.

- **`renderedCellGrid`'s "pool and column order are already document order" claim is wrong; the walk needs a swap guard `## Architecture Decisions` doesn't anticipate.** A pool row's element is appended to the DOM once, in creation order, and never physically reordered afterwards — `alignPoolWindow` only rotates which Row *object* a given array slot currently references, and `bindAndPositionRows` always assigns `_boundIndices[i] = firstRow + i`, so a pool slot's *array* index already tracks row-major (data-index) order with no sort needed. What is **not** row-major is the DOM: a specific row's fixed creation-order position is unrelated to which data index it currently shows, so the browser's `startContainer`/`endContainer` — ordered by that fixed DOM position — can disagree with this already-correctly-ordered grid. (An earlier version of this fix wrongly diagnosed the cause as pool-slot order itself and added a redundant `_boundIndices` sort in `renderedCellGrid`; caught in a second audit round and removed — the array was never out of order to begin with.) A live-browser check confirmed the *symptom* this causes: selecting two adjacent, visually-ordered rows straddling a pool recycle point resolved to a *reversed* row-major span and produced an **empty clipboard payload while still suppressing the browser's default copy** — strictly worse than doing nothing. Fixed in [`Body.ts`](packages/lib/src/typescript/lib/component/table/Body.ts)'s `buildSelectionText`: it now swaps its resolved start/end grid positions (and their offsets) whenever `start` sorts after `end` in the grid's row-major order, since the browser's own `startContainer`/`endContainer` labels track raw DOM position, not row-major order. Column order needed no equivalent fix: `Row.setColumnWindow`'s `sortComponents` call reorders `getComponents()`'s *array*, which `renderedCellGrid` reads, so a row's cells stay in visible-column order regardless of DOM sibling order — the same array `resolveClickedColumn` already relies on. Covered by two offline tests (`Body.test.ts`, "recovers correct row-major order when the Range reports start/end reversed" and "swaps the start/end character offsets along with the reversed positions") that stub a `DocumentSelectionRange` whose start/end containers resolve to the grid's later row first — the same shape a real DOM/data-index divergence produces — rather than depending on exact scroll arithmetic to reproduce a real pool wrap.

- **`DOMSource` gaining a new required member is a breaking change; `## Documentation Impact` omitted the changelog entry for it.** `packages/lib/docs/reference/changelog/next.md` had no `## Breaking changes` section; the closest precedent, `0.4.0.md`'s `DOMSource gains two required members` entry, was not followed. Added a `## Breaking changes` → `### Core` entry for the new `getDocumentSelection()` member, matching that precedent's shape and scope note ("Only a consumer implementing its own `DOMSource` is affected").
