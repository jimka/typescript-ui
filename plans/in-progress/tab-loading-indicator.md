...
Two small deviations surfaced during implementation, neither changing the design in `## Architecture Decisions`:

- **`TabButton.destructor()` also disposes `_busyIndicator`.** `## Internal Structure` listed the new `_busy` / `_busyIndicator` fields and `setBusy` / `isBusy`, but did not call out updating the destructor. `_busyIndicator` is raw-appended onto the button's element exactly like `_closeButton` (the plan's own precedent — see `buildCloseButton`'s doc comment on why `super.destructor()`'s child recursion can't reach a raw-appended overlay), so it needs the same explicit `.dispose()` call `_closeButton` already gets. Without it, `packages/lib/tests/core/ComponentDispose.test.ts`'s "leaves zero new rule-cache keys when the tab closes while its build is in flight" failed: closing a lazy tab mid-build left the busy indicator's own per-instance component-scoped style rule in the module rule cache. Added `this._busyIndicator?.dispose();` beside `this._closeButton?.dispose();`.
- **`ComponentDispose.test.ts` updated to exclude `.TabBusyIndicator`.** Even with the destructor fix above, that same test still saw one new rule-cache key survive the tab's close: `.TabBusyIndicator`, the module-level shared class rule `ensureBusyIndicatorClassRule` registers once on first use (mirroring `SortPriorityBadge`'s `.SortPriorityBadge`, per `## Architecture Decisions`). That rule is permanent and module-scoped by design — the same category the test file already carves out for the framework-wide `:where(.ts-ui-component)` selector, with a comment explaining why a first-use-in-this-file appearance isn't a leak. This test happens to be the first in its file to mark any tab busy, so `.TabBusyIndicator` legitimately appears relative to its `before` snapshot the same way. Added a second excluded selector, `BUSY_INDICATOR_SELECTOR = '.TabBusyIndicator'`, alongside the existing `FRAMEWORK_SELECTOR` one, with a comment following the existing one's shape.
- **`[^dock-manual]`'s "the automated browser" is not the offline test harness — the Dock↔Tab `"busychange"` forwarding turned out to be testable offline, and now is.** The footnote's 0×0-measurement problem is a real, pre-existing limitation, but of a live/real-browser end-to-end tool, not of the Vitest offline harness. `Dock.lifecycle.test.ts`'s existing `mountDock()` / `captureRaf()` / `flush()` harness already drives a lazy panel's full activation-through-materialization cycle offline (see its `'materializes the content on first activation, once'` test) — the same mechanism `resolvePanel`'s `onBusyChanged` subscription rides. Added `'marks the panel busy on the outer region tab while it builds, and clears it on resolve'` to `Dock.lifecycle.test.ts`'s `Dock addLazyPanel` block, using that harness plus the `useReducedMotion`-style `matchMedia` mock `Tab.lazy.test.ts` already uses to let `onReady` settle without a real `transitionend`. Verified non-vacuous by temporarily removing the `tab.on("busychange", onBusyChanged)` subscription and confirming the new test fails. Case 19 in `## Expected Behaviour` still has a real manual component — the visual wash's legibility in both themes — so it stays in the manual list, but the state-transition mechanism it also covers no longer rests on manual verification alone.
