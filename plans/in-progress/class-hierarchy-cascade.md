...
Six deviations from the plan's literal text, found while implementing and verifying `## Expected Behaviour`:

1. **The base case needed `Component`'s real defaults, not `{}`.** `resolveClassLevel`'s base case (`Object.getPrototypeOf` reaching past `Component`) is sketched as `{ defaults: {}, resolved: FRAMEWORK_DECLARATIONS }`. With a literal empty `defaults`, `resolveDeclarations({})` does *not* reproduce `FRAMEWORK_DECLARATIONS`: its `minWidth`/`minHeight` fallback (no `minSize` present) is `"auto"`, and its `overflowX`/`overflowY` fallback (no `overflow` present) is `"visible"` — both differ from the framework's hoisted `"0px"`/`"hidden"`. A participating class that never restates `minSize`/`overflow` (e.g. `Cell`, which customises only colour/background/border) therefore got two spurious deviations on its class rule, forcing every instance's `#id` rule to needlessly materialise. Fixed by adding `FRAMEWORK_DEFAULTS = { minSize: {0,0}, overflow: "hidden" }` — the `ClassStyleDefaults` input that *does* round-trip through `resolveDeclarations` to `FRAMEWORK_DECLARATIONS` — as the base case's `defaults`, matching what every already-registered class's `_defaultOptions` merge chain already carries transitively from `Component`'s own base bag.
2. **`Text`'s `ownClassStyleDefaults` needed the resolved `font` bag, not a filtered `_defaultTextOptions`.** The opt-in table's literal instruction ("Own field value: `_defaultTextOptions`") doesn't work: `_defaultTextOptions` carries no top-level `ClassStyleDefaults` key at all (its font fields are flat `TextOptions` keys, not nested under `font`), so assigning it verbatim would make `Text`'s static contribution a no-op — while the *actual* font sharing this plan's Overview promises depends on `Text.getClassStyleDefaults()`'s override, a *separate*, instance-level mechanism the plan says is "unaffected." The two paths would disagree (the invariant `## Internal Structure` requires), and `Label`/`Legend` would silently stop sharing font declarations at all. Fixed by giving `Text.ownClassStyleDefaults` the same resolved `font` sub-bag `getClassStyleDefaults()` needs, and refactoring `getClassStyleDefaults()` to read it from that one static field instead of recomputing it inline — so the two paths cannot drift apart by construction, rather than by convention.
3. **A `callable()`-wrapped constructor and its raw class are different `Object` references, and the walk sees both.** Footnote 437 (`^callable-preserves-chain`) states the hierarchy walk "needs no special-casing for the callable wrapper" — this is incorrect. `this.constructor` (an instance's own leaf class, entering via `Component.applyStyle`) is always the *raw* class; `Object.getPrototypeOf` on a subclass returns whatever reference sat on the right of its `extends` clause, which — per ARCHITECTURE.md's own "Imports always use the callable name... This holds even for extends clauses" — is always the *callable* Proxy. The same conceptual class (e.g. `Text`) is therefore keyed under two different `Map` entries depending on which path reached it, splitting its `_owners` registration and silently defeating the name-collision check: a `Text` instance constructed directly (raw key) left `Text`'s font rule registered under the raw reference, while `Label`/`Legend`/`SelectableText`/`Link` (reaching `Text` via `Object.getPrototypeOf`, the callable key) found no registration there, fell through as if colliding, and lost the whole font group. Fixed with a `canonicalCtor` helper (`ctor.prototype.constructor`, which a `callable()` Proxy forwards straight to the raw target) applied at every point a constructor reference enters `resolveClassLevel`, `getStyleClassChain`, `chainParticipates`, `ensureClassStyleRule`, and `registerStyleChainRoot`, so every one of `_levels`/`_owners`/`_bags`/`_classChains` is keyed consistently regardless of which path supplied the reference.
4. **`ensureClassStyleRule`'s final body needed an explicit participation gate and a leaf-level collision short-circuit**, confirming the plan's own caution that this function was "a sketch, not a final body." A class with no `ownClassStyleDefaults` anywhere in its chain must keep using the pre-hierarchy flat `classDeviations(defaults)`-against-`FRAMEWORK_DECLARATIONS` path — not `resolveClassLevel`'s pass-through — because that flat path is the only one that reads the *caller-supplied* (instance-derived) `defaults`, which is where a not-yet-migrated class's own `subclassDefaults` customisation actually lives; `resolveClassLevel`'s static walk has nothing to read for such a class and would silently report "no deviation." A `chainParticipates` check (walk the chain once, checking `ownDefaultsOf` at each level) decides which path applies. Separately, row 7's expected behaviour (a name-colliding class disables the class tier entirely, `null`, not a partial pass-through) required a top-level name-collision check in `ensureClassStyleRule` *before* delegating to `resolveClassLevel` — `resolveClassLevel`'s own internal collision handling (needed for mid-chain ancestor collisions) resolves to a valid pass-through bag, not `null`, which is right for an ancestor but wrong for the leaf itself.
5. **Ten more leaves beyond `Link`/`SelectableText` needed their own `ownClassStyleDefaults`**, found by a systematic sweep (per the worker standard's "Inheritance / dependency sweep" step) after the `Link`/`SelectableText` case surfaced via a pre-existing test failure. Any class that forwards its own hoistable field (`cursor`, `backgroundColor`, `border`, …) as `subclassDefaults` to one of the plan's eight opted-in middle classes, without itself registering `ownClassStyleDefaults`, silently loses that field to a pass-through — invisible offline unless a test happens to assert on the specific `#id`-vs-class-rule split. Five more `TextInput` leaves (`TextField`, `PasswordField`, `UsernameField`, `TextArea`, `PickerInput`) and five more `AnimatedDropdown` leaves (`ComboBoxDropdown`, `TimePickerDropdown`, `AutoCompleteDropdown`, `PopupPanel`, `AbstractCalendarDropdown`) all had this shape and now register their own field, extracting a named CSS-only constant from any constructor that also forwards a fresh-per-instance value (a `layoutManager`/`insets`) inline, so no stateful object is accidentally shared across instances via a module-level constant. `AbstractCalendarDropdown` was the most consequential: `DatePickerDropdown`/`DateTimePickerDropdown` forward no `subclassDefaults` of their own, so before this fix both would have silently rendered with zero background/border/radius/shadow chrome the moment `AbstractCalendarDropdown` started participating in the hierarchy walk. Verified live in a browser (not just offline) for this reason. `## Documentation Impact`'s changelog entry already describes these at the right level of generality ("`TextInput` and its subclasses", "the `AnimatedDropdown` family"), so it needed no further edit; `## Architecture Decisions`' "The class tier is hierarchy-aware" paragraph in ARCHITECTURE.md was widened to name them rather than imply `Link`/`SelectableText` are the only such leaves.
6. **`getStyleClassChain`'s "unconditional" widening (`## Internal Structure`) needed a participation gate — a defect an independent audit caught, not caught by this plan's own manual verification.** The plan's `## Architecture Decisions` "Rollout is scoped to the confirmed-safe chains" section explicitly excludes `Button`/`ToggleButton`/`TabButton`/`SpinButton` from DOM-class widening, because each still has its own independent flat `.ClassName` rule (including an independently-created state-tier `.pressed` rule) at the same specificity as every other level's rule — widening their DOM classes without also making the state tier hierarchy-aware would let two same-specificity rules start matching one element, decided by insertion order. But `## Internal Structure`'s literal sketch for `getStyleClassChain` has no such exclusion, and neither did the first implementation: `getStyleClassChain` walked every ancestor unconditionally for every class, `Button` family included, since nothing in `Component.init()`'s call site or `getStyleClassChain` itself checked for it. Fixed by gating the widening on `chainParticipates(ctor)`: a chain with no `ownClassStyleDefaults` anywhere keeps today's exact pre-hierarchy behaviour (its own name only), which generalises the plan's named `Button`-family exclusion to *every* non-participating chain — including `MenuButton`/`PopupButton`, two more `Button` descendants the plan's own exclusion list didn't name — rather than hand-maintaining a fixed list. Verified live: before the fix, a rendered `TabButton` carried `ts-ui-component Button ToggleButton TabButton`; after, `ts-ui-component TabButton`, matching pre-plan output exactly.
