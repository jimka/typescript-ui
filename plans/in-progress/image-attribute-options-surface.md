...
**`handleLoad()`'s actual shape keeps an `if`/`else` with a `notifyIntrinsicSizeChanged()` branch that `## Internal Structure`'s snippet doesn't show.** This plan's own Overview flagged that the dependency plan wasn't implemented anywhere at authoring time, so `handleLoad()` was anchored to a description rather than real line numbers; by the time it landed (see `image-intrinsic-size-lifecycle`'s own `## Implementation Notes`), its audit loop had restructured the single `if (!this.getPreferredSizeConstraint()) { …publish… }` this plan's snippet assumed into `if (this.getPreferredSizeConstraint()) { this.notifyIntrinsicSizeChanged(); } else { …publish… }`, so an explicit `preferredSize` still relays a natural-size-driven min-size change upward. This plan's two edits — swap the gate condition, call `super.setPreferredSize` in the auto-publish branch — were applied to that actual `if`/`else` shape instead: `this.getPreferredSizeConstraint()` became `this._hasExplicitPreferredSize` in the `if`, the `else` branch's `this.setPreferredSize(...)` became `super.setPreferredSize(...)`, and the `notifyIntrinsicSizeChanged()` branch itself is untouched, per this plan's own `## Non-Goals` ("everything else in `handleLoad()` is the dependency plan's, unchanged"). The existing test covering that branch (`Image.test.ts`'s "relays a min-size change upward when an explicit preferredSize skips the publish") continues to pass unmodified.

**The constructor's `_hasExplicitPreferredSize` seed reads `this.getPreferredSizeConstraint() !== null`, not the plan's literal `??= false` from `## Internal Structure`.** The audit caught a real regression the plan didn't anticipate: `_hasExplicitPreferredSize` is only ever set by a *dispatched* setter, but a `subclassDefaults`-seeded `preferredSize` (the documented `Component` mechanism a subclass uses to pin a default size — see [ARCHITECTURE.md](ARCHITECTURE.md)'s "Class-level defaults must survive the getter") is never dispatched through `setPreferredSize` at all; `Component.applyOptions` only calls it for a caller-supplied `options.preferredSize`. With the plan's literal `??= false` seed, a subclass with a defaulted `preferredSize` and no explicit option would have its pinned size silently overwritten by the natural size on the very first `load` — a real regression against the pre-plan code, whose `getPreferredSizeConstraint()` gate *did* fold the class default in. The fix seeds from `getPreferredSizeConstraint() !== null` instead, which folds the same default, restoring the pre-plan behaviour for that case while leaving the no-default path (`null` constraint → seeds `false`) unchanged. Covered by a new test constructing `Image` with a `subclassDefaults`-only `preferredSize` and asserting it survives a load; mutation-verified (reverting to `??= false` fails that test).

**Two more tests were added for the `## Internal Structure` overrides `setPreferredSize` / `clearPreferredSize`, neither of which the plan's `## Expected Behaviour` named directly.** Both were previously reachable only indirectly (through construction options or `handleLoad`'s own bypass via `super.setPreferredSize`), so a direct post-construction `setPreferredSize` call and a `clearPreferredSize` call were each untested; deleting either override's one-line body left the full suite green. New tests exercise both directly.

**`packages/lib/docs/reference/changelog/next.md` and `packages/lib/docs/reference/migration/next.md` gained entries, though the plan's `## Documentation Impact` never listed either page.** This branch adds public API (seven `ImageOptions` fields, seven getter/setter pairs) and, incidentally, narrows one: `Image.render()` had no access modifier before this plan (implicitly public, overriding `Component`'s `protected render()`), and deleting the now-dead override drops it back to `protected`, so a direct `image.render()` call is now a compile error. Every comparable public-API change on `master` updates both pages (the dependency plan's own `## Implementation Notes` records the identical gap and fix for `next.md`); `migration/next.md` gained a `## Image.render() is no longer public` section with the `getElement(true)` replacement. (A first pass appended the `Image.render()` bullet as a second, duplicate `### Components` heading under `next.md`'s `## Breaking changes`, next to the existing one for `Slider`; fixed to a single bullet list under the one heading, matching every other page in `changelog/`.)

**The plan's closing manual-verify step ("open the `Image` docs demo … confirm `alt` text appears in the accessibility tree … and that swapping `src` on a live instance re-measures … without a console error") is now performed and passes.** Neither existing demo called `setSrc` — `image-basic` pins an explicit `preferredSize` (so it wouldn't re-measure even if it did), and `image-auto-fit` had no way to trigger a second `load`. `image-auto-fit.ts` gained a "Swap image" button (`Button`, `listeners.action`) that alternates `logo.setSrc(...)` between two differently-sized inline SVGs (`data:` URIs, no external asset), and the `height` export grew from `120` to `160` to fit the added button row. Verified live via `npm run docs:dev` + Chrome DevTools MCP at `/typescript-ui/components/Image`: the `image-basic` image's accessible name is "Circle logo" and `image-auto-fit`'s is "Auto-fit demo shape" (both from `alt`, confirmed via an accessibility-tree snapshot); clicking "Swap image" changes the status line from "loaded at 64×64" to "loaded at 40×40" and back, with `list_console_messages` showing no error at any point (one pre-existing, unrelated `CodeEditor` height-correction warning from a different demo's fenced code block).
