...
- **No fix to the `syncCodeEditors` width-resync one-cycle lag** that `markdown-code-editor-autogrow-height`'s own Implementation Notes already flagged as a real, unfixed, pre-existing timing gap. It was not confirmed related to this report (the reproductions above never triggered a width-driven correction at all — no window resize occurred during any navigation-time trace) and fixing already-shipped code for a different, separately-documented issue is out of scope here.
