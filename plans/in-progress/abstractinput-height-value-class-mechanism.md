...
- **This plan's precedent survey (`## Architecture Decisions`, the Files table) is stale relative to this branch's own base commit.** It states `Text.setLineHeight` is `setValueStyleState`'s "one current caller" and that this plan is "the first use of a value class for a *layout* constraint." Both were true when the prose was drafted but not by the time this branch forked: [`ListItem.ts:111`](packages/lib/src/typescript/lib/component/list/ListItem.ts#L111) (landed at `8ece9f11`, already an ancestor of this branch's phase-start commit) publishes a `minSize` value class for `ListItemMarkerText` the same way. That file's own doc comment ([`ListItem.ts:35-45`](packages/lib/src/typescript/lib/component/list/ListItem.ts#L35)) states an invariant this plan diverges from: a size-key value-class token must be seeded from every axis the shared rule declares, or two different `{width, height}` pairs can collide on one rule and the first caller silently decides the CSS for the second. [`AbstractInput.pinSingleLineBoxHeight`](packages/lib/src/typescript/lib/component/input/AbstractInput.ts#L244) seeds its token from height alone (`h + "px"`), not from both `minSize`/`maxSize` axes.
  - **The divergence is a decision, not an oversight.** It is safe here specifically because the width half of `pinSingleLineBoxHeight`'s patch is pinned to fixed framework constants (`0` / `UNBOUNDED`) rather than read back from the instance — see the `@remarks` on `pinSingleLineBoxHeight` and the `[^normalised-widths]` footnote above, both already in this plan. Every caller passes the identical width pair regardless of which instance mints the rule first, so there is no second `{width, height}` combination for a height-only token to collide with. `ListItem`'s marker widths, by contrast, are genuinely per-list — which is exactly why its token has to carry both axes. The two call sites reach the same safety property (one CSS body per token) by different means because their inputs differ, not because either is wrong.
- **`packages/lib/tests/component/input/TextInputClassTier.test.ts` needed updating, though it isn't in this plan's Files table.** Two of its pre-existing tests pinned the *old* per-instance `#id` behaviour this plan's whole point is to change: "row 1: a TextField renders with no font declaration on its own #id rule" asserted the real declarations left on `#id` were exactly `['maxHeight', 'minHeight']`, and "every single-line AbstractInput leaf writes maxHeight before minHeight on its own #id rule" asserted that pair's *order* for every leaf (both from `abstractinput-height-dedup.md`, the precedent plan this one explicitly builds on). Both are updated to assert the new contents instead: empty for a class this plan opts in (`TextField`, `ComboBox`, `NumberSpinner`, and every `TextField` subclass reached through it — `NumberSpinner`'s and `AutoCompleteField`'s inner fields), and the original real pair for a class this plan's `## Non-Goals` explicitly holds back (`PasswordField`, `UsernameField`, `AbstractPickerField`/`DateField`).
- **Row 3 of `## Expected Behaviour` ("all resolving the same h") doesn't hold under this repo's offline test DOM.** `TextInput`'s border (`TextField`/`ComboBox`) is `var(--ts-ui-input-border)` with no inline fallback, which resolves to nothing against the test fixture's empty `themeVars` and contributes 0; `NumberSpinner`'s own border is a hard-coded `"1px solid var(--ts-ui-button-border, rgb(200, 200, 200))"` literal, whose leading `1px` is parsed directly regardless of the variable, contributing a real 2px. So `NumberSpinner` resolves 2px taller than `TextField`/`ComboBox` offline, even though `## Architecture Decisions`' cross-class analysis is correct that they coincide under a real shipped theme. `SingleLineHeightValueClassSharing.test.ts`'s row 3 reads each component's own resolved height back independently instead of assuming equality, which still exercises the scenario's actual point — three distinct classes get three distinct selectors, none writing a real declaration to its own `#id` rule.
- **Row 10's manual verification ran against `#/baseline`, `#/misc`, `#/column`, and `#/style-audit`, not `#/inputs`** — that route doesn't exist; `main.ts` registers no section by that name. `TextField`/`ComboBox`/`NumberSpinner`/`DateField`/`TimeField`/`DateTimeField`/`AutoCompleteField` are exercised across `BaselinePanel` and `MiscPanel`; `PasswordField`/`UsernameField` are not constructed by either — they live in `LayoutTestPanel` (`ColumnPanel`/`VBoxPanel`, reached via `#/column`/`#/vbox`), so `#/column` was added to cover them. A dev server was started from this worktree (confirmed via `readlink /proc/<pid>/cwd`) and driven with the `chrome-devtools` MCP tools, both `getComputedStyle` and a direct stylesheet walk for each class's own `#id` rule and its value-class rule.
  - **This plan's three classes, opted in:** every rendered `TextField`/`ComboBox`/`NumberSpinner` element (the outer composite, for `NumberSpinner`) carries no materialised `#id` rule and resolves through `.TextField.h24px`/`.ComboBox.h24px`/`.NumberSpinner.h24px`, each with exactly `{ minWidth: 0px, minHeight: 24px, maxWidth: none, maxHeight: 24px }` (row 4's contract) — coinciding at 24px under the real `ModernTheme`, unlike the offline fixture (see the row-3 note above). `NumberSpinner`'s inner `NumberSpinnerField` and `AutoCompleteField`'s inner `AutoCompleteTextField` are also opted in (both extend `TextField` and inherit the pin) but are borderless by class default, so they resolve a *different* height (22px) through their *own* per-class rules — `.NumberSpinnerField.h22px` / `.AutoCompleteTextField.h22px`, each also matching the row-4 contract exactly. `getComputedStyle` on every instance read back the height its class resolves to.
  - **This plan's Non-Goals, confirmed untouched:** `PasswordField` (×2) and `UsernameField` (×1) on `#/column` each still carry a real `{ minHeight: 24px, maxHeight: 24px }` on their own `#id` rule; `DateField`/`TimeField`/`DateTimeField` on `#/baseline` likewise each still carry a real 24px pair on their own `#id` rule (all three route through `AbstractPickerField.updateHeight`, which this plan does not touch). All rendered at the correct height with intact border/focus chrome — no visual regression.
  - The Style Audit's post-refresh scan (`#/style-audit`) still lists a duplicate `AbstractInput`-labelled `{ max-height: 24px; min-height: 24px; }` bucket; per the per-class-rule check above, that bucket is contributed by the still-untouched `DateField`/`TimeField`/`DateTimeField` (and, once `#/column` is visited, `PasswordField`/`UsernameField`) — never by this plan's three opted-in classes, which was directly confirmed rather than inferred from the bucket's label.
