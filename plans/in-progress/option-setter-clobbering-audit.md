...
**Step 8 (`TabBar.ts`) needed a second guard the plan didn't call for.** The plan's Step 8.4 instructed folding the caller's `border` option only inside `applyUnderBorder`'s *full-width* branch (`this.setBorder(options?.border ?? {...theme-token object...})`), leaving the early-return branch (`if (!this._underBorderFullWidth) { this.clearBorder(); return; }`) untouched. A contract-derived test written before this step (`new TabBar({ border: {...} }).getBorder()` should reflect the caller's value, per Expected Behaviour item 4) failed against that literal reading: the shipped Modern and Dark themes both default `tab.underBorderFullWidth` to `false` ([`ModernTheme.ts:110`](packages/lib/src/typescript/lib/core/themes/ModernTheme.ts#L110), [`DarkTheme.ts:107`](packages/lib/src/typescript/lib/core/themes/DarkTheme.ts#L107)), and `ThemeManager`'s own default is Modern ([`Theme.ts:1292`](packages/lib/src/typescript/lib/core/Theme.ts#L1292)) — so under the framework's actual default configuration, `applyUnderBorder(options)` takes the early-return branch regardless of what the caller passed, and its unconditional `clearBorder()` call still clobbers a caller-supplied `border` set moments earlier by the `super()` cascade's chrome dispatch. This is exactly the bug class this plan exists to fix, just one branch over from the one the plan named.

The fix that keeps: `applyUnderBorder` now checks `options?.border` first, before either branch — `if (options?.border !== undefined) { this.setBorder(options.border); return; }` — so a caller-supplied border wins outright at construction, and only when the caller passed none does the method fall through to the existing theme-driven full-width/cleared logic exactly as before. This is the same options-aware-fold technique the plan already established for this method (and for `TabButton`'s `backgroundColor`), just applied at the point that actually decides the value in both cases rather than only one. The three runtime call sites (`setUnderBorderFullWidth`, `setSide`, the theme-change subscription) all still call `applyUnderBorder()` with no argument, so `options?.border` is always `undefined` there and their behaviour is unchanged — verified by `grep -n "applyUnderBorder(" packages/lib/src/typescript/lib/component/container/TabBar.ts` showing exactly one call passing `options` (the constructor) and three passing nothing.
