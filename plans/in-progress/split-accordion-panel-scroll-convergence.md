...
**`Menu.ts`'s `padding` was not migrated — the plan's claim that `setInsets` writes through `StyleBag.padding` is false.** `Component.setInsets` writes `_options.insets`, a purely layout-internal field with zero direct CSS effect of its own (`grep -n "insets" core/ClassStyleRules.ts` finds no `insets` key in `StyleBag` at all); `StyleBag.padding` is a separate mechanism backed by `Component.setPadding`/`_instanceStyle.padding`, real CSS `padding`. They only *look* related because `getContentInsets()`/`getInnerSize()` both sum insets and padding together for layout math — a coincidence of that summed usage, not evidence `setInsets` is a `StyleBag`-backed setter. Worse, `Menu`'s own `getOrCreateResizeGutter`-adjacent positioning method (the one behind `show()`) independently re-asserts `this.setInsets(new Insets(4, gutter, 4, 0))` on *every* `show()` call, hardcoding the same `4` top/bottom a third time — unconditionally overwriting whatever `applyPersistentChrome`/`applyRebuildChrome` set at construction. Had `padding: new Insets(4, 0, 4, 0)` been added to `ownClassStyleDefaults` as the plan specified, `getContentInsets()` would then sum the new CSS `padding` (4px) *and* the still-hardcoded `insets` (4px) into an 8px top/bottom inset the moment any menu is shown — a real, silent visual regression, not a refactor-neutral change. `padding` is excluded from `_defaultMenuStyleDefaults`; only `borderRadius` (a confirmed, isolated `StyleBag` field with no such interaction) migrated. `setInsets(new Insets(4, 0, 4, 0))` in both chrome methods is unchanged from before this plan.

**`applyRebuildChrome`'s `backgroundColor`/`border`/`shadow` calls were kept, not deleted, contradicting the plan's literal Internal Structure instruction — the plan's own `ownClassStyleDefaults` snippet never actually declared them anywhere, so deleting them would leave a rebuild-mode (context) menu with no background/border/shadow at all.** The plan's prose ("rebuild's values are now the resting default") only makes sense if "resting default" means *rebuild's per-instance write acts as the value seen whenever `.persistent` isn't set* — exactly what keeping the calls, isolated automatically onto `#id:not(.persistent)` by `isRestingChromeIsolated` (since `backgroundColor`/`border`/`shadow` are also part of `.persistent`'s state bag), already achieves. This mirrors `SplitGutter`'s own `_expandedBackground` precedent (a per-caller resting value coexisting with a declared state) the same plan cites elsewhere. Only `applyPersistentChrome`'s three calls became `this.setStyleState(".persistent", true)`, as specified.

**`CollapseButton.ts`'s `.strip` width toggle was not migrated at all — `StyleBag` has no `width` field.** Confirmed against `core/ClassStyleRules.ts:44-93` (the same file the plan cites for every other field check in this phase): no `width` key exists anywhere in the interface, so `ownStyleStates`' `extract: (defaults) => StyleBag` cannot type-check a `{ width: ... }` return value, let alone apply it. This is the "existing pattern demonstrably cannot carry this case" branch of `pattern-conformance.md`, not a judgment call — `setStripMode` is left completely unchanged (`this.createStyleRule("").set("width", ...)`), and no `CollapseButton.classStyleHoisting.test.ts` was created, since nothing migrated to test.

**`ScrollStripArrowButton` needed `Button`'s own `_defaultOptions` seeded to the same values as its `ownClassStyleDefaults`, not just the class-tier bag — the plan's snippet omitted it, and without it `Button`'s own "2px ridge" border and default shadow/background leaked onto the arrow buttons' own `#id` rule at the *highest* possible specificity, completely defeating the class-tier override.** `Button.ownClassStyleDefaults` and `Button`'s own `_defaultOptions` are two independently-consulted tiers that happen to share one literal object (`_defaultButtonOptions`) only because `Button` deliberately forwards it to both; a subclass overriding one without the other leaves `_defaultOptions` — which a native `<button>` element's own UA-style-defeating per-instance write reads from — still pointing at `Button`'s values. `_defaultScrollStripArrowButtonStyleDefaults` is forwarded as `super(undefined, options, ...)`'s third argument. The first implementation pass hardcoded that bag directly with no `subclassDefaults` parameter on `ScrollStripArrowButton`'s own constructor to layer over it — a dead end `ARCHITECTURE.md`'s "Constructors forward `subclassDefaults`" rule calls out by name, and a real divergence from the `Scrollbar.ts` `ScrollArrowButton` / `TabCloseButton` precedent the code's own comment claimed to mirror "exactly"; the audit loop caught it (see below) and the constructor now takes `subclassDefaults?: Partial<ButtonOptions>` and spreads it over the class's own defaults, matching `TabCloseButton`'s identical shape. Separately, `border`/`shadow` use the literal string `"none"`, not `null` as the plan's snippet had it: `resolveDeclarations` truthy-gates both keys (`if (defaults.border) ...`, `if (defaults.shadow) ...`), so an explicit `null` resolves to *no declaration at this class tier* — letting `Button`'s own class-level border/shadow show through — rather than reproducing `clearBorder()`/`clearShadow()`'s actual written value (`"none"`, confirmed against their own bodies in `Component.ts`). Verified empirically via `RecordingDOMSink` writes before and after both fixes, not just by reasoning about the type signatures.

**`tests/diagnostics/StyleAudit.regression.test.ts`'s ceiling moved from 68 to 69, per its own "raise it only with a comment explaining why" rule.** `SplitGutter.setOpaque`'s migration isolates the pre-existing, deliberately-per-instance `_expandedBackground` write (see `## Architecture Decisions`) onto its own `#id:not(.opaque)` rule instead of folding it into the same combined write `applyCursor`'s direction-derived `cursor` used to share. The live gallery's two identically-configured (`'horizontal'`, default `expandedBackground`) `SplitGutter` instances still represent exactly *one* redundant pair of declarations either way — splitting a previously-combined body into two separately-tracked ones just makes two groups record that one pair instead of one group recording it, which is bookkeeping overhead, not new duplication. Confirmed by reverting only `SplitGutter.ts` and re-running the gallery test (passes at the old ceiling), then diffing the second instance's exact rule writes before/after the migration.

**`Menu.ts`'s new `_defaultMenuStyleDefaults` const, first placed between `Menu`'s own class-level JSDoc block and the `class Menu` declaration, silently detached that JSDoc from the class it documents — TypeDoc attaches a doc comment to the *next* declaration, so the const inherited Menu's real summary and `class Menu` fell through to `DismissableLayer`'s (the interface it implements) instead.** This corrupted the *published* `docs/api/overlay/classes/Menu.md` page (rendering `DismissableLayer`'s "a portaled overlay surface..." text as Menu's own description) and, as a downstream symptom, pushed `npm run docs:llms`'s site-variant manifest (`packages/docs/public/llms.txt`) 12 tokens over its 6940 budget, since the wrong (longer) summary flowed into its `Menu` catalog row — `npm run build:pages`, which `.github/workflows/docs.yml` runs on every push to `master`, would have failed outright. Caught by the audit, not by this plan's own Verification steps (`docs:llms`/`build:pages` are not among them; `npm run docs:api` alone doesn't fail on a misattributed-but-still-valid doc comment). Fixed by moving the const (with its own leading comment) to *before* Menu's JSDoc block, matching the established ordering `PopupPanel.ts`'s `_defaultPopupPanelOptions` already uses ahead of `PopupPanel`'s own class doc. Confirmed via a from-scratch build in a disposable worktree at this branch's start point (`2ab97955`): baseline is 6095/6931 tokens (root/site); after the fix this branch reproduces exactly 6095/6931 — zero net growth, `packages/lib/llms.txt` (the committed variant) unchanged.

**Manual smoke tests (per `## Expected Behaviour` and the plan's escape-hatch verify step) were run against a real browser** (this worktree's own dev server on a scratch port, driven via `chrome-devtools` MCP), not simulated: Accordion demo, resizable + multi-open mode, dragging a resize gutter via synthetic `mousedown`/`mousemove`/`mouseup` — the drag starts (`ts-ui-dragging` class + cursor applied), fires `sectionresize` with a real sizes payload, and ends cleanly with zero stuck state and zero console errors. Dialog demo — opened, then closed immediately (Escape) before its `afterNextLayout`-queued `resizeToContent`/`focusFirst` callbacks could fire — zero console errors on the frame those callbacks would have run on. Docs app — clicked "Show source" on a live demo block, then immediately triggered a client-side (non-reloading) navigation to a different page before the measurement callback's flush — zero console errors. The HBox `itemAlign` fix was *not* separately smoke-tested visually: no demo page happens to construct the exact non-zero-inset, mid-range-preferred-height combination the bug needs, and the fix already has a red-then-green automated regression pair pinning it exactly.
