...
One deviation from the plan's literal design, found during review rather than implementation:

**`TableHeader.reconcileColumnCells` orders its children from the reconciler's own `assigned` array, not by re-sorting on `Field.getOrder()`.** The plan's pseudocode ends the reconcile with `row.sortComponents(by constraint Field.getOrder())  // unchanged from rebuildCells`, carrying over what `rebuildCells` did. That is correct only while cells are built once per column in column order, which is exactly what this plan stops doing. `Field.getOrder()` returns a `-1` sentinel for every field that declares no `order` — the common case for a consumer-supplied model, and the shape the demo app's own `table-store.ts` uses. With every key tied at `-1` the comparison returns `0` throughout, `sortComponents` is a stable no-op, and a recycled cell keeps whatever array index it already held. Slot then stops mapping to column, so `getColumns()[s]` is no longer the cell for column `getColumnWindowStart() + s`. Everything downstream of that mapping misreads at once: `positionColumnCells` writes column `firstCol + slot`'s x and width onto whichever cell occupies the slot (header cells visibly misaligned with the body beneath them), `columnIndexOf` returns the wrong index so a resize drag resizes the wrong column, `syncSortIndicators` paints the arrow on the wrong cell, and `applyFocusedColumn` underlines the wrong cell. It only manifests once the window has slid, so no pre-layout or fits-the-viewport case catches it. The fix builds a `Map` from the `assigned` array the reconciler has already filled in slot order and sorts on that — a total order over distinct integers, which cannot tie. The plan's `## Potential Challenges` bullet "Slot order and DOM order diverge" is what licensed the original assumption: it asserts `sortComponents` reorders the children array into column order, which is true only when the order keys are distinct.

The companion body plan's `Row.setColumnWindow` inherited the same pseudocode and the same defect, and takes the same fix on its own branch; that plan's `## Implementation Notes` records it as its third deviation. Pinned here by two cases in `HeaderColumnWindow.test.ts` (27, 27b) that model twenty columns declaring no `order`, slide the window, and assert both the rendered field-name sequence and each cell's x against the column its own `getFieldName()` names — the latter keyed on the cell's field rather than its slot, since asserting per slot passes even when the wrong cell occupies the slot.
