...
- **`purgeComponent`'s map-iteration keys needed an explicit `String()` normalisation the plan's `## Implementation` snippet didn't show.** `listenerMap` / `subtreeListenerMap` / `viewportListenerMap` are declared `Map<String, Map<String, CompFunc>>` — object-wrapper `String` keys, even though every write passes a primitive (the same quirk `_registeredComponentIds`'s own comment already calls out). Destructuring `for (const [type, typeMap] of listenerMap)` therefore yields `type: String`, not `string`, and passing that into `Set<string>.add`, `uninstallBaseListener(type: string)`, or `captureOpts(type: string)` fails `tsc` with "Argument of type 'String' is not assignable to parameter of type 'string'". Fixed by wrapping each such use in `String(type)` (which returns the primitive), matching the normalisation `_registeredComponentIds` already does for the same reason. No behavioural change — `String(x)` on an already-primitive-backed key is a no-op conversion — confirmed by the full `Event.purgeComponent` / `_registeredComponentIds` test suite passing (E1–E5, R1–R6) and `npm run typecheck` finishing clean.

- **`MenuBarButton`'s R3 registry row (`dispose-listener-teardown.test.ts`) was red for a reason beyond the plan's own accounting, and that reason is expected, not a defect.** Before `Component.destructor()` called `Event.purgeComponent`, R3 failed with the button's own id still registered — not from its `click` / `mouseover` pair (its own destructor already unhooked those), but from `Tooltip.attach(this, str)` (`Button._rebuildTooltip`, fired because `MenuBarButton('File', …)` has truthy text), which registers `mouseover` / `mousemove` / `mouseout` / `mousedown` through `Event.addListener` on the same component id. `MenuBarButton`'s hand-rolled unhook never knew about those — only the id-wide central purge reaches them, which is exactly the gap this plan closes. The row went green once step 4 wired the purge into `destructor()`, with no test change needed. `Tooltip`'s own `attachments` map still leaks a stale entry for the same case; that gap is the plan's own named non-goal ("Purging `Tooltip.attachments`"), unaffected by this finding.
