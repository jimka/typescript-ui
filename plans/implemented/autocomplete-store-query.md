...
- **`getAll()` also bypasses the store's active sorters, not just its active filters — a consequence `## Architecture Decisions` (specifically `[^why-getall]`) reasoned about only for filters.** `store.getRecords()` ([AbstractStore.ts:1913-1933](packages/lib/src/typescript/lib/data/AbstractStore.ts#L1913-L1933)) applies the store's active sorters before returning `_records`; `store.getAll()` returns `_allRecords` in load order, unsorted. So beyond the filter-independence the footnote already argues for, a store-backed field's suggestions previously followed the store's active sort (via the old `getRecords()` read) and now list in the store's load order instead — and, once `maxSuggestions` truncates the list, a query with more matches than the cap can surface a different *subset* of records, not merely a different order. This is the same "independent of unrelated UI" rationale `[^why-getall]` already gives for filters, extended to sorters: a `Table` sorted elsewhere on screen must not silently reorder — or reshuffle the composition of — this field's suggestions. Pinned by a new test case ("ignores the store's active sorter") and called out in the component doc page and the changelog, both of which as originally written covered only the non-mutation guarantee (filters/sort/view/page untouched), not this consumption difference.
- **`npm run docs:api` finishes with 14 pre-existing warnings, not the zero the plan's `## Verification` describes.** Confirmed pre-existing by running `typedoc` against `master`: the same 14 warnings, none naming `AutoCompleteField` or any file this plan touches (they concern `SpatialNavigation`, `MarkdownViewer`, `MarkdownEditor`, and `FieldDecorator`). This branch introduces none of its own.
