...
**Manual verification (`## Verification`'s non-negotiable browser check) was performed** against a dev server started from this worktree (`npx vite --port 8017 --strictPort` from `packages/lib`, confirmed via `readlink /proc/<pid>/cwd`), driven live through `chrome-devtools` MCP tools, covering `## Expected Behaviour` rows 5-7 across all three shipped themes (modern, classic, dark) via the `packages/lib` demo app's Misc and Accordion panels:

- `PickerButton` (Misc panel, "Animated dropdowns:" row — the `DateField`/`TimeField`/`DateTimeField` trigger buttons) was confirmed at rest in all three themes via `getComputedStyle`: `background-color: transparent`, `background-image: none`, no border, `box-shadow: none`, with `color` correctly resolving to each theme's text token (`rgb(0, 0, 0)` modern/classic, `rgb(220, 220, 220)` dark). The `.PickerButton`/`.PickerButton.pressed` class rules were also read directly from the live document's `CSSStyleSheet` rule list and matched the plan's `## Expected Behaviour` rows 1-2 exactly.
- `AccordionHeader`'s title button (Accordion panel, "Personal Info" section) was confirmed the same way in all three themes, both with the panel's own `Themed: ON` and `Themed: OFF` toggle — `applySectionTheming`'s `setForegroundColor`/`clearForegroundColor` calls on the header component produced no difference in the title button's own computed style in any theme, confirming the Architecture Decisions' "orthogonal" claim.
- The Style Audit panel (`## Expected Behaviour` row 7) was checked after visiting Misc and Accordion in all three themes (255 total rules materialised, up from the pre-visit baseline): neither `PickerButton` nor `AccordionHeaderTitleButton` appears anywhere in its duplicate-rule table.

**Deviation: `:hover` needed the same pin as `.pressed`, contrary to `## Architecture Decisions`'s original conclusion.** Round 3 of the audit loop found that claim false: `chromeless`'s bare `#id` resting write used to outrank `.Button:hover:not(.pressed)` for `backgroundColor`/`backgroundImage`/`boxShadow` "for free," but once the resting write moved onto the lower-specificity `.PickerButton`/`.AccordionHeaderTitleButton` class rule (this plan's whole point), that suppression stopped happening automatically — both buttons picked up Button's generic raised hover chrome (background fill + shadow) for the first time, a real visible regression the manual verification above did not catch because it only exercised resting state. The fix adds a `:hover` entry to each class's `ownStyleStates`, pinning `hoverBackgroundColor`/`hoverBackgroundImage`/`hoverShadow` (added to `_defaultPickerButtonOptions`/`_defaultAccordionHeaderTitleButtonOptions`) to the same transparent/none values as resting — the same shape `WindowControlButton`/`TabCloseButton`/`MenuBarButton` already use for their own hover pins, rather than restating `Button.ownStyleStates[1]` unchanged as `## Internal Structure` originally prescribed. Both classes' test files gained hover coverage in the same priming test that already covered rows 1/2 (resp. row 4), since the module-level rule cache only materialises `.PickerButton:hover:not(.pressed)` / `.AccordionHeaderTitleButton:hover:not(.pressed)` once per test file, on the first construction.

This was re-verified live (not just re-documented): against a dev server started from this worktree on a spare port, confirmed via `readlink /proc/<pid>/cwd`, driven through `chrome-devtools` MCP tools. For both the Misc panel's first `DateField` trigger button and the Accordion panel's "Personal Info" title button, `element.matches(':hover')` was confirmed `true` (genuine browser hover engaged, not a synthetic event) in the same script evaluation that read `getComputedStyle`, which showed `background-color: transparent`, `background-image: none`, `box-shadow: none` — identical to the resting values and to the old chromeless behaviour, confirming the regression is fixed. This was checked in the modern theme only: `hoverBackgroundColor`/`hoverBackgroundImage`/`hoverShadow` are literal `"transparent"`/`"none"` constants, not theme tokens (unlike `WindowControlButton`'s `var(--ts-ui-window-control-hover-bg)`), so the result cannot vary by theme and re-checking classic/dark would add no information. (Automation note: the `chrome-devtools` `hover` tool's simulated `:hover` state proved to decay within roughly one further tool round-trip in this environment — most single hover-then-read attempts observed `matches(':hover') === false` by the time they ran — so confirming genuine engagement required retrying until a read landed inside the live window and pairing it with a screenshot taken in the same confirmed-hovered batch, which showed no visible highlight.)
