...
**`_applyFlatChrome()`'s rewrite needed a new `Component.cacheBorderSpec()` the plan's `## Internal Structure` didn't call for.** Deleting the per-instance `this.setBorder("1px solid transparent")` call, exactly as specified, leaves `Component`'s private `_border`/`_borderWidths` fields holding whatever `super.applyChromeOptions()` cached during the base construction cascade — `.Button`'s own raised `"2px ridge ..."` default — while the shared `.Button.flat` rule now paints a real `1px solid transparent` border. `getBorderSize()` (the layout-geometry read every `doLayout` pass depends on) would then report a stale 2px width against a 1px painted border. Worse, `BorderWidths.ts`'s `measureBorderWidths` caches its `getComputedStyle` result in a *module-level* map keyed by the border spec string, not by the measured pixels — so a flat button reusing the raised spec's cache key would either read back a wrong cached width, or (if it measured first) poison that shared entry for every raised, non-flat `Button` with the identical default spec. Fixed by adding `Component.cacheBorderSpec()`, a CSS-free sibling of `setBorder()` that updates the cached spec/width fields only, called from `_applyFlatChrome()` in place of the deleted `setBorder()`. It deliberately skips `setBorder()`'s theme-resubscription line: `Button`'s chromeful `applyChromeOptions()` always dispatches a real `setBorder()` during the base construction cascade before `_applyFlatChrome()` ever runs (flat is only reachable from the chromeful branch), so the per-instance theme listener that nulls `_borderWidths` on a theme change is already armed by the time `cacheBorderSpec()` runs — re-arming it would be redundant, not incorrect.

**`TableHeaderMenuButton`'s constructor could not wire `action` through the options-bag `listeners` field the plan's `## Internal Structure` showed.** `Button`'s own constructor only calls `this.applyListeners(options?.listeners)` when `Object.getPrototypeOf(this) === Button.prototype` — by its own doc comment, a subclass is expected to wire its listener bag itself, from its own constructor body, after `super()` returns. Passing `listeners: { action: onAction }` through to `super()` as specified is therefore a silent no-op once `TableHeaderMenuButton` exists as a real subclass (caught by `HeaderMenuButton.test.ts`'s real-DOM-dispatch case, which failed with zero calls). Fixed by dropping `listeners` from the options bag and adding `this.on("action", onAction);` directly after the `super()` call, the same pattern `PopupButton`/`MenuButton` already use for their own `action` wiring.
