...
- **Step 10's `ARCHITECTURE.md` edit contradicted itself as written, and was reconciled.** Step 10 replaces the "canonical pattern" snippet for a module-level shared class rule with the `deferStyleSheetWrite` form, unqualified, which reads as the recommended way to write *any* module-level shared class rule. The same step amends the "Module-level shared class rules" bullet under *Defer DOM work to render time*, about 75 lines further on, to say a shared rule is made by an `ensureXClassRule()` singleton called on first use, "never from the module's top level". Implemented literally, the document contradicted itself; the first audit round caught it as BLOCKING. The fix scopes each pattern to its own case rather than choosing one: `ensureXClassRule()` for a **new** shared rule, matching the existing precedent (`SortPriorityBadge.ts`, `ResizeHandle.ts` and the other singletons `ARCHITECTURE.md` already named), and `deferStyleSheetWrite` for an **existing** load-time write that must keep its position at the module's top level — the rule blocks, focus-ring calls and `@keyframes` this plan converts (for example `TextArea.ts:18`, `ComboBox.ts:393`). That is the plan's own reasoning ([^precedent], [^constructor-alternative]) made explicit, not a new design. A second round found the *Defer DOM work to render time* section's own two bullets still lacked the same qualifier; both were updated.
- **The Module load bullet says more than step 10 asked.** The new-versus-existing split alone would send a new control's focus ring to first use, which [^constructor-alternative] rejects, so the bullet states that a new ring is registered at the module's top level through the ring helpers, and that a new `@keyframes` is wrapped in `deferStyleSheetWrite` like the existing ones. It names `component/display/Glyph.ts`'s module-level `prefers-reduced-motion` listener as a second recorded exception beside `Body`, since this plan deliberately leaves it alone. And it says the queue holds stylesheet writes only, so the queue is not read as a home for any other `DOM.sink` write.
- **The flush isolates each queued write**, which the plan did not specify. The queue is drained before any write runs, so one write that throws would otherwise lose every write queued after it for the session, and the error would surface from whichever unrelated render triggered the flush — eagerly, the same throw only broke its own module's import. A failure is reported with `console.error` and the rest continue, following the layout flush's per-entry isolation (`reportFlushFailure` in `core/Component.ts`). E9 pins it. E8 was added too: E7 passes whether the flush sets its flag before or after running the queue, and E8 fails if the flag is moved after the loop.
- **The changelog entry departs from step 11's wording.** Step 11's counts were wrong: the rings cover twelve controls, not seven — it left out the three `registerFocusWithinRing` callers, covering `NumberSpinner`, `DateField`, `TimeField`, `DateTimeField` and `AutoCompleteField` — and 13 modules route shared rules or `@keyframes` through the queue, not "a dozen". Its "nothing renders differently" was also too broad. Rule order within the library's stylesheet is unchanged, but the `<style id="Base">` element is now added to `<head>` at the first real write rather than at import, so an app that injects a `<style>` of its own in between now has the library's sheet after its own. The entry says so.
- **One cosmetic departure.** The test file's config constant is `DEFER_TEST_CONFIG` rather than step 3's `CONFIG`.
- **Verification.** M2 was run in a real browser against this branch's dev server and `master`'s, after the same interactions: the 125-rule lists were identical. M1 was run only in part during implementation; a later hands-on check found the focus rings behaving as they should. M3 can only run after merge, but its substance was run before it: with Loom's `@jimka/typescript-ui` symlink pointed at this worktree's build, Loom's suite passed (22 files, 493 tests, including `tests/languages.test.ts`), and the symlink was restored afterwards. `docs:api` cannot finish with zero warnings as the plan asks, because `master` already has 14; this branch adds none.
