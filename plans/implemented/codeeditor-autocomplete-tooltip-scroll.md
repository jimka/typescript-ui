...
**`CodeEditor.isForeignWheelTarget` gates the `.cm-tooltip` match on the tooltip actually having scroll room, which this plan's `## Internal Structure` did not call for.** The plan-as-written returns `true` for any wheel whose target climbs to a `.cm-tooltip` ancestor, unconditionally. An audit round found this claims-without-preventing a wheel that has nothing to absorb it — every hover/lint tooltip (never scrollable) and a completion list already at its scroll bound — which defeats an enclosing overlay's wheel trap (`core/WheelTrap.ts`'s `swallowUnconsumedWheel`, installed by `Dialog`, `Drawer`, `Popover`, and `AbstractWindow`). The trap stands down on any wheel already marked consumed, on the assumption a claim means the gesture was handled; an inert claim leaves the trap disarmed while the browser's own scroll chaining carries the un-prevented gesture past it to whatever real scroll container it finds next — for a `CodeEditor` inside a modal `Dialog` or `AbstractWindow`, that can be the page behind the overlay. This is not a corner case: every `trapWheel` call site traps its whole subtree unconditionally, so any `CodeEditor` hosted in one of those four overlay kinds hits it as soon as the pointer rests over a non-scrollable tooltip, or over a completion list the user has scrolled to either end of.

The fix adds a module-level `hasWheelExtent(handle, e)` in `CodeEditor.ts`, mirroring `Component.onWheelScroll`'s own established idiom of claiming an axis only when it has somewhere to go: `canX`/`canY` there is overflow style conjoined with remaining extent (`isOverflowScrollable(...) && getMaxScrollLeft() > 0`), and `hasWheelExtent` applies the same two-part test to foreign DOM — `DOM.source.getComputedOverflow(handle)` for the style half, `DOM.source.getScrollMetrics(handle)` for the extent half, per axis. (A first pass at this fix checked extent only; a second audit round found that admits content which merely overflows a non-scrolling box — an ellipsised completion row's `overflow-x: hidden` reports `scrollWidth > clientWidth` without being scrollable — back into the same failure this note opened with, so the overflow-style half was added.) `isForeignWheelTarget`'s climb records the walked handles and, at the `.cm-tooltip` match, returns whether any of them satisfies `hasWheelExtent(handle, e)` — not an unconditional `true`. A non-scrollable tooltip, an overflowing-but-not-scrolling one, or a completion list at its bound, now reports `false` and falls through to the normal claim-and-prevent path (the editor's own eased scroll takes it, or nothing does if the editor has no extent either) — every such outcome leaves any enclosing `WheelTrap` correctly armed. `Component.ts`'s hook and call site are unchanged; the fix is confined to `CodeEditor`'s override.

This also corrects three rows of the `## Architecture Decisions` routing table (`:38-44`). Row 4 ("a hover/lint tooltip … claimed, not prevented … the browser chains to `.cm-scroller` and the document scrolls natively rather than eased") is wrong on two counts: `.cm-scroller` is a sibling of `.cm-tooltip` under `.cm-editor`, not an ancestor, so it was never actually reachable by native chaining from inside a tooltip; and post-fix, a hover/lint tooltip is never treated as foreign at all (it has no scrollable descendant), so the wheel claims and prevents normally — an eased scroll of the document, when the document has extent — the same outcome `## Non-Goals` already gave for `.cm-panels`, just reached through the extent check rather than a selector exclusion. Rows 3 and 5 ("a completion list row" / "…with an `autoScroll` `Panel` hosting the editor" → `true`) are no longer unconditional: both now read `true` only while the list has scroll room left in the wheel's direction, and fall to the same claim-and-prevent outcome as row 4 once it doesn't. The `## Non-Goals` bullet on `trapWheel` / `untrapWheel` ("the tooltip is not portaled and its wheels are claimed, so there is nothing to trap") is superseded by this note: nothing new needed trapping because a room-less tooltip's wheel is no longer claimed at all, not because a claimed wheel was already safe.

Six tests were added beyond the plan's `## Expected Behaviour` list to pin this, all in `packages/lib/tests/component/code-editor.test.ts`: `isForeignWheelTarget` (1) returns `false` for a `.cm-tooltip` match with no scrollable descendant at all; (2) at a scrollable list's bottom bound, returns `false` for a further downward wheel but `true` for an upward one; (3) at its top bound, the reverse; (4) is direction-aware horizontally too, at both the left and right bound; (5) returns `false` for content that merely overflows a non-scrolling box (the ellipsised-row leak a second audit round found in a first, extent-only pass at this fix); and (6) `onWheelScroll` falls through to claim-and-prevent when the matched tooltip has no room. Each of the four clauses in `hasWheelExtent`'s per-axis, per-direction check has at least one assertion that fails if that clause alone is removed — a third audit round found the first draft of these tests exercised only one of the four. The plan's original behaviour 1 and 5 tests were extended to stage a scrollable, `overflow-y: auto` `ul` inside the tooltip chain, since a room-less default no longer satisfies them. `tests/dom/TestDOM.ts`'s `makeEvent` gained an optional `deltaX` field (additive, no existing call site affected) so the horizontal cases could be expressed.

The climb in `isForeignWheelTarget` collects the walked handles into an array and evaluates `hasWheelExtent` over them only once a `.cm-tooltip` match is found, rather than folding it in at every step. A third audit round found the eager version made every wheel tick — including the common case of no tooltip at all — pay a `getComputedStyle` and a layout-forcing `getScrollMetrics` read per climbed handle, contradicting `## Potential Challenges`' own cost claim that the climb is "a small constant number of [`matches`] calls — far cheaper than the `getScrollMetrics` reflow `onWheelScroll` already performs on the same tick." Deferring the extent checks to the match branch restores that claim exactly: an ordinary wheel over editor content now costs only the `matches` climb, unchanged from before this note.

The plan's `## Verification` grep check ("`grep -rn "isForeignWheelTarget" packages/lib/src/` — exactly three hits") now returns four: `hasWheelExtent`'s doc comment carries one `{@link CodeEditor.isForeignWheelTarget}` cross-reference (`packages/lib/src/typescript/lib/component/editor/CodeEditor.ts:164`), sanctioned by `CODE_CONVENTIONS.md`'s "the link inside an internal symbol's own JSDoc is fine" (both symbols are non-public). The declaration, the call site, and the override the original check named are still exactly three of the four; the fourth is a comment mention, not a second implementation.

A fourth review, closing the audit loop's iteration cap, found two of the tests above did not actually discriminate the behaviour they claimed to pin. `"stops the climb at the editor's own element"` returned `false` regardless of whether the `handle === root` stop ran, because the falsely-matched `.cm-tooltip` ancestor reachable only by climbing past that stop had no scroll room mocked in, so `hasWheelExtent` answered `false` either way; the fix gives that ancestor real scroll room so the two code paths diverge. `hasWheelExtent`'s `overflowY` gate had no vertical counterpart to the existing `overflow-x: hidden` ellipsised-row case, so deleting `scrollableY &&` from both vertical clauses left every test green; a new `overflow-y: hidden`-with-extent case mirrors the horizontal one. Manual verification (`## Verification`, steps 3-5) was then carried out live against this worktree's dev server: dispatching a `WheelEvent` at a genuinely overflowing completion list (`scrollHeight`/`clientHeight` 583/140, reached by filtering local `cat0`…`cat30` bindings past the tooltip's `10em` cap) left `event.defaultPrevented` `false` and `.cm-scroller`'s `scrollTop` unmoved, while the same dispatch over ordinary editor content and the gutter reported `defaultPrevented` `true` and drove the eased scroller in both directions — matching behaviours 8-10 exactly. Step 6 (a `CodeEditor` nested inside a scrolling `Markdown` fenced block) was not independently reproduced live in this session — the demo's fenced editors did not accept programmatic focus and typing — but the mechanism it exercises (an ancestor's own `onWheelScroll` seeing `consumeWheel` already return `false`) is exactly what `tests/core/PanelScrollChaining.test.ts` pins, and that suite is unchanged by this fix and still green.
