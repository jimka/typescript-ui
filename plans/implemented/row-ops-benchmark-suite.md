...
- **`benchTablePoolGrow`'s `meanMs` is near zero, not "well clear of zero" as `## Expected Behaviour` case 13 predicts.** Live-browser verification (`await bench.benchTablePoolGrow()` at the default 10,000 rows) confirmed the fix itself works — `await store.load()` does populate `store.getRecords()` before the loop runs — but the method's synchronous `performance.now()` window around `table.setSize(...)` still measures near-nothing (`mean=0.005ms`). Direct instrumentation showed why: `setSize` on an already-appended, already-first-laid-out `Table` schedules its relayout via `requestAnimationFrame` rather than running it synchronously — in a standalone repro, the row count stayed flat across the synchronous call and only grew after one `requestAnimationFrame` tick — so the pool-grow work this method's own JSDoc describes measuring happens entirely outside its timed span. This is a pre-existing characteristic of the unmodified method, not something introduced by this plan's `await store.load()` addition: an unloaded store would also read near-zero, but for the unrelated reason that there is nothing to bind. Per `## Non-Goals`, the five existing benchmarks get no changes beyond return type, `summarize` routing, and the two `store.load()` calls, so no `requestAnimationFrame` wait was added here; correcting the measurement window is separate follow-up work. One practical consequence worth flagging: `tablePoolGrow`'s seeded baseline entry (`meanMs: 0.005`) is small enough that `compareToBaseline()`'s 15% threshold trips on ordinary timer-quantization noise for this op specifically (observed `0.005ms → 0.025ms`, a `+400%` warning, during an otherwise-normal repeat run) — a limitation of this one baseline entry, not of the regression-check machinery, which was separately verified correct via a controlled halved-baseline trial on `tableScroll` that produced a single `+98.5%` warning as expected.
- **Case 11's "no regression warnings on an idle machine" was verified on a busy machine, not an idle one.** This session's host had several concurrent dev servers and agent processes running throughout verification, so repeat `compareToBaseline()` runs occasionally warned on ops other than `tablePoolGrow` (e.g. `storeAppend`, `recordUpdate`, `componentInit`) with deltas in the 16–50% range. Every observed warning's `delta=` matched the `(current / baseline - 1) * 100` formula exactly, confirming the regression-check logic itself is correct; the warnings are machine jitter of exactly the kind `## Potential Challenges` already anticipates ("Numbers drift with machine state... re-run a warning once before believing it").
