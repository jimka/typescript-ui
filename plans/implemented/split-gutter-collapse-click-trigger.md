...
- **`## Expected Behaviour` items 7 and 8 turned out to be unit-testable offline, contradicting the plan's rationale for marking them manual-verify.** The plan classified them "Not unit-testable — requires a real browser", on the premise that "the offline recording DOM sink does not dispatch `click`/`dblclick` to the window-level capture handler" (citing a comment at the then-current `CollapseButton.test.ts:63-67`). That premise was wrong: `Event.fireEvent(component, makeEvent(handle, type))` routes through the recording sink's modelled window-level dispatch (`RecordingDOMSink.dispatchEvent` in `tests/dom/TestDOM.ts`), the same mechanism `tests/component/input/Link.test.ts` and `tests/component/diagram/DiagramView.test.ts` already use to test real DOM-event wiring offline — the stale comment the plan cited predated this plan and was itself inaccurate. An audit round caught that the initial implementation, following the plan, left the actual DOM-event-type wiring (which of `click`/`dblclick` `CollapseButton` registers, per `trigger`) untested — the only tests written covered `getTrigger()` echoing its constructor argument and the tooltip text, neither of which observes the `Event.addListener` call itself. A test was added as the first `describe` block in `CollapseButton.test.ts` (first, per a documented ordering constraint on `Event`'s window-level base listener surviving `DOM.reset()` — see the file's own comment) that dispatches real `click`/`dblclick` events via `Event.fireEvent`/`makeEvent` and asserts `collapse` fires only on the configured trigger, in both trigger modes. This automatically covers items 7 and 8; the stale comment the plan cited was corrected as part of the same fix. Items 9-11 remain genuinely manual-verify as the plan describes — drag suppression, a full `Split` end-to-end collapse animation, and rapid re-toggle behaviour are still beyond what the offline harness models.
