...
Step 9's literal instruction (`const width = this._contentFitWidth;`) was implemented exactly, but doing so exposed a gap the plan did not anticipate: four pre-existing tests in `Menu.test.ts` (`Menu open() — horizontal placement`) construct a persistent `Menu` and call the inherited `Component.setWidth()` directly on it before `open()`, using that as a test-only shortcut to pin a deterministic width for horizontal-placement math. Once `open()` always re-derives its width from `_contentFitWidth` (as the plan specifies), `applyScrollbarGutterWidth` overwrites that pinned value on every open — which is correct per the established precedent (rebuild mode's `showAnchored` already overwrites `getWidth()` from `_contentFitWidth`/`_menuWidth` on every `show()`, so a bare `setWidth()` pin was never respected there either), but it changed the width these four tests asserted against.

An earlier draft of this step instead kept `this.getWidth()` and added a heuristic (`isExplicitWidth = width !== this._contentFitWidth`) to guess whether the width had been "explicitly" pinned, skipping the gutter re-evaluation when it looked explicit. That heuristic has no precedent in this codebase and is wrong on repeat opens: after the first overflowing `open()`, `getWidth()` permanently differs from `_contentFitWidth` (it now includes `TRACK_WIDTH`), so the heuristic misreads every subsequent open as "explicit" and never re-evaluates the gutter again — the widening becomes sticky forever, exactly the failure mode the plan's "never sticky" cases (6, 7, 10) exist to prevent. It was reverted in favour of the plan's literal instruction.

The four affected tests were updated instead: two assertions never actually depended on the pinned value (the non-flipping placements), so the pin was simply dropped; the two flip-threshold tests now read the natural width directly off the same `Menu` instance immediately after construction — `buildPersistentItems` already commits `_contentFitWidth` as `getWidth()` at construction time, before any `open()` call — and compute their expected coordinate from that measured value instead of a hardcoded pin.
