...
- **Sidebar labels are hand-authored, split from `DocPage.title` (audit fix).** The plan carries a latent contradiction: `## Expected Behaviour` requires `getPage().title` to be a page's first `# ` heading, while step 7 requires the sidebar to use the config.mts titles — and for three pages those differ (`/guide` heading "Getting Started" vs config "Introduction"; `/concepts` heading "Concepts" vs config "Overview"; `/concepts/dom-seams` heading ``DOM seams (`DOMSink` / `DOMSource`)`` vs config "DOM seams", the heading also carrying inline-code backticks). The first implementation reused the single `DocPage.title` for both, so the tree rendered the wrong labels for those three and leaked literal backticks on the DOM-seams entry. Resolved by giving the nav its own hand-authored label: a new `NavEntry { path; label }` type, `NavGroup.pages` retyped from `DocPage[]` to `NavEntry[]`, `getNav()` copying each `label` verbatim from config.mts, and `DocsSidebar` rendering `page.label`. `DocPage.title` stays the first `# ` heading, so `getPage`'s `## Expected Behaviour` is unchanged. This is a deviation from `## Public API`'s `NavGroup { pages: DocPage[] }` declaration, forced by that section contradicting step 7; a new `pages.test.ts` case pins the labels to the config.mts titles and asserts no label contains a backtick.
