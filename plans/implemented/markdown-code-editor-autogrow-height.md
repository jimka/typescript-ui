...
- **`on`/`off`'s implementation signature widened to `listener: Function`, not `listener: (payload: CodeEditorChange) => void`.** The **Public API**/**Internal Structure** sketches show only the three overload signatures, not the implementation signature beneath them. Writing it as literally `(payload: CodeEditorChange) => void` (the pre-existing implementation type) fails to typecheck once a third overload (`"heightchange"`, payload `CodeEditorHeightChange`) is added: TS2394 rejects an overload whose listener payload type is neither identical to nor a subtype of the implementation's declared payload type, and `CodeEditorChange`/`CodeEditorHeightChange` share no such relationship. Rather than union the payload type (which produces the same TS2394 failure, just relocated to the `"change"` overload), the implementation now types `listener` as `Function`, matching the established codebase pattern for an `on`/`off` surface with more than one distinct non-empty payload shape (`AutoCompleteField.ts`, `TextInput.ts`, `Header.ts`, `Cell.ts`, `Tree.ts`, `Scrollbar.ts` all do this). `emit`'s overloads needed no equivalent change — its implementation signature was already able to take an optional `payload?: CodeEditorChange | CodeEditorHeightChange` without a TS2394 conflict.

- **Two files outside the `## Files to Create / Modify / Delete` table were touched.** `packages/lib/src/typescript/MarkdownPanel.ts` (the lib's own demo gallery panel) was extended per `implement/worker.md` step 7 ("Add a demo/example of the new feature... if it has one") — its `typescript` sample block was lengthened past 20 rows to show the row-cap-with-vertical-scroll behaviour, and a second, short `sql` block with one long line was added to show the horizontal-scrollbar-reserve behaviour: an over-cap block's height is already pinned by `Math.min(desired, capPx)` regardless of the reserve (the reserve only changes the outcome when it's what pushes an *otherwise-under-cap* `desired` past the cap — see the round-2-audit-added `code-editor.test.ts` case), so a single combined block cannot demonstrate "the box grows taller for both" — only a short, under-cap block with a long line can, and the plan's own manual-verify step (Ordered Implementation Steps, step 11) already describes the over-cap-plus-long-line case differently ("stays capped... with both scrollbars working inside it") from the under-cap-plus-long-line case ("the box measurably taller than the no-scrollbar case"). An earlier version of this note (and the long block's inline comment) conflated the two; both are now corrected to match. `packages/lib/src/typescript/lib/component/editor/index.ts` had `CodeEditorHeightChange` added to its type re-exports — mechanically required, not optional: the interface is `{@link}`-ed from public JSDoc (`CodeEditor.ts`'s `on`/`off`/`emit` heightchange overloads) and CODE_CONVENTIONS.md's link rule requires `{@link}` targets to be part of the public docs, so `npm run docs:api` warns without it (confirmed: the warning appeared and cleared when this export was added).

- **Footnote `[^cm6-resize-observer]`'s citation is inaccurate.** The `this.resizeObserver = … measureSoon()` line it quotes (`node_modules/@codemirror/view/dist/index.js:10300`) belongs to CM6's tooltip plugin, not `EditorView` itself; `EditorView`'s own resize path is `DOMObserver.resizeScroll` (`index.js:7152-7158`), which explicitly *suppresses* its callback within 75ms of the view's last update (`if (this.view.docView?.lastUpdate < Date.now() - 75) this.onResize()`) — a window `syncAutoHeight`'s own `setHeight()` call, issued from inside the update listener, always falls inside. This does not leave the design broken: `syncAutoHeight` is also called explicitly and synchronously at the end of `mount()` (Internal Structure, and `CodeEditor.ts`'s post-mount call), and CM6's own constructor schedules an initial `requestMeasure()` independent of `resizeScroll`, so the first real measurement still lands via the update listener shortly after mount. The footnote's specific claim that `CodeEditor`'s *own* `setHeight()` calls "are picked up and re-settled" by that observer is left uncorrected in the footnote text itself (plan-file edits are limited to this Implementation Notes section per `implement/worker.md`'s "Deviating from the plan"), but the underlying design it argues for is sound for a different, real reason than the one it cites.

- **Manual verification (Ordered Implementation Steps, step 11) — corrected record.** An earlier version of this note said this was checked in "the docs app (`npm run docs:dev`)"; that is wrong — `packages/docs` has no `Markdown`/`CodeEditor` demo. It was actually run against the lib's own demo gallery (`npm run dev`, the `MarkdownPanel` tab, `packages/lib/src/typescript/MarkdownPanel.ts`). Live-verified there: the 20-row cap with vertical scroll (first sample block) and the horizontal-scrollbar reserve growing a short block's box with the last row staying visible above the bar (second sample block) both settle to the values `syncAutoHeight`'s formula predicts (host height equals `capPx`; `.cm-scroller`'s `clientHeight` equals `offsetHeight` minus the measured scrollbar width), with no visible flash or clipping — confirmed by directly narrowing/widening a mounted `CodeEditor`'s own root element and observing CodeMirror's scroller geometry and `syncAutoHeight`'s reaction update correctly. The width-driven re-settle case (step 11's "resize the browser window … confirm a block's height re-settles") was attempted and **not confirmed**: resizing the demo window does reach `Markdown`'s own width (its root element visibly resizes), but the already-mounted `CodeEditor`'s width, resynced by the pre-existing `Markdown.syncCodeEditors()` (unchanged by this plan, shipped in `markdown-code-editor-highlighting`), was observed to lag one resize cycle behind — each resize's `syncCodeEditors()` call appears to read the wrapper's `clientWidth` before that same `setWidth` call's own buffered width write is committed to the live DOM, so the editor is resized to the *previous* resize's width, not the current one. This looks like a real, pre-existing timing gap in `syncCodeEditors`/`measureContentHeight`'s call ordering (`syncCodeEditors` runs before `measureContentHeight`'s own `commitElementStyle()` flush) — not a defect in this plan's own `syncAutoHeight`, which reacts correctly to whatever width it is actually given (verified directly, above). Fixing it would mean changing `markdown-code-editor-highlighting`'s already-shipped code, which is out of scope here; flagged for the user's attention rather than fixed.
