...
`tests/component/chart/Chart.test.ts` is **not** modified — its existing test is the primary gate and stays as-is.
...
- **Charts fix (steps 1–5) shipped exactly as planned.** `entriesEqual`/the `setEntries`/`setOrientation` guards, the `expectNoSelfReschedule` helper, and the ChartLegend idempotency tests all match the plan's `## Internal Structure` verbatim. The pre-existing red regression test at `tests/component/chart/Chart.test.ts` went from "Number of calls: 2" to green with no changes to the test itself.
