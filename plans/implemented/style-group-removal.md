...
Step 5's terminal check ("`grep -n 'roup' packages/lib/src/typescript/lib/core/Component.ts` — exactly one match should survive") undercounts. Three references to the unrelated, still-implemented `plans/cross-class-style-groups.md` (the trait-tier plan; lines ~539, ~1940, ~1956 after this plan's edits) also match `roup`, because that filename contains "styleGroups" as a substring. These citations are correct and out of scope — they document the trait mechanism's own history, not the removed `styleGroup` mechanism — so they were left unedited rather than forced to satisfy the literal grep count. The actual objective (no dangling `styleGroup`/`_groupLayer`/`ensureStyleGroupRule`/`styleGroupClassSuffix`/`_groupBags` reference) was verified directly instead, and holds.

Row 8's required manual browser verification (`## Verification`) was performed against a dev server serving this worktree (confirmed via `readlink /proc/<pid>/cwd`), on port 8016. The Misc tab's "Show window with table (slow)!" demo was opened; `getComputedStyle` on the rendered header menu-button glyph read `width`/`height`/`minWidth`/`minHeight` of `8px`, its `className` was `ts-ui-component ButtonIconGlyph ts-ui-trait-table-header-menu-glyph`, and the live stylesheet's only rule naming `table-header-menu-glyph` was `.ts-ui-component.ts-ui-trait-table-header-menu-glyph { min-width: 8px; min-height: 8px; max-width: 8px; max-height: 8px; }` — no `ButtonIconGlyph--table-header-menu-glyph` selector existed. The Style Audit tab rendered only its title, description, Refresh button, and the results table (no button row above it); clicking Refresh produced no row or text containing `warning-demo` or `ButtonIconGlyph--table-header-menu-glyph`.
