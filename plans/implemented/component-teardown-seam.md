...
Fixed by: moving all fourteen remaining `dispose()` overrides onto `protected destructor()`, ending each in `super.destructor()` (the public `dispose()` they came from stays inherited, unoverridden, so no external call site changes); adding a layout-manager-detach step to `Component.destructor()`, resolved directly against `_options.layoutManager ?? _defaultOptions.layoutManager` and gated on `getContainer() === this` (never through `getLayoutManager()`, whose lazy-attach branch would re-attach, and then immediately re-detach, a manager that was never actually in use — breaking idempotency on a second `destructor()` call) and ordered *before* the element is removed, because `Accordion.detach()` (a second `LayoutManager.detach()` override, alongside `Tab`'s) re-parents live content through `container.getElement()`, which would otherwise hit `getElement()`'s create-on-demand path against a component whose `_element` field the next few lines are about to clear; and deleting `Menu.dispose()` outright rather than relocating it — its body was a manual loop re-disposing `_menuItems`, guarded to persistent mode only via `assertPersistentMode`, which would have made ancestor teardown throw for a rebuild-mode menu (`Table._columnContextMenu`, `TabBar._contextMenu`) the moment the loop moved onto the always-reached `destructor()`. Both modes register every item via `addComponent` (`showAnchored` / `buildPersistentItems`), so the base class's recursive teardown already disposes them once each item's own cleanup lives on `destructor()` — the loop, and the guard that made it throw, were fully redundant rather than needing a guard rewrite.
