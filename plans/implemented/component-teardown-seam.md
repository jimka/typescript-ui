...
**Three pre-existing tests had assumptions invalidated by the fuller `dispose()` contract and needed narrow updates**, not because they were wrong about the feature they test, but because `dispose()` now does more — or reaches its target differently — than they assumed:
...
- The same file's store-binding "dispose unbinds the store" test spied on `_legend.dispose` to confirm the internally-owned legend is torn down. That spy was never invoked: `_legend` is a registered child, so its teardown is reached exclusively through `super.destructor()`'s recursive `child.destructor()` call, never through `_legend`'s own public `dispose()`. The spy was retargeted to `_legend.destructor` (a fourth-review-cycle fix, documented below alongside the double-teardown removal it accompanies) to match how teardown actually reaches a registered child.

**`ScrollStrip` was missing from the `tests/component/dispose-full-teardown.test.ts` registry, and step 12's mandated stop-and-report on a count mismatch did not happen.** `ScrollStrip.dispose()` (added above, for the `_clip` / `_leadArrow` / `_trailArrow` fix) is a real `Component`-subclass override and belongs in the registry the same as the other fifteen; it was never added, and the registry-vs-source count was never re-checked against step 12's grep before this plan was marked implemented. Fixed in review follow-up: added a `ScrollStrip` row (with `_clip` / `_leadArrow` / `_trailArrow` as `extraSubtrees`, since all three are raw-appended outside `_components` the same way `TabBar`'s chrome overlays are) and a matching `TabBar.dispose()`-style scenario test in `tests/core/ComponentDispose.test.ts` that attaches a live item and forces the paging arrows open via `layoutContent()` before disposing, so the assertion covers recursion into `_clip`'s contents and not just `_clip` itself — verified to fail when `_clip.dispose()` is removed, then restored. Re-running step 12's grep (`^\s*dispose(` in `src/typescript/lib`) returns 18 hits; excluding `Component.ts:706` (the base definition, per step 12's own exclusion), `Component.ts:726` (a local variable call, not a definition), and `core/StyleTarget.ts:298` (`StyleRule.dispose()` — an unrelated class hierarchy that never constructs a `Component` and so isn't eligible for this registry) leaves 15 real overrides, matching the registry's 15 rows after the fix. The two counts agree; no further stop-and-report is warranted.

**`## Documentation Impact`'s directive — add a `dispose()` row to the "Subclass hooks" table and switch the `performance.md` / `troubleshooting.md` samples to override `dispose()` — was not followed, and the deviation was never recorded here.** An earlier docs pass did literally follow that directive; commit `c43f5814` ("Restore destructor() as the documented dispose override hook") reverted it once review established why it was wrong: `Component.destructor()`'s child recursion calls `child.destructor()`, never `child.dispose()`, so a sample (or any real subclass) that overrides only `dispose()` has that override silently skipped whenever it is torn down as a registered child — the normal case for anything added via `addComponent`. `component-lifecycle.md`'s table therefore keeps `destructor()`, not `dispose()`, as the one documented override hook; `performance.md`'s sample shows `Text` needing no override at all (its own `subscribeTheme` migration already made it self-cleaning) plus a second example for a component held only in a field, cleaned up via a `destructor()` override; `troubleshooting.md`'s sample — exactly that held-only-in-a-field case — is likewise `destructor()`-based. The final state is correct; what was missing was the record of why it diverges from this section, which an unrelated reviewer reading only the plan would otherwise have no way to reconstruct.

**A later review confirmed that same defect — cleanup living on `dispose()` instead of `destructor()` — was still live in the code, not just in the docs the pass above already fixed**, and found two further gaps in the same seam. First, `ScrollStrip.dispose()` (added by this plan) and the other fourteen surviving `dispose()` overrides were exactly the pattern `c43f5814`'s docs fix described as broken: none of them ran when the owning component was torn down as a registered child, only when `.dispose()` was called on it directly. Second, `Component.destructor()` never detached the component's layout manager, so `Tab.detach()` — `_bar.dispose()`'s only call site, and the one path that reaches the raw-appended `TabBar` at all (`Tab.attach()` appends it straight to the container element instead of registering it as a child) — was reachable only from `setLayoutManager` replacing the manager, never from the container's own teardown. A `Tab`-managed container's whole strip subtree (`TabBar` → `ScrollStrip` → tab cells) therefore leaked on every close, exactly the leak class this plan exists to close, and exactly what the plan's own skipped manual check (`## Expected Behaviour`, "Manual verification") would have caught.

Fixed by: moving all fourteen remaining `dispose()` overrides onto `protected destructor()`, ending each in `super.destructor()` (the public `dispose()` they came from stays inherited, unoverridden, so no external call site changes); adding a layout-manager-detach step to `Component.destructor()`, resolved directly against `_options.layoutManager ?? _defaultOptions.layoutManager` and gated on `getContainer() === this` (never through `getLayoutManager()`, whose lazy-attach branch would re-attach, and then immediately re-detach, a manager that was never actually in use — breaking idempotency on a second `destructor()` call) and ordered *before* the element is removed, because `Accordion.detach()` / `Split.detach()` (both `LayoutManager.detach()` overrides, alongside `Tab`'s) resolve their own header/panel-wrapper/gutter components' elements through `getElement()`'s `getElementById` fallback, which only succeeds while the container's own element is still connected to the document — removing that element first would detach the whole subtree and break those lookups (see the correction below re: the originally-stated reason); and deleting `Menu.dispose()` outright rather than relocating it — its body was a manual loop re-disposing `_menuItems`, guarded to persistent mode only via `assertPersistentMode`. Both modes register every item via `addComponent` (`showAnchored` / `buildPersistentItems`), so the base class's recursive teardown already disposes them once each item's own cleanup lives on `destructor()` — the loop was fully redundant (see the correction below re: the originally-stated reason for the guard).

**Correction (third review cycle) — two of the reasons given just above, and in commit `22d3b3c0`'s message, are wrong; the decisions they justify are still correct.** First, the claim that `Menu.dispose()`'s loop, if relocated onto `destructor()`, "would have made ancestor teardown throw for a rebuild-mode menu (`Table._columnContextMenu`, `TabBar._contextMenu`)" describes a path that cannot occur: both fields are plain fields, and no `Menu` anywhere in the library is ever passed to `addComponent` — so an ancestor's recursive `destructor()` can never reach a `Menu` at all, let alone trigger its guard. The loop was redundant for a simpler reason: every `MenuItem` / `MenuSeparator` is registered via `addComponent` in both modes (`Menu.ts:293`, `Menu.ts:900`), so the base class's recursive teardown already disposes them regardless of whether `Menu` itself is ever torn down as someone else's child. Second, the claim that the layout-manager-detach step must precede element removal because `Accordion.detach()` "re-parents live content through `container.getElement()`" is unreachable as stated: the child-destruction loop immediately above in `Component.destructor()` already sets `_components = []`, so by the time `Accordion.detach()` runs, `container.getComponents()` (`Accordion.ts:1136`) is always empty, `components[i]` is `undefined`, and the guarded `container.getElement()!` (`Accordion.ts:1140`) never evaluates. The ordering is still correct, for the reason now recorded in `Component.ts`'s comment and above: descendant components resolve their own element via `getElement()`'s `getElementById` fallback when their `_element` field is uncached, and that fallback only resolves while the container's element is still connected to the document (`Accordion.ts:1143-1144`, `Split.ts:1033`).

Two tests were added test-first, per this plan's own precedent (`## Ordered Implementation Steps` 1-2), to `tests/core/ComponentDispose.test.ts`: one builds a plain parent/child pair with a synthetic `destructor()` override and asserts the child's cleanup ran when the parent is disposed (this already passed before the code change above — the child-recursion mechanism itself was never broken, only the fifteen classes whose cleanup lived on the wrong method were — so it stands as a regression guard for the contract the rest of this fix now depends on); the other builds a `Container` using a live `Tab` layout manager with a real tab, disposes the *container* (never calling `Tab.detach()` or `bar.dispose()` directly), and asserts zero surviving rule-cache keys across the `TabBar` / `ScrollStrip` / tab-cell subtree — this one failed red before the layout-manager-detach fix, exactly reproducing the leak this review found, and is the test the plan's skipped manual check should have been backed by from the start. `tests/component/dispose-full-teardown.test.ts`'s registry and header comment were updated to match: 14 rows now cover real relocated `destructor()` overrides, and the `Menu` row — no longer an override at all — stays as coverage for the ancestor-recursion contract its own item disposal now depends on entirely.

**A fourth review cycle found that three of the fourteen relocated `destructor()` overrides — `AbstractChart`, `VideoPlayer`, `MarkdownEditor` — still explicitly disposed a field (`_legend`, `_video`, `_codeEditor` respectively) that was ALSO registered via `addComponent`, so `super.destructor()`'s recursion below ran that child's `destructor()` a second time.** This is the same double-teardown hazard `26c70437`'s message names, just missed by the registry/enumeration bookkeeping above at the time it landed. The three redundant explicit calls were removed (`AbstractChart.ts:1090`, `VideoPlayer.ts:555`, `MarkdownEditor.ts:819`); each site's comment now explains why the call is unnecessary rather than performing it. `CodeEditor.destructor()` (`CodeEditor.ts:473`) additionally nulls `_view` after destroying it — cheap insurance against a second `destructor()` pass finding a live `EditorView` and calling its own, not idempotency-guarded, `destroy()` again; unreachable today (nothing double-disposes `CodeEditor`) but not provably so. `tests/core/ComponentDispose.test.ts:160-187` gained a base-class regression test — a synthetic `CountingChild` counting `destructor()` invocations — pinning the child-runs-exactly-once contract in general; because it exercises a synthetic child under a plain `Component`, it cannot by itself catch a *subclass* reintroducing the pattern, so two class-specific guards were added to match: `tests/component/display/VideoPlayer.test.ts`'s "VideoPlayer dispose" describe block spies on `_video.destructor` and asserts `toHaveBeenCalledTimes(1)`, and `tests/component/markdown-editor.test.ts`'s "MarkdownEditor dispose" describe block does the same for `_codeEditor.destructor` — both mirroring the pre-existing `Chart.test.ts:176` guard for `_legend.destructor` (itself retargeted from the never-invoked `_legend.dispose`, see the corrected enumeration above). Both new guards were mutation-tested: reintroducing the redundant explicit call turned each red (`toHaveBeenCalledTimes` reporting 2, not 1), then the mutation was reverted.

**The same review found `Popover._arrowComponent` leaking on teardown — a fourth instance of the raw-appended-outside-`_components` shape this plan's `WindowBorder` precedent, and `ScrollStrip`'s own fix, already cover.** `Popover.ensureArrow()` inserts the arrow tail straight into the DOM via `DOM.sink.insertBefore` (not `addComponent`), so `_components` recursion can never reach it; `Popover.destructor()` only nulled the field. Fixed the same way as `ScrollStrip`: `this._arrowComponent?.dispose()` before the field is nulled and before `super.destructor()`. The sibling fields nulled in the same block — `_titleComponent`, `_bodyComponent`, `_actionsRow` — were each checked individually rather than blanket-disposed (explicitly disposing a registered child here would reintroduce the exact double-teardown bug fixed just above): all three are inserted via `insertComponent` / `addComponent` (`setTitle`, `setBody`, `addAction`), so recursion already reaches them, and none needed an explicit call. The `Popover` row in `tests/component/dispose-full-teardown.test.ts`'s registry previously missed this because `ensureArrow()` is lazy — only reached from `show()` — so a bare `new Popover()` never builds `_arrowComponent`. Rather than drive a full `show()` (anchor element, `LayerManager`, fade timers) just to reach a one-line lazy builder, the row's `make()` now casts to call the private `ensureArrow()` directly, following the same "materialise the lazy part before asserting" approach `ScrollStrip`'s dedicated `ComponentDispose.test.ts` scenario uses (forcing `_leadArrow` / `_trailArrow` open via `layoutContent()`) rather than the registry's own `ScrollStrip` row, which leaves those two arrows unbuilt and out of scope.

**Correction (fourth review cycle) — the layout-manager-detach ordering justification, even after the third-cycle correction above, was still wrong; do not add a fifth attempt.** The third-cycle correction fixed the *unreachable-code* half of the original claim but kept its *mechanism*: that `Accordion.detach()` / `Split.detach()` need the container's element still connected because their header / panel-wrapper / gutter components resolve their own element through `getElement()`'s `getElementById` fallback. That mechanism cannot fire for either class: `Accordion`'s headers and wrappers (`Accordion.ts:1357-1358`) and `Split`'s gutters (`Split.ts:1185`) are each rendered via `getElement(true)` at creation time, which caches the result in `_element`; `Component.getElement()` (`Component.ts:927-940`) returns the cached field immediately when set and only falls through to `getElementById` when `_element` is unset — so the fallback this justification depends on never runs for either class's own detach-time `getElement()` calls (`Accordion.ts:1140/1143-1144`, `Split.ts:1032`). Reordering the detach to run *after* element removal left the full suite green (221 files / 2813 tests) — this branch's own regression baseline — so the ordering is not demonstrably load-bearing for anything currently in the codebase. `Component.ts`'s comment and the ordering itself were left as-is (detach-before-removal, defensively: a *hypothetical* `LayoutManager.detach()` override touching the container's element through an uncached `getElement()` would need it connected, and reverting a working, harmless default on an unproven basis is its own risk), but the comment no longer names a mechanism — it states plainly that no current override demonstrably requires this order, and that the offline test harness cannot settle the question either way, because its modelled `getElementById` (`TestHandleTable._byId`) is never evicted when `removeElement` runs, so a stale id resolves regardless of ordering. Three successive review cycles each named a wrong mechanism for this ordering; this is the fourth, and it is the last one to attempt — an unproven "kept defensively, no known reason" is the honest final state.
