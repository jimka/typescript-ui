...
- **`removeOverlayScrollbars`'s gutter-clear write had to become unconditional.** The plan's pseudocode guards the trailing `setScrollbarGutter(0, 0)` on `this._scrollbarGutter.right !== 0 || this._scrollbarGutter.bottom !== 0`, mirroring `setAutoScroll`'s native-path gutter-clear. That guard crashed (`Cannot read properties of undefined (reading 'right')`) under `PanelFlushInsets.test.ts`'s `new _Panel({ flush: true, layoutManager: new Fit() })`: passing a `layoutManager` construction option makes `Component.applyOptions` call `setLayoutManager` — and Panel's override calls `setAutoScroll` — from *inside* `super.applyOptions(options)`, i.e. before Panel's own `applyOptions` body has run at all, so `_scrollbarGutter` (seeded at the top of that body) is still `undefined` at that point. The pre-existing `setAutoScroll` gutter-clear survives the same early call only because it's additionally gated on `mode === "none"`, which short-circuits before the read (`mode` is `undefined` on that first call). `refreshOverlayScrollbars` has no equivalent mode gate, so its teardown path always reaches the gutter-clear on this early call. Fix: drop the guard and call `this.setScrollbarGutter(0, 0)` unconditionally — the assignment is a cheap plain-object write with nothing to read-before-write, so removing the optimization removes the crash for free. No behavioural change versus the plan's intent (the value written is the same 0/0 either way).
- **No test file needed pinning under step 18.** `tests/component/input/TimePickerDropdown.test.ts`, `tests/overlay/DialogCappedScroll.test.ts`, and `tests/component/input/PickerColumn.test.ts` all stayed green unmodified under the overlay default — the plan's prediction that the reserved gutter falls on the cross axis (width, not height) held, so none of their height-based geometry assertions shifted. Only `tests/core/PanelScrollChaining.test.ts` (step 15, unconditional per the plan) was pinned to `scrollbarStyle: 'native'`.
- **`npm run docs:build`'s `vitepress build` step could not be verified in this environment.** It was OOM-killed by the kernel (`dmesg` confirms `Out of memory: Killed process ... (node-MainThread)`) across four attempts with different heap sizes (the default 6144MB twice, 4096MB once, 6144MB once more after a retry), consistent with this repo's known pre-existing WSL2 memory-pressure issue (unrelated processes — VSCode extension hosts, tsserver, chrome-devtools-mcp — hold enough RSS that swap stays exhausted). This is environmental, not caused by this change: the `docs:api` (TypeDoc) step, which does process every new JSDoc comment this plan added, completed cleanly with **0 errors and no new warnings** (163 pre-existing "Accordion" link-resolution warnings, unrelated to this plan, unchanged before/after). The vitepress static-site build itself was not exercised end-to-end.
- **Manual verification (per `## Expected Behaviour` and `## Verification`) was run live** against a `vite` dev server serving this worktree (port 8016, isolated from the main tree's own dev server on 8015) via `chrome-devtools` MCP, on the Misc panel's `autoScroll: "auto"` demo window plus the new `scrollbarStyle: "native" (compare)` button. Confirmed via DOM inspection: the overlay host installs as the element's first child (`position: sticky; width: 0px; height: 0px`), `scrollbar-width: none` is applied, and two `Scrollbar` elements are positioned at the exact effective-viewport trailing edges the plan's formula predicts (`clientWidth - 12` / `clientHeight - 12`). Confirmed **no phantom scroll range**: `scrollHeight - clientHeight` / `scrollWidth - clientWidth` matched the panel's actual max scroll offset exactly (401 / 450 against an 800x600-in-350x199 fixture). Confirmed the bars stay pinned to the same viewport rect before and after scrolling (sticky positioning). Confirmed the full round-trip both directions: writing `scrollTop`/`scrollLeft` natively repositions the thumb (sync path), and a synthetic click on the vertical bar's track pages the *native* `scrollTop` (200 -> 387; bar-to-native path). Confirmed the `"native"` opt-out button renders the OS scrollbar (`scrollbar-width: auto`, no sticky host, 2 children instead of 3) side-by-side with the overlay window. Confirmed a Dialog's content-region `Panel` (`autoScroll: "y"`) installs the same overlay mechanism (`scrollbar-width: none`, sticky host present) — its demo content happened not to overflow enough to show a visible thumb, which is itself the documented auto-hide behaviour, not a gap. Confirmed the top `TabBar`'s `ScrollStrip`-backed strip and all of its ancestors keep `scrollbar-width: auto` (untouched) with no sticky host anywhere in the chain. Not exercised live: Ctrl+F match reveal, keyboard scroll, and caret scroll in a nested input — these were not re-verified beyond the design argument (native scroll mechanics are literally untouched by this change, only the bar's paint + an additive overlay), consistent with the plan's own reasoning for why the overlay approach is safe to default.
