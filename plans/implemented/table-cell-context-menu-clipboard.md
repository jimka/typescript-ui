...
- **`editor.test.ts`'s pre-existing `Event.fireEvent`/dispatch staleness required a different workaround than `text-input-context-menu-clipboard.md` used, not the same technique.** That plan's own Implementation Notes documented the same root cause — `Event.ts` arms a DOM event type's window-level capture listener exactly once per test-file module lifetime and never re-arms it against a later `installTestDOM()` sink unless every listener of that type is disposed first — and fixed it there by disposing every field the affected test files construct, letting the type re-arm cleanly. `editor.test.ts` already constructs many undisposed `StringEditor`/`NumberEditor`/`DateEditor` instances across pre-existing describe blocks (keydown-forwarding, Tab-suppression, `Cell.onKeyDown` commit/cancel) that are out of this plan's scope to retrofit with disposal discipline — doing so would touch a large surface of tests this plan does not own, for a fix this file already has its own established alternative to. `editor.test.ts`'s own header comment documents exactly that alternative for the analogous `"input"` case (`typeInto()` calls the private `onInput()` directly instead of dispatching), and its "Tab suppresses native focus-shift" describe block does the same for `"keydown"` (via `findKeydownListener`, extracting and calling the registered listener directly) with a comment citing this identical staleness mechanism. The new `TextInputCellEditor`/`StringEditor`/`NumberEditor` Cut/Copy/Paste menu tests (Expected Behaviour 31, 34, 35, 36) follow that same established idiom: `handleContextMenu(event)` is invoked directly instead of through `Event.fireEvent(component, makeEvent(...))`. The cut()/paste() re-sync assertions (Expected Behaviour 32, 33) call the subclass's private `onInput()` directly after the public `cut()`/`paste()` call, mirroring `typeInto()`, since `cut()`/`paste()`'s internal `Event.fireEvent(this, "input")` — correct, unchanged production code — is not delivered by this file's already-poisoned `"input"` registration either. None of this affects the shipped Cut/Copy/Paste mechanism itself, which is exercised through its real public methods (`copy()`/`cut()`/`paste()`/`handleContextMenu()`) exactly as the plan specifies; only the means of reaching `handleContextMenu`/`onInput()` in this one file's tests differs from `text-input-context-menu-clipboard.md`'s own tests.

- **Two pre-existing "mechanically enforced" registry tests needed updates for `TextInputCellEditor`'s new `_contextMenu: Menu` field and `contextmenu` listener**, both correctly reflecting real, intended side effects of this plan's design:
  - `dispose-full-teardown.test.ts`'s `DateEditor` row's `covers` gained `'TextInputCellEditor'` — disposing a `DateEditor` already exercises `TextInputCellEditor.destructor()`'s new `_contextMenu.dispose()` through the `super.destructor()` chain, so no new row was needed.
  - `dispose-listener-teardown.test.ts`'s `UNCLAIMED_LISTENER_CLASSES` baseline gained `'TextInputCellEditor'`, joining `DateEditor`/`DateTimeEditor`/`TimeEditor` — its own concrete subclasses — already listed there unclaimed for their pre-existing `"focus"`/`"blur"`/`"input"` registrations.

- **`npm run build:docs` (the VitePress docs site build) fails independently of this plan.** `vite build` in `packages/docs` errors in its `spa-fallback` plugin's `closeBundle` hook (`ENOENT` copying `dist/index.html` to `dist/404.html`) — confirmed via `git stash` that this reproduces identically with none of this plan's changes applied, on this branch's own pre-existing tip. `npm run docs:api` (the TypeDoc API-reference generation this plan's own public API additions are checked against) succeeds with 0 errors and the same 9 pre-existing warnings the base branch already carries (all in `MarkdownViewer`/`MarkdownEditor`, untouched by this plan). `npm -w packages/lib run build` (the library's own production build) succeeds cleanly.

- **Manual smoke test (behaviours 37-40) performed live** via `packages/lib`'s own `vite` dev server (port 8025) and `mcp__chrome-devtools__*`, against `MiscPanel`'s "Show window with table (column spec)!" (a `Name`/`Active`/`Score`/`Role`/`Joined`/`Meeting`/`LastSeen`/`Manager` table exercising every affected column type) and "Show window with tree table!" demos:
  - **Behaviour 37 (range Ctrl/Cmd+X / Ctrl/Cmd+V):** a real click-drag range selection followed by a genuine `Control+x` keydown correctly cleared both cells in a two-cell range (`Score`+`Role`) and left the range's other columns untouched — confirmed live. `Control+v` into a different cell was attempted but the automated browser session's `navigator.clipboard.readText()` is denied by the browser's permission model for a page with no user-granted clipboard-read permission (confirmed independently: a bare `await navigator.clipboard.readText()` call hangs on the browser's own permission prompt, which nothing in this session can dismiss) — `DOM.source.readClipboardText()`'s existing `try { ... } catch { return null; }` correctly converts that denial into `pasteAtSelection()`'s documented no-op, matching the exact "browser denied the read" contract `text-input-context-menu-clipboard.md`'s own manual verification hit and documented for `TextInput.paste()`. The successful-read round-trip itself is exercised only by the automated suite (Expected Behaviour 16-28, clipboard stubbed).
  - **Behaviour 38 (right-click menu, Table and TreeTable):** right-clicking a `Table` cell opened a `Cut`/`Copy`/`Paste` menu in that order; clicking `Cut` correctly cleared the clicked cell's value. Right-clicking a cell in the `TreeTable` demo (`TreeTable — file-system layout`) opened the identical `Cut`/`Copy`/`Paste` menu — confirmed live.
  - **Behaviour 39 (spreadsheet round-trip):** not verified live — reproducing it needs a real external spreadsheet application, outside an automated browser session's reach. The exact property it describes (a copied grid, once pasted elsewhere and copied back, reproduces the original values including a tab/quote/newline-bearing cell) is what Expected Behaviour 5 already pins automatically: `parseRectangularTSV(buildRectangularTSV(grid))` deep-equals `grid`, for a grid mixing plain and special-character fields.
  - **Bug found while preparing this manual pass, fixed in `pasteIntoRange`: a `time`-typed column's Cut/Paste round-trip silently lost the value.** `buildCopyText` formats a `time` cell via `TableExporter.formatValue`'s locale display text (e.g. `"02:30 PM"`, no date component); `pasteIntoRange` fed that text straight to `field.convertValue`, whose `time`/`date`/`datetime` branch is a bare `new Date(raw)`. `date`/`datetime`'s locale text happens to round-trip through that (verified: `new Date(d.toLocaleDateString())` and `new Date(d.toLocaleString(...))` both parse back to the original instant in this runtime), but a bare time string never can — `new Date("02:30 PM")` is always `Invalid Date`, since there is no date to anchor it to. `convertValue` therefore returned `undefined`, `pasteIntoRange`'s failure check correctly treated that as unparseable, and the cell was silently skipped — a genuine hole in the "a Cut-then-Paste-elsewhere round-trips losslessly" guarantee this plan's `[^empty-clears]` footnote states as the feature's own reason for existing, for one of `isFieldClearable`'s own explicitly-listed clearable types. Every existing `Body.test.ts` case left `_columns` empty (skipping `formatValue` entirely and falling back to `record.get(...)`'s raw value, which for a `time` field is a `Date` whose default `String(...)` stringification a bare `new Date(...)` parses fine) — none exercised the `Column`-wired path `Table` actually uses in production, so this was untested either way, not a regression against a passing test. Fixed at the same call site, `Body.ts`'s `pasteIntoRange`: for a `time` field only, the candidate text is padded onto the fixed reference date (`1/1/1970`) `TimeEditor.onInput`/`onTimeSelected` already store every `time` value against, before handing it to `field.convertValue` — mirroring `DateEditor.onInput`'s own identical padding for the opposite (date-only) case, so no unsanctioned new pattern is introduced. A regression test (`Body.test.ts`, `'Body range selection — paste'`) pins the fix with a real `Column`-wired `Body`, cutting then pasting a `time` cell and asserting the round-tripped value matches the original. `date`/`datetime` needed no fix — confirmed to already round-trip correctly through the unmodified `field.convertValue` path.
  - **Behaviour 40 (Date/Time/DateTime cell-editor menu):** double-clicking a `Meeting` (time) cell opened its `TimePickerDropdown` and its real `<input>`; right-clicking inside it opened a `Cut`/`Copy`/`Paste` menu identical in shape to `StringEditor`/`NumberEditor`'s own, coexisting with the open dropdown with no visual interference — confirmed live, satisfying the behaviour's "shows the same menu" clause. Its second clause ("Cut/Copy/Paste act correctly") surfaced a real interaction-order caveat, reproduced identically on the **unmodified, out-of-scope `NumberEditor`** before concluding it was worth recording rather than treating as a defect in this plan's own code: clicking a `Cut`/`Copy`/`Paste` **menu item** (as opposed to a native keyboard shortcut) moves focus off the cell's input first, which fires `CellEditorPool`'s existing blur-to-commit handling and returns the cell to its display state *before* the clicked item's `cut()`/`copy()`/`paste()` runs — so a menu-triggered Cut/Paste has no visible effect on the cell in this exact interaction shape. This reproduces byte-for-byte on `NumberEditor` (right-clicking its `_textField`, selecting a digit, clicking `Cut`: the cell's value is unchanged afterward), confirming it predates this plan and is not a regression in the new `TextInputCellEditor` mechanism — it mirrors the identical, already-shipped behaviour it was asked to match. Fixing it would mean changing how `Menu`'s rows take focus (e.g. a `pointerdown`-`preventDefault()` guard on every row, mirroring `DatePickerDropdown`'s own calendar-day rows) or how `CellEditorPool` recognises a menu-internal blur — a `Menu`/`CellEditorPool`-wide change with no precedent and no `## Architecture Decisions` entry authorising it here, well outside "give these three editors the menu `StringEditor`/`NumberEditor` already have." Native keyboard `Ctrl+X` (the browser's own text-editing shortcut, unrelated to this plan's `cut()`) on a real selection inside the same `TimeEditor` input worked correctly end-to-end and confirms the rest of the behaviour: the browser natively removed the selected substring, the resulting native `"input"` event reached `TimeEditor`'s own listener, and the edited value committed and re-rendered correctly on blur (`"11:15:40"` → cut `"11"` → committed and displayed as `"12:15:40 AM"`).
