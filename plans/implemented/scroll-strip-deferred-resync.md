...
- **A mounted-but-unrealised strip needs one extra settling pass before a test can assert a silent (0-read) pass; the plan's literal "mount, then `drainFrames()`" sequence (steps 8-9; Expected Behaviour cases 4, 5, 9) does not reach one.** `laidOutItemsExtent` reads each item's committed `getX()`/`getWidth()`, which `HBox.doLayout` only ever writes once its container's `getInnerSize()` is non-null — which itself requires the clip to have a realised element (`Component.getInnerSize`'s own doc: "or null if the element is not yet in the DOM"). In this offline harness the clip is not realised until `ensureArrows`'s `this.getElement(true)` runs, and that call happens inside `layoutArrows`, i.e. *after* that same pass's `layoutItems` already computed and cached `_lastItemsExtent` — so the very first pass unavoidably caches a placeholder (items not yet positioned; `laidOutItemsExtent` sums to `NaN` for a strip with no items positioned) as its baseline. Draining the mount pass's settle relay does not correct this: `flushResizeSettle` only re-runs `refreshArrows`, never `layoutItems`, so `_lastItemsExtent` stays at the placeholder. The next pass — the one each new test's assertions are actually about — then compares a real, freshly-committed extent against that placeholder and always reads `!==`, i.e. always "changed", regardless of whether anything the plan cares about actually moved.

  A real browser mount does not have this problem: rendering and layout finish before a user can start a resize drag, so by the time a burst begins, the strip's own last committed extent is already real. The fix keeps the same shape but adds one further identical pass between mount and `drainFrames()` — `mountAndSettle` in `ScrollStrip.resizeResyncCoalescing.test.ts`, and a second `realDragFrame` call in `ScrollStrip.resizeResyncCoalescingRealtime.test.ts` — so `_lastItemsExtent` acquires its first real baseline before any "nothing changed" assertion runs, matching what a real mount already guarantees. No production code changes for this; `Tab.stripRelayoutEconomy.test.ts` (steps 10-11) does not need it because `hostTab()` already calls `host.getElement(true)` before any layout pass runs, so its very first pass already sees a realised clip.

- **`resetScroll` and `setMainScroll` can leave the arrows stale under the new gating, once `resetScroll`'s one caller was traced; the fix follows the `scrollBy`/`revealItem` precedent already in the file.** The plan's `## Architecture Decisions` and footnote `[^cache-readers]` enumerate every *reader* of the clip's scroll cache but not the *writers* that move the scroll offset outside a layout pass without changing the clamp signature. Under the pre-plan code, `layoutItems` resynced live on any pass with no resize burst in flight regardless of whether the clip's extent had actually changed, so a stray scroll write was always corrected by the next pass; under this plan's clamp-signature gate, a pass whose signature is unchanged is now silent, so a write that moves the scroll without changing the box is never corrected. `TabBar.setSide` calls `ScrollStrip.resetScroll()` on a side switch, and a flip that lands on an identical box (same thickness, same tab widths) is exactly such a silent pass. `scrollBy` and `revealItem` already follow every native-scroll write with `this.refreshArrows()`; `resetScroll` gets the same call, so all three of the strip's own scroll-moving methods now refresh the arrows themselves rather than depending on the next layout pass to notice. `setMainScroll` itself is unchanged — both of its two callers already refresh explicitly, matching the existing pattern, and it has no callers outside this file — but it is public, so its JSDoc, the `ScrollStrip` docs page, and this branch's changelog entry now tell an external caller to follow a write with `refreshArrows()`; the plan's Documentation Impact ("no consumer-visible behaviour change") did not anticipate that. The previous unreleased changelog entry for the resize-burst detector was amended in the same pass to describe the one read a layout pass still performs, rather than the two it originally withheld, because both entries ship in the same release. Doc comment for `resetScroll` describes the paging step in prose rather than linking `scrollBy` — `scrollBy` is private, and `CODE_CONVENTIONS.md` forbids a public symbol's JSDoc linking a non-public one.

- **Expected Behaviour case 3's literal sequence (`layoutAt(W1), layoutAt(W2), layoutAt(W2)`, draining to assert "exactly one catch-up, not two") cannot discriminate the invariant it claims to pin, because `_arrowRefreshOwed` is a boolean, not a counter.** In that sequence the mount pass (`W1`) leaves its own settle relay armed (steps 8-9's "harmless" mount quirk), so the `W1`→`W2` change lands on an *already-armed* relay and marks `_arrowRefreshOwed = true` on its own, correctly, before the case's own same-signature `W2` pass ever runs. A mutant that sets `_arrowRefreshOwed = true` unconditionally on every mid-burst pass (instead of only when the signature changed) is unobservable here: the flag was already `true`, so the mutant's redundant write changes nothing a call-count assertion can see, and `drainFrames()` performs exactly one catch-up either way. Verified both ways by mutation: with the literal sequence, the mutant leaves all 15 tests in both coalescing files green. The fix isolates a fresh burst first — `mountAndSettle(strip, W1)` before installing spies — so the burst's arming change (`W1`→`W2`) is a *live* read (the `arrowRefreshDueThisPass` branch taken when no relay is armed, which never touches `_arrowRefreshOwed`) rather than a withheld one, leaving `_arrowRefreshOwed` at its clean `false` baseline going into the case's own same-signature `W2` pass. The assertion after `drainFrames()` is accordingly `SYNC_PER_LIVE_PASS`/`REFRESH_PER_LIVE_PASS` (no catch-up at all), not `2 *` those — the literal plan text's counts were mutation-tested and found not to hold against the correct implementation either. Re-verified by mutation after the fix: the same mutant now fails this test (`expected 1, got 2`).
