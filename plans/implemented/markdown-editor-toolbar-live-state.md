...
[^togglebutton-self-flip]: `ToggleButton.onAction()` (`ToggleButton.ts:281-285`) runs on the DOM `"click"` listener wired in its own constructor, ahead of any external `"action"` listener (which fires on the `"change"` event `onAction()` itself dispatches) — so `setSelected(!isSelected())` always runs before `MarkdownDocumentPanel`'s `() => this._editor.toggleBold()` handler does. This is harmless here because `toggleBold()`'s own `editor.update(...)`/`editor.dispatchCommand(FORMAT_TEXT_COMMAND, ...)` calls are synchronous commits that each run the `registerUpdateListener` callback (and therefore `updateSelectionState()`) before `toggleBold()` returns — so by the time the click handler stack unwinds, `applySelectionState` has already been called with the real post-toggle value, correcting whatever the self-flip guessed. No change to `ToggleButton` or to the wiring is needed to make this correct.
  ...
  **Implementation correction (see Implementation Notes below):** this footnote's "synchronous commits ... before `toggleBold()` returns" claim does not hold in this codebase — `dispatchCommand`'s underlying update is not `{ discrete: true }`, and Lexical defers a non-discrete update's `registerUpdateListener` flush to the next microtask rather than running it inline. The correction still happens — just one microtask later, not literally within the same synchronous call stack — which is still imperceptible in a real browser and required no code change, only an `await` in the tests that pin this specific behaviour.

---

## Implementation Notes

- **Footnote [^togglebutton-self-flip] overstates the timing.** It claims `toggleBold()`'s `dispatchCommand` call runs the `registerUpdateListener` callback (and therefore `updateSelectionState()`) synchronously, before `toggleBold()` returns. Empirically (confirmed both via a standalone probe and via `markdown-document-panel.test.ts`'s own pre-existing comment on its dirty-relay test, which already noted "a toggle command's dispatchCommand path settles on a later flush in this headless harness, while setValue()'s `{ discrete: true }` update commits synchronously"), `dispatchCommand(FORMAT_TEXT_COMMAND, ...)`'s underlying `editor.update()` is not marked `discrete`, so Lexical defers that update's listener flush to the next microtask rather than running it inline. The Lexical *state* itself still mutates synchronously — `getValue()` reflects the new format immediately, which is what every pre-existing command-API test already relies on — only the update-listener-driven emit (`"selectionstate"`, and, by the same pre-existing mechanism, `"change"`) lands one microtask later. This changes nothing about the shipped code (the design is unaffected: the emit still fires, just not perfectly synchronously) but two tests pinning the plan's Expected Behaviour needed an `await Promise.resolve()` rather than a synchronous assertion: `markdown-editor.test.ts`'s "a format toggle that does not move the selection still triggers a selectionstate emit", and `markdown-document-panel.test.ts`'s click-dispatch test's "a stale pre-click `.selected` guess is corrected by the live selectionstate emit" assertions. Both are otherwise unaffected — this is a test-timing detail, not a functional gap.
- **`MenuButton.contextActive` (the Table button's persistent-highlight mechanism) was backed out** after the user pointed out, in review, that the framework already has this: `Button.setEnabled(false)` already dims a button, sets the native `disabled` attribute (which suppresses pointer events and blocks `MenuButton.toggleMenu()` from ever running), and switches the cursor — exactly the "this doesn't apply right now" signal the Table button needs, with no new CSS state, no new tokens, and no new public API. `MarkdownDocumentPanel.applySelectionState` now calls `this._tableBtn.setEnabled(state.inTable)` instead of a bespoke `setContextActive`; `MenuButton.ts`/`MenuButton.test.ts`/`MenuButton.md` are reverted to their pre-`.contextActive` shape entirely (that state had no other caller). The Architecture Decisions section above (`.contextActive` reuses `ToggleButton`'s visual tokens, [^why-menubutton-not-a-new-class]) and the Public API section's `MenuButton` addition are superseded by this note — disabling the whole button, not highlighting it, is now the shipped design.
