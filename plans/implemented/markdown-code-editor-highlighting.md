...
- **`mapFenceLangToEditorId` is exported from `Markdown.ts` itself, but not re-exported from the `component/display` barrel.** The plan's Expected Behaviour section calls for asserting the mapped id directly ("assert the mapped id via `mapFenceLangToEditorId`"), and no existing module-level helper in `Markdown.ts` (`slugify`, `alignmentClass`) is exported even for tests — the test file already imports `Markdown` from the source module path rather than the barrel, so a matching source-level (non-barrel) export keeps the "no exported symbol changes" claim in **Public API** true for the package's actual public surface while satisfying the plan's own test guidance.
- **The "displayed:false defers the loader" and "upgrades to a live CodeEditor" tests bypass the real `requestAnimationFrame`-coalesced layout flush**, rather than mocking `requestAnimationFrame` and flushing a captured frame (the approach `OnFirstLayout.test.ts` uses in its own, single-purpose file). `Markdown.test.ts` is a large shared file where many earlier tests already drive `scheduleLayout()`/`onFirstLayout()` under the harness's default (callback-dropping) `requestAnimationFrame`; that pins the module-level `rafHandle` in `Component.ts` non-null for the rest of the file's run, so a locally-mocked `requestAnimationFrame` installed later never gets called (`ensureFlushScheduled`'s `rafHandle === null` guard short-circuits) and the flush-based approach is unreliable — it depends on test execution order elsewhere in the file. The wiring tests instead spy on `onFirstLayout` itself, assert it was called (not `loadCodeEditorUpgrade` directly, proving the deferral), and then invoke the captured callback directly — the same effect a real flush would have, without depending on the shared queue's cross-test state.
- **Awaiting the dynamic import in a test uses the spied call's own returned promise (`spy.mock.results[0].value`), not a fixed number of `await Promise.resolve()` ticks** as the plan's step 9 suggested. `CodeEditor.js`'s dynamic import pulls in real `@codemirror/*` packages through Vite's test module runner, which is a genuine module-graph load rather than a same-tick microtask; a couple of `Promise.resolve()` awaits proved insufficient and occasionally left the import still in flight when a test's `afterEach` reset the environment (surfaced as an `EnvironmentTeardownError` from a later test). Awaiting the actual promise a `vi.spyOn` call-through returns is deterministic regardless of how long the real transform takes.
- **The dynamic-import kickoff is gated by `Component.onEffectiveVisibilityChange`, not solely by `onFirstLayout`** — a deviation from the plan's Architecture Decision text, which relied on `onFirstLayout` alone to keep the kickoff pending "for as long as the panel stays collapsed." That guarantee only holds for a component's very first render: `onFirstLayout`'s "already connected" fast path (taken once an element has been connected at least once, e.g. a second `setMarkdown()` call on a previously-shown, now-hidden `Markdown`) routes into `Component.afterNextLayout`, which fires unconditionally on the very next layout flush with no `displayed`-gating at all — so the deferred kickoff could start downloading CodeMirror while still hidden. An audit round caught this, and a first fix attempt (re-arming via `this.onFirstLayout(kickoff)` when still hidden) traded that bug for a worse one: an unbounded per-animation-frame poll for as long as the component stayed connected-but-hidden, exactly the anti-pattern `Canvas`/`WebGLCanvas`/`Glyph` already eliminated via `onEffectiveVisibilityChange` (`Canvas.ts`'s own comment: "the replacement for the old per-frame `isEffectivelyVisible()` poll"). The final design follows that precedent instead: `appendCode`'s deferred callback routes through a new `startCodeEditorImport(entry: QueuedCodeUpgrade)` that checks `isEffectivelyVisible()` exactly once — starting the import if visible, or pushing `entry` onto a new `_awaitingVisibilityKickoffs` array otherwise — and `Markdown` now overrides `onEffectiveVisibilityChange` to flush that array (edge-triggered, once per visibility flip to `true`) the same way `Canvas` reconciles its animation loop. `clearContent` clears the array on every rebuild/dispose alongside `_pendingCodeUpgrades`.
