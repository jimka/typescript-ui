...
- **`resolveDeclarations` (in `core/ClassStyleRules.ts`) is now `export`ed, not left module-private.** `ensureStyleGroupRule`'s Public API signature takes an already-resolved `declarations: Record<string, string | null>`, and `resolveInstanceStyleDeclarations` (the private `core/Component.ts` helper the plan's own call site — `ensureStyleGroupRule(this.constructor, group, resolveInstanceStyleDeclarations(this))` — requires) has to *produce* that resolved record from an instance's per-field getters. Reusing `resolveDeclarations` "unchanged", as the plan directs, is only possible across the file boundary if it is reachable from `Component.ts`; it was `function resolveDeclarations(...)` (module-private) beforehand. Adding `export` is the minimal change that keeps `ensureStyleGroupRule`'s signature exactly as specified while still avoiding a second, duplicated fold of the same resolution logic.
- **`ComponentOptions.styleGroup` is typed `string | null` (not the plan's literal `string`).** The plan's `## Public API` pseudocode gives the option field as `styleGroup?: string;` but the setter as `setStyleGroup(group: string | null): this` — a nullable clear path with no nullable option-bag field to source it from. The nearest existing precedent for this exact shape (`ComponentOptions.name?: string | null` paired with `setName(name: string | null)`, both backed directly by `this._options.name`/`this._options.styleGroup` with no dedicated private field) types the option field nullable too, so `setStyleGroup`/`getStyleGroup` were implemented as `name`'s exact mirror. No behavioural difference for any construction-time call in `## Expected Behaviour`, which never passes `null`.
- **Manual browser verification (`## Expected Behaviour` rows 8-9) was performed**, per `## Ordered Implementation Steps` step 9 and `## Verification`'s "non-negotiable" requirement. `StyleAuditPanel.ts` gained a "Shared Instance Style Groups" demo section (five `Button`s sharing `styleGroup: "warning-demo"` plus three ungrouped `Button`s with the identical `backgroundColor`); a dev server was started from this worktree and the `#/style-audit` route was driven via `chrome-devtools`. Confirmed via `evaluate_script` against the live `#Base` stylesheet: the five grouped buttons render identically to the three ungrouped ones (same `rgb(181, 137, 0)` fill) and carry the `Button--warning-demo` DOM class with zero per-instance `background-color` rules of their own; one shared `.Button--warning-demo` rule carries `background-color: rgb(181, 137, 0)` plus the rest of the group's hoisted style; the three ungrouped buttons each still write their own identical `#id:not(.pressed) { background-color: rgb(181, 137, 0); }` rule — the literal three-way duplication the Style Audit panel's dedup table is built to flag, absent entirely for the five grouped instances.
- **A `styleGroup` token containing whitespace crashed at render, found during audit.** The plan's `## Non-Goals` ("Validating or restricting the `styleGroup` string's characters… not restricted at the API surface") reasons by analogy to a component id, but that analogy breaks for this one case: an id is only ever written through `DOM.sink.setId`/an escaped selector string, never through `classList`, so it has no whitespace constraint — a `styleGroup` token, however, becomes a literal `classList.add()` token in `init()`, and `classList.add` throws `InvalidCharacterError` on any token containing ASCII whitespace. CSS-escaping (used for the class-tier selector) does not help: `CSS.escape` backslash-*prefixes* a space rather than removing it, so an escaped token can still contain a raw space and `classList.add` still rejects it. Fixed with a new exported `styleGroupClassSuffix(group)` helper (`core/ClassStyleRules.ts`) that replaces ASCII whitespace with `-`; both `init()`'s `addClass` and `ensureStyleGroupRule`'s selector construction call it, so the literal DOM class and the CSS selector that must match it always agree. This is a minimal fix to make the plan's own design safe for realistic input (a caller-chosen label like `"my group"`), not a validation/restriction added at the `setStyleGroup` API surface — the setter still accepts any string; only the internal token-construction sites were made crash-safe. Covered by a new offline test (locks in that the recorded `addClass` patch never contains whitespace) and re-verified live via the same `StyleAuditPanel.ts` browser session (temporary probe button, reverted after confirming no console error and a correctly-matching `.Button--has-space` rule).
- **A follow-on cache-key bug in that same fix, also found during audit and fixed in the same round.** `ensureStyleGroupRule`'s `_groupBags` cache was keyed on the *raw* `group` string while the DOM class/selector it produces are derived from the *normalised* suffix — so two differently-spelled tokens that normalise to the same suffix (e.g. `"brand warning"` and `"brand-warning"`) missed the cache and silently re-inserted the shared rule, the second instance's value clobbering the first's. Fixed by keying `_groupBags` on `styleGroupClassSuffix(group)` instead of the raw token, matching what the selector is actually keyed on. Covered by a new offline test asserting the shared rule is written exactly once and a second, differently-spelled-but-same-suffix instance with a genuinely different value writes its deviation to its own `#id` instead of clobbering the shared rule.
