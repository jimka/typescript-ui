...
Two deviations from this plan's literal `## Internal Structure` code samples were made during the audit loop, both corrections rather than redesigns:

- **`release()` additionally calls `cancelTransitions(element)`.** The plan's `release()` sample (`## Internal Structure`) omits it. An in-flight `Animation.play` transition against the outgoing element — its two-frame entrance dance, or its `transitionend`/fallback-timeout completion — queues a deferred write that resolves the handle after `release()` has already called `DOM.sink.release(element)`; in production that throws (`DOM handle X is not registered`), exactly the hazard `destructor()` already guards against with the identical call ([Component.ts](packages/lib/src/typescript/lib/core/Component.ts), mirroring its existing `cancelTransitions` loop). `release()` now calls it once, on the single outgoing handle, between `DOM.sink.removeElement` and `DOM.sink.release`/`untrackHandle` — the same relative position `destructor()` uses. A regression test (`tests/component/element-release.test.ts`, "Component.release() cancels pending transitions") mirrors `tests/core/DisposedPendingTransition.test.ts`'s technique and was confirmed to fail without the fix and pass with it.
- **`restoreReleasedState()` routes its scroll write through `this.getScrollElement()`, not the raw `element` parameter the plan's sample used.** Every other scroll-writing site in `Component` (`setScrollLeft`, `setScrollTop`, `syncScrollOffsets`) resolves `getScrollElement()` first, precisely because a subclass whose scroll happens on an inner element (e.g. `Panel`'s overlay-scrollbar mode) overrides it so all scroll plumbing follows the inner scroller. Writing the raw root `element` instead would silently mis-target scroll restoration for any such subclass once one opts in. The focus branch is unaffected and still targets the root `element` parameter, matching `focus()`'s own established convention of using `getElement()` rather than `getScrollElement()`.

Both fixes were folded into the code commit that introduced `release()`/`restoreReleasedState()` via `git commit --fixup` + `git rebase --autosquash`, per the `commit` skill's fixup convention — they are not visible as separate commits on the finished branch.

**Manual verification of Expected Behaviour cases 10, 11, and 13** (the plan's `## Verification` "Manual smoke" step) was performed in a real browser against this worktree's dev server (`packages/lib`, `npm run dev`, localhost:8015) using a `ReleasableProbe`-style test subclass constructed via the browser console (`chrome-devtools` MCP `evaluate_script`), attached to `document.body` via `DOM.source.getBody()` / `DOM.sink.appendChild` (the framework's own Handle-based attach primitives — `document.getElementById` cannot resolve a component's element before it is first attached, since attachment is a container's job, not `render()`'s). Driven sequence: set `scrollTop` to 150 and call `.focus()` on a real, connected, `overflow:auto`, `tabindex="0"` element; `release()`; confirm `document.getElementById` genuinely misses the id (production evicts the id index, unlike the offline harness — see `[^offline-byid]`); call `getElement(true)` to rebuild; re-append the fresh element (the caller's responsibility, matching how a real consumer would re-splice a released-then-rebuilt component back in); call `flushLayout()` to drive the first-connected-layout drain. Result: the fresh element's `scrollTop` read back as `150`, and `document.activeElement` was the fresh element — both cases 10 and 11 confirmed landing correctly in a real browser, with no console errors or warnings. Case 13 (releasing mid-CSS-transition) is additionally covered by the automated offline regression test added above, which supersedes its original "manual-verify only" classification — the `DisposedPendingTransition.test.ts` technique it was modelled on turned out to make this case offline-testable after all.
