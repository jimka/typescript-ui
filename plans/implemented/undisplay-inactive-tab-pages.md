...
- **`npm run docs:api` does not finish at zero warnings** — `## Verification` item 5 calls for zero. The run after this change reports 0 errors and 14 warnings, all pre-existing and unrelated to any file this plan touches (`SpatialNavigation.ts`, `FieldDecorator`, `MarkdownViewer.ts`, `MarkdownEditor.ts` — none in `## Files to Create / Modify / Delete`). None of the five files this plan's JSDoc changes touch (`Component.ts`, `Tab.ts`, `Card.ts`, `Panel.ts`, `CodeEditor.ts`) appear anywhere in the warning list. Confirmed pre-existing by diffing against `master`. Left unfixed as out of this plan's scope.
- **The WebKitGTK MiniBrowser drag measurement (`## Verification` item 7) was not run by the implementer.** It opens full-screen windows on the developer's desktop and was explicitly reserved for the invoking session to run afterward.
