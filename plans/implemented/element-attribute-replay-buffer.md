...
- **Step 1 test example, `data-insets`.** The plan's `## Expected Behaviour` row 13 illustrates the `data-*`-is-unaffected case with `p.setDataAttribute("insets", "0,0,0,0")`, expecting `data-insets="0,0,0,0"` after render. Live behaviour differs: `Component.applyStyle` (Component.ts, called from `init()`) unconditionally re-derives `data-insets` from `getInsets().render()` on every render — `"0px 0px 0px 0px"` for the zero default — which overwrites a manually-set `data-insets` value regardless of this plan's change. This is pre-existing, unrelated behaviour specific to the `insets` key, not something the buffer introduces or fixes. The test (`ElementAttributeReplay.test.ts`) uses a neutral key (`probe-marker`) instead so the case actually exercises the `_attributes` replay path the row is testing.
- **Step 8, stale-comment sweep.** The plan predicted six files with comments asserting `setElementAttribute` drops detached writes (`TextInput.ts`, `TextArea.ts`, `Video.ts`, `FileField.ts`, `MarkdownEditor.ts`, `TextInputCellEditor.ts`). Running the specified grep, and reading each file's `init()` override and its callers of `setElementAttribute`, found the false claim present in `FileField.ts` (one comment), `MarkdownEditor.ts`'s `WysiwygSurface` (two comments — the `init()` override and `setContentEditable`), and `Video.ts` (two comments — the `init()` override and `replayMediaOptions`) — five comments across three files, all fixed. `TextInput.ts`, `TextArea.ts`, and `TextInputCellEditor.ts` have no JSDoc on their hand-rolled replay at all (nothing false to correct). An earlier pass of this note incorrectly judged `Video.ts` clean, reasoning that its comments only describe the `getElement()`-timing constraint on `replayMediaOptions`; a follow-up audit (see branch history) caught that the same comments *also* justified the replay's existence with "Setter DOM writes no-op ... dropping `src` and the rest," which is false the same way the fixed files were — `Video.setSrc`/`setPoster`/`setPreload`/`setAutoplay`/`setLoop`/`setMuted` all route through `setElementAttribute` and are now buffered and replayed by `Component.init()` onto the same passed `element`. `replayMediaOptions` is *not* fully redundant, though: `volume`, `playbackRate`, and `muted`-as-live-property have no reflecting attribute and are never covered by the base cache, so the method still has to run for those. The corrected comments say so — the `setAttr` portion is redundant with the base replay, kept for parity; the IDL-property portion is not.
