...
- **Step 8, stale-comment sweep.** The plan predicted six files with comments asserting `setElementAttribute` drops detached writes (`TextInput.ts`, `TextArea.ts`, `Video.ts`, `FileField.ts`, `MarkdownEditor.ts`, `TextInputCellEditor.ts`). Running the specified grep, and reading each file's `init()` override and its callers of `setElementAttribute`, found the false claim actually present in only two files: `FileField.ts` (one comment) and `MarkdownEditor.ts`'s `WysiwygSurface` (two comments — the `init()` override and `setContentEditable`). `TextInput.ts`, `TextArea.ts`, and `TextInputCellEditor.ts` have no JSDoc on their hand-rolled replay at all (nothing false to correct). `Video.ts`'s comments describe why `replayMediaOptions` must write directly to the passed element handle rather than through a `getElement()`-based setter (the element isn't registered as `this.getElement()` yet at that point in `init()`) — a timing fact this change doesn't alter, so nothing there was false either. Fixed only the three comments that were actually wrong; left the rest untouched per "don't improve adjacent code that isn't broken."
