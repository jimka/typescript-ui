...
- **Case 3's `_ownedSelectors` assertion was rewritten from a hard-coded single-element array to a before/after snapshot comparison.** The plan's Architecture Decisions table illustrates `_ownedSelectors` holding one entry (`["#a"]`) at each step, but any component whose class declares style states — which includes the base `Component` class itself, via its `.undisplayed`/`.invisible` `ownStyleStates` — also lazily allocates and tracks a "resting isolation" rule selector (`#<id>:not(.undisplayed):not(.invisible)`) the first time `flushStyleBag` resolves a framework-baseline key (`displayed`/`visible`) during render, regardless of whether the component declares `backgroundColor` or any other real style. So a rendered `Component` tracks two selectors, not one, before `setId` is ever called. This is an existing, unrelated mechanism `setId`/`destructor` don't touch — confirmed empirically (a bare `new Component({})` tracks the same second entry) — so case 3 ("re-setting the same id changes nothing") now snapshots `_ownedSelectors` before the same-id `setId` call and asserts the array is unchanged afterward, which states the plan's actual invariant ("no dispose, no duplicate tracking entry") without hard-coding a count that depends on this orthogonal mechanism. Cases 1, 2, and 4 needed no change — their assertions are about the `#<id>` base selector and the materialised rule cache specifically, which are unaffected.
- **Case 5 uses the literal event type `'mouseup'` instead of the plan's suggested `'mousedown'`.** `'mousedown'` already has a dedicated literal-type test in the `PRIMARY_BUTTON_TYPES per-type default` describe block further down `events.test.ts`, and that block's own comment explains why a literal type may appear in only one test in the whole file: `installBaseListener` installs the native window listener on a type's first registration only, and that installation isn't undone by `DOM.reset()` between tests, so a second test registering the same literal type sees nothing dispatched against its own fresh sink. Adding a second `'mousedown'` registration reproduced exactly that failure in the existing per-type-default test when the two ran in the same file (confirmed: the existing test passed in isolation, failed once case 5 ran first). `'mouseup'` is unused elsewhere in the file and is otherwise identical for the test's purpose (a `PRIMARY_BUTTON_TYPES` member with no special-case handling in `baseListener`, unlike `'click'`).
- **`next.md`'s `## Fixed` section was not header-only, contrary to the plan's assumption.** The stacked `component-lifecycle-leak-fixes-round-2` branch this branch is built on already added a `## Fixed` section with three uncategorized bullets. Rather than inventing a heading for that pre-existing content (out of scope, and not this plan's to categorize), the new `### Core` subsection was appended after it, leaving the existing bullets untouched.
- **Three classes joined `dispose-full-teardown.test.ts`'s `UNCLAIMED_DESTRUCTOR_CLASSES` baseline as a direct consequence of the `override` normalisation.** That test's `classesDeclaringDestructor()` scan matches `/^\s*protected destructor\(/`, which does not match `protected override destructor(`— so `DiagnosticsOverlay`, `StyleAuditOverlay`, and `StyleAuditView` were invisible to the scan before this plan's step 13 and became newly "unclaimed" (no `covers` row, not yet in the baseline) the moment `override` was dropped, with no change to their destructor bodies. The test's own registry comment calls this out as the expected failure mode for a class gaining its first matched `destructor()`, so the fix is the documented one: add the three names to the baseline, with a comment explaining why, rather than building new dispose-registry coverage for the diagnostics overlays (out of this plan's scope).
