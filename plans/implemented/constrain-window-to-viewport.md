...
- **Full-window containment, configurable (request).** The drag default changed from header-grabbable to full containment: `clampDragDelta` now bounds the window to `x ∈ [0, vw − w]`, `y ∈ [0, vh − h]` so every border stops at the viewport edge. It is gated by the new `constrainToViewport` option (default `true`) with `setConstrainToViewport`/`isConstrainToViewport` accessors; when `false` it falls back to the original `EDGE_MARGIN_PX` header-reachable clamp. The clamp order flipped to `Math.max(Math.min(...))` so an oversized window floors at the viewport top-left (header on-screen) instead of the bottom-right.
...
- **Configurable margin value.** The 24 px slab used by the header-reachable fallback is a fixed internal constant — the `constrainToViewport` option toggles the *mode* but the margin size itself is not exposed.
