...
A private `getChildHost()` returns `this._contentFrame ?? this.getElement()`, mirroring `getAttachNode()` ([Component.ts:687](../src/typescript/lib/core/Component.ts#L687)) exactly (no-arg, same `HTMLElement | undefined` shape): `getAttachNode` answers "what node does THIS component occupy in its parent"; `getChildHost` answers "where do MY children attach". `addComponent` ([Component.ts:3313](../src/typescript/lib/core/Component.ts#L3313)) and `insertComponent` ([Component.ts:3366](../src/typescript/lib/core/Component.ts#L3366)) append/insert through `this.getChildHost()?.…` (the optional-chain handles the nullable union; both call sites are already past their `if (!element) return` guard, so it never short-circuits) so a child added while the frame is active lands inside the frame, not behind it. `removeComponent` calls `component.removeElement()` ([Component.ts:3398](../src/typescript/lib/core/Component.ts#L3398)), which uses `element.remove()` and is parent-agnostic — no change needed there.
...
class Component {
    ...
    /** Where this component's children attach: the content frame when active, else the element. Mirrors getAttachNode. */
    private getChildHost(): HTMLElement | undefined;  // = this._contentFrame ?? this.getElement()
...
4. **`Component.ts` — `getChildHost()`.** Add `private getChildHost(): HTMLElement | undefined { return this._contentFrame ?? this.getElement(); }` with JSDoc, placed near `getAttachNode` and mirroring its no-arg shape.
...
6. **`Component.ts` — route appends through `getChildHost`.** In `addComponent` change `element.appendChild(compElement)` ([Component.ts:3313](../src/typescript/lib/core/Component.ts#L3313)) to `this.getChildHost()?.appendChild(compElement)`. In `insertComponent` change the `element.insertBefore(...)` target ([Component.ts:3366](../src/typescript/lib/core/Component.ts#L3366)) likewise (`this.getChildHost()?.insertBefore(...)`). Verify `removeComponent`/`removeAllComponents` need no change (they call `component.removeElement()` which uses `element.remove()`, parent-agnostic — confirmed at [Component.ts:569](../src/typescript/lib/core/Component.ts#L569)).
