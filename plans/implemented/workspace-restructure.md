...
Deviations made during implementation (the plan otherwise implemented as written):

- **`typedoc` pinned to exact `0.28.19` in `packages/lib/package.json` devDependencies** (was `^0.28.0`). Step 7's mandated lockfile regeneration (`rm -rf package-lock.json && npm install`) resolved `typedoc` to `0.28.20`, which relocated its internal `dist/lib/converter/symbols.js` — the exact file `typedoc-callable-plugin.mjs` deep-imports via `await import()` — breaking `docs:api`. Pinned to `0.28.19` (the version master's lockfile carries and the plugin is built against). This touches only a devDependency, so the plan's byte-identical `name`/`exports`/`files`/`sideEffects` published-surface invariant is unaffected. **Critical follow-on:** `typedoc` must resolve to a *single hoisted* instance at the workspace-root `node_modules/typedoc`, shared by the root-hoisted `typedoc-plugin-markdown`; a workspace-local copy under `packages/lib/node_modules/typedoc` produces two `typedoc` instances and the markdown theme fails its `instanceof Reflection` check ("The page model should be a reflection…"). A clean `npm install` hoists it correctly.
- **Docs-app `main.ts` uses `Body.getInstance().addComponent(new Header({ text }))`**, not the plan's placeholder `Label` / `Body.add`. Step 12 explicitly instructed confirming valid constructors against the barrels: `component/display` exports `Header` (no `Label`), and the add method on `Component` (Body's base) is `addComponent`. This is the plan's sanctioned "adjust to a valid current constructor", not a design change.
- **`exports`-map resolution verified via `import.meta.resolve`, not an executing `import()`.** The Verification step's `node -e "import('@jimka/typescript-ui/core')…"` fails outside a DOM because lib modules touch `document` at import scope; `import.meta.resolve` proves the specifier maps to `packages/lib/dist/lib/core.es.js` (the thing being verified) without executing DOM code.
- **Pre-existing debt, not introduced here (out of scope — the plan forbids source edits under `packages/lib/src`):** `npm run lint` reports 5 errors and `typecheck:test` reports 2 errors; both reproduce identically on unmodified `master`. The restructure is behaviour-neutral — the full unit suite (211 files, 2617 tests) passes, `build:lib`, lib `typecheck`, `docs:api`, and the docs-app build all pass.
- **VitePress `docs:build` (retained, not CI-deployed) was OOM-killed (exit 137) in the constrained implementation environment (~4 GB RAM).** Its config is byte-identical to master (only relocated), and every input step — `docs:api` (0 errors, matching master) and `docs:llms` — succeeds; the failure is memory pressure, not the restructure. Verify on an adequately-resourced machine/CI. This is the retained-legacy path; Pages deploys `packages/docs` instead.
