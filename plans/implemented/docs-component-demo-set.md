...
## Addendum: review pass over the rendered catalogue

Reviewing the finished catalogue in a 1920px window overturned three of the plan's decisions. The plan's text above is left as written; this section is what the code now does.

**The marker sits directly after the page's summary, not at the end of the section it illustrates.** `### The marker sits at the end of the section it illustrates` put each page's demo several screens down, so a reader arriving at `/components/Table` met three code fences before anything live. Every page's *first* demo now sits between the summary paragraphs and the first `##` heading — the shape `components/Button.md` already had. A page's *second* demo (`Button`, `Table`, `VBox`) stays at the end of the section it illustrates, because it exists to illustrate that section rather than the component.

**The live area is capped at 900px wide.** The plan fixed a demo's height and left its width to the pane, which is full-bleed: at 1920px a four-column table spread its columns to ~390px each, a chart's bars grew as wide as a hand, and a form's inputs ran the length of the page. `DocsDemo` now carries `maxSize: { width: 900, height: UNBOUNDED }` — on the block rather than on the stage, so the east-anchored "Show source" toggle stays against the stage's right edge. The cap is a ceiling only; below 900px the block still fills the pane.

**A demo returning a bare control gets a non-stretching row.** The stage is a `Fit()`, so a `create()` returning the component itself hands it the whole live area. `button-basic` always wrapped its buttons in an `HBox` row; `splitbutton-menu` and `combobox-store` did not, and rendered as a page-wide button and a page-wide selector. Both now return `Panel({ layoutManager: HBox(), components: [control] })`. The data views (`Table`, `Tree`, `TreeTable`, both charts) still fill the live area, which is right for them.

**`table-store`, `table-cell-types`, `tree-nodes`, and `treetable-hierarchy` drop from the `320` height class to `200`.** `320` was chosen from the scale's "a table, tree, or chart" row, but these four carry five to six rows against a canonical dataset that never grows: at `320` roughly a third of each frame was empty. The charts keep `320`.

**Unresolved: the "Show source" toggle overlaps the stage on `/components/ComboBox` and `/components/LabeledFieldSet`.** On those two pages only, the toggle is positioned as if the stage were as tall as its content (`top: 51px` against a 120px stage; `top: 142px` against a 200px stage) and lands inside the frame. Root cause is in `VBox`, not in the docs app: [`VBox.resolveChildHeight`](packages/lib/src/typescript/lib/layout/VBox.ts#L585) applies the maximum *after* the minimum, so a child's maximum wins over its explicit minimum — the reverse of the invariant [`Component.clampPreferredToConstraints`](packages/lib/src/typescript/lib/core/Component.ts#L2709) documents ("an explicit minimum is always honoured, because the floor is applied last"). `ComboBox` sets a hard maximum height on itself ([`ComboBox.ts:736`](packages/lib/src/typescript/lib/component/input/ComboBox.ts#L736)); `Fit.getMaxSize` propagates it to the stage, which caps the stage's cell to ~47px even though the stage's explicit `minSize.height` is the demo's `height`. The stage element is then floored back to 120px by `clampHeight` (a `Panel` clamps to its own explicit constraints), but `VBox` has already advanced its cursor by 47px. Swapping the two clamps in `resolveChildHeight` is the candidate fix; `HBox.resolveChildWidth` should be read for the same ordering. Both live under `packages/lib/src`, which this plan's Non-Goals put out of reach, and the change touches every column in the library — it belongs in its own plan with the layout suite run against it.
