...
**`CheckboxBox.applyState`'s two CSS state classes are not actually mutually exclusive as the *caller* can invoke it, contradicting `## Architecture Decisions`' stated premise — found during the plan's own mandatory live-browser verification, not by a unit test.** The plan's `## Architecture Decisions` states as settled fact that "the two are mutually exclusive by construction: `applyState` never sets both", and derives the resting-isolation selector `#id:not(.selected):not(.indeterminate)` from that premise. This is true of `applyState`'s *own* body in isolation, but false of what a caller can pass it: `Checkbox.setIndeterminate` (unchanged by this plan, and unchanged from before it) deliberately leaves `selected` untouched — `this.applySelected(this.isSelected(), next)` — so calling `setIndeterminate(true)` on an already-selected `Checkbox` reaches `_box.applyState(true, true)` with both flags true. `setSelected` always forces `indeterminate` to `false`, so the reverse direction is safe; only this one direction is affected.

Pre-plan, this composite call was harmless: the old `applySelected` resolved a single CSS value per property through a JS ternary that unconditionally prioritised `indeterminate`, so "both true" produced one deterministic, correct visual with no CSS-cascade ambiguity. This plan's class-based mechanism changes that: `applyState` originally toggled `{ selected, indeterminate }` verbatim onto the element, so "both true" placed *both* `.selected` and `.indeterminate` on the same DOM node. Both are same-specificity sibling class rules, so which one visually wins would depend on stylesheet source order — accidentally correct today, but fragile, and a direct contradiction of the resting-isolation selector's own stated premise that the two classes never coexist.

Fixed by applying the same indeterminate-wins priority the style branch already uses to the class toggle itself: `applyState` and `render()`'s re-assert now toggle `.selected` only when `selected && !indeterminate`, so the two CSS classes are actually mutually exclusive rather than merely claimed to be. Pinned by a new test in `Checkbox.stateClassHoisting.test.ts` (`setIndeterminate(true) on an already-selected Checkbox never carries .selected onto _box alongside .indeterminate`), confirmed red against the pre-fix code and green after, and re-verified live in the browser (dual-class DOM state before the fix, single `.indeterminate` class with correct computed colours after). `RadioButtonRing` has only one state class, so it has no analogous gap and needed no equivalent change.
