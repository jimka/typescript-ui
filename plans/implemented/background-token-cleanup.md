...
**Step 13 (`TabButton.ts`) needed a guard the plan said was unnecessary.** The plan's Step 13.5 instructed deleting *both* `applyTabStyling()` lines (`setBackgroundColor` and `setBackgroundImage`) outright, on the claim that `super()` already applies the effective value once `_defaultTabButtonOptions` exists. That holds for `backgroundImage` — `Button`'s `applyChromeOptions` folds `_defaultOptions.backgroundImage` properly (`options.backgroundImage ?? this._defaultOptions.backgroundImage`) — but not for `backgroundColor`: `Button`'s `applyChromeOptions` override *also* unconditionally repaints a plain resting background (`BUTTON_RESTING_BACKGROUND = "var(--ts-ui-button-bg, transparent)"`) via a direct `this.setBackgroundColor(...)` call whenever the caller passed no explicit `backgroundColor`, bypassing `_defaultOptions` entirely (it checks the raw `options` argument, not the merged defaults bag). Since `ToggleButton`/`TabButton` always forward `options: undefined` to `Button` (only `subclassDefaults` and, later, `applyOptions(options)` carry the caller's values), this fires on every `new TabButton(...)` before `applyTabStyling()` runs, and — because `setBackgroundColor` writes into `_options.backgroundColor` — it permanently defeats `getBackgroundColor()`'s pure-fallback read of `_defaultOptions.backgroundColor` for the bare-construction case. A contract-derived test written before this Step (`new TabButton('x').getBackgroundColor()` should equal `_defaultTabButtonOptions.backgroundColor`) failed exactly this way, confirming the gap empirically rather than by inspection alone.

The fix that keeps: `applyTabStyling(options?: TabButtonOptions)` now takes the constructor's `options` and reasserts `backgroundColor` with an options-aware fold — `this.setBackgroundColor(options?.backgroundColor ?? (this._defaultOptions.backgroundColor as string))` — instead of a bare hardcoded literal. This preserves both required outcomes (a caller override still wins; the bare-construction default is `_defaultTabButtonOptions.backgroundColor`, not `Button`'s resting background) without touching `Button.ts`, which is out of this plan's scope. `backgroundImage`'s line was deleted exactly as the plan specified — no fold was needed there. The plan's Expected Behaviour item 5 illustrative assertion (`new ToggleButton("x", undefined, { backgroundColor: "green" }).getBackgroundColor() === "green"`) is also affected by the same `Button` mechanism and does not hold at the bare `ToggleButton` level (no consumer beside `TabButton` needs it to); the shipped test instead pins the plumbing via `backgroundImage`, which is unaffected, and documents the `backgroundColor` caveat inline.
