...
- **The step 7 registry row's `make()` needed `await store.load()` plus explicit `setWidth`/`setHeight`/`doLayout()` calls, and the registry's `make` signature needed to support a `Promise<Component>` return.** The plan's own `## Potential Challenges` flagged this row's exact pool-access cast as "a sketch, not a fully verified sequence" and called for confirming it against the step 6 test's own version first. That confirmation (in `CellEditorPool.styleRuleDisposal.test.ts`) showed `Body.getRowPool()` stays empty after a bare `table.getElement(true)` — `Body.getVisibleRecords()` reads from the store's loaded-records list, which `MemoryStore` only populates once its inherited `AbstractStore.load()` resolves, not merely once `proxy.setData()` has run at construction. The `Table` row already in `dispose-full-teardown.test.ts` sidesteps this because it never renders a data row (its store's data array is empty and it drives `showColumnMenu` instead of a cell edit). Fix: the new `'Table (cell-editor pool)'` row's `make()` is `async`, awaiting `store.load()` before `getElement(true)`/`setWidth`/`setHeight`/`doLayout()`; `REGISTRY`'s `make?: () => Component` widened to `make?: () => Component | Promise<Component>`, and the shared test loop's `it(name, () => {...})` became `it(name, async () => {...})` with `await make()` at both the warm-up and real call sites — a no-op change for every pre-existing synchronous row, since `await` on a non-Promise value resolves immediately.
