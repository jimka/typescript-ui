...
> **Implementation correction (during /implement).** The premise "the browser reports the rotated width/height through `getBoundingClientRect`, so the existing preferred-size measurement stays correct" does **not** hold here: [`Button.getPreferredSize`](../../src/typescript/lib/component/button/Button.ts) is **analytic** (computed from text metrics + insets, not from the DOM), so `writing-mode` rotates the painted/hit-tested element but leaves the reported preferred size un-rotated. Hit-testing is therefore correct (the real DOM box rotates), but **layout sizing is not**. Resolution implemented: the manager derives every tab's main-/cross-axis extent itself, orientation-aware — `buttonMainExtent` returns `p.width` for north/south and west/east-rotated text but `p.height` for west/east-horizontal text; `buttonCrossExtent` is the complement. The west/east strip thickness is **computed dynamically** (`max(STRIP_THICKNESS, max button cross extent, tool-group cross extent)`) instead of the fixed `STRIP_THICKNESS` seed, so a west/east strip with horizontal text grows to the longest label. The `"content"` and overflow width branches pin/floor each rotated-text wrapper to its derived main extent (the box would otherwise read the un-rotated preferred extent); the `"equal"`/`"fixed"`/`"fill"` branches already route through the derived `buttonMainExtent`.

### Tool group is a hand-positioned overlay, not a weighted spacer box child

> **Implementation decision (during /implement).** The Internal-Structure sketch realises the tool slot + `start`/`end` alignment with a *weighted spacer child* inside the toolbar box. That makes the toolbar's box children a mix of wrappers, spacer, and tool group whose ordering flips with alignment — which breaks the 1:1 mapping between tab wrappers and toolbar child indices that this plan's own `reorderTab(wrapper, toIdx)` and the DnD slot-math rely on. Resolution implemented: the **tab wrappers stay the toolbar box's only children** (indices line up with `_tabs`). The **tool group is a hand-positioned overlay** (its own inner `HBox`/`VBox`) raw-appended next to `_indicator`; its main-axis space is **reserved via the toolbar's insets** (right/left for north/south, bottom/top for west/east, picked from `tabAlign`), and `end`-alignment of the tabs within the remaining region is realised by a **hand-offset** applied to each wrapper after the box runs. Observable behaviour matches the plan; the box-child churn and its index hazard are avoided.
...
| Modify | `src/typescript/lib/core/Component.ts` (add the tracked `setWritingMode`/`getWritingMode` CSS setter the vertical-orientation feature applies to tab-button elements — `setElementStyles` is `protected`, so the writing-mode write needs a public typed CSS setter mirroring `setOverflowX`/`setPointerEvents`) |
