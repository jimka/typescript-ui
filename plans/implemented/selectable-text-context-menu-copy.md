...
- **Two files outside the plan's "Files to Create / Modify / Delete" table needed updating:** `packages/lib/tests/component/dispose-full-teardown.test.ts` and `packages/lib/tests/component/dispose-listener-teardown.test.ts`. Both are class-wide registries that scan the library source at test time (`classesDeclaringDestructor()` / `classesRegisteringEventListeners()`) and fail when a class gains its first `protected destructor()` or `Event.addListener(...)` registration with no covering row and no baseline entry. `SelectableText` gained both for the first time in this plan, so each registry needed a new `covers: ['SelectableText']` row — mirroring the existing `Menu`/`MenuButton` lazy-overlay row for the teardown registry (materialise `_contextMenu` by calling the private `handleContextMenu` directly, then let `dispose()` clean it up), and the existing `Link` row (SelectableText's own wire-once precedent) for the listener registry.
- **Two pre-existing `Markdown.test.ts` assertions needed adjusting**, not because of a defect but because of a widened invariant: `SelectableText`'s and `Markdown`'s permanent `contextmenu` registration means `Event._registeredComponentIds()` now always contains a live instance's id, regardless of its viewport-watch state. Two viewport-gate tests ("`onViewportPass()` ... disarms the watch" and "`setMarkdown()` drops any kickoff ...") used `.not.toContain(md.getId())` as a proxy for "the viewport listener was torn down" — a proxy this plan's change makes permanently true. Both were changed to a before/after delta on `Event.listenerCounts().viewport` instead (mirroring `tests/unit/core/Event.test.ts`'s own before/after idiom), which isolates the viewport-specific registration the tests actually care about.
- **`npm run build:docs` fails in this environment independently of this plan.** A clean run on this branch's base (`feature/clipboard-context-menu-foundation`, no changes from this plan applied) fails identically with `ENOENT: ... copyfile '.../dist/index.html' -> '.../dist/404.html'` from the `spa-fallback` Vite plugin — confirmed by stashing every change from this plan and re-running the build. `packages/docs`'s own `typecheck` script independently fails too, on an unrelated stale-`dist` issue (`separator-basic.ts` referencing a `Separator` export `packages/lib`'s built `dist` doesn't currently have) that also reproduces on the same unmodified base. Neither is caused by, or fixed by, this plan; `packages/docs/tests/links.test.ts` (211 cases) and `packages/docs`'s own `test` script (2548 cases) both pass cleanly against this plan's documentation edits.
