...
**`getMinSize()`'s new `.broken` gate uses `!this.instanceLayer().authored.minSize`, not `!this.getMinSizeConstraint()`.** The plan's `## Internal Structure` snippet gated the new branch on `!this.getMinSizeConstraint()`, but `getMinSizeConstraint()` folds in the class-level `{0, 0}` default every `Component` carries (`ComponentDefaults.ts`'s `BASE_DEFAULTS.minSize`), so it is never `null` and the literal gate would never fire — dead code. This is the exact same defect `image-object-fit-aspect-ratio`'s own Implementation Notes already recorded for its sibling `_aspectMinSize` branch in this same method, which already uses `!this.instanceLayer().authored.minSize` (true only after an actual `setMinSize` call) for this reason. The new `.broken` branch was written to match that live precedent instead of the plan's stale snippet, keeping both early-return guards in `getMinSize()` consistent with each other.

**`handleError()`'s explicit-preferredSize branch uses `_hasExplicitPreferredSize` / `notifyIntrinsicSizeChanged()`, not `getPreferredSizeConstraint()` / `setPreferredSize()`.** The plan's `## Internal Structure` snippet for `handleError()` was written to "mirror `handleLoad()`'s own shape," but against an earlier version of `handleLoad()` that gated on `getPreferredSizeConstraint()` directly. `image-attribute-options-surface`'s own Implementation Notes record that `handleLoad()`'s gate was already changed to the declared `_hasExplicitPreferredSize` flag before this plan landed, specifically because `getPreferredSizeConstraint()` cannot distinguish an explicit caller size from `handleLoad()`'s own prior auto-publish. `handleError()` was implemented against `handleLoad()`'s real current shape rather than the plan's snippet: it checks `_hasExplicitPreferredSize`, calls `super.setPreferredSize(...)` directly in the auto-publish branch (so the placeholder publish doesn't itself trip the flag), and calls `this.notifyIntrinsicSizeChanged()` in the explicit branch — relaying the `.broken`-driven `getMinSize()` change upward even though `preferredSize` itself is pinned, the same relay `handleLoad()`'s own explicit branch already performs for the equivalent natural-size-driven case. A test (`'relays a min-size change upward when an explicit preferredSize skips the publish on error'`, `Image.test.ts`) pins this behaviour.

**A first audit round found four BLOCKING gaps, all fixed:**
- **The demo built for the plan's own manual-verify step (`## Expected Behaviour`'s closing bullet, `## Verification`'s smoke-test bullet) couldn't show what it was meant to show.** `image-loading-error.ts` originally pinned `preferredSize: {64,64}` on both images, so `handleError()` took the `_hasExplicitPreferredSize` branch and never published the 48x48 placeholder — the broken box stayed pinned at 64x64. Both `preferredSize` options were dropped so the demo actually exercises the auto-publish path the manual-verify step names.
- **`image-loading-error.ts`'s `on('load', ...)` / `on('error', ...)` listeners were inline arrow functions**, violating `ARCHITECTURE.md`'s *Listeners must reference a named function* (the adjacent `image-auto-fit.ts` demo this batch already added does it correctly). Replaced with named `reportOkLoaded`/`reportBrokenFailed` functions declared inside `create()`, which now also call `isLoading()`/`isBroken()` themselves rather than setting a literal string — so the demo genuinely exercises the two new public methods the docs page claims it does.
- **The declared `.loading`/`.broken` `ownStyleStates` rules — the entire point of choosing that mechanism over hand-written CSS — had no test asserting their actual chrome, dedup, or guard clause.** Added `Image.classStyleHoisting.test.ts`, mirroring `SplitGutter.classStyleHoisting.test.ts` (the plan's own cited precedent): one test confirms `.Image.loading` and `.Image.broken:not(.loading)` each materialise with their intended `backgroundColor` on the priming instance and that a second instance writes no duplicate `#id` declarations; a second test confirms an instance-level `setBackgroundColor` isolates onto `#<id>:not(.loading):not(.broken)`.
- **Two shipped doc statements about `getMinSize()`'s `{0,0}` default became false** once `.broken` could raise a floor: `Image.ts`'s `ImageOptions` `@remarks` and `Image.md`'s matching Notes bullet both said `{0,0}` held until `setMinSize` or `preserveAspectRatio` raised it, with no mention of a failed decode. Both now name the broken-state floor as the third case that raises it.

**A second audit round found the first round's own fixes introduced new problems, all fixed:**
- **`changelog/next.md`'s new bullet claimed an explicit `preferredSize` overrides the broken-state minimum floor.** It doesn't — only `setMinSize` does, per `## Expected Behaviour`'s own size table and the test that pins it. Reworded to state the preferred-size and minimum-size overrides separately.
- **`image-loading-error.ts`'s doc comment made three claims the demo didn't actually back up:** the SVG had no explicit `width`/`height`, so its live natural size is 150x150, not the claimed 64x64 (fixed by adding `width="64" height="64"` to the SVG root, matching `image-auto-fit.ts`'s own established convention for a deterministic natural size, rather than just correcting the prose to describe an accidental number); "grows into" the placeholder box overstated a direction that wasn't checked (reworded to "settles into"); and both sources resolve within a frame, so the `.loading` wash is not something this specific demo can show a reader — the comment no longer claims it is (see below).
- **The Implementation Notes' manual-verification paragraph implied the plan's full non-automatable bullet was covered, when only the broken-state half was.** Corrected below: the `.loading` wash's *rule* (that `.Image.loading` carries the intended `backgroundColor`) is verified offline by `Image.classStyleHoisting.test.ts`; its live appearance specifically in `image-loading-error.ts` was not captured, because both sources resolve within a frame in that demo and repeated attempts to catch it mid-flight via Chrome DevTools network throttling (`Slow 3G`) were defeated by the docs dev server's own bundle-load time dominating the throttled window. Recorded honestly rather than left implied.
- **A relative path in this section's first draft (`../.claude/skills/_shared/worktree.md`) didn't resolve from either the repo root or the worktree.** The correct reference is `~/.claude/skills/_shared/worktree.md`, used below.

**A third audit round found one BLOCKING gap, fixed:** `image-loading-error.ts`'s `height` doc comment still said "the two 64x64 images" — the second round's fix gave the SVG an explicit `width`/`height="64"` so the *loaded* image genuinely is 64x64, but left this earlier line's claim that *both* images are, when the failed one is 48x48. Reworded to name each image's own size. The round also confirmed the round-2 fix for the `.loading` manual-verify gap (below) is an honest documented substitute, not a silent skip: the live-measured `.Image.broken:not(.loading)` rule (higher specificity, same declaration shape) and `Image.classStyleHoisting.test.ts`'s offline assertion on `.Image.loading` together leave nothing about the `.loading` rule's correctness unverified — only its live *timing* in this one demo is undemonstrated, and that gap is named explicitly rather than implied away.

**Manual verification, precisely what was and wasn't checked:** `packages/docs/src/demos/image-loading-error.ts` was loaded on the live `/components/Image` docs page (Chrome DevTools, against a worktree-local `node_modules/@jimka/typescript-ui` symlink pointing at this worktree's own `packages/lib` rather than the main tree's — the pitfall `~/.claude/skills/_shared/worktree.md` and this project's own memory both flag — so the page genuinely exercised this branch's build, not a stale one). Confirmed via `getBoundingClientRect()`/`getComputedStyle()`: the failed-source image renders at exactly 48x48px, carries the DOM classes `ts-ui-component Image broken`, and computes `backgroundColor: rgba(220, 60, 60, 0.08)` (the `--ts-ui-image-broken-bg` fallback); the loaded image carries neither `loading` nor `broken` and computes a transparent background. Both status lines read `isLoading()`/`isBroken()` correctly ("loaded" / "broken"). **Not captured live:** the `.loading` wash's on-screen appearance before a source settles — both this demo's sources resolve too fast, and forcing a slow one via network throttling was not achieved within a reasonable effort. The `.loading` rule's correctness (the CSS it publishes) is instead pinned by `Image.classStyleHoisting.test.ts`'s offline assertion that `.Image.loading` materialises with `backgroundColor: var(--ts-ui-image-loading-bg, rgba(0, 0, 0, 0.06))`.
