...
- **Step 2's "assert via the same mechanism the field's own existing debounce tests use" (AutoCompleteField) had no existing debounce test to follow.** `AutoCompleteField.test.ts`'s file header explicitly scoped debounce timing as out of scope prior to this plan. Behaviour 20's test was designed from scratch instead: `vi.useFakeTimers()`, a real `_textField.paste()` call, `vi.advanceTimersByTime(50)` (via an explicit `debounceMs: 50` construction option, since no `getDebounceMs()` getter exists to read the default back), and a `vi.spyOn(dropdown, 'show')` assertion — mirroring the `vi.spyOn(Menu.prototype, 'show')` idiom this plan already uses for the base `TextInput` menu tests.

- **A real cross-file test-harness fragility, not covered by the plan, had to be diagnosed and fixed for the plan's own real-dispatch tests (behaviours 6, 12, 15-17, 18-20) to pass reliably.** `Event.ts` arms its window-level capture listener for a DOM event type exactly once per module lifetime (once per test *file*, given Vitest's default file isolation) and never re-arms it against a later `installTestDOM()` sink unless every listener of that type is disposed first, draining the type back out of `Event`'s internal bookkeeping. `TextInput`'s constructor now registers both `"contextmenu"` (new) and `"input"` (pre-existing) unconditionally, so any undisposed `TextField`/`TextArea`/`PasswordField`/`DateField`/`AutoCompleteField`/`NumberSpinner` constructed anywhere earlier in the same test file — including ones built at `describe()`-collection time by a helper like `AutoCompleteField.test.ts`'s pre-existing `matcherFor()` or `DateField.test.ts`'s `parser()`/`formatter()` — permanently pins that registration to whichever sink was active at that construction, silently breaking every later `Event.fireEvent`-based real-dispatch assertion in the file (a call that looks correct but the dispatched event simply never reaches the listener, since the currently-active sink's own listener map was never populated).
  - Fixed by disposing every field these pre-existing helpers/tests construct (`TextInput.test.ts`'s new blocks track their field in an outer variable and dispose it from `afterEach` — robust against a thrown assertion, unlike a manual end-of-test `dispose()` call, which a mid-test failure would skip; `DateField.test.ts`'s `parser()`/`formatter()` and its two previously-undisposed tests; `AutoCompleteField.test.ts`'s `matcherFor()` and its two previously-undisposed tests).
  - This is a pre-existing test-hygiene gap the plan's own cross-component real-dispatch tests were the first to depend on and expose — not a change to any production code path, and not a deviation from the plan's design. `NumberSpinner.test.ts` needed no such fix (it wires no `"input"` listener of its own and had no pre-existing DOM-touching test to begin with).

- **Two pre-existing "mechanically enforced" regression-guard tests needed updates for `TextInput`'s new unconditional `_contextMenu: Menu` field**, both correctly reflecting real, intended side effects of the plan's own design rather than bugs:
  - `Component.test.ts`'s hand-counted theme-listener-count assertion for a fresh `TextField` moved from `base + 2` to `base + 3` — the third subscription is `Menu`'s own `applyRebuildChrome()` dispatching a themed `setBorder()` during its (now eager, per-`TextInput`-instance) construction.
  - `dispose-full-teardown.test.ts`'s class-scan registry gained a `TextInput (via TextField)` row (`covers: ['TextInput']`), exercising `handleContextMenu` before disposal so the lazily-materialised menu rule is actually present to be released — mirroring `Table`'s existing `_columnContextMenu` row's own comment and shape.

- **Manual smoke test (behaviours 22-26) confirmed live** via the running demo app (`packages/lib`'s own `vite` dev server, port 8016 since 8015 was already in use by another worktree): right-click on a plain `TextField` opens an enabled Cut/Copy/Paste menu at the click point over a selection, and a Copy/Cut-disabled Paste-enabled menu over a collapsed caret; `Cut` correctly removed the selected text and wrote the clipboard (verified by the field going empty); the same menu opens correctly positioned inside a `NumberSpinner`'s inner field and inside a `DateField`'s inner text input (not offset by either composite's own chrome); right-clicking an `AutoCompleteField` while its suggestion dropdown is open shows both simultaneously with no visual interference, confirming `Menu.show()` does not blur the field. Two behaviours could not be fully verified live and are covered only by the automated suite: behaviour 23's read-only/disabled Copy-only case (the demo app has no read-only/disabled text field to right-click) and behaviour 25's paste-into-`DateField` value update (the automated browser session's `navigator.clipboard.readText()` was denied by the browser's own permission model even after a real synthetic click, so `paste()` correctly resolved `false` and left the field unchanged per its documented "browser denied the read" contract — the exact case `TextInput.test.ts`'s stubbed-`null` test already pins).
