...
**`handleLoad()`'s same-size early return also checks `!this.isBroken()`, a term the plan's own `## Internal Structure` snippet and step 20's/`## Verification`'s `grep -n "handleError\|isBroken\|\.broken"` check both said would introduce zero matches.** `handleError()` (from `image-loading-error-states`, already landed on this branch) never clears `_naturalSize`, so a same-size recovery — an errored candidate followed by a `load` for a candidate whose natural size happens to match what was cached *before* the error — hit the literal guard's early return before it ever reached the `setStyleState(".broken", false)` line two statements below, stranding `.broken` (and its 48×48 `getMinSize()` floor) on a healthily-recovered image forever. The guard now reads `if (!this.isBroken() && this._naturalSize && …)`, so a `.broken` instance always falls through to the full republish path regardless of whether the freshly measured size matches the stale cache. A regression test (`'a same-size load after an error still clears broken and republishes, instead of being swallowed by the size-diff guard'`, `Image.test.ts`) pins this: it drives `_onLoad()` → `_onError()` → `_onLoad()` with the same seeded natural size across all three and asserts `.broken` clears, `"load"` fires, and the size republishes. Step 20's own instruction ("this plan never assumes `image-loading-error-states` has landed") is narrower than what the plan's diff actually needed: the size-diff guard's correctness turns out to depend on `handleError()`'s real shape once that plan has landed, not just on whether `_onError()` exists as a callable field.

**Two audit rounds found BLOCKING gaps, all fixed:**
- **The first round found the `.broken`-stranding bug above, a self-contradicting `handleLoad()` doc comment** (the retained "Clears both `.loading` and `.broken` unconditionally… Re-emits `"load"` either way" directly contradicted the paragraph describing the early return, added two sentences later — reworded to state the guard's actual, now `.broken`-aware, condition), **two false-negative tests, and two documentation gaps.** The two disposal-settlement tests (`'a decode settlement after disposal does not throw and does not fire "load"/"error"'`) passed even with `triggerDecode`'s `this.getElement()` disposal recheck deleted, because `Component.destructor()` clears `_element` but `getElement()`'s `DOM.source.getElementById` fallback keeps resolving through `TestHandleTable._byId`, which nothing evicts on removal in the offline harness (confirmed against `Component.ts:1146`/`1254-1267` and `TestDOM.ts`'s own `_byId` map). `fired` stayed `0` only because `registerListenerBag`'s `onDestroy` hook clears the `ListenerBag`, not because the guard's `getElement()` branch no-oped — both tests now carry a comment recording exactly this, naming what they do and don't prove, rather than silently overclaiming; the two srcset/sizes cache-invalidation tests asserted only `decodeImage` call counts, never that `_naturalSize` was actually cleared, so two more tests were added that drive a same-size settle *after* `setSrcset`/`setSizes` and assert the republish the cleared cache should produce. Documentation: `changelog/next.md` had no entry for this plan (both sibling Image plans' own doc commits each added one), and no `srcset`/`sizes` demo existed, so the plan's own manual-verify bullet had nothing to point at — `packages/docs/src/demos/image-srcset.ts` and a new "Responsive sources" section in `Image.md` were added, alongside the two `next.md` entries (`## Breaking changes` → `### Core` for `DOMSink.decodeImage`, `## Changed` → `### Components` for the `srcset`/`sizes` + proactive-decode feature). Both files are additions beyond this plan's own `## Files to Create / Modify / Delete` table, which predates the audit loop that added them.
- **The second round found the `.broken` fix above was undocumented** (this note), and that the first round's own demo fix repeated the *exact* defect `image-loading-error-states`' own audit had already found and fixed in `image-loading-error.ts`: pinning `preferredSize` on the demo's `Image`(s) routes `handleLoad()` down the `notifyIntrinsicSizeChanged()` branch, making the natural-size auto-publish this plan's whole size-diff rule governs structurally unobservable, and the demo's per-image fixed-pixel `sizes` values could never cross a breakpoint at all. Rebuilt as a single `Image` with no `preferredSize` and a real `(min-width: 600px) 480px, 100px` breakpoint `sizes` value. Manually verified against a real Chrome instance (`npm run docs:dev`, resized past and reloaded across 600px): a fresh page load at each width resolves the correct candidate and republishes the matching natural size with no error, matching the plan's `## Expected Behaviour` manual bullet. A *live* in-place resize across the breakpoint does update the underlying `<img>`'s `naturalWidth`/`naturalHeight` (confirmed via `evaluate_script` against `window.devicePixelRatio`/`img.naturalWidth`) but did not reliably re-fire the native `load` event in the Chrome instance tested, so the demo's status line does not always update without a reload — recorded honestly in the demo's own doc comment as a platform limit of relying on `load`, not a defect in `handleLoad()`'s guard, which the offline test suite already pins independently of what any one browser does with a live resize.

**A third round found three BLOCKING gaps, all fixed:**
- **`handleError()` had no equivalent of `handleLoad()`'s own same-state guard, so a genuinely broken source double-fired `"error"`.** A rejected `decode()` and the native `error` event can both independently fire for the same failed source (`decode()` forces the same fetch the native listener also observes completing) — exactly the `load`-side double-fire scenario `## Architecture Decisions`' *A `load` signal republishes only when the measured size actually changed* / `[^size-diff-churn]` already reasons about, but the reasoning was never carried over to the error side, and neither `handleError()` nor `triggerDecode()`'s catch branch guarded against it. `handleError()` now returns early when `this.isBroken()` is already `true`, mirroring `handleLoad()`'s shape. A regression test (`'a rejected decode() followed by the native error event for the same failed source does not double-fire "error"'`, `Image.test.ts`) drives a rejected `decode()` then the native `_onError()` bridge for the same source and asserts `"error"` fires exactly once. This is a second, independent deviation from `handleError()`'s plan-predates-`image-loading-error-states` shape, for the same underlying reason as the `.broken` guard above: this plan's `_onError()`-is-a-stable-field assumption held, but `handleError()`'s *behavioural* shape once landed still needed a guard this plan's own snippets didn't anticipate.
- **The `applyOptions` dispatch lines for `srcset`/`sizes` (step 11) had no test constructing `new Image(src, { srcset, sizes })`.** The only options-bag construction test predates this plan and never gained a `srcset`/`sizes` case, so deleting both dispatch lines left the whole suite green. Added `'writes srcset and sizes attributes from the options bag on first render'` (`Image.test.ts`), extending `describe('Image options-bag construction', …)`.
- **The class-level `Image` JSDoc and `Image.md`'s matching Notes bullet both claimed the natural-size publish and `"load"` re-emit happen "only when" the measured size differs — an absolute claim the `.broken` exemption (this note, above) had already made false.** Both reworded to name the exemption explicitly, matching the already-accurate private `handleLoad()` doc comment cycle 1 fixed.
