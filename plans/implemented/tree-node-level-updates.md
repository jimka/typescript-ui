...
- **Re-inserting a node whose own load is still in flight** could let the old load commit and the new `expandNodeAsync` resolve `false`. `setNodes` with the same node objects had the same edge; at `f08be3da` the identity-checked `_pendingExpansions` cleanup ([Tree.ts:875-882](packages/lib/src/typescript/lib/component/tree/Tree.ts#L875-L882)) kept that map consistent. Not addressed here; fixed afterwards on `feature/tree-reveal-structural-races`, which deleted `_pendingExpansions` and gave each load a seq of its own to check, so only the newer load commits (see *Implementation Notes*).
...
- **`revealByPredicate` walking a subtree that a concurrent structural call detached** was listed here as a Non-Goal because "that race predates this plan". That was true only of `setNodes`. `removeNode` and `setChildren` are new, so their races with a reveal were new too: a reveal could return a removed node, expand it, and cache its loaded children. The guard in step 5 covers only a parent whose children were taken over. The follow-up branch `feature/tree-reveal-structural-races` fixed these races; see *Implementation Notes*.
...
**The reveal guard also covers a rejected load.** The `_ensureChildrenLoaded` snippet in `## Internal Structure` guarded only the success path. A reveal load that a structural call took over and that then rejected still returned `[]`. The reveal skipped the caller's children and could resolve `null` with the target in the tree. That breaks the plan's own rule: "the reveal keeps the children the caller supplied and discards its own loader result". The `catch` then applied the same `_loadedNodes` check, the way `_loadAndExpand` checked for an orphaned load on both its success and failure paths. The follow-up branch below replaced both with one `_settleLoad`, which settles the node's single shared load on either path, so neither that `catch` nor `_ensureChildrenLoaded` survives. A test pins the rejected case next to the resolved one.

**`revealByPredicate`'s walk re-reads the tree after every await** (corrected on the follow-up branch `feature/tree-reveal-structural-races`). The plan did not touch the walk, but the audit found a race this branch introduced. The walk looped over the live sibling array across `await`s, and `insertNode` and `removeNode` now splice that array in place. Removing an earlier sibling while the walk waited on a load therefore shifted it past a later sibling the tree still held, and the reveal resolved `null` with a match in the tree. This branch first made the loop iterate a snapshot, `[...nodes]`, and an earlier version of this note called what the snapshot left "two narrower races, both left alone as the Non-Goal intends". That was wrong on both counts. A later review showed the snapshot made two interleavings worse. A sibling removed ahead of the walk was still visited, and a lazy one was loaded and marked loaded while detached. A sibling inserted ahead of the walk was never visited, so the reveal resolved `null` with the match in the tree. Neither is the pre-existing `setNodes` race the Non-Goal describes. The cited precedent was wrong too: `Split` and `MultiSelectList` snapshot a collection their own synchronous loop body mutates, whereas here another caller changes the tree during an `await`.

The codebase handles a result made stale during an `await` with a check after the await: the orphan check `_loadAndExpand` made after its await (at `f08be3da`, before this fix; `_settleLoad`'s seq check below now does that job), `AbstractStore.load`'s `_loadSeq`, and `Image`'s `_decodeGeneration`. The walk has that shape. `setNodes`, every structural call, and every lazy load that commits a node's children bump `_structureGeneration`. When it has moved after an await for any reason but a load the walk waited on, the walk re-reads the live array (`this._nodes` or `parent.children`), stops if its level has left the tree, and otherwise restarts the level from its first child. Nodes it has already tested are passed over but still descended into. So a node inserted anywhere is found, including under a branch already searched or into a list `setChildren` replaced, and a node removed ahead of the walk is never reached. After `setNodes`, the root level restarts on the new roots. A load the walk waits on commits nothing once the tree has dropped its node, even if the node is put back before the load settles; the last note below says how. The match is expanded in the same synchronous step that tested it, so no structural call can make its path stale between the two. Each structural call's interleavings with a running walk have a test, and three pin `setNodes`: the walk searches the new roots from the first rather than indexing them at its old position, abandons a level the reset detached, and commits nothing from the load of a replaced node.

**A node has at most one lazy load, shared by every reveal and expand that needs its children** (same follow-up branch). Before this fix (at `f08be3da`), a reveal (`_ensureChildrenLoaded`) and an expand (`_loadAndExpand`) each called `loadChildren` on the same node. An expand whose call settled second overwrote the children the reveal had committed, detaching the subtree the walk was still searching, so the reveal could return a node the tree no longer held. Two calls can also split, one resolving and the other rejecting, and then no answer for the expand fits the documented rejection contract in every settle order, so the calls are shared rather than arbitrated. `_pendingLoads` maps a node to its one load, and `_loadLazy` joins it or starts one. That is the join `_expandLazy` made at `f08be3da` through `_pendingExpansions` for concurrent expands, moved down to the load so that a reveal joins too; since the load now commits the expansion, joining the load joins the expansion, and `_expandLazy` and `_pendingExpansions` are gone. `_settleLoad` commits the outcome once, before any caller resumes, the way `ElkLayoutEngine.ensureElk` runs its adopt-or-terminate step once inside the construction every caller waits on. A resolve attaches the children and marks the node loaded, and when an expand is waiting (the node is in `_loadingNodes`, which also drives the spinner) it expands the node, renders, and emits `"expand"`. A rejection leaves the node collapsed and unloaded, so the next expand or reveal starts a fresh load, and emits `"loaderror"` only when an expand is waiting; a load only reveals wait on fails silently, as a reveal's load did at `f08be3da`. Committing inside the load rather than in each caller keeps the result independent of which caller resumes first: otherwise a reveal that resumed first could expand the node, as its match's ancestor, before the expand looked, and the expand would skip its `"expand"`. The load's entry is registered before the loader is called, so a loader that calls back into the tree synchronously — a `setChildren` placeholder, `removeNode`, `setNodes` — drops its own load just as a caller doing it after an `await` does, and one that expands its own node joins that load; at `f08be3da` the expand path got this from marking the node loading before calling the loader, and the reveal path did not have it at all. The entry's `settled` promise is therefore created before `_settleLoad` runs and takes it over afterwards, and the loader is called inside a `Promise` executor, so one that throws instead of rejecting still settles the load, as `f08be3da` already turned such a throw into a `"loaderror"`. The render and event emission that close the commit run consumer code, so `_settleLoad` records what a renderer or an `"expand"`/`"loaderror"` listener throws in its result instead of rejecting the promise every waiter shares: the expands waiting on the load rethrow it, as the expand's caller saw such a throw at `f08be3da`, and a reveal sharing the load carries on, as its separate load let it then.

A structural call drops a shared load for every caller: each waiting expand resolves `false`, and nothing commits. A reveal tells the two kinds of drop apart by whether the node is loaded afterwards. `_supersedeLoad` marks a node whose children a structural call took over as loaded, and the walk searches the caller's children without testing the node again. A node the tree dropped is left unloaded, so the walk forgets it tested the node and, should it be back, tests and loads it afresh, joining the newer load if an expand has already started one. Tests run a reveal and an expand sharing one load in both orders against the same assertions. On a resolve, `loadChildren` is called once, the children are committed once, `"expand"` fires once, and the spinner shows while the expand waits. On a rejection, `"loaderror"` fires once, the expand resolves `false`, the node stays unloaded while the reveal finds a match elsewhere, and the next toggle calls `loadChildren` again. Others pin a reveal-only rejection (no `"loaderror"`, no spinner), a reveal and two expands firing `"expand"` once, a `removeNode` dropping a shared load before a fresh load on re-insert and re-expand, the commit landing before any caller resumes, a throwing loader, and a takeover leaving its node tested once. Removing each part of the change fails at least one of them.

**A load's commit bumps the structure generation** (same follow-up branch), since attaching children changes which nodes the tree holds. The bump sends a reveal back for children a later load commits under a node it passed after the load it waited on for that node failed; at `f08be3da` it resolved `null` there with the match in the tree. A reveal leaves out the bump of each load it waited on, whether it started the load or joined it, counted in `RevealWalk.ownCommits`: that commit only filled in the children of the node the walk was about to search beneath, and restarting for it would re-walk everything already searched once per load. Leaving out a joined load's bump is as safe as leaving out a started one's, because a load commits only while it holds its node's entry, which any structural call on that node's children deletes, so a commit only ever fills in absent or empty children. `_expandPathTo` relies on this when it asserts `this._locate(node)!`: everything that can detach a node bumps the generation, the only bumps a walk skips detach nothing, and a fallback would only hide a break in that rule. Tests pin each load path's bump (an expand, and a second reveal, loading the match under a node the walk passed) and the walk searching each level once when only its own loads change the tree.

**Each load checks that it still owns its node** (same follow-up branch). Before this fix (at `f08be3da`), `_loadAndExpand` asked only whether `_loadingNodes` still held the node. After `removeNode(L)`, `insertNode(…, L)` and a second `expandNodeAsync(L)`, the first load found the second's entry, committed its children, emitted `"expand"` and resolved `true`, and the second then resolved `false` with `L` expanded. That broke the JSDoc of both `removeNode` and `expandNodeAsync`, and a rejection of the dropped load fired a spurious `"loaderror"` while the newer load was in flight. Each load now takes a seq from `_loadSeq`, the `AbstractStore.load` token applied per load, and its `_pendingLoads` entry holds that seq. `_settleLoad` commits only if the node's entry still holds it. `setNodes`, `_pruneDetachedState` and `_supersedeLoad` delete the entry, so a dropped load commits and fires nothing, even when the node is back and a newer load holds the entry; that newer load alone commits, clears the spinner and settles its callers. `setNodes` handed the same node objects back is the same edge, the one *Potential Challenges* notes. Tests pin the resolved and rejected cases after a remove and re-insert, and the resolved case after `setNodes`.
