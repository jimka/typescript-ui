...
- **The `build/keepNames.ts` Architecture Decision (`## Architecture Decisions` → "One shared `keepNames` module…") and its `[^shared-keepnames]` footnote justify the approach only on technical-feasibility grounds — Vite's config loader bundling the relative import, and `moduleResolution: bundler` resolving it under TypeScript — without addressing why this repo's established cross-package sharing mechanism (an npm-workspaces package dependency: a bare `@jimka/typescript-ui/…` specifier resolved through `node_modules`, the pattern every existing `packages/docs` → `packages/lib` reference uses) doesn't apply here. Recording that now: the workspaces-package mechanism is for *published, consumer-facing* code reached through a package's own `exports` map — it requires a `package.json`, a workspace-array entry, and a devDependency edge, none of which make sense for four lines of build-time-only config consumed exclusively by Vite's own config loader, never by application source, and never shipped in any tarball (`packages/lib`'s `files` array covers only `dist/lib` and three metadata files, so a repo-root import cannot leak into a published package either way — see `[^shared-keepnames]`). Standing up a workspace package for it would be exactly the kind of unrequested ceremony `CLAUDE.md` §2 rules out. A repo-root relative import — a pattern with no prior instance in this codebase, confirmed by grep (`find . -maxdepth 1 -type d`, `grep -rn "from '\.\./\.\./"` across every package's `src` and Vite configs) — is the plan's genuinely new pattern for this one build-tooling case, distinct from and not competing with the workspaces convention that governs runtime code sharing.
- **Both `## Expected Behaviour` → Manual verification steps were performed and passed.** `npm run build:docs` was run, `packages/docs/dist` was served with `vite preview`, and a real browser (chrome-devtools MCP) clicked through `/recipes/drag-and-drop`'s repaired `DragManager` link, confirming it navigates to `/api/overlay/namespaces/DragManager` and renders the real API page (Functions: `cancel`, `isDragging`, `makeDragSource`, `makeDropTarget`) rather than 404ing; `/components/Button` was also loaded directly and its console carried no errors, confirming the GitHub-repointed `ARCHITECTURE.md` link renders cleanly. Separately, `node packages/create-app/bin/create-tsui-app.js --help` was run directly: it printed the usage text and exited 0, confirming the bin wrapper path (not covered by the unit tests) works end to end.
