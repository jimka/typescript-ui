...
**Expected-Behaviour case 7's `LayerManager.getTopLayer()` assertion targets the child submenu, not the parent.** The plan's case 7 (persistent-mode submenu leaf with `closeOnActivate: false`) says the check should show "the parent is still the top layer." Reading `LayerManager.ts` shows `register()` is reached only from `Menu.showAnchored` (rebuild mode, via `show()`/`toggleFor()`) and `Menu.open()` (persistent mode); the persistent-mode constructor (`new Menu(items, onClose)`) never calls it, and `getTopLayer()` returns whichever registered layer is topmost on `LayerManager`'s internal stack. The test's top-level parent menu is a bare `new Menu(...)`, built the same way as the plan's other persistent-mode cases (6) and the existing `'Menu focus navigation (persistent)'` block, and is never `.open()`ed — so it can never literally be the registered top layer. The child submenu, opened via `handleItemOpenSubmenu`'s `submenuPanel.open(...)`, is the one that registers and becomes topmost. The implemented test (`packages/lib/tests/overlay/Menu.test.ts`, `'Menu closeOnActivate' > 'a closeOnActivate: false leaf inside an open submenu leaves the submenu open (persistent mode)'`) therefore asserts `LayerManager.getTopLayer()` equals the child submenu panel, not the parent menu — the literal wording is unsatisfiable given how the parent menu is constructed in this test, but the underlying intent (the submenu chain stays open, nothing was torn down) is preserved and verified.
