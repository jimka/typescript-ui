...
- **Expected Behaviour item 1's boundary formula is off by `GUTTER_SIZE / 2`; the test uses the corrected invariant instead.** The plan states the widened gutter's centre equals `leadingPane.getX() + leadingPane.getWidth()`. Tracing `Split.ts`'s actual `doLayout` math (the running `x` a gutter is placed at is the leading pane's trailing edge; the widened box is `[x - overhang, x + GUTTER_SIZE + overhang]`, so its centre is always `x + GUTTER_SIZE / 2`, not `x`) shows the true, GUTTER_SIZE-independent invariant is the midpoint between the leading pane's trailing edge and the next pane's leading edge: `(leadingPane.getX() + leadingPane.getWidth() + nextPane.getX()) / 2`. `Split.gutterHitBox.test.ts` asserts that corrected formula rather than the plan's literal one; both are derived purely from the panes' own reported geometry, so this satisfies the plan's actual intent ("not hardcoding the gutter constant") — only the literal wording was imprecise.
- **`SplitGutter.setMovable(false)` now clears `.hover` directly**, a mechanism the plan's Internal Structure section didn't specify. Locking a gutter while the mouse sits stationary over it produces no native `mouseover`/`mouseout` (a resize under a motionless cursor fires neither), so without this the `.hover` wash could stay stuck on a locked, `cursor: default` gutter — contradicting manual-verify item 13. `setOpaque` needs no equivalent: `.opaque` outranks `.hover` in `ownStyleStates`' priority order, so a stale `.hover` there is masked in the generated CSS. Found and fixed during the audit loop; see `SplitGutter.hover.test.ts`'s `'clears .hover when locked mid-hover'` case.
- **Expected Behaviour item 11 and the `[^transition-clobber]` footnote's "persists through a drag for free" claim were backwards — found via live manual testing, not the audit loop.** `beginPointerDrag` (`PointerDrag.ts`) suppresses `pointer-events` on every `<body>` descendant for the drag's duration — the gutter itself included, since it lives under `<body>` like the rest of the app. That takes the gutter out of hit-testing, so the browser fires a **real `mouseout` on the gutter the instant the drag starts** (hit-testing falls through to `<html>`, which the same mechanism hands the drag cursor to), clearing `.hover` immediately instead of leaving it alone. The reported symptom: the highlight vanished the moment a drag began. Fixed by mirroring `Scrollbar`'s existing `_thumbHovered`/`_thumbDragging` split (`Scrollbar.ts`'s `updateThumbFill`, `_onDragStart`/`_onDragEnd` — a precedent this plan's own research pass missed): `SplitGutter` now tracks `_hovered` (real mouseover/mouseout) and `_dragging` (`onDragStart`/`onDragStop`) independently, and `applyHoverState()` derives `.hover` from `_hovered || _dragging`, so the wash stays on for the whole drag regardless of the suppression-triggered mouseout, and correctly drops at drag-end unless a real hover is also still active — see `SplitGutter.hover.test.ts`'s two new `'... for a drag ...'` / `'keeps the hover wash on through the mouseout ...'` cases.
- **The constructor's `mouseover`/`mouseout` wiring changed from `Event.addListener` to `Event.addSubtreeListener` — found via a second round of live manual testing.** Reported symptom: hovering the gutter, moving onto the chevron, then moving away from the gutter entirely (without re-crossing its plain background) left the highlight stuck on, clearable only by hovering the gutter again and leaving cleanly. Root cause: `Event.addListener` fires only when a real DOM event's exact `target` is the registered element (see `Event.ts`'s `baseListener`, and the codebase's established "`addListener` matches exact target ID only" characteristic) — it does not walk ancestors the way native bubbling does. The chevron (`_collapseButton`) is a real hit-testable descendant raw-appended straight into the gutter's element (`init()`'s `DOM.sink.appendChild(el, this._collapseButton.getElement(true)!)`), so a mouseout whose target is the chevron itself never reached the gutter's `onMouseOut` at all, leaving `_hovered` stuck `true`. `Event.addSubtreeListener` is the framework's existing primitive for exactly this ("container event delegation"): it walks from the real event target up through ancestors, so it now also matches when the target is the chevron. This is a "self" registration (`Event.addSubtreeListener(this, ...)`), not a cross-component `Event` bypass — consistent with `ARCHITECTURE.md`'s event rules. `onMouseOver`/`onMouseOut`'s own logic (including the `containsEventTarget` internal-crossing guard) needed no changes; only the registration API did. This is the one behaviour in the feature that could not be verified by calling `onMouseOver`/`onMouseOut` directly (the existing test style) — it required a real event dispatched through `DOM.sink.dispatchEvent(DOM.source.getWindow(), ...)` with the chevron as the event's `target`, to actually exercise the routing gap. Added in its own file, `SplitGutter.hoverSubtreeRouting.test.ts`, for the same reason `ComboBoxDropdownClose.test.ts` (this session's other test-isolation fix) needed one: none of this feature's `SplitGutter` test instances are disposed, so `Event`'s `installedListenerTypes` bookkeeping — which outlives `DOM.reset()` — would otherwise already show `mouseover`/`mouseout` installed against a stale window/sink by the time a same-file test needing real dispatch ran.
