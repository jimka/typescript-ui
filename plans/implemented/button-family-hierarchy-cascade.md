...
- **`resolveClassStateLevel`'s "own contribution" gate is `hasOwnExtractor` alone, not `hasOwnExtractor && ownBag`.** The plan's own sketch (`## Architecture Decisions` and `## Internal Structure`) gated a level's own contribution on `Object.prototype.hasOwnProperty.call(ctor, extractorMethodName) && ownDefaultsOf(ctor)`. Implemented literally, this silently drops `ToggleButton`'s `.selected` contribution: `ToggleButton` declares its own `extractSelectedClassDeclarations` but deliberately has **no** `ownClassStyleDefaults` of its own (`## Architecture Decisions`' "`Button` opts into the resting tier" section — `ToggleButton` contributes nothing new to the *resting* tier), so `ownDefaultsOf(ToggleButton)` is `null` and the `&&` gate would resolve `own` to `{}` even though `ToggleButton` plainly does own `.selected` declarations. That contradicts Expected Behaviour row 9, which requires `.ToggleButton.selected:not(:hover)` to exist. Implemented as: `hasOwnExtractor` is the sole gate (matching row 4's own wording, which cites `hasOwnProperty` alone), and `ownDefaultsOf(ctor) ?? {}` is only the *argument* passed to the extractor when it does own the extractor — never a second precondition. Verified against all 7 synthetic mechanism tests plus live `.ToggleButton.selected:not(:hover)`/`.TabButton.selected:not(:hover)` inspection in the browser (Expected Behaviour rows 9/13).
- **`resolveClassStateLevel` canonicalizes `ctor` and its parent, and terminates at `_rootCtor`, matching `resolveClassLevel` — neither appeared in the plan's sketch.** Both are necessary for correctness, not cleanup: `Object.getPrototypeOf` on a subclass returns the `callable()`-wrapped ancestor reference (per `ARCHITECTURE.md`'s "imports always use the callable name... this holds even for extends clauses"), and `resolveClassStateLevel` shares the `_owners` collision registry with `resolveClassLevel`, which already canonicalizes before touching it — an uncanonicalized reference would register a second, spurious `_owners` entry for the same class and could misreport a collision. `_rootCtor` termination avoids walking into `Component`'s own base class for no benefit (it declares no state extractor), mirroring `resolveClassLevel`'s own termination exactly.
- **Manual browser verification used `#/misc`, `#/tab`, `#/menubar`, and `#/style-audit` instead of the plan's literal `#/buttons`/`#/tabs`/`#/inputs`.** No such routes exist in `src/typescript/main.ts`'s route table (`slugify()`'d tab labels: `misc`, `tab`, `menubar`, `style-audit`, …) — the plan named the demo content descriptively rather than by its actual slug. `#/misc` covers plain `Button`/`SpinButton` (including the app's own top-nav `Tab`/`TabButton` strip, already selected/rendered there); `#/tab` covers the dedicated `TabDemoPanel`. All rows 8-13 were verified directly against the live `document.getElementById('Base').sheet.cssRules` and computed styles from a second dev server (port 8091) started from this worktree, not screenshots alone — confirmed `.Button.pressed` exists and `.TabButton.pressed` does not; `.TabButton.selected:not(:hover)` carries the tab-specific fill (`rgb(255, 255, 255)`) distinct from `.ToggleButton.selected:not(:hover)`'s generic grey; a `TabButton` element carries `ts-ui-component Button ToggleButton TabButton`; a `SpinButton` element carries `ts-ui-component Button SpinButton` and its own `.SpinButton` rule carries only the deviating border, not `backgroundColor`/`cursor`/`foregroundColor`/`shadow`; and pressing a live `SpinButton` correctly paints `.Button.pressed`'s shared background-image/foreground colour with no `.SpinButton.pressed` rule involved.
