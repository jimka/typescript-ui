...
The flagged `WindowBorder` instance gets a CSS class (`snap-target`) toggled on its DOM
element. The state rule itself is allocated via `Component.createStyleRule(".snap-target")`
(the same builder `ToggleButton` uses for `.selected`) so the box-shadow rule is materialised
through the framework's batched style channel. Only one border carries the class at a time —
swapping is `classList.toggle(false)` on the old winner followed by `classList.toggle(true)`
on the new winner, driven by the mousemove handler. The `WindowBorder` constructor allocates
the rule lazily and sets `boxShadow: var(--ts-ui-window-snap-glow)` on it.
...
1. **Register glyphs at the WindowHeader call site.** The glyph modules already exist
   under `src/typescript/lib/glyphs/solid/` (`window_minimize.ts`, `window_maximize.ts`,
   `window_restore.ts`) — they were generated by the FA-Free import sweep. In
   [WindowHeader.ts](../src/typescript/lib/component/container/WindowHeader.ts) add
   `import` statements for `window_minimize` and `window_restore` next to the existing
   `window_maximize` import, then extend the `Glyph.register(...)` call to include them.
   Glyphs.ts itself does NOT need editing — the registry is populated per consumer module,
   not centrally.
...
14. **Add stylesheet rule.** In `WindowBorder`'s constructor, allocate a lazy
    `.snap-target` rule via `this.createStyleRule(".snap-target")` and set
    `boxShadow: "var(--ts-ui-window-snap-glow, 0 0 0 2px rgba(30, 100, 200, 0.7))"` on it.
    Mirror the lazy-getter pattern from `ToggleButton`'s `.selected` rule. Verify by
    inspecting the document's `<style>` tags after window open.
...
| Modify | [src/typescript/lib/component/container/WindowHeader.ts](../src/typescript/lib/component/container/WindowHeader.ts) (also: import `window_minimize`/`window_restore` glyph modules and register them) |
| Modify | [src/typescript/lib/component/container/WindowBorder.ts](../src/typescript/lib/component/container/WindowBorder.ts) (snap-target style rule + classList toggle on `onDragStop`) |
| Modify | [src/typescript/lib/core/Theme.ts](../src/typescript/lib/core/Theme.ts)                                    |

No new files. No deletions. (Glyphs.ts itself is unchanged — the registry is per-consumer
via `Glyph.register(...)` since the FA-Free import sweep.)
