...
- **`Split` gained a `_collapsing` flag, mirroring `Border._collapsing`, and its `doLayout` tail sweep is guarded on `!this._collapsing` — not on `this._collapseAnimation === null` as step 5 specified.** `runCollapse` calls `container.doLayout()` *before* it returns the canceller `setPaneCollapsed` stores in `_collapseAnimation`, so from idle that field still reads `null` during the very layout pass that starts a collapse: the sweep would have undisplayed the pane's content before the reveal animation ran a single frame (case 2 now pins this). Under reduced motion the field is also left holding `animateLayout`'s no-op canceller after `onIdle` has already nulled it, so the sweep would never have run again. The flag is set with the collapsed flag (before `panecollapse` fires), cleared in `onIdle`, and cleared in `detach` exactly as `Border.detach` clears its own.
- **`transferPaneSize` does not transfer the snapshot as step 9 specified; it undisplays `to`'s own content and gives `from`'s back.** Both real callers move `from` and `to` around each other: `DockRegion`'s wrap nests `from` inside `to`, and `Dock`/`DockRegion`'s single-pane collapse hoists `to` — `from`'s direct child, so one of the very children `from`'s entry undisplayed — into `from`'s slot. With the plan's transfer, a hoisted `to` would stay `isDisplayed() === false` and `getLaidOutComponents()` would drop it out of the split entirely (no strip, no gutter); with no transfer at all, `from`'s children would stay undisplayed forever. The order is: capture `to`'s scroll offsets while `from`'s content is still out (the walk skips those boxless nodes, and a hoisted `to` — itself one of them — captures nothing, so its subtree's cached offsets survive), then `redisplayPaneContent(from)`, then snapshot `to` and undisplay its children — so a `to` that wraps `from` is snapshotted with `from`'s content present rather than childless. Case 10 covers the plain swap, the wrap and the hoist shapes.
- **`Border.doLayout`'s and `flooredMainExtent`'s region-size reads route through the snapshot helpers too; steps 12–13 listed only `getPreferredSize`/`getMinSize`/`computeTotalMinSize`.** `doLayout` reads each edge's `getPreferredSize()`/`getMinSize()` live to size the region's full, clipped box (`northExtent` and siblings) and throws when the preferred size is `null` — which a `Container`/`Fit` region reports the moment its only child is undisplayed, i.e. on the first resize while collapsed. Pinned by case 15's "keeps laying the collapsed region out at its full pre-collapse extent across a resize".
- **The reconcile sweeps test what the layout actually clips, not the raw collapsed flag, and run in both directions.** `Split.reconcileCollapsedContent` also requires a serving gutter (`doLayout`'s own collapsed test) and `Border.reconcileCollapsedRegions` also requires `isRegionCollapsible` (`applyRegionClip`'s gate) and a `laidOut` region, so content only ever leaves a pane/region that is really clipped away behind its strip. The plan's sweeps only ever *added* undisplay; the audit showed that strands content when a still-flagged pane/region stops being clipped — a `Split` pane whose trailing sibling is removed becomes the last pane and loses its serving gutter, and `setRegionCollapsible(edge, false)` sends a `Border` region down the unclipped frame branch — while `setPaneCollapsed`/`setRegionCollapsed` refuse such a pane/region, so nothing could bring the content back. Both sweeps therefore also redisplay (and lay out, then scroll-restore) a pane/region that carries a snapshot but was laid out unclipped this pass; the stale collapsed flag itself is left alone, as it was before this plan. The scroll-restore drain now runs after the sweep and keys on "content is in the render tree" (no snapshot entry) rather than the collapsed flag, so a just-redisplayed pane/region restores in the same pass.
- **Case 16's "the new component starts fully displayed and not collapsed" is read as a statement about the content bookkeeping, not about `isRegionCollapsed`.** `Border._collapsed` is keyed by placement and pre-dates this plan; step 15 asks only for the two new maps to be cleared on a replacement, never for the collapsed flag. The test therefore asserts no stale snapshot and the new component's children displayed at the moment of replacement, and pins the consequence of the still-collapsed edge: the next layout hides the new component's content against the new component's own snapshot.
- **The snapshot also carries the pane/region's `getMaxSize()`, which `## Non-Goals` deliberately left reading live.** The plan assumed a childless container reports a larger or unbounded max; every box manager reports the bare perimeter instead (`BoxLayout.aggregateMaxSize` — `{0,0}` for an inset-free `Container`; `Grid` likewise) the moment it has no laid-out children. `Split.recalculateSizes` re-clamps every stored size through `clampMain`'s live `getMaxSize()` on each layout, so a collapsed `VBox`/`HBox` pane's stored size fell to 0 on the first idle layout and the pane expanded to nothing; `Border.getMaxSize` likewise capped the border's width to a collapsed box-managed north/south region's 0. `clampMain`, `isPinnedMain`, `onDrag` and `Border.getMaxSize` now read a collapsed pane/region's max through the snapshot (`paneMaxSize` / `regionMaxSize`) exactly as its preferred/min are read. Pinned by "A collapsed pane's stored size and expanded geometry survive its childless reports" and "The border's max report survives a collapsed region's childless max".
- **The snapshot records which direct children were displayed, and an expand redisplays only those** — `## Internal Structure` loops over `getComponents()` both ways. A `Card`-managed pane keeps an inactive page undisplayed itself and only re-hides on a page switch, so the plan's unconditional `setDisplayed(true)` left every page of such a pane visible after one collapse/expand cycle (`Tab` re-sweeps on every layout, which is the next note's problem, not an escape). The docs note's second consequence is worded for this behaviour. Pinned by "An expand puts back only the children the collapse took out" in both suites.
- **A pane/region whose content is out is committed as a box only — its own layout manager does not run while it stays collapsed — and the idle sweep re-asserts the undisplay of the children it took out.** `LayoutManager.commitBounds`, and `CollapseSupport.commitRect` with `relayout: true` for every participant of a sibling's animation, run the pane's own `doLayout`; a pane managed by `Tab` — the Dock's split-region shape (`DockRegion`/`Dock` build regions as `Container({ layoutManager: new Tab(...) })`) — re-selects its page there with `setDisplayed(true)`, which an idempotent sweep would never take back out, so the page was back in the render tree on the first idle layout after a collapse. The plan's `## Architecture Decisions` treats the pane's own manager as a bystander and `## Potential Challenges` calls the race an accepted trade for a rare shape; the Dock is that shape. `Split.commitPanes` and Border's collapsible-branch placement (`placeRegionBox`) therefore commit an undisplayed pane's/region's box through `CollapseSupport.commitRect(…, relayout: false)` — the geometry-only commit the gutters already use, now exported — and both managers pass `relayout: false` for such a pane/region in a sibling's animation; `undisplayPaneContent`/`undisplayRegionContent` re-take the recorded children out on every idle sweep, so a stray layout of the pane (its own scheduled one) or a consumer's `setDisplayed(true)` is undone at the next idle layout. `Split.onDrag` — which lays both of a divider's neighbours out on every drag frame, and the divider beside a collapsed pane is still draggable — skips the collapsed neighbour's layout for the same reason. Pinned by "A pane's/region's own layout manager cannot put its content back while it stays collapsed" in both suites.
- **The recorded children are only touched while they are still the pane's/region's children.** The re-assert and redisplay loops check `getParentComponent()` against the pane/region, so a child re-homed elsewhere while the pane stays collapsed (the Dock moves stacks and units between containers) is neither re-hidden by the old pane's next idle layout nor force-redisplayed by its later expand over whatever its new owner decided. Pinned by "The recorded children stop being this manager's once they leave the pane/region" in both suites.
- **`captureSubtreeScroll` is guarded on `isEffectivelyVisible()`** in `undisplayPaneContent` and `undisplayRegionContent`, as `Tab.doLayout` and `Card.undisplayChild` guard theirs — the sibling plan's audit found the unguarded form (which `## Internal Structure` shows) clobbers a boxless subtree's cached offsets with zero.
- **Smaller departures from the plan's snippets:** `Split`'s `onIdle` reconciles `container.getLaidOutComponents()` rather than the `components` array closed over at toggle time, so a pane removed mid-animation is not touched; `Border.setRegionCollapsed`'s redisplay sits after the `container` null-guard rather than before it, so a bail-out leaves no half-applied state; `Border.setLayoutConstraints` drops a placement's entry only when the component actually changes, since re-registering the same collapsed component (to change `collapsible`, say) must not discard a snapshot the next sweep would retake childless; the snapshot lookups use entry presence (`get(...) !== undefined`) rather than `?.preferred ??`, so a pane/region that reported `null` before its content left keeps reporting `null` instead of falling through to a live read; `Split`'s snapshot-aware reads go through `panePreferredSize`/`paneMinSize` helpers mirroring the plan's own `regionPreferredSize`/`regionMinSize` for `Border`, rather than the inline `??` expressions steps 6–7 show; `undisplayPaneContent`'s snapshot-and-undisplay half is its own `snapshotAndUndisplay` helper so `transferPaneSize` can order the scroll capture around the outgoing pane's redisplay; and the Border bookkeeping clears go through one `forgetRegionContent(placement)` helper called from both constraint hooks.
- **The animated cases are settled by driving `requestAnimationFrame`, not by timers alone.** Step 16 says completion is always reached through the fallback timer; that holds for the primed CSS transitions' cleanup but not for `runCollapse`'s `onIdle`, which fires only from `animateLayout`'s rAF loop — and the offline DOM swallows `requestAnimationFrame` (see `tests/dom/TestDOM.ts`). The new suites and the two extended teardown tests capture frames through a `DOM.sink.requestAnimationFrame` spy and run them past the collapse duration, the precedent set by `tests/core/Animation.test.ts`.
- **Case 4's second toggle targets pane 1, not pane 2.** The existing teardown test's `setPaneCollapsed(2, true)` is refused: the last pane collapses toward the start by default and so has no serving gutter, which made its "different pane" re-toggle a no-op. The extended test toggles the middle pane so the supersession actually happens; the original assertions are unchanged and still pass.
- **Border's case 15 baseline is the during-animation report** (flag set, content present): `Border.getPreferredSize`/`getMinSize` already substitute `COLLAPSE_STRIP_SIZE` on the main axis the moment the flag is set, so "the instant before it collapsed" is read as the instant before the content left. `Split`'s `computeContentSize` never looks at the flag, so case 8 compares against the pre-collapse values as written.
- **Tests beyond `## Expected Behaviour`** (each of the other additions is named in its own bullet above; these two are the ones not tied to a deviation): a `Border` scroll round trip mirroring case 5 (the Border restore path is new behaviour the plan's Border cases did not cover) and a `Split` prune check for step 8.
- **The WebKitGTK MiniBrowser collapse-drag measurement (`## Verification` item 6) and the manual smoke tests (item 7) were not run by the implementer** — they open windows on the developer's desktop and were reserved for the invoking session.
- **A pane/region re-collapsed before its expand's scroll restore landed keeps the offsets it was collapsed with** (independent audit, round 1, BLOCKING). `undisplayPaneContent`/`undisplayRegionContent` captured live offsets whenever the pane was effectively visible; after an expand whose restore was still queued in `_pendingScrollRestore`, the live offsets were the engine's post-`display: none` zeros, so the capture overwrote the cache and the reconcile drain then dropped the queued restore. The capture is now skipped while the pane/region is still awaiting a restore — its cache is the authoritative value until a restore has been written. Pinned by "keeps the cached offset when the pane/region is re-collapsed before its expand settles" in both suites and by the `setPaneCollapsedImmediate(false)`/`(true)` variant in the Split suite; the plan's own `[^reconcile-sweep]` design missed this because it only considered the drain, not the re-capture.
- **`Split`'s scroll-restore drain only restores panes the container still owns** (independent audit, round 2, BLOCKING). `setPaneCollapsed`'s `onIdle` runs `reconcileCollapsedContent` outside `doLayout`, before `recalculateSizes` has pruned `_pendingScrollRestore`, so a pane expanded and then removed — and disposed — before its animation settled was still queued when the last frame fired, and `restoreSubtreeScroll` would have written through its released handle (the production sink throws on that; the offline sink records the write). The drain now checks each queued pane against `container.getComponents()` first, as `Border`'s drain already resolves through the live region slot. Pinned by "does not write scroll offsets into a pane removed and disposed before its expand settled".
- **`Component` gained an `@internal` `setContentClampSuspended(suspended)` seam, and `Split`/`Border` drive it around a pane's/region's content leaving and returning** (independent audit, round 3, BLOCKING; the seam was the user's chosen fix). A general `Component` pane/region — one that `clampsToContentSize()`, i.e. anything but a `Container`/`Panel` — clamps its own `setWidth`/`setHeight` to the live merged max, which a box manager with no laid-out children reports as its bare perimeter, so every box-only commit while collapsed was clamped to 0×0 (probe: a `Component({ layoutManager: new VBox() })` pane went 198×300 → 0×0 on the first idle layout; a WEST region 80×300 → 0×0 on the next resize) and the expand animated from that box. The manager-level snapshot could never cover the component's own clamp. The seam suspends the content-derived clamp — leaving the explicit `setMinSize`/`setMaxSize` constraints in force, the same policy `Container` applies permanently — from `snapshotAndUndisplay`/`undisplayRegionContent` until `redisplayPaneContent`/`redisplayRegionContent`, the `recalculateSizes` prune (a pane that left while collapsed) or `forgetRegionContent` (a replaced/removed region; the snapshot now records its component for this). Pinned by `tests/core/ComponentContentClamp.test.ts` (default clamp, suspension, explicit constraints still honoured) and by "A general Component pane/region keeps its own box while its content is out" in both suites. The shared touch on `core/Component.ts` is recorded here rather than in the frontmatter, which is left as filed.
- **A queued scroll restore survives a consumer-side undisplay of the pane/region itself** (independent audit, round 3, advisory): both drains used to clear `_pendingScrollRestore` wholesale even when the restore was skipped because the pane/region had no boxes, losing it; an entry whose pane/region is still owned but not displayed is now retained for the layout that follows its re-show. Pinned by "A queued scroll restore outlives a consumer-side undisplay of the pane" (Split suite).
- **Docs:** `docs/layouts/Split.md` and `docs/layouts/Border.md` now carry the render-tree paragraph the sibling plan added to `Tab.md`/`Card.md`, and `docs/concepts/performance.md` records the seam and the focus change (advisories from rounds 2–3).
- **`CollapseParticipant` gained a `gutter` flag; the strip cross-fade selects on it, not on `!relayout`** (independent audit, round 4, BLOCKING). Passing `relayout: false` for a collapsed pane/region (so its own layout does not run mid-animation) had made it a "gutter" to `runCollapse`, which primes the `background-color` cross-fade on the first `!relayout` participant and targets its `transitionend` — panes precede gutters in both lists, so a collapsed sibling became that target, and expanding it inside the 240 ms fallback window had the stale track's cleanup null its clip-path transition mid-reveal. Both managers now enrol panes/regions with `gutter: false` and gutters with `gutter: true`; the contract comment says what each flag means. Pinned by "a sibling's toggle does not prime the cross-fade on a collapsed-and-undisplayed pane/region" in both suites.
- **`Split.detach`/`Border.detach` release every content snapshot: clamp resumed, recorded children put back** (independent audit, round 4, BLOCKING). A manager swap left the children `display: none` and the clamp suspended with nothing left to undo them, and the LayoutSerialization restore shape — leaves parked, old tree disposed, fresh `Split` re-applying `setPaneCollapsedImmediate` — had the fresh manager snapshot an already-emptied pane, so the expand redisplayed nothing. The liveness test is the pane's/region's own parent link against the detaching container: `Component.destructor` clears a container's child list but never the children's parent links, so on the dispose shape a pane still wired to the now-childless container was destroyed with it and is skipped, while a parked or re-homed pane points elsewhere and is alive (round 4 first stated this as "a destroyed pane no longer owns its children", which is false — corrected in round 5). Pinned by "a manager swap puts a collapsed pane's/region's content back and resumes its clamp" in both suites and "a pane parked and re-homed under a fresh Split expands with its content" (Split suite).
- **Plan-file edits outside this section reverted** (independent audit, round 4, BLOCKING): the `touches-shared` frontmatter addition and the sentence appended to Verification item 7 are withdrawn; both facts live here instead. Manual-verify addition for item 7: put keyboard focus inside a pane/region and collapse it — focus is expected to leave the content when the collapse settles, where the clip-path-only collapse used to keep it.
- **Naming and a redisplay detail** (round 4 advisories): the seam is `Component.setContentClampSuspended(boolean)`, the `setAutoCommitStyle(boolean)` shape, rather than the earlier `suspendContentClamp`; `Border.redisplayRegionContent` resumes the clamp on the snapshot's recorded component rather than the live slot.
- **`detach`'s liveness test corrected** (independent audit, round 5, BLOCKING): the round-4 rationale — "a destroyed pane no longer owns its children" — was false (`Component.destructor` recurses into the children and clears `_components`, but `_parent` is only cleared by `unwireChild`), so on the plain dispose shape the redisplay loop wrote `setDisplayed(true)` to destroyed children, harmless in production only because the released element no longer resolves. Both `detach`s now skip a pane/region that is still wired to the detaching container when that container has no children left (destroyed with it) and touch only parked/re-homed ones; the clamp reset stays unconditional as a plain field write. Pinned by "writes nothing to a collapsed pane's children when the container is disposed with them" (Split suite) and "does not call setDisplayed on the child when the host is disposed while WEST is collapsed" (Border suite).
- **Round-5 advisories taken:** `setContentClampSuspended` returns `this`, matching `setAutoCommitStyle`; `ARCHITECTURE.md`'s "which minimum binds" paragraph now mentions the `@internal` suspension beside `clampsToContentSize()`. **Known live read left in place:** `LayoutManager.resolveBounds` reads a pane's/region's preferred/min/max live when the pane's own `LayoutConstraints.fill` overrides the `FillType.BOTH` the collapsed placement passes (`placeGutterAsStrip`/`placeRegionBox`); with the default fill `resolveBounds` still calls them but does not use the results, and an expand's start layout re-sizes the box with content present, so the effect is confined to a non-BOTH-fill pane's/region's already-clipped box while collapsed. Recorded here rather than routed through the snapshot; a follow-up if such a constraint is ever used on a collapsible pane.
- **`detach` releases every live recorded child by `Component.isDestroyed()`, wherever it is parented** (independent audit, round 6, BLOCKING; replaces round 5's parent-link test). `LayoutSerialization.parkLeaves` lifts the *leaf* out of a Tab-managed stack pane before the old tree is torn down, so the leaf failed the parent check, arrived in the fresh stack still `display: none`, and the fresh `setPaneCollapsedImmediate` recorded an empty `displayed` set — the stack's own `Tab.doLayout` then put the leaf back and nothing ever took it out again while collapsed. Parent links say nothing about liveness (`destructor` clears a container's child list but not the children's parent links; `disposeAllComponents` clears the links but destroys the children), so `Component` gained an `@internal` `isDestroyed()` accessor over its existing `_destroyed` flag and both `detach`s redisplay every recorded child that is not destroyed. Pinned by "a collapsed Tab-stack pane survives a serializeLayout/restoreLayout round trip" (Split suite) alongside the existing dispose and manager-swap cases; Verification item 4's grep ("every `setDisplayed` hit is inside the four helpers") no longer holds — both `detach`s call it too, by design.
- **Round-6 advisory taken:** `reconcileCollapsedContent` returns before allocating its owned-pane set when no scroll restore is queued, keeping its "cheap when there is nothing to reconcile" promise.
- **`detach` puts back only a child the pane/region still owns or one parked with no owner** (independent audit, round 7, BLOCKING): round 6's "wherever it is parented" would have force-displayed a child re-homed under another owner — a Dock unit moved out of a collapsed stack into another `Tab`-managed stack as an inactive page — over that stack's active page when the old container was disposed. The test is now `!isDestroyed()` and (`getParentComponent() === null` or `=== pane`), matching every other touch of a recorded child. Pinned by "leaves a child re-homed under another owner alone when the old container is disposed" (Split suite).
- **Why the branch keeps its own geometry-only commit rather than `Component.pauseLayout()`/`resumeLayout()`** (independent audit, round 7, pattern conformance): that seam — a host pausing another component's layout for an animation's duration, `AbstractWindow` ~2873/2883 — would indeed make `commitBounds`, `commitRect(relayout: true)`, `onDrag`'s `doLayout()` and the pane's own scheduled layouts no-ops while collapsed, retiring the exported `commitRect`, the `gutter` flag, `placeRegionBox` and the `onDrag` skip. It was not adopted because the seam is a public, boolean, non-nesting flag with a synchronous `doLayout()` on resume: a collapsible pane's *consumer* is entitled to `pauseLayout()` it for reasons of its own (the seam is public API), and a manager co-owning the flag would either clobber that pause on expand (`resumeLayout()` clears the boolean and lays out) or be clobbered by the consumer's `resumeLayout()` while collapsed, which runs the pane's layout and re-shows a `Tab` page — the very stray layout the idle re-assert exists to undo, now with no re-assert if the consumer's resume happens to be the last word. Recording `isLayoutPaused()` at collapse and resuming only what the manager paused would mitigate the co-ownership the same way the displayed-state ownership already does, but two things it would not mitigate decide it: `isLayoutPaused()` is read by widgets for behaviour beyond layout recursion (`Tree`, `AbstractMarkerList`, `VirtualRowView` consult it) and `scheduleLayout` swallows every queued pass while paused, so an indefinite manager-imposed pause on a pane that *is* one of those widgets changes its semantics, not just its layout; and a counting or owner-tagged pause would be a `Component` API change with wider blast radius than this plan's. The honest cost of the choice: a pane's *own* scheduled layout (a child's size relay, a strip change, a font-load re-measure) can put a `Tab` page back behind the strip until the split's/border's next idle layout — possibly a resize away — where a pause would have closed that window; `docs/concepts/performance.md` states that consequence.
- **Round-7 advisories taken:** the suites' `SplitInternals`/`BorderInternals` now type the snapshot map values as the real shapes; the performance doc records the detach-versus-removal asymmetry (a removed pane keeps its children undisplayed for the consumer to re-home; a detached manager puts them back because it cannot see the end of the tear-down). Not taken: moving `isDestroyed()` beside `dispose`/`destructor` and giving the `ARCHITECTURE.md` edit its own tooling commit are folded into the autosquash step.
- **Round-8 (clean) advisories:** (a) a manager swap over a collapsed pane/region puts the content back but drops its queued scroll restore — the cached offsets survive in each node, nothing under the new manager replays them; recorded here as a known limit rather than routed through a seam the new manager would have to know about. (b) The plan never enumerated focus (the sibling plan carried an explicit "Focus and ARIA need no change" decision); the change — focus already inside the content is dropped at settle, as for a `Tab` page, while reveal-then-focus still works because `setPaneCollapsed(false)` redisplays synchronously — is documented in `performance.md`, `Split.md`, `Border.md` and the manual-verify step above, and is a plan omission, not a code defect. (c) Not taken, to keep the audited code as folded: an idempotency guard in `snapshotAndUndisplay` for a `to` that already carries a snapshot (no caller shape produces it), moving `isDestroyed()` beside `dispose`/`destructor`, and extracting the scroll-restore drain from the two reconcile methods. (d) The `ARCHITECTURE.md` edit is re-targeted at fold time into its own tooling-bucket commit rather than the documentation commit.
