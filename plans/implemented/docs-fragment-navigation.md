...
Seven deviations from the plan text, discovered during implementation, manual verification, and audit. None change the public API or the plan's architecture decisions; each is a correction to an assumption the plan text made about the surrounding codebase or test harness, or (items 6-7) a bug the audit's own live browser testing found.
...
**7. `applyFragment`'s scroll-to-top branch didn't clear `_pendingFragment`, so a fragment-then-bare-path navigation within the same animation frame could scroll back down after the "scroll to top" had already run — a second, related race the audit found in the same mechanism as item 6, pre-existing in the plan's own `## Internal Structure` snippet (its `showPath` pseudocode has the identical gap) and not introduced by item 6's fix.** Failure scenario, reproduced live: navigate to path A with fragment `F1` (`applyFragment` sets `_pendingFragment = 'F1'` and queues `Component.afterNextLayout(handleScrollToFragment)`); before that queued callback drains — a roughly-one-frame window — navigate again with an empty fragment, either to the same path or a different one. `applyFragment('')` ran `setScrollTop(0)` and returned without touching `_pendingFragment`, which stayed `'F1'`; the still-queued callback then fired, read the stale `'F1'`, and scrolled back down to it — silently overriding the "scroll to top" the second navigation asked for, and directly contradicting the "last target wins" invariant `_pendingFragment`'s own field comment and `onScrollToFragment`'s own docblock already claimed. Fixed by having the `fragment === ''` branch clear `_pendingFragment = null` before writing the top scroll, so a callback already in flight from an earlier call finds `null` on drain and no-ops, matching the invariant the code already documented. Reproduced the exact scenario live (two synchronous clicks in one tick — a fragment link then a bare-path link on the same page, both landing before the next `requestAnimationFrame`) both before this fix (scroll ended on the stale fragment) and after (scroll correctly stayed at the top).
