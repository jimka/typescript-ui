...
- **`doLayout()`'s literal doc comment `{@link}`s four private methods, violating `CODE_CONVENTIONS.md`'s "Don't `{@link}` internal symbols from public JSDoc" rule; the links were replaced with prose.** The plan's *Internal Structure* snippet writes `{@link resizeScrollShadowOverlay}`, `{@link measureScrollbarGutter}`, `{@link updateScrollShadows}`, and `{@link deferScrollMetricsWhileResizing}` into `doLayout()`'s own doc comment — but `doLayout()` is public and all four targets are `private`, which TypeDoc excludes from the rendered docs. Copying the snippet verbatim raised `npm run docs:api` from the pre-existing baseline of 14 warnings to 50: one set of four for `Panel.doLayout` itself, and another four for *every* subclass that overrides `doLayout()` without its own doc comment (`PickerCellList`, `Form`, `MarkdownMinimap`, `ChartLegend`, `ScrollStrip`, `FloatingPanel`, `DiagramNode`, `DiagramGroupNode`) — TypeDoc renders the inherited comment on each of their pages too. Fixed by describing the mechanism in prose instead of linking to it (mirroring how `ScrollStrip.layoutItems()`'s own public doc comment describes its private settle relay), which restores the 14-warning baseline exactly. The four methods' own (private) doc comments still `{@link}` each other freely — the convention only constrains a *public* symbol's comment, and TypeDoc excludes private members' pages entirely, so no warning is emitted either way for links inside them.

- **`PanelOverlayScrollbar.test.ts`'s pre-existing "re-sizes the inner scroller to the CURRENT panel viewport on every layout (never lags a resize)" test needed a settle-relay drain, not just new coverage.** The plan's Expected Behaviour case 9 states this suite "passes unchanged... none of them drives a resize burst" — true for every case except this one, which calls `panel.doLayout()` three times in a row with a different stubbed `getScrollMetrics` viewport each time, asserting each pass's inner-scroller width matches that pass's viewport immediately. Under the new mechanism, the *first* `doLayout()` call always arms the settle relay (the `-1` baseline rule), and because this test never advances a real animation frame between its three calls, the relay is still armed — and therefore withholding — for calls two and three, which is exactly the intended behaviour for a burst, just not what this specific test expected. Confirmed in isolation: this was the *only* failure across the entire 7207-test suite (two independent full runs, one of which also surfaced an unrelated, pre-existing `ColumnFilterRow.test.ts` test-order flake — reproduced once, absent on a second full run with no code change, confirmed unrelated to this plan by running that file alone against the unmodified baseline). Fixed by adding the same `Map`-keyed `requestAnimationFrame`/`cancelAnimationFrame` capture-and-drain harness the new test files use, scoped locally to this one test (not the file's shared `beforeEach`, to keep the diff minimal), and draining to quiescence after each `doLayout()` call before its assertion — modelling each viewport change as a genuinely *settled* one-off resize (a sidebar toggle, a window resize) rather than three back-to-back burst frames, which is the scenario the test's own title already claims to cover. The title's parenthetical was updated to "on every settled layout (a one-off resize never lags)" to keep the test's own words accurate.

- **Expected Behaviour case 5 ("a change landing while a settle frame is already armed extends the burst") does not converge in the two drained batches its prose describes; the test instead mirrors `ScrollStrip.resizeResyncCoalescing.test.ts`'s own "extends the burst" case.** Tracing the literal `deferScrollMetricsWhileResizing`/`flushScrollMetricsSettle` code: the withheld S2 pass in case 2 already sets `_panelSizeMoved = true` (it finds the relay armed from case 1 and `sizeChanged` is true), so from that exact state, draining the relay's first hop (re-arm) and second hop (sees `_panelSizeMoved` true, consumes it, *extends* rather than catching up) reproduces only the same "extended, not settled" waypoint the case's own first two assertions describe — a real catch-up needs a further two-hop cycle once no further size change lands, not the single additional batch the prose implies. This is the identical shape `ScrollStrip.resizeResyncCoalescing.test.ts`'s own "extends the burst when a change lands between the settle relay's two hops" case already tests and resolves correctly, so `PanelResizeMetricsCoalescing.test.ts`'s equivalent case follows that file's structure instead: drain exactly the two hops that consume S2's contribution (asserting `_panelSizeMoved === false` and `_scrollMetricsSettleHandle !== null` — extended, not settled — matching that precedent's own assertions almost verbatim), introduce S3 in the window right after the second hop's re-arm (`_panelSizeMoved === true`, this pass's own contribution), then drain to quiescence and check the final state. The mechanism itself is unchanged from the plan; only this one test's choreography differs from the Expected Behaviour prose's literal "two batches" framing.

- **Second audit round found two more issues in the first round's own fixes; both are fixed here.**

- **BLOCKING (design gap, not just an implementation slip): withholding `measureScrollbarGutter()` also withheld the inner overlay scroller's own size, freezing the visible content viewport for the whole burst, not one frame.** `layoutOverlayScrollbars` (reached only through the now-gated `measureScrollbarGutter`) is what resizes `_overlayScrollElement` — the element that hosts and clips every child in `scrollbarStyle: "overlay"` mode (the default) — to the panel's current size. Gating it along with the other two helpers meant a withheld pass left that element, and the content inside it, stuck at its pre-burst size: a live widening drag would visibly open a gap between the frozen inner viewport and the growing outer panel border for the whole burst, not the single-frame staleness the plan's own Potential Challenges section analysed for `_scrollbarGutter` feeding `getInnerSize()`. The plan's Non-Goals explicitly rule out reordering `layoutOverlayScrollbars`'s own reads-before-writes, so the fix does not touch that method (or `measureScrollbarGutter`): `doLayout()`'s newly-added `else` branch (taken exactly when the remeasure is withheld) now writes the inner scroller's size directly, from already-cached data — this panel's own `width`/`height` (already read earlier in the same `doLayout()` pass), this panel's own cached `getBorderSize()`, and the last-known `_scrollbarGutter` — so it costs no new `getScrollMetrics` call and reintroduces none of the forced flushes this plan removes. This duplicates (against cached geometry instead of a fresh read) `layoutOverlayScrollbars`'s own pre-read sizing write rather than extracting a shared helper, to avoid touching that method at all. Pinned by two new cases in `PanelResizeMetricsCoalescing.test.ts` ("keeps the overlay inner scroller tracking this panel's live size even while withheld" and, added once the second audit round caught the first version subtracting border-box `width`/`height` without also subtracting this panel's own border — mismatching `layoutOverlayScrollbars`'s `clientWidth`-based live-pass write on any bordered panel — "accounts for this panel's own border when tracking the live size while withheld"), both confirmed to fail without their respective fix.

- **BLOCKING: the five new fields used plain initialisers instead of `declare`, missing `CODE_CONVENTIONS.md`'s "Fields written during the `super()` cascade must use `declare`" rule — and fixing that naively surfaced a second, more serious latent bug.** `Panel.applyOptions` dispatches `setAutoScroll` from inside the `super()` cascade; for any non-`"none"` mode this can itself trigger a `doLayout()` call (via `LayoutManager.setOverflowing` → `getContainer()?.doLayout()`, and `getContainer()` here resolves to this same panel) before construction finishes — the same trap the file's own `_shadowEdges`/`_lastContentExtent` fields already guard against. With plain initialisers, that phantom pass's writes to the five new fields were silently reverted by the field initialisers running immediately after `super()` returns — latent today (nothing this phantom pass would do has any effect before the element exists), but a real, if currently harmless, instance of the documented trap: confirmed via a probe that mid-cascade, `_scrollMetricsSettleHandle === null` reads `false` (`undefined === null` is `false`), sending that phantom pass down the *withheld* branch of `deferScrollMetricsWhileResizing` on a pass its own `@remarks` says must always be live.

  Converting the five fields to `declare` (seeded in `applyOptions`, mirroring the established pattern) fixes the convention violation but, on its own, makes it *worse*: the phantom pass's write to `_scrollMetricsSettleHandle` now *persists* past construction — confirmed via the same probe, which showed `scheduleScrollMetricsSettle()` actually firing during construction and leaving `_scrollMetricsSettleHandle` non-null once the constructor returned. Since a settle frame armed once is only ever cleared by an eventual `flushScrollMetricsSettle()`, every panel built with a non-`"none"` `autoScroll` *option* (not just `setAutoScroll` called later) would find its own genuine first post-render pass already "mid-burst" and wrongly withhold it — confirmed by running the full suite after the `declare` fix alone: 5 failures, all in this plan's own new tests, because `mountPanel()`'s `new _Panel({ autoScroll })` construction pattern is exactly this case. `deferScrollMetricsWhileResizing` now also short-circuits (returns `false`, touching no state) whenever `!this.getElement()` — a pass with no element yet has nothing for any of the three helpers to measure (each already no-ops on a missing element or overlay) — so the phantom cascade-time pass can no longer arm anything that outlives it. Pinned by a new case in `PanelResizeMetricsCoalescing.test.ts` ("constructing with a non-\"none\" autoScroll option arms no settle frame, and the first post-render pass still remeasures live"), confirmed to fail without the guard.

- **BLOCKING (first round): `PanelResizeMetricsCoalescingRealtime.test.ts` never rendered its panels (`getElement(true)` was missing), so `getScrollMetrics` was called zero times per pass in both tests and every assertion passed vacuously.** Fixed by adding `panel.getElement(true)` after each `new _Panel(...)`, matching `PanelResizeMetricsCoalescing.test.ts`. This uncovered the same mount-baseline mistake the offline file's own "extends the burst" case had already been caught and fixed for above: the second test's `oneLivePass` baseline was taken as an absolute call count including `getElement(true)`'s own `init()`-time reads, not the delta of the first live `doLayout()` pass alone — fixed the same way, by capturing the post-mount count first.

  **BLOCKING (second round): that fix removed the symptom, not the hole.** Both tests still passed with `getElement(true)` deleted again, because neither asserted that a live pass measures *anything* — test 1 only compared a delta to zero (`0 === 0` is still zero when nothing is ever measured), and test 2's `oneLivePass` was never checked to be positive before being compared against itself. Fixed by adding `expect(before).toBeGreaterThan(afterMount)` (test 1) and `expect(oneLivePass).toBeGreaterThan(0)` (test 2), mirroring the equivalent guard the offline file already had (`PanelResizeMetricsCoalescing.test.ts`'s `oneLivePass` cases). Both new assertions confirmed to fail when `getElement(true)` is removed again.

- **Manual-verify performed against a real browser; the first round's write-up had two inaccuracies, both corrected here.** No existing docs demo combines `Split` with an overflowing `autoScroll` `Panel` (confirmed, as the plan's own Manual-verify bullet anticipated), so a temporary, uncommitted demo (`packages/docs/src/demos/scratch-split-panel-drag.ts`, plus a matching `<!-- demo: ... -->` marker temporarily added to `Split.md`) built a `Split` whose left pane is a `Panel({ autoScroll: 'y' })` holding 60 rows. Both were deleted/reverted before this note was written — neither is part of the diff.

  **Correction 1 (demo geometry):** the pane was given `preferredSize: { width: 300, height: 220 }` **and separately** `maxSize: { width: UNBOUNDED, height: 220 }` — the first round's note described both as "`{ width: 300, height: 220 }`", which reads as the *width* being capped at 300 too. It was not: only the *height* is capped (so the pane genuinely overflows and scrolls internally instead of the `Split` growing to fit all 60 rows — `Panel` does not clamp to its content-derived size, and `Split`'s own cross-axis preferred size is the largest pane's *unclamped* preferred height, so height needed both an explicit preferred value and a cap). Width is `UNBOUNDED` specifically so the gutter drag can resize it freely; an actually-300-capped width would have made the drag a no-op, which did not happen — the gutter visibly moved in every run below.

  **Correction 2 (drag numbers):** the first round reported "133 forced `Layout` events before the fix, 1 after, zero during the drag itself" from one before/after pair. Re-running the identical script several more times (after rebuilding for the fixes above) produced 133 on some fixed-branch runs too, not 1 — including on runs where a direct `fetch` of the served chunk confirmed the settle-relay guard code was actually loaded, ruling out a stale build as the explanation. The likely cause, only found on this re-run: `Split`'s own gutter-drag handling is not necessarily rAF-batched the way `Split.flushDrag`'s doc comment describes for a real pointer drag, so a synthetic `mousemove` dispatched with `await requestAnimationFrame` between each one can let the settle relay fully cycle *between* dispatches — each step then lands as its own fresh, already-settled "one-off resize" rather than a continuous burst, which the mechanism (correctly, by design) never withholds. This is the same failure mode `ScrollStrip.resizeResyncCoalescing.test.ts`'s own realtime-file header comment and this plan's own `PanelResizeMetricsCoalescingRealtime.test.ts` exist to guard against for *unit* tests (driving passes with no intervening frame, vs. one real frame each) — and the ScrollStrip precedent's own manual-verify note independently flagged the mirror-image version of this same CDP-timing sensitivity ("`requestAnimationFrame` timing was irregular... the settle relay had ample idle time to fully resolve between several of the 50 dispatched moves").

  Given this sensitivity, the offline and realtime unit test suites — which control frame timing exactly rather than depending on real browser/CDP scheduling — are the authoritative, reproducible evidence that the mechanism withholds and catches up correctly; the live trace is corroborating, not primary, evidence. What the live trace does support reliably, confirmed across every run regardless of the above sensitivity: the gutter genuinely resizes the panel across a real multi-frame `mousedown`/`mousemove`×50/`mouseup` sequence (827px → ~984–1092px depending on the run), and at least one before/after pair (same script, same page, only `Panel.ts` differing via `git stash`) showed a 133-to-1 reduction in forced `Layout` events for the identical drag — directionally consistent with, though not a precise per-frame measurement of, the mechanism's intended effect. A cleaner live number (as the `ScrollStrip` precedent's own note also concluded of itself) would need a real, foregrounded interactive session rather than CDP-dispatched synthetic events.
