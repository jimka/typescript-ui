...
- **`demos/button-basic.ts` uses `new Button(...)`, not the plan's `Button(...)` callable-form snippet.** `Callable<T>` ([Callable.ts:10](packages/lib/src/typescript/lib/core/Callable.ts#L10)) types its call signature from `ConstructorParameters<T>`, which TypeScript resolves to only the *last* declared overload — `Button`'s `constructor(options: ButtonOptions)` ([Button.ts:430](packages/lib/src/typescript/lib/component/button/Button.ts#L430)), dropping the `constructor(text?: string, options?: ButtonOptions, ...)` overload. So `Button('Save')` (callable, no `new`) fails typecheck, while `new Button('Save')` resolves against the real overloaded constructor and passes — matching every existing call site in the codebase (`packages/lib/src/typescript/**`), all of which already use `new Button(text)`, never the bare callable form with a string argument. The plan's own doc-corpus example (`Button.md`'s `const saveButton = Button('Save');`) is the same shape and has never been typechecked — see the plan's own [^fences-untypechecked] — so this is the first place that exact snippet was actually compiled, and it failed. `button-basic.ts` is compiled (by `packages/docs`'s `typecheck`), so it uses `new Button(...)` instead. No plan text otherwise affected.

- **`demos.test.ts` needs `// @vitest-environment jsdom` and a `jsdom` devDependency.** The plan's footnote [^eager-globs] claims resolving a demo needs no DOM, "verified by importing `@jimka/typescript-ui/component/button` in bare Node." That check didn't cover `@jimka/typescript-ui/core`: `button-basic.ts` imports `Panel` from `core`, and the bundled `core` module (`dist/lib/core.es.js`, a single file) evaluates a top-level `Body` singleton (`private static readonly INSTANCE: Body = new Body();`, [Body.ts:23](packages/lib/src/typescript/lib/core/Body.ts#L23)) that reads `document` at import time — so any test importing `demos.ts` (which eagerly globs every demo module) needs a real DOM the moment it imports it. This only affects `demos.test.ts`; `blocks.test.ts` and the rest of the suite are untouched and still run under vitest's default `node` environment. `jsdom` was added to `packages/docs/package.json`'s `devDependencies` (matching `packages/lib`'s pinned `^29.1.1`) — it was already resolvable via workspace hoisting from `packages/lib`'s own dependency, but every other `packages/docs` test dependency is declared explicitly rather than relied on implicitly, so this follows that pattern. `packages/lib` itself is untouched (`installTestDOM`'s modelled-DOM harness that `packages/lib`'s own tests use is a private `tests/` utility, not part of the published package, so it isn't reachable from `packages/docs`).

- **Case 27's leak check is not flat under real, repeated browser navigation.** DOM element count stays flat (no DOM leak; confirmed via DevTools `document.querySelectorAll('*').length` across repeated cycles). CSS rule count does not, and a stack-trace-instrumented `CSSStyleSheet.prototype.insertRule`/`deleteRule` capture (`window.__ruleLog`, run live against `npm run docs:dev`) isolated the shape precisely:
  - **Pure navigation, never interacting with the demo** (clicking only the "ToggleButton" / "Button" content links, never "Show source" or the demo's own buttons): stylesheet rule count is flat within measurement noise (+2 over one clean instrumented round trip, fully attributable to the two pages' differing prose link counts, not a leak). `DocsDemo`'s own dispose ordering and composition were separately confirmed clean by a synchronous jsdom reproduction that mirrors `DocsContent.showBlocks`/`buildBlock` exactly — the real `Button.md` source (both Markdown blocks plus the `DocsDemo`), built, `flushLayout()`'d, and disposed in the same dispose-then-empty-then-rebuild order `showBlocks` uses, ten times over — whose stylesheet rule count came back exactly flat, before and after.
  - **Clicking "Show source" and then immediately navigating away** (before the next animation frame) reproduces real, if noisy, growth: instrumented deltas over 10 such cycles were `57, -9, -9, 33, 12, -1, 33, 12, 54, -59` (net +123, element count flat at 602 throughout). This is the actual leak case-27 is catching.
  - Investigating that path found one real, concrete defect in this plan's own code: `DocsDemo.onToggleSource` calls `Component.afterNextLayout(this.handleSourceMeasured)` ([DocsDemo.ts](packages/docs/src/shell/DocsDemo.ts)), and `Component.afterNextLayout` has no cancellation ([Component.ts:5335](packages/lib/src/typescript/lib/core/Component.ts#L5335)) — a callback queued by a "Show source" click that lands just before the block is disposed stays queued and later fires against a torn-down component. **Fixed**: `DocsDemo` now tracks a `_disposed` flag, set in an overridden `destructor()` before `super.destructor()` runs, and `handleSourceMeasured` no-ops when it's set.
  - That fix did **not** resolve the leak (re-measured after the fix, same stress test: net +123 over 10 cycles, essentially unchanged) — ruling it out as the dominant cause, though it was worth fixing regardless (an uncancelled callback touching a disposed component is a real hazard independent of whether it's *the* leak). With `DocsDemo`'s own async path closed off, and the leak specifically gated behind toggling `_source`'s visibility (never behind plain navigation), the remaining suspects are inside `packages/lib` and outside this plan's control from `packages/docs`: most plausibly `Markdown`'s own internal deferred height (re)measurement for `_source` when `setDisplayed` flips it from hidden to shown (per the plan's own "Two relays are needed" note under `## Internal Structure` § `DocsDemo`, `Markdown.measureContentHeight` already schedules work on the next layout independently of the caller), or `Panel`'s `autoScroll` scrollbar machinery reacting to the stage's real, DOM-measured overflow (jsdom does not compute real geometry, which is also why the jsdom reproduction above cannot exercise this path at all). Both are internal to components this plan does not own or modify.
  - An independent audit (`aac6c1c2458cc25b2`) reproduced the underlying leak live and rated it BLOCKING, additionally noting that the Tooltip-attachment theory floated in an earlier draft of this note does not hold up (`Tooltip` is a singleton with one shared rule set, so a leaked attachment/listener would not explain a growing CSS *rule* count) — that theory has been dropped from this note in favor of the toggle-source-specific reproduction above.
  - Given `packages/lib` is out of scope for this plan (`## Non-Goals`: "If a library change turns out to be unavoidable, stop and re-plan rather than widening a library API mid-implementation"), and the remaining leak requires editing `Markdown` or `Panel` internals to close, this is recorded as a known, out-of-scope limitation rather than silently worked around. A follow-up library-side plan should pick this up starting from `Markdown.measureContentHeight`'s deferred remeasurement path and `Panel`'s `autoScroll` scrollbar lifecycle under real `requestAnimationFrame` timing.
