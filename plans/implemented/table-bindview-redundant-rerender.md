...
**Manual verifications (`## Expected Behaviour` case 9, `## Verification`'s two manual bullets) were performed, not skipped:**

- **Case 9 — an open cell edit commits onto the record it was opened on.** Verified live against both this branch and `master` on the `Rotated` demo (`RotatedRecordPanel`, normal mode, 30 records × 20 columns): double-clicked a cell (`firstName` on the record with `id: 2`) to open its editor, typed a new value without blurring, then clicked the rotate toggle — which switches display mode while that edit is still open. After the switch, both the rotated projection and (after toggling back) the normal view show the edited value on `id: 2`, not on whichever record the pool slot the editor's row happened to occupy was rebound to. Confirms the plan's [^edit-commit] reasoning: `syncPoolCells`'s `commitEditsOutsideWindow(null)` now runs before any rebind, so the commit still sees the cell bound to its original record.
- **Manual smoke** (`npm run dev` → Rotated section): toggled rotated ↔ normal mode repeatedly. Grouped-field separators (Identity / Employment, including the non-adjacent-group split), their tint, group-member indent, and the checkbox cells all rendered correctly across every round trip; no console errors.
- **Manual perf trace:** captured a DevTools performance trace over one rotate-toggle click on this branch and, for comparison, an identical trace against `master` serving the same demo. Toggle-click INP: **680ms** on this branch vs **788ms** on `master` (dev/unminified build, no throttling, single click). Directionally confirms the fix, though this demo's 30-record × 20-column table is small enough that the redundant work this plan removes — which scales with column count, not record count (see the Addendum's pass-by-pass trace) — shows as a modest rather than dramatic difference; the plan's own profiled numbers (~10,179 `cellKeyFor` calls, ~1,888 cell constructions) are the more decisive evidence, and both are pinned by the automated `syncPoolCells`/`renderWindow` call-count tests.

**Expected Behaviour case 7's test could not be written exactly as specified.** The plan's step 9 requires cases 7-8 to run against pre-fix source too, but case 7 is worded as "`bindViewState` runs the same `filterUnhideable` pass `setHiddenColumns` runs" — `bindViewState` doesn't exist pre-fix, so a test that calls it cannot pass against `master`. Resolved by splitting into two tests in `BindViewRenderEconomy.test.ts`: a guard (`case 7 (guard)`) that calls `Body.setColumns` + `Body.setHiddenColumns` directly, which does run and pass against both sources (proving the shared `filterUnhideable` logic they both call survived the extraction), plus a second, post-fix-only test (`case 7 (bindViewState)`) that calls `bindViewState` itself directly with an unhideable column named in `hiddenColumns`, in the same red-first style as cases 1-4.
