...
> **Implementation resolution (recorded during `/implement`).** Three points the plan deferred were settled by reading the code:
> 1. **`retainsFocus` stays trivial (base returns `false`).** `ComboBoxDropdown` calls `setFocusOnRowClick(false)` and `AbstractCustomList.onPointerDown` `preventDefault`s pointerdown, so the host ComboBox keeps DOM focus throughout the dropdown's lifetime — exactly the cell-editor embedding it was designed for. Row clicks never blur the editor, so the only blur is a true outside exit, which *should* commit. No `LayerManager.containsAcrossLayers` override is needed; it is omitted from the public API below.
> 2. **The editor builds explicit `{ key, label }` items.** `ComboBox.setItems` auto-keys *plain strings by array position* (`String(i)`), which would make `getValue()` return `"0"`/`"1"` instead of the option value. So the editor always normalises to `{ key: value, label }` pairs.
> 3. **A new public `ComboBox.openDropdown()` is added.** ComboBox exposes no public way to pop its dropdown (`toggleDropdown` is private), which the editor's `focus()` needs so a double-click lands the user straight in the option list. The one-line method delegates to the existing open path and ships in the feature commit.
