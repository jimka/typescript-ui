...
7. **The row wants exactly the band.** For the same composition, the layout manager's `getPreferredSize().height === 22`, not 21 as originally measured here: `HBox.getPreferredSize()` adds the container's `getPerimeterSize()` (insets + border) to `computeRowHeight`'s raw 21px result, and the bar's 1px top border makes that `21 + 1 = 22` — exactly `STATUS_BAR_HEIGHT`, confirmed against `Component.getPerimeterSize()` (`Component.ts:2635`), an established, unchanged mechanism. The row's internal `computeRowHeight` value is still 21 (the band), which the no-clipping test (behaviour 6) already asserts directly via child `y + height`. Pins the "fits exactly, zero slack" result and is the earliest tripwire if a widget's metrics drift.
