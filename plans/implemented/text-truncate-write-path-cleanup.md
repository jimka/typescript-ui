...
**Row 1's claim that a bare `new Text('x')` materialises no `#id` rule at all does not hold — the rule still materialises, empty.** Steps 1-5 and 7 were implemented exactly as written and are correct as far as they go: `setTextOverflow`/`clearTextOverflow`/`applyStyle`'s `textOverflow` write all route through the reconciled path, and the *value* that ends up on `#id` for a bare `Text` is a removal for both `whiteSpace` and `textOverflow`, not a real string — verified by both the offline test suite and a live Style Audit comparison (below). But `setTruncate`'s unconditional constructor-time dispatch (`applyOptions`, inside `super()`) calls `setTextOverflow("ellipsis")` before `_inheritedStyleBag` exists, so it always queues `"ellipsis"` as a *real* value into `_styleRule`'s dirty bag. `whiteSpace`'s matching construction-time queue is corrected in time because its render-phase fix (`applyMiscInlineStyles`) is one of `Component.applyStyle`'s own phases, sharing the same flush boundary (`materialiseStyleRule()`, called at the end of `Component.applyStyle`'s own body) as the construction-time write. `textOverflow`'s render-phase fix lives in `Text.applyStyle`, which — per the depended-upon `text-applystyle-class-hoisting` plan's "`Text.applyStyle` flushes twice" — runs, and flushes, *after* `super.applyStyle()` returns. So `Component.applyStyle`'s own `materialiseStyleRule()` call sees the stale, still-real `"ellipsis"` before `Text`'s own correction ever runs, and materialises the `#id` rule on that basis; `Text`'s later correction still resolves `textOverflow` to `null` (and the offline tests below confirm no real value survives), but too late to stop the now-empty rule from having been inserted. Closing this gap fully would need a genuine design decision — a way for `Text` to reconcile `textOverflow` before `Component`'s own first flush, which is a new pattern with no precedent anywhere in the codebase today (`TabBar`/`Markdown`/`Legend`, the only three other `applyStyle` overrides in the library, all call `super.applyStyle()` first and act after, never before) — so it was not attempted here; see the plan's own `## Follow established patterns` constraint. The new test file's row 1 case, and the analogous "the other five" `lineHeight`/`textOverflow` assertions step 9 asked for in `TextClassStyleHoisting.test.ts`, were written to match this actual, verified behaviour instead of the plan's original claim — none of the seven `lineHeight` assertions actually flip from `toBeNull()` to `toBeUndefined()`, because the same construction-time queue keeps the rule materialised (empty) in every one of those cases too. `CellTextSelection.test.ts`'s `SelectableText` case and a third pre-existing file the sweep in step 10 turned up, `TextLineHeightValueClassSharing.test.ts` (whose row 5 test already documented "materialised only because textOverflow forces #id to exist" from a prior plan, corroborating this finding), were updated the same way.

**Live verification confirms the mechanism still delivers substantial, real savings, just not the full "rule never inserted" claim.** Using this worktree's dev server (`npx vite --port 8041 --strictPort` from `packages/lib`, confirmed via `readlink /proc/<pid>/cwd`) and a fixed navigation sequence (open a table window, a confirm/cancel Dialog, and hover a tooltip button on `#/misc`, then `#/style-audit`), compared with `git stash` toggling `Component.ts`/`Text.ts` between the pre- and post-fix state: the duplicate-rule group that previously read `137` instances / `6.51 KB` with body `{ white-space: nowrap; text-overflow: ellipsis; }` becomes `134` instances / `0.39 KB` with body `{ }` (an empty rule) — a ~94% size reduction for that group, not full elimination (the small instance-count difference between runs reflects normal session-to-session render count variance, not the fix). Every other duplicate-rule group that previously included `white-space: nowrap; text-overflow: ellipsis;` in its body lost exactly those two declarations and nothing else (e.g. the 117-instance Button-label group: `15.41 KB → 10.20 KB`; the 32-instance right-aligned group: `2.06 KB → 0.67 KB`). Overall: total stylesheet size `126.84 KB → 111.45 KB`, estimated dedupeable size `70.93 KB → 57.55 KB`. Computed `white-space`/`text-overflow` were confirmed identical before and after on a plain `Text`, a `Button` label, a table cell (`StringRenderer`'s `SelectableText`), a `Dialog` title, a `MenuItem` title (all resolving `nowrap`/`ellipsis` via the class or framework tier, not `#id`), and a `Tooltip` (`white-space: pre-wrap`, a real per-instance deviation, unaffected). No console errors during the session.

**Step 6 (`setTruncate`'s `false` branch routing through `clearWhiteSpace()`) was not implemented — it introduces a real rendering regression, caught by a pre-existing test.** `clearWhiteSpace()` resets the `_whiteSpace` *field*, not just the CSS declaration. `Text.applyOptions` dispatches `setWhiteSpace(options.whiteSpace)` before `setTruncate(options.truncate ?? ...)`, so `new Text(x, { truncate: false, whiteSpace: 'normal' })` — a real, deliberate combination used by `tests/component/TextBatchMeasure.test.ts`'s case 11 to get wrapping text — first sets `_whiteSpace = 'normal'`, then `setTruncate(false)` immediately wipes it back to `null` via `clearWhiteSpace()`. Since `applyMiscInlineStyles`'s render-phase write reads the `_whiteSpace` field directly (`if (this._whiteSpace) { ... }`), the caller's explicit `'normal'` is permanently lost — not merely stale, as the plan's `[^cleared-whitespace-cache]` footnote assumed, which only analysed the case where `_whiteSpace` was still at its constructor default (`"nowrap"`) when `setTruncate(false)` ran. The *previous* code (`this.setElementCSSRule("whiteSpace", null)`, a raw write that never touches the field) accidentally avoided this because the field survived untouched and the render phase re-derived the correct real value from it on first render. Confirmed by reverting just this one line and re-running the full suite: `TextBatchMeasure.test.ts`'s case 11 (which fails on the unmodified plan's step 6 with `expected +0 to be 2`, since the wrapping text silently stops wrapping) passes again, and no other test regresses. `setTruncate`'s `false` branch keeps its original `this.setElementCSSRule("whiteSpace", null)` write; `getWhiteSpace()` keeps returning the pre-existing stale `"nowrap"` after `setTruncate(false)`, unchanged from before this plan — the new test file's row 10 case was rewritten to assert that, rather than the plan's original claim. The `whiteSpace`/`textOverflow` grep invariants for `Text.ts` in `## Verification` no longer hold exactly as written: `setElementCSSRule("whiteSpace"` in `Text.ts` now has **one** match (the retained `setTruncate` raw write), not zero.

**Everything else — steps 2-5, 7-13 — was implemented and verified exactly as planned.** Full suite: 329 files, 5122 tests, all green. `npm run typecheck`, `npm run lint`, and `npm run docs:api` (0 errors, 1 pre-existing warning unrelated to this change) all pass cleanly.
