...
- **`PopupPanel.test.ts`'s placement tests need a real child, not just `preferredSize`.** The plan's *Expected Behaviour* preamble says the placement table is "reproducible without building children" because `getPreferredSize` returns an explicit `preferredSize` constraint ahead of the layout manager's report. That holds for the *measured* width/height fed into `resolvePlacement`, but not for the *committed* `getHeight()`/`getWidth()` after `setHeight`/`setWidth` clamp: `Component.getMaxSize()` always merges the panel's own cap with `this.getLayoutManager().getMaxSize()` (`Math.min`), and a childless `VBox`'s `aggregateMaxSize` (`BoxLayout.ts`) reports only the perimeter — not `UNBOUNDED` — when it has zero components to sum. A `PopupPanel` built with only `preferredSize` and no `components` therefore clamps every committed dimension down to its insets (8px), regardless of the cap `showAt` computes. The fix: every placement test in `tests/overlay/PopupPanel.test.ts` that asserts a committed `getHeight()`/`getWidth()` also passes one trivial child (the `unboundedChild()` helper) alongside `preferredSize`, so the layout manager's own max is `UNBOUNDED` (any real child defaults to it) and the panel's `setMaxSize` cap is what actually binds — matching real usage, where a popup always has content. The placement math itself (`x`/`y`, and the four `OverlayPosition.test.ts` rows for `positionAnchoredFlexible`) is unaffected; only the committed-size assertions needed this.

- **Both new test files dispose every fixture in `afterEach`, not just unregister it from `LayerManager`.** The precedent files (`AnimatedDropdown.test.ts`, `Menu.test.ts`, `MenuButton.test.ts`) only drain the `LayerManager` module singleton in `afterEach`; they never call `dispose()` on the dropdowns/menus/buttons they construct. That leaves any in-flight `Animation.play()` fallback `setTimeout` armed — `Component.destructor()` is the only path that cancels it via the pending-transition registry. Running the full suite after implementing both new test files (24 tests, most driving `showAt`/`toggleFor`/`hideAnimated`, i.e. `Animation.play` with the default `animated: true`) surfaced an intermittent unhandled rejection in an unrelated, later test file (`tests/component/table/ColumnFilterRow.test.ts`): a stale fallback timer fired after `DOM.reset()` had released the handle it targeted, in a `HandleRegistry.resolve` throw. Confirmed via `git stash` that the full suite (296 files) was clean without this branch's tests, and reproduced non-deterministically (1 failure in 3 full-suite runs) with them. This is a pre-existing, already-diagnosed framework-level race (`Animation.ts`'s pending-transition registry exists specifically for this), not a defect in `PopupPanel`/`PopupButton` themselves — but this branch's test volume raised its trigger probability. Fix, scoped to this branch's own two test files only: `tests/overlay/PopupPanel.test.ts` and `tests/component/button/PopupButton.test.ts` now dispose every `PopupPanel`/`PopupButton` they construct in `afterEach` (`Component.destructor()` cancels the fixture's own pending transitions as a side effect, subsuming the `LayerManager` drain). Verified clean across 3 consecutive full-suite runs afterward. The same latent exposure remains in the untouched precedent files; fixing it there is out of scope for this branch.

- **`scripts/llms/generate.mjs`'s `TOKEN_BUDGET` needed a third raise, from 6110 to 6260.** Not listed in *Files to Create / Modify / Delete*. Adding the two new manifest rows (`PopupPanel`, `PopupButton`) per *Documentation Impact* and regenerating pushed the site variant of `llms.txt` to ~6258 tokens, over the existing 6110 ceiling. The constant's own comment already documents two prior identical raises (for `CheckboxMenuRow`, then `RadioMenuRow`) with the same "nudge up by the minimum needed" rationale; this raise follows the same precedent and adds a third comment paragraph in the same style. No hand-authored catalog wording was trimmed to avoid the raise.
