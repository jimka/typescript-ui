...
`ScrollArrowGlyph`'s constructor accepts and forwards a `subclassDefaults?: Partial<GlyphOptions>` parameter that `## Internal Structure`'s snippet omitted (`super("unicode-arrow-" + direction, undefined, _defaultScrollArrowGlyphOptions)`, passing the constant straight through). The project's `local/require-subclass-defaults` ESLint rule (`packages/lib/scripts/eslint/require-subclass-defaults.js`, backing `ARCHITECTURE.md`'s "Constructors forward `subclassDefaults`") rejects any constructor that takes a parameter yet hands its `_default<Name>Options` constant to `super()` unforwarded — it exempts only zero-parameter constructors like `CheckboxCheckGlyph`'s, which the plan's snippet mirrored. `ScrollArrowGlyph` cannot use that exemption because it takes `direction` to select which arrow glyph to construct. The fix mirrors `ScrollArrowButton`'s own constructor in the same file, which already takes this exact shape (`direction`, `subclassDefaults?: Partial<ComponentOptions>`, spread over its own defaults constant): `constructor(direction: ArrowDirection, subclassDefaults?: Partial<GlyphOptions>) { super("unicode-arrow-" + direction, undefined, { ..._defaultScrollArrowGlyphOptions, ...(subclassDefaults ?? {}) }); }`. No caller passes a `subclassDefaults` value today — same as `ScrollArrowButton`'s own unused parameter — this only keeps the class from being a dead end for a future subclass, per the rule's rationale.
