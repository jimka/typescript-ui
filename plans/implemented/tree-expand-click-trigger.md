...
- **`_handleDblClick`'s first-ever tests surfaced a pre-existing, unrelated gap in how a real double-click on the caret is delivered — not a defect in this plan's change.** [`TreeRow.setRowData`](packages/lib/src/typescript/lib/component/tree/TreeRow.ts#L144) tears down and recreates the toggle `Glyph` on every expand/collapse rebind (lines 144, 167, 171). A browser's native `"dblclick"` event freezes its target to whatever the second click's own pre-handler hit-test resolved — the toggle element as it existed right before that same click's handler ran. That handler then toggles again, rebinding the row and detaching that exact element. A detached bubbling-event target has no ancestors to bubble through, so `Event.addSubtreeListener`'s delegated listener likely never receives it, meaning [`_handleDblClick`](packages/lib/src/typescript/lib/component/tree/Tree.ts#L1225) may not run at all for a genuine double-click landed on the caret, in either `expandTrigger` mode. This is orthogonal to `expandTrigger` (the caret's own toggle-then-recreate churn and `_handleDblClick`'s row-matching both pre-date this plan) and out of scope to fix here per the plan's Non-Goals (*No change to the caret's own toggle behavior*). The plan's own Potential Challenges section anticipated exactly this risk ("`_handleDblClick` had no test coverage before this plan... treat a failure there as a possible pre-existing bug, not just a regression"). The two caret-double-click tests in `Tree.test.ts` are documented inline as white-box unit tests of `_handleDblClick`'s skip-condition logic (re-reading the toggle element fresh before each dispatch to reach the current one), not as a faithful reproduction of native dblclick delivery to the caret — that delivery gap is unverified against a real browser and is left for separate investigation.
