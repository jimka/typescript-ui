...
- **`TabOptions` has no `barVisible` field, so `setTabOptions`'s exclusion list drops it.** The Architecture Decisions section and the `setTabOptions`/`DockOptions.tabOptions` snippet both list `reorderable`, `barVisible`, and `listeners` as the three fields stripped from a caller's `TabOptions` before it is stored/forwarded. `TabOptions` (`Tab.ts`) never had a `barVisible` field to begin with — only a private `_barVisible` field plus the runtime `setBarVisible`/`isBarVisible` pair, never exposed on the construction-time options bag — so destructuring `barVisible` out of a `TabOptions`-typed parameter does not compile (`Property 'barVisible' does not exist on type 'TabOptions'`). The implemented `setTabOptions` strips only `reorderable` and `listeners`; the doc comments on `Dock.setTabOptions` and `DockOptions.tabOptions` were written to match. Nothing else about the exclusion's rationale changes: a caller could never have passed `barVisible` through `TabOptions` in the first place, so there is nothing left to guard against.

- **`onFloatBeforeClose` is wired through a local closure, not a direct method reference, in `subscribeFloatWindows`.** The plan's `## Internal Structure` snippet wires it as `win.on("beforeclose", this.onFloatBeforeClose)`, mirroring the direct-reference style used for `win.on("close", onFloatClose)`. But `AbstractWindow`'s `"beforeclose"` overload calls its listener with one argument (`(controller: WindowCloseController) => void`), while `onFloatBeforeClose`'s signature is `(window: AbstractWindow, controller: WindowCloseController) => void` — two required parameters — which TypeScript rejects as an argument to a one-parameter call signature ("Target signature provides too few arguments"). The fix follows the pattern already used twice in the same function for the same reason (capturing `win`): a named local closure, `const onFloatBeforeClose: (controller: WindowCloseController) => void = (controller): void => { this.onFloatBeforeClose(win, controller); };`, passed to `win.on("beforeclose", onFloatBeforeClose)` instead of the bare method reference. `tab.on("beforetabclose", this.onPanelBeforeClose)` and `tab.on("tabdblclick", this.onPanelDoubleClicked)` needed no such wrapper — their signatures match `Tab`'s call signatures exactly, the same as the existing `onPanelClosed`/`onPanelFocused`/`onPanelDetached`/`onPanelDocked` wiring.

- **`setTabOptions`/`applyTabOptions` also exclude `TabOptions.tools`, and `wireRegion` now applies `_tabOptions` to a newly-wired region.** Two related gaps the plan's `## Architecture Decisions` and `## Internal Structure` didn't account for, both found during the audit's independent review:
  - `TabOptions.tools` (`Tab.ts`) can carry a live `Component`, which can only ever have one parent. The plan's exclusion list names only `reorderable`/`barVisible`/`listeners`; forwarding `tools` unfiltered into `_tabOptions` meant the *second* region built from the same stored options (`newTabRegion()`'s `new Tab({ ...this._tabOptions })`) would throw `Component <id> already has a parent` the moment a `TabOptions.tools` entry was a live component and more than one region existed. `setTabOptions` now also destructures `tools` out before storing, matching `reorderable`/`listeners`. `Dock.panelPresentation.test.ts`'s first regression case for this (`splitOnEdge`-driven second region) was itself a false positive — `splitOnEdge`/`DockRegion.newStack()` never reads `_tabOptions` at all (see the next bullet), so the test passed whether or not `tools` was excluded. It was rewritten to use a two-stack *construction-time* `DockLayoutSpec`, which calls `newTabRegion()` twice from the one stored `_tabOptions` — the path the bug actually reproduces on — confirmed red without the exclusion and green with it.
  - Separately, `DockRegion.newStack()` (`DockRegion.ts:541`, used by `splitOnEdge` — the actual drag-and-drop edge-split path, not just the plan's cited `newTabRegion()` call sites) constructs its own plain `new Tab({ reorderable: true, compact: true })` with no knowledge of `Dock._tabOptions` at all. This meant `setTabOptions`'s documented "applies … to every region it builds later" was false for the single most common real way a user creates a new region — a drag-driven edge split. Rather than threading a new callback into `DockRegion` (a class this plan doesn't mention, name in its Critical Files, or authorize a new cross-class mechanism for), the fix stays inside `Dock.wireRegion` — already the idempotent "make every region correct" pass the sweep runs after any structural change (including a `splitOnEdge`) — adding `this.applyTabOptions(tab, this._tabOptions)` alongside the existing `tab.setReorderable(true)` when a region is wired for the first time. This reuses an existing hook rather than inventing one, and makes the plan's own stated contract for `setTabOptions` actually true. The existing "is picked up by a region built later" test in `Dock.panelPresentation.test.ts` turned out to be a false positive too, for a different reason — it used `regionForFrame(frameOf(dock, 'b'))` to locate the split-off region, but a raw `Component.moveComponent` (what `splitOnEdge`/`newStack` use, not a full Tab-driven tear-off) never prunes the source `Tab`'s own `_contents` bookkeeping, so `regionForFrame` kept matching frame `'b'`'s stale entry in the *original* region rather than the new one, silently re-testing already-covered behavior. It was rewritten to locate the new region structurally (the root `Split`'s child that isn't the pre-split root) and confirmed red before the `wireRegion` fix, green after.

- **`packages/lib/docs/layouts/LayoutSerialization.md` needed updating too, though the plan's `## Documentation Impact` section names only `Tab.md`, `Dock.md`, and `changelog/next.md`.** That page hand-documents `PanelNode`'s captured-field shape (a prose sentence and a table row, both enumerating exactly `glyph`/`tooltip`/`closeable`/`disposeOnClose`) independently of the `LayoutConstraints.ts`/`LayoutSerialization.ts` changes in `## Public API`/`## Internal Structure`, so adding `italic`/`modified` to `PanelNode` made both spots factually stale (found during the audit, not by grepping the plan's own file list). Updated both to list all six fields and to note that a state written before `italic`/`modified` existed carries neither, mirroring the page's existing pre-0.4.1 `glyph`-only callout.

- **A third `_tabOptions`-blind region-construction site: `TabWindow` (`TabWindow.ts:81`), built on every default (non-Shift, `detachWindowMode: "strip"`) tab tear-off.** Found in a third audit round, by the same reasoning as the `DockRegion.newStack()` fix above: `TabWindow`'s constructor builds its own `Tab` with hardcoded options (`{ reorderable: true, widthMode: "fixed", textAlign: "start", compact: true, barIgnoreParentInsets: true }`), with no knowledge of `Dock._tabOptions`, so `setTabOptions`'s "every region it builds later" promise was false for the single most common way a user detaches a tab — a plain tear-off — even after the `newStack()` fix. `subscribeFloatWindows`'s `if (win instanceof TabWindow)` block is the equivalent "first time this window is seen" hook `wireRegion` is for a tiled/adopted region, so the fix is the same one-liner in the same shape: `this.applyTabOptions(tab, this._tabOptions)` added there. `Dock.panelPresentation.test.ts` gained a case that tears a tab off via `Tab`'s private `detachTabToWindow(id, content, x, y, forceBare: false)` (the same private surface `Tab.closeDisposal.test.ts`'s T4 already reaches, with `forceBare: false` instead of `true` so it builds a `TabWindow` rather than a bare `Window`) and confirms the resulting `TabWindow`'s own `Tab` reflects a prior `setTabOptions` call; confirmed red before this fix, green after.

  While specifically checking every `new Tab(...)` construction site in the library for this same blind spot (per the audit's explicit request), a further, structurally different, **pre-existing and out-of-scope** issue turned up and is recorded here for visibility rather than fixed: `LayoutSerialization.ts:529`'s `populateContainer` — the generic restore-path region builder every `Dock.setLayoutState`/`restoreLayout` call reaches for a `"tab"`-kind node — also builds a plain `new Tab({ reorderable: true, compact: true })` with no `_tabOptions` awareness, but unlike the two fixed sites, `Dock.wireRegion`'s identity-based idempotency guard (`this._wiring.get(region)`, keyed by the *container*, not its layout manager) cannot even see this one as "new": `restoreLayout` calls `container.setLayoutManager(tab)` on the *same* root `Container` instance `Dock.getRootRegion()` already returned before the restore, so `_wiring.get(root)` still finds the pre-restore entry with `tabWired: true` already set, and `wireRegion` skips the whole tab-wiring block — not just `applyTabOptions`, but the pre-existing `tab.on("tabclose"|"activate"|"detach"|"dock", …)` wiring too. Verified empirically (root's `Tab` instance is a different object after `setLayoutState`, `_wiring.get(root)` still resolves to the identical pre-restore entry, and `removePanel` after a restore returns `true` — `Tab.closeTab` ran — while `dock.on("close", …)` never fires) and confirmed present identically on `master` at the branch's own start point, unrelated to any change in this plan. Fixing it would mean changing what `wireRegion`'s idempotency keys on (the container, the manager instance, or detecting a manager swap) — a change to a wiring-identity model this plan's `## Architecture Decisions` never discusses and has no call to make unilaterally; left for a separate bug report/plan rather than folded in here.
