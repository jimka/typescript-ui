...
Follow-up to [`plans/container-base-class.md`](container-base-class.md), which introduces a `Container` station between [`Component`](../src/typescript/lib/core/Component.ts) and [`Panel`](../src/typescript/lib/core/Panel.ts). `Container` is the fit-parent (`clampsToContentSize() === false`), zero-default-inset, no-autoScroll / no-body-host / no-scroll-shadow base; `Panel extends Container` keeps the `Insets(4,4,4,4)` content padding and the full native-scroll stack. **That plan must land first** — `Container.ts` does not exist on disk yet, so every reference below is to the dependency plan's spec, not current code.
...
For each migrated class the change is two lines plus an import: `extends Panel<TOptions>` → `extends Container<TOptions>`, and `interface XOptions extends PanelOptions` → `extends ContainerOptions`. Because `Container` adds no fields and no setters (`ContainerOptions extends ComponentOptions` with no new members, per the dependency plan), and because every option these classes actually pass to `super` (`tag`, `insets`, `layoutManager`, `id`, `name`, `components`, …) lives on `ComponentOptions`, the options-side change is type-only — no runtime behaviour shifts. The `super(options, defaults)` two-arg form keeps working: `Container`'s constructor forwards `(options, subclassDefaults)` to `Component` unchanged, exactly as `Panel` did.
...
[`TabBar.ts`](../src/typescript/lib/component/container/TabBar.ts) already calls `this.clearInsets()` in its constructor ([`TabBar.ts:517`](../src/typescript/lib/component/container/TabBar.ts#L517)) and its class doc states it relies on `clampsToContentSize() === false` so the strip fills its edge ([`TabBar.ts:402`](../src/typescript/lib/component/container/TabBar.ts#L402)). After migration the top-level `clearInsets()` on `this` is redundant (Container is already zero) and is removed; the `clearInsets()` / `setInsets(0,…)` calls on TabBar's **internal** `_tabClip` / `_toolGroup` / `_leadGroup` (which stay `Panel` — see breakage check) are left untouched. Update the class-doc reference from `Panel.clampsToContentSize()` to `Container.clampsToContentSize()`.
...
- **AbstractWindow** is the abstract base for `Window` and `TabWindow`. It calls `this.getInnerSize()` inside its `doLayout` border-sizing math ([`AbstractWindow.ts:1482`](../src/typescript/lib/core/AbstractWindow.ts#L1482)). `getInnerSize` is overridden on `Panel` to subtract the scrollbar gutter ([`Panel.ts:351`](../src/typescript/lib/core/Panel.ts#L351)); losing that override means falling back to `Component.getInnerSize` ([`Component.ts:2264`](../src/typescript/lib/core/Component.ts#L2264)). For a window the gutter is `{right:0, bottom:0}` (windows never set `autoScroll` on themselves — verified: no `setAutoScroll` in the file), so the two return identical values **today** — but this is a real coupling to a Panel-only override, which is exactly why this class is deferred. It also owns its own `_bodyHost` field + `findBodyHost()` ([`AbstractWindow.ts:191`,`545`](../src/typescript/lib/core/AbstractWindow.ts#L191)) and uses `getInsets()` as the resize-border thickness ([`AbstractWindow.ts:1474`](../src/typescript/lib/core/AbstractWindow.ts#L1474)); its default insets come from `_defaultWindowOptions` ([`AbstractWindow.ts:106`](../src/typescript/lib/core/AbstractWindow.ts#L106)) — verify that block to confirm it sets its own insets (window border) rather than relying on Panel's 4px before migrating.
...
- **`: Panel` parameter/return/field types** — hits, each assessed:
  | Site | Type | Handled? |
  | --- | --- | --- |
  | [`DockRegion.ts:274`](../src/typescript/lib/layout/DockRegion.ts#L274) `newStack(): Panel` | Constructs and returns a real `new Panel(...)` (a scrollable tab stack) — **not** a migrated class. | Unaffected. |
  | [`MiscPanel.ts:1077`,`1111`](../src/typescript/MiscPanel.ts#L1077) `const scrollPanel: Panel = new Panel({...})` | Real `Panel` instances in a demo. | Unaffected. |
  | [`TabBar.ts:416`,`459`,`469`](../src/typescript/lib/component/container/TabBar.ts#L416) `_tabClip / _toolGroup / _leadGroup: Panel = new Panel()` | TabBar's **internal** children, kept as `Panel`. The *outer* `TabBar` migrates; these inner fields stay `Panel` (they are explicitly typed and constructed as `Panel`). | Keep as `Panel`; do not migrate the inner clip/group panels. |
  | [`StatusBar.ts:80`,`81`](../src/typescript/lib/component/container/StatusBar.ts#L80) `_leftZone! / _rightZone!: Panel` | StatusBar's internal zone panels, `new Panel({ insets: 0 })`. | See decision below. |
...
3. **`container/StatusBar.ts`** — `StatusBarOptions extends PanelOptions` → `extends ContainerOptions` ([L34](../src/typescript/lib/component/container/StatusBar.ts#L34)); `class StatusBar extends Panel<StatusBarOptions>` → `extends Container<StatusBarOptions>` ([L82](../src/typescript/lib/component/container/StatusBar.ts#L82)). **Keep** `_defaultStatusBarOptions.insets = new Insets(0,6,0,6)` ([L53](../src/typescript/lib/component/container/StatusBar.ts#L53)). Migrate internal zones: `private _leftZone!: Panel` / `_rightZone!: Panel` → `Container` ([L80-81](../src/typescript/lib/component/container/StatusBar.ts#L80)); `new Panel({ insets: new Insets(0,0,0,0) })` → `new Container()` (drop the redundant zero-inset) ([L112-113](../src/typescript/lib/component/container/StatusBar.ts#L112)). Import: now needs `Container` and `ContainerOptions`; the `Insets` import stays (outer inset uses it) — verify before any removal. Update class-doc "`StatusBar` extends `Panel`" ([L60](../src/typescript/lib/component/container/StatusBar.ts#L60)) to `Container`.

4. **`menubar/ToolBar.ts`** — `ToolBarOptions extends PanelOptions` → `extends ContainerOptions` ([L38](../src/typescript/lib/component/menubar/ToolBar.ts#L38)); `extends Panel<TOptions>` → `extends Container<TOptions>` ([L102](../src/typescript/lib/component/menubar/ToolBar.ts#L102)); import swap. **Inset regression guard:** ToolBar's class-doc says it "inherits the standard 4-pixel insets" ([L72-73](../src/typescript/lib/component/menubar/ToolBar.ts#L72)) and the compact toggle sets insets explicitly ([L228](../src/typescript/lib/component/menubar/ToolBar.ts#L228)). Read the `setCompact`/toggle body around [L211-228](../src/typescript/lib/component/menubar/ToolBar.ts#L211) and the constructor: if the **non-compact** state relied on inheriting Panel's 4px (rather than setting its own), add an explicit default inset (e.g. seed `insets: new Insets(4,4,4,4)` in a `_defaultToolBarOptions` or in the toggle's non-compact branch) so the toolbar's resting appearance is preserved. If the toggle already sets both states explicitly, no inset addition is needed — only update the doc wording away from "inherits the standard 4-pixel insets." Decide by reading the toggle; do not guess.
...
8. **`container/TabBar.ts`** — `TabBarOptions extends PanelOptions` → `extends ContainerOptions` ([L122](../src/typescript/lib/component/container/TabBar.ts#L122)); `class TabBar extends Panel<TabBarOptions>` → `extends Container<TabBarOptions>` ([L408](../src/typescript/lib/component/container/TabBar.ts#L408)). **Keep the `Panel` import** — `_tabClip`/`_toolGroup`/`_leadGroup` stay `new Panel()` ([L416,459,469](../src/typescript/lib/component/container/TabBar.ts#L416)). Add `Container`/`ContainerOptions` to the import. Remove the now-redundant top-level `this.clearInsets()` ([L517](../src/typescript/lib/component/container/TabBar.ts#L517)) — Container is zero by default. Leave the internal `_tabClip.clearInsets()` etc. ([L530,540,554](../src/typescript/lib/component/container/TabBar.ts#L530)) untouched (those clear Panel's 4px on the inner panels). Update the class-doc `Panel.clampsToContentSize()` reference ([L402](../src/typescript/lib/component/container/TabBar.ts#L402)) to `Container.clampsToContentSize()`.
...
11. **`core/AbstractWindow.ts`** — **First read** `_defaultWindowOptions` ([L106](../src/typescript/lib/core/AbstractWindow.ts#L106)) and confirm it (or the subclasses' defaults) sets the window's own border insets rather than leaning on Panel's 4px. Then `WindowOptions extends PanelOptions` → `extends ContainerOptions` ([L77](../src/typescript/lib/core/AbstractWindow.ts#L77)); `abstract class AbstractWindow extends Panel<WindowOptions>` → `extends Container<WindowOptions>` ([L152](../src/typescript/lib/core/AbstractWindow.ts#L152)); import swap. The `this.getInnerSize()` call in `doLayout` ([L1482](../src/typescript/lib/core/AbstractWindow.ts#L1482)) now resolves to `Component.getInnerSize` — verify the window's `_scrollbarGutter` was always `{0,0}` (no `setAutoScroll` on the window) so the result is identical; if any code path enables window-body scrolling, **abort this class and keep it on `Panel`**, flagging it. `_bodyHost`/`findBodyHost` are AbstractWindow's own members (not Panel's) — unaffected.
...
| Modify | `src/typescript/lib/core/AbstractWindow.ts` (Phase 2) |
...
The only doc-surface touch is in-source JSDoc class-prose that names the parent class: change "A `Panel` subclass…" / "extends `Panel`" to `Container` in `Header`, `AccordionPanel`, `StatusBar`, `TabPanel`, `Popover`, and the `Panel.clampsToContentSize()` reference in `TabBar`. These are descriptive sentences (and `{@link}`-free where they currently use markdown `/api/...` links); keep the existing link style. The TypeDoc-generated API pages for these classes pick up the new base automatically. Catalog tables in `docs/` reference the components by name, not by base class, so they need no edits.

---

## Potential Challenges

- **ToolBar resting inset (highest-risk Phase 1 item):** if ToolBar's non-compact state silently relied on Panel's 4px, dropping it shifts every toolbar's button spacing. Mitigation: read the compact toggle and constructor in Step 4 and add an explicit default inset only if needed; verify against `master` in the visual smoke.
...
- **Dependency ordering:** this plan is inert until `container-base-class` lands (`Container.ts` + barrel export + `Panel extends Container`). The `depends-on` frontmatter encodes that; `/implement` should not start this before the dependency is merged.

---

## Critical Files

- [`plans/container-base-class.md`](container-base-class.md) — the dependency: defines `Container`, `ContainerOptions`, the moved `clampsToContentSize`, and `Panel extends Container`. Read first; `Container.ts` won't exist on disk until it lands.
- [`src/typescript/lib/core/Panel.ts`](../src/typescript/lib/core/Panel.ts) — the base being shed: `getInnerSize` gutter override ([L351](../src/typescript/lib/core/Panel.ts#L351)), `setAutoScroll` stack ([L216](../src/typescript/lib/core/Panel.ts#L216)), `_defaultPanelOptions` 4px ([L97](../src/typescript/lib/core/Panel.ts#L97)) — confirms what each migrated class is no longer inheriting.
