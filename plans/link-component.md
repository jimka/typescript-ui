...
### `foregroundColor` / `cursor` / `tag` are class defaults; the underline rides `styleRules`
...
- `styleRules` **cannot** go in `subclassDefaults` — `applyOptions` dispatches it only from the caller's bag ([Component.ts:533](src/typescript/lib/core/Component.ts#L533)) and never consults `_defaultOptions`. It must be merged into the options bag via the array-merge idiom at [Component.ts:82-88](src/typescript/lib/core/Component.ts#L82), which also preserves caller-supplied rules.
...
| 9 | `new Link("x", { styleRules: [{ suffix: ":hover", styles: { opacity: "0.8" } }] })` keeps **both** the caller rule and the underline — the merge appends, never replaces. |
...
6. **Link cells** — there is **no demo-app table with a link column** (verified: nothing under `src/typescript/*Panel.ts` uses `LinkCellRenderer`; its only in-repo consumers are the barrel, `ColumnConfig`'s JSDoc, and the test). So the renderer's coverage is R1-R3 plus the 9 existing tests, and the sqladmin check below. **Do not add a link column to the demo** — it is not what this change is for.
7. **The non-focusable invariant** (the one the unit tests deliberately cannot reach). In sqladmin's StructurePanel FK grid, Tab must never land on a link cell's text, and pressing Enter while a link cell is *selected* must trigger only the Table's own handling — never a second activation from the cell's inner `Link`. This is the real-browser check that a non-interactive link is never a keydown target.
...
- **`styleRules` in `subclassDefaults` is silently dropped** ([Component.ts:533](src/typescript/lib/core/Component.ts#L533)) — test row 9 catches it.
...
- **A `Link` demo column in a demo-app table.** No demo table uses `LinkCellRenderer` today, and adding one to exercise a refactor the unit tests already cover is scope creep.
...
- **A separate `--ts-ui-link-underline` token.** An underline is not a theme colour; a consumer who wants it gone passes `styleRules`.
