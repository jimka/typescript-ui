...
| `aria-*` attributes                         | Stay on `Aria` helper   | `Aria` (in `src/typescript/lib/core/Aria.ts`) is a sibling helper, not a `Component` subclass — it holds a reference to a Component and writes to it via `setElementAttribute`. The visibility flip in CR2 would lock it out. **Solution:** add a narrow `protected applyAriaAttribute(name: string, value: string | null)` on `Component`, and refactor `Aria` to call it (no widening of the public surface — `Aria` is constructed via `Component#getAria()` which is itself inside `Component`, so the trust boundary is fine). Alternative: keep `setElementAttribute` public but document it as `@internal`. The protected-helper route is cleaner — pick during implementation by surveying whether `Aria` ever runs outside the framework's own internal flows. |
...
| `setGlyph(null)`                     | `clearGlyph()`                       | On `Button` and on `WindowHeader`. Both gain `clearGlyph()`.                            |
