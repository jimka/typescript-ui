...
| C10 `TreeCellRenderer` drops the old caret `Glyph` undisposed | `treetable-rows` · `toggle` | seam: `ensureStyleRule` > 0 per unit, and `deleteStyleRule` absent | `deleteStyleRule` = `ensureStyleRule` |
...
| C35 a context-less `WebGLCanvas` animates forever | `canvas-idle` · `idle` | `before.host.webglAnimating` = n while `webglContexts` = 0; one `requestAnimationFrame` per such canvas per unit | those loops stop |
| C36 a canvas added to a hidden parent keeps animating | `canvas-idle` · `idle` | `before.host.hiddenAnimating` = n; one `requestAnimationFrame` per hidden canvas per unit | 0 |
...
- **Components.** The root is `Panel(VBox({ spacing: 8 }))` holding two panels:
  - `shown`: `Panel(HBox)` holding n `WebGLCanvas({ preferredSize: { width: 160, height: 120 }, onFrame: drawNoGlFrame })`;
  - `hidden`: `Panel(HBox, displayed: false)`, empty at build.
- **afterMount.** Add n `Canvas({ preferredSize: { width: 160, height: 120 }, onDraw: drawNoCanvasFrame })` to `hidden`, then call `startAnimation()` on all 2n surfaces. This is C36's shape: a canvas added to a parent that is already hidden. It returns `{}`.
- **Geometry, describe.** Geometry: `shown`. `describe()`: `{ surfaces: n, webglContexts, webglAnimating, hiddenAnimating }`, counted with `getContext() !== null` and `isAnimating()`.
- **Callbacks.** `drawNoGlFrame` and `drawNoCanvasFrame` are named module functions that draw nothing.

Each panel's `description` names what it builds. A panel that reproduces a figure also names the figure's source and symptom, per the QA app plan's symptom rule. For example: `list-items: 300-item List. Reproduces slice 18 F18.2 (lists are not virtualised): 906 applies per unchanged pass.`
...
- **M9 `treetable-rows` (F22.5 D7; F22.3 D8; C10).**
  ...
  - `toggle` with `seam=1`: `seam.sink.ensureStyleRule` equals `seam.sink.setRuleStyles`, and `deleteStyleRule` is absent.
...
- **M11 `list-items`, n = 300 (F18.2; F18.1).**
  - `passes` with `seam=1`: `seam.sink.apply` = 906.
  - `key` with `seam=1`: `apply` = 603 and `dispatchEvent` = 1.
...
- **M14 `canvas-idle` (F26.7; F26.8).** `before.host.hiddenAnimating` = 4 and `webglAnimating` = 4. `idle` with `seam=1`: `seam.sink.requestAnimationFrame` = `hiddenAnimating + webglAnimating` ±0.05.
...
If a figure differs, compare it with the offline census in [^offline] before concluding anything. The panel is not validated until the difference is explained. A plausible explanation for M11 is the real engine's overlay scroller, which slice 18's probe Q did not model.
...
**Every command below opens a full-screen window on the user's desktop for about a minute. An implementer or agent must not run it.** Run from the repository root, with the library built. The default host is MiniBrowser; once `qa-app-tauri` lands, the same lines run in Tauri with `--host tauri`.
...
python3 packages/qa/bin/qa-table.py packages/qa/results val- --seam \
  --before host.editorViews,host.treeRows,host.webglAnimating,host.hiddenAnimating
```
