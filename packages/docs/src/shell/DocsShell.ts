...
import { Border, HBox, AnchorType, Anchor, AnchorConstraints } from '@jimka/typescript-ui/layout';
...
class DocsShell extends Container {
    ...
    // handler below.
    private readonly handleContentSettled: () => void = () => this.rehugMinimap();
    ...
    /**
     * Lays out the shell as usual, then re-hugs the minimap against
     * `DocsContent`'s current text column — `MarkdownMinimap.placeNextTo`
     * needs calling after every pass that can move either the window's width
     * or the content pane's rendered width (see its own doc comment for why
     * it's an owner-driven call rather than a `MarkdownMinimap`-internal
     * `doLayout` override).
     *
    ...
    /** Re-hugs the minimap against `DocsContent`'s current first block. */
    private rehugMinimap(): void {
        this._minimap.placeNextTo(this._content.getTextColumnReference());
    }
    ...
    /**
     * Reflects the current page's heading outline into the minimap, then
     * re-hugs it once the incoming page's blocks have actually laid out.
     * `DocsContent.showBlocks` only re-lays-out its own (unchanged) bounds —
     * it never bubbles a layout pass up to this shell — so without this,
     * `rehugMinimap` would run against the outgoing (disposed) block or, on
     * the very first page, against no block at all.
     *
