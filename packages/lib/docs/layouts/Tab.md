...
| `tabdblclick` | the double-clicked tab's content component, and its index | A tab button in the strip is **double-clicked**. |
...
`tabdblclick` reports the gesture only — the first click of the pair has already activated the tab through the button's normal press handling, so a listener always runs with the double-clicked tab already selected. A double-click on the strip's blank area or its fixed chrome (the tool group, a scroll arrow) fires nothing, and neither does one on a lazy tab whose deferred content has not been built yet.
...
```typescript
layout.setTabItalic(consolePanel, true);
// ...
