...
Every category that was once a holdout now funnels through the seam: computed border widths and overflow (`getBorderWidths` / `getComputedOverflow`), the `<legend>` box (`getOffsetSize` / `isConnected`), native event listeners (`addListener` / `removeListener` / `dispatchEvent`), DOM traversal (`querySelector` / `contains` / `closest` / `matches` / `getParentElement`), globals (`matchMedia` / `requestAnimationFrame` / `getActiveElement` / `getDocumentElement` / `getLocationHash` / `setLocationHash` / `replaceLocationHash`), structural and form-control writes (`setId` / `insertBefore` / `setValue` / `setSelectionRange` / `setSelectedIndex`), and the irreducible text/scrollbar **measurement leaf** — the off-screen probe, the canvas font-metrics context, and the scrollbar-width probe — which now lives inside `ProductionDOMSource` itself.
