...
- Pool slots are rebound to new data only when needed, not on every scroll tick: a `Table` row calls `setData()` when its pool slot's data index changes — DOM nodes stay in place and only their bound data shifts. A `Tree` row calls `setRowData()` when what it was last bound to (its node, depth, expanded/loading state, and a few other tracked values) has actually changed, not merely when its slot's data index does — so an expand/collapse that shifts which node a slot displays only rebinds the rows whose content genuinely changed.
