...
Because there is one editor, there is one owner — the cell that acquired it — and anything taking the editor away from that cell commits its open edit first. An interrupted edit is therefore saved onto the record it was opened against, never discarded and never written onto a different row. Three things can take the editor away: another cell acquiring it, a factory re-registered over a key whose editor is cached (`register` also disposes the editor it drops), and [`Body`](/api/component/table/classes/Body) rebinding the editing cell's pooled row to a different record — as the user scrolls, sorts or filters, or as a store change shifts rows. `release(cell)` names its caller for the same reason: a cell whose edit was already committed when another cell took the editor over cannot unhook the cell that owns it now.
