...
- **Only the nodes near the visible area are attached to the document.** On a large graph, a node more than about half a viewport outside the visible area is built, measured, and positioned but not mounted until panning or zooming brings it into range. Selection, emphasis, and every centring method (`focusNode`, `revealNode`, `zoomToFit`, `resetView`) work against a node whether or not it is currently mounted. A custom `nodeRenderer`'s component may therefore have no element for part of the view's life.
