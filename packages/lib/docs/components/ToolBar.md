...
| `overflowSide` | `"end"` | Edge the `"menu"` overflow chevron sits on. `"end"` pins it to the bar's trailing edge (a flex spacer fills the gap after the last visible button); `"start"` leads the buttons. The overflowing buttons are the trailing run either way. No effect unless `overflow` is `"menu"`. |
...
- `setOverflowSide(value)` — moves the `"menu"` chevron to the `"start"` or `"end"` (default) edge; `getOverflowSide()` reports it. On `"end"` a flex spacer pins the chevron to the bar's trailing edge. Only the trigger's position changes — the overflowing buttons remain the trailing run.
...
- A bar whose parent re-commits it at the rectangle it already holds is not re-laid-out (see [the write is diffed](/concepts/layout-system#the-write-is-diffed)), so dragging a gutter elsewhere does not re-flow it every frame. The bar's own setters, adding, removing or moving a child, and a `Button` child's label or glyph change all still lay it out. Changing insets, padding or a border, swapping or reconfiguring a manager through `getLayoutManager()`, hiding or showing a child with `setDisplayed`, rewriting a child's layout constraints (a `Spacer`'s `setFlex` included) or re-sorting children — on the bar or on anything inside it — marks the bar owed, so the next pass that reaches it lays it out, as before. One change does not announce itself: a custom child that changes its intrinsic size without calling `setPreferredSize` or `notifyIntrinsicSizeChanged`, which should be followed by `bar.scheduleLayout()`.
