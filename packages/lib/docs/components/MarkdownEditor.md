...
Images, task lists, thematic breaks (`hr`), and raw HTML are **not** part of the dialect — they are excluded so the editor's output always round-trips cleanly through the viewer. A column's alignment (from the delimiter row's `:---` / `:---:` / `---:` markers) is **preserved** across a load/edit/save round-trip; unlike alignment, column width and cell merges **are** authorable in the WYSIWYG surface, through `setTableColumnWidth` / `mergeTableCells` / `unmergeTableCell` and the table-cell context menu's Column width…/Merge cells/Unmerge cell items.
