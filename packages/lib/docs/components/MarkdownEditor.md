...
| Block alignment / multi-column region | `::: {align=... columns=... gap=...}` … `:::` |
...
| `setColumnCount(count, gap?)` | Set (or, with `null`, clear) the column count — and optionally the column gap — of the top-level blocks the selection spans, with the same wrap/unwrap behaviour as `setBlockAlignment`. |
