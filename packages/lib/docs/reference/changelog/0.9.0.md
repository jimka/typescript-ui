...
- **`AbstractStore.getActiveSorter()` is removed**, in favour of
  ...
  [Migration](/reference/migration/0.9.0) for the full replacement.
...
- **`Slider`'s deprecated `setMinValue` / `getMinValue` / `setMaxValue` /
  ...
  See [Migration](/reference/migration/0.9.0) for the full replacement.
- **`Image.render()` is no longer public.** It falls back to `Component`'s
  ...
  instead. See [Migration](/reference/migration/0.9.0) for the full replacement.
- **`ChartStoreBinding` is removed.** It described a store-bound chart's
  `store` / `xField` / `yField` / `seriesField` fields as a standalone
  interface, but `AbstractChartOptions` already carries the same fields
  directly and is what every chart's construction options and
  `AbstractChart.setStore` actually route through — the interface had no
  consumers anywhere in the library. See [Migration](/reference/migration/0.9.0)
  for the full replacement.

### Diagram

- **`DiagramViewOptions.elkWorkerUrl` and `ElkLayoutEngineOptions.workerUrl`
  are removed.** Neither ever achieved off-thread ELK layout — this library
  always imports elkjs's bundled build, whose own worker-availability check
  can never succeed against it — so both silently ran layout on the main
  thread regardless. Use `elkWorkerFactory` for real off-thread execution.
  See [Migration](/reference/migration/0.9.0) for the full replacement.
...
- **`CodeEditor` gains code folding, line wrapping, a search panel, parser-error
  ...
  grammar's own parse tree) are newly exported from `component/editor`. The
  search panel is a framework-built floating card, pinned to the editor's
  upper-right corner and overlaying the document rather than docking a strip
  that pushes it down; `Ctrl-F` (`Cmd-F` on macOS) and `Escape` open and
  close it, every match tints while it's open, and its nine controls (match
  case, whole word, regular expression, find previous/next, select all,
  close, replace, replace all) are glyph-only `Button`/`ToggleButton`
  controls with hover tooltips. No consumer action is needed.
