...
- **`Button`'s pressed-state text color now shares one CSS rule across every
  default-styled instance of the same class**, instead of each instance
  repeating an identical `color` declaration on its own `#id.pressed` rule —
  the same dedup `Button`'s other `.pressed` chrome already applies. An
  instance with a customized `pressedForegroundColor` still writes its own
  value on its own rule, unaffected. `ToggleButton` and `TabButton` inherit
  the same behaviour. No consumer action is needed; nothing changes
  visually.
...
- **`TabButton`'s busy indicator (`TabBusyIndicator`) no longer duplicates
  its background color on every instance's own CSS rule.** Each now shares
  one CSS rule across every busy indicator in the app. Nothing changes
  visually; no consumer action needed.
...
- **A `Dialog`'s Enter/Tab keydown handling reacted to the dispatcher's
  broadcast rather than to a specific target, so two stacked `Dialog`s could
  both handle the same keypress** — a "save preset" prompt opened from
  inside a login dialog, for example, would have the backgrounded login
  dialog also confirm/submit itself on the same Enter press. `Dialog`'s
  keydown handling now bails unless it owns keyboard routing, checked via a
  new `LayerManager.isTopmostInputLayer(layer)` query that shares the same
  topmost-non-manual-layer walk `Escape` already uses. No consumer action is
  needed.
