...
`ComponentOptions.styleGroup` is removed, together with
`Component.getStyleGroup()` and `Component.setStyleGroup()`, in favour of
the new cross-class `StyleTrait` mechanism (see *Added* below). See
[Migration](/reference/migration/0.8.0) for what to use instead.

The theme `scale` block's `titleGlyph` and `tabCloseGlyph` tokens are gone,
replaced by a five-step icon scale (`glyphXs`/`glyphSm`/`glyphMd`/`glyphLg`/
`glyphXl`). See [Migration](/reference/migration/0.8.0) for which new token
replaces which.
...
- **A table's row-viewport body now loses its old `Body` class and instead
  ...
  consumer stylesheet selector that targets any of these names — see
  [Migration](/reference/migration/0.8.0) for the full class-list breakdown.
  The public export (`Body`, from `component/table/index.ts`) is unchanged;
  only the rendered DOM class list changes.
- **[`DiagramView`](/components/DiagramView) now opens its graph fitted to
  the viewport by default.** A new `fitOnLoad` option controls the view's
  first successful centring, and **defaults to `true`**. See
  [Migration](/reference/migration/0.8.0) for the full breakdown of when it
  applies and how to opt back out.
