...
[`Tree`](/components/Tree) recycles a row pool the same way for scrolling; it flattens the visible subtree into a linear list, and a pool slot follows its node across an expand/collapse instead of following a shifted flat position, so a row that is still on screen keeps its DOM element and rebinds only when what it was last bound to has actually changed — see [Performance](/concepts/performance#virtual-scrolling).
