...
class TreeTablePanel extends Container {
    ...
    constructor(store: AbstractStore, spec: TreeTableSpec) {
        ...
        this._syncBtn.on("action", () => { void this._treeTable.sync(); });
     ...
     * `showOverlay`, which raw-appends it onto the tree table's element rather
     * than registering it as a child of this panel, so the inherited recursion
     * over the child list never reaches it.
     */
     ...
     * as TSV" entries in the underlying tree table's column context menu.
     *
    ...
    exportTSV(options?: ExportOptions): void {
        this._treeTable.exportTSV(options);
    }
