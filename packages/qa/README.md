...
first unit that decorator's error tooltip is on screen, it types one character
into the second field, whose error re-attaches its own tooltip. If
`geometry.tooltip` is `null` in every unit, the hover never armed, and the run
says nothing about C21. The sequence leaves both fields in error and the
tooltip up, so give `call` a run of its own.
