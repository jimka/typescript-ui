...
| `scroll-panes` | A `Grid` of `n` scrolling panes, six across, each holding 40 `Text` rows — more than a cell of the board can show. `n`: panes (default 24). The panes are a `Panel` **subclass**, and have to be: `Panel.canSkipUnchangedLayout` opts a plain `Panel` into the unchanged-commit layout skip by prototype identity, so a plain pane whose rectangle the board re-commits unchanged never runs `doLayout` and never reaches `remeasureScrollMetrics` — the call this cell measures. | `passes,wheel,idle:4` (default: the board's passes, then a wheel on pane 0), `resize`. `work=1` adds `pane.remeasure@<class>` — the counter wraps `Panel.prototype`, so the key carries the receiver's class: `pane.remeasure@ScrollPane` once per pane per layout pass, plus `pane.remeasure@Panel` once for the board root, whose own re-measure returns at once because it does not scroll (and which `g16.panel-settled` passes through uncounted for the same reason) — and `pane.scrollTick`, one per scroll event pane 0 delivers | `board`, `pane0`, `paneLast`, and `pane1` and `row1` (pane 1's first row) above `n = 1` | None recorded: it is G16's surface, carrying both a settled pass and a real scroll on the same scrollable panes in one cell, with a population large enough that the avoided work clears a phase's bracket. `before.host.scrollablePanes` must equal `n`, and `pane.scrollTick` be at least 1 per wheel unit, before either arm is read. **Read the number as scoped:** a subclassed pane is the realistic shape — a real application's scrolling pane almost always is one — and the plain `Panel` the panel first used is the case stage 1's own opt-in already covers, so what G16 has left to win exists only for panels that are *not* opted in. | |
