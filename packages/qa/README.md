...
| `bin/` | `qa-verdict.py`, `qa-table.py`, `qa-forced.py`, `qa-ab.py`. |
| `sweeps/` | Scripted sweeps: `w3-0.sh`, the W3.0 bounding sweep (see *Sweeps*). |
...
| `step=<px>` | pixels per unit for `drag`, `dragout`, `resize`, `hover`, `park` and `pan` | 3 |
| `resize=live\|outline` | sets the app-wide resize mode through `Body.setResizeMode` before the panel is built; a library build without it fails the run | none (the library's default, `live`) |
...
| `grip=`, `hover=`, `wheel=`, `toggle=`, `update=`, `rowfilter=`, `passes=`, `type=`, `click=` | which element or which mutation a panel gives a driver (see the panel table); a value the panel does not list fails the run | the panel's first listed value |
...
`getPreferredSize` and `setClipPath`; `sizeHintMiss`, one per live size-hint
computation, that is per miss of the per-pass size-hint memo;
`getLaidOutComponents`, one per child list a layout builds;
`reserveContentFrame`, one per content-frame walk of a layout manager; and
`measureContent@Grid`, `split.doLayout`, `split.recalculateSizes`,
...
|---|---|---|---|---|---|
| `chart-line` | A 3-series `LineChart`, legend and point markers on. `n`: points per series (default 50). | `resize` (default), `passes`, `update` (swaps two data sets), `hover` (the chart) | `chart`, `point` | Slice 26 F26.1 (fixed): a chart rebuilt every SVG mark per layout pass — at `n = 50`, 1,081 sink calls (`apply` 241, `createElementNS`, `appendChild`, `removeChild` and `release` 210 each) and 12 `measureText` calls per unchanged pass before the fixes; after `text-measurement-without-reflow` an unchanged pass makes no `measureText` call, and after the chart repaint gate no `createElementNS`, `appendChild`, `removeChild` or `release`. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-pass-main`) — the census exactly, at 8.0 ms per pass over 236 elements.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-pass`, `ta-resize-a`) — the same census exactly, at 8.6 ms per pass (p90 12) and 73.7 ms per resize unit (p90 92).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `cl`: `passes` 4.6, `resize` 70.8 ms over 236 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g18i1-clq-*`, `crgi1-clq-*`, `crgi1-clu-*`, `edwi1-clh-*`, `g19i1-clh-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `clq` (`passes`) first loses its 12 `measureText` calls (4.79 → 2.75 ms), then all but 30 of its 1,081 sink calls with `createElementNS` 210 → 0 (2.74 → 0.42 ms) — the *Reproduces* census now reads 0 of each; `clu` (`update`) keeps `createElementNS` at 208.6 on both arms, so no real repaint is gated away, and only the gate's own write leaves `apply` (239.39 → 238.40); `clh` (`hover`) loses three quarters of the event ancestor walk, 11.98 → 3.04 source reads, at flat frame time. Geometry `=` on every run of every cell. |
| `chart-dashboard` | A `Split` of a 2×2 grid of store-backed `LineChart`s, 3 series each, beside two grouped `BarChart`s. `n`: points per line series (default 200). | `resize` (default), `passes` (the grid), `drag` (the split's gutter), `update` (moves one point), `hover` (the first line chart) | `grid`, `chart0`–`chart3`, `bars`, `point0`, `bar0` | M12, slice 26 F26.1 and F26.2 (both fixed): at `n=50`, `passes` with `seam=1` gave `seam.sink.createElementNS` 840 (4 × 210) and `seam.source.measureText` 48 (4 × 12) before the fixes — F26.2's by `text-measurement-without-reflow`, F26.1's by the chart repaint gate — and gives 0 of each after. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-charts`) — M12 exactly: `createElementNS` 840 and `measureText` 48, in 4,329 sink calls at 25.1 ms per pass over 1,112 elements.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-charts`) — M12 exactly, the same 4,329 sink calls, at 26.5 ms per pass (p90 34).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `cd50` (n=50): `passes` 23.0 ms over 1,112 elements; `cd`: `resize` 160.7, `drag` 161.0 ms over 2,912 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g18i1-cdq-*`, `crgi1-cdq-*`, `crgi1-cdu-*`, `crgi1-cdd-*`, `crgi2-cdu-*`, `lri1-cdq-*`, `edwi1-cdh-*`, `edwi2-cdh-*`, `g19i1-cdh-*`, `stki1-cdq-*`, `stki1-cdh-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit at `n=50`: an unchanged `passes` goes 23.1 → 0.59 ms against `master`, `seam.sink.createElementNS` 840 → 0 and 4,329 sink calls → 53 — M12 with F26.1 and F26.2 closed in the engine; `update` and `drag` hold `createElementNS` at 208.6 and 1,006, so no real repaint is suppressed, and the gate's own cost on a changed `update` is 0.2–0.4 ms; `hover` loses 76% of the ancestor walk (17.37 → 4.09 source reads) and reads −6.50 ms against `master`. Geometry `=` on every run of every cell. |
| `shell-deep` | **S1 stand-in.** A menu bar; an explorer `Accordion` (open file tree, open outline, closed history) beside a `Dock` of 2×2 regions of `CodeEditor`s under a toolbar; a status bar. `n`: tabs per region (default 1, four editors). | `drag,wheel:120` (default), `resize`, `passes`, `hover`, `park` (the explorer gutter), `type` (the first painted editor), `toggle`. `grip=` `dock-h`, `dock-v`, `sidebar`, `section`, `tab`; `hover=` `toolbar`, `menubar`, `tabs`; `wheel=` `tree`, `editor`; `toggle=` `section` (the closed History section), `pane` (the explorer pane's collapse). `work=1` adds `sidebar.doLayout`, `main.doLayout`, `history.doLayout`, `history.getPreferredSize` and `onThemeChange@CodeEditor` | `sidebar`, `files`, `outline`, `history`, `main`, `dock`, `editor0`–`editor3`, `header0` (the first page's `Header`), `status` (the status bar), `resizeOutline` (the resize outline; `null` unless an outline drag is on screen) | M5, slice 06 F06.3: `park` with `work=1&geom=1` gives `sidebar.doLayout` and `main.doLayout` 1.00 ±0.02 per unit, `geometry.sidebar` the same in every unit. M6, slice 08 F08.3: `drag` with `grip=sidebar&work=1` gives `history.doLayout` 1.00 ±0.02 and `history.getPreferredSize` at least 1.00. M7, slice 23 F23.1 (C7): `theme:4` grows the note's rule total by about 51 × 4 × `before.host.editorViews`. | `minibrowser`: 2026-09-20, lib `608544c9` — **S1 stand-in baseline**, at `n=1` over 2,484 elements: `park` (`val-sdeep-park`) 20.5 ms per unit, `sidebar.doLayout` and `main.doLayout` 0.99, the sidebar at 8,41,160,1999 in all 150 units; `drag` with `grip=sidebar` (`val-sdeep-drag`) 84.6 ms per unit, `history.doLayout` and `history.getPreferredSize` 1.00; `theme:4` (`val-sdeep-theme`) 206.0 ms per switch, 517 → 1,333 rules, which is 51 × 4 editor views × 4 switches exactly. Re-verified on 2026-09-20 against lib `2df6a50d`, current `master`, in a same-session A/B (`tr-*`, old-new-old): census, counters and rectangles identical and the times inside the old arm's own bracket, so these numbers still describe `master` after the tree-reveal merge rewrote `Tree`.<br>`tauri`: 2026-09-20, lib `608544c9` — **S1 stand-in baseline**, the same counters, rectangles and rule growth as MiniBrowser to the digit: `park` 9.4 ms per unit (p90 14), `drag` with `grip=sidebar` 64.5 ms (p90 71), `theme:4` 179.7 ms per switch (p90 197).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `sdh`: `drag` 55.6, `wheel` 80.8 ms over 2,484 elements; `sd4h` (n=4): `drag` 60.2, `wheel` 78.3, `theme` 194.3 ms over 2,604 elements; `sds` (grip=sidebar): `park` 9.0, `drag` 69.3, `theme` 172.0 ms over 2,484 elements; `sdv` (grip=dock-v): `drag` 39.6 ms over 2,484 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g18i1-sdy-*`, `g9i1-sd*`, `lri1-sd*`, `lri2-sdr-*`, `g12i1-sdt-*`, `g24i1-sdq-*`, `g28i1-sdh-*`, `roi1-sd*`, `stki1-sd*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `type` 55.3 → 21.8 ms once the status bar's width probe stops forcing an editor re-layout; a settled `passes` 2.80 → 0.17 ms against `master`, work 4,938 → 166 and writes 373 → 16; `resize` 64.9 → 63.1 ms with work 6,656 → 3,155, then a further −16.6% of its size-hint calls (1,806.4 → 1,506.5) at +0.9 to +1.9 ms, which reproduced and is engine-side (no counter of the 152 the cell reports rose); `toggle=pane` 22.4 → 19.0 ms against `master`, work 1,085 → 400 and `sidebar.doLayout` 0.18 → 0.03; `theme` work 10,490 → 7,612. With `resize=outline`, a `grip=sidebar` drag goes 66.9 → 18.2 ms (work 5,832 → 17.0, `apply` 551 → 1.7), `grip=dock-h` 53.4 → 17.0 and `grip=dock-v` 38.0 → 9.5, the page holding its pre-press layout in every unit, `resizeOutline` non-`null` in every drag unit and `null` at rest, the outline tracking the base's growth unit for unit, and the layout at rest identical to live mode's; the live arm is unchanged to a tenth of a percent of work. |
| `shell-shallow` | **S3 stand-in.** `shell-deep`'s chrome and explorer around a `Dock` of one region. `n`: tabs (default 1, one editor). | As `shell-deep`, but `grip=` `sidebar`, `section` and `hover=` `toolbar`, `menubar` | As `shell-deep`, with `editor0` only, `header0`, `status` and `resizeOutline` | M5–M7, as for `shell-deep`. | `minibrowser`: 2026-09-20, lib `608544c9` — **S3 stand-in baseline**, at `n=1` over 1,618 elements: `park` (`val-sshal-park`) 9.2 ms per unit, `sidebar.doLayout` and `main.doLayout` 0.99, the sidebar at 8,41,160,1999 in all 150 units; `drag` with `grip=sidebar` (`val-sshal-drag`) 67.3 ms per unit, `history.doLayout` and `history.getPreferredSize` 1.00; `theme:4` (`val-sshal-theme`) 147.7 ms per switch, 325 → 529 rules, which is 51 × 1 editor view × 4 switches exactly. Re-verified on 2026-09-20 against lib `2df6a50d`, current `master`, in a same-session A/B (`tr-*`, old-new-old): census, counters and rectangles identical and the times inside the old arm's own bracket, so these numbers still describe `master` after the tree-reveal merge rewrote `Tree`.<br>`tauri`: 2026-09-20, lib `608544c9` — **S3 stand-in baseline**, likewise identical in counters, rectangles and rule growth: `park` 10.3 ms per unit (p90 17), `drag` with `grip=sidebar` 57.9 ms (p90 67), `theme:4` 132.0 ms per switch (p90 144).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `sss` (grip=sidebar): `park` 9.8, `drag` 61.5, `theme` 134.3 ms over 1,618 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g18i1-ssy-*`, `g9i1-ss*`, `lri1-ss*`, `g12i1-sst-*`, `g24i1-ssq-*`, `roi1-sss-*`, `roi1-ssc-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `type` 53.5 → 18.7 ms; `passes` 2.14 → 0.15 ms with work 4,120 → 169; `resize` 59.2 → 56.2 ms with work 5,833 → 2,337, then size-hint calls 1,381 → 1,189; `toggle=pane` work 518 → 353 and `sidebar.doLayout` 0.17 → 0.03 at flat frame time; `theme` 132.2 → 129.7 ms with work 8,028 → 5,220. With `resize=outline`, a `grip=sidebar` drag goes 59.6 → 12.2 ms and work 5,219 → 13.1, removing 95% of the drag's excess over idle; `grip=section` is the exception — work 1,145 → 19.0 but frame time flat at 12.9 ms, that cell sitting close to idle before anything is skipped. |
| `table-rows` | A `Table` over a 12-field model with every cell type, the filter row shown. `n`: rows (default 10,000). | `wheel` (default), `resize`, `passes`, `key` (ArrowDown and ArrowUp on the body), `update`, `drag` (the first column's edge), `click` (sorts by `name`). `update=` `record` (one visible row's score), `filter` (the department filter). `rowfilter=` `off` (default), `title` (a body row filter admitting two titles in five, through `Table.setRowVisible`); `describe()` adds `rowFilter`, `storeRecords`, `filteredRows` and `bodyRowFilter`. `work=1` adds `getVisibleRecords@TableBody` and `getRecords@MemoryStore` | `table`, `header`, `body`, `focused` (the cell showing the focus style; `null` while none does), `cell` (the body's first `StringCell`) | M8, slice 19 F19.2: `key` with `work=1` gives `getVisibleRecords@TableBody` 6.00 per unit. C34: `wheel` with `seam=1` gives `seam.sink.dispatchCustomEvent` above 0 before the boolean-cell fan-out opt-out — one synthetic click per boolean pool slot per rebind — and 0 after. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-table`) — M8 exactly: `getVisibleRecords@TableBody` 6.00 per `key` unit at 10,000 rows, 21.4 ms per unit over 4,766 elements.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-table`) — M8 exactly, `getVisibleRecords@TableBody` 6.00, at 20.8 ms per `key` unit (p90 23).<br>**Store-worker fix confirmed in-engine**: 2026-09-20, `master` `5a46ff52` (`store-worker-verify`, MiniBrowser) — the panel mounts and scrolls with the same 4,766-element census as the baseline above, so the inlined `blob:` worker boots and the 10,000-record store builds its view. This is the one claim in `store-worker-fail-safe` that no offline test could settle.<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `tr`: `key` 17.2, `update` 38.1, `passes` 1.6, `wheel` 109.1 ms over 4,766 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record. |
| `treetable-rows` | A `TreeTable` of folders, each with 3 subfolders of 3 files: 13 rows per root. `n`: root folders (default 200, 2,600 rows). | `toggle` (default: collapse all, then expand all), `resize`, `passes`, `wheel`, `key` (ArrowDown and ArrowUp), `update` (one row's size), `click` (a size cell). `work=1` adds `getVisibleRecords@TreeBody` and `getRecords@MemoryStore` | `table`, `header`, `body`, `focused` (the cell showing the focus style) | M9, slice 22 F22.5: `key` with `work=1` gives `getVisibleRecords@TreeBody` 6.00. F22.3 (C10): `toggle` with `seam=1` gives `ensureStyleRule`, `setRuleStyles`, `createElement` and `removeElement` equal — one caret minted and one detached per visible branch row — and no `deleteStyleRule` in that path; the deletes a long phase shows follow elapsed time, not toggles. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-treetable`, `c10-probe`) — M9 exactly: `getVisibleRecords@TreeBody` 6.00 per `key` unit at 200 roots, 16.6 ms per unit over 1,699 elements; `toggle`'s four glyph-swap counters 70.5 each at 201.2 ms per unit, with `deleteStyleRule` absent over 2 units and 55.7 over 20 — the collector, not the swap. C10 is unfixed and its leak is GC-bounded; see the panels plan's M9.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-treetable`) — M9 exactly, the same 6.00 and the same four swap counters at 70.5, at 17.0 ms per `key` unit (p90 17) and 196.1 ms per `toggle` (p90 227). The C10 probe was run under MiniBrowser only.<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `tt`: `key` 17.2, `toggle` 199.1 ms over 1,699 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g17i1-ttg-*`, `g17i2-ttg-*`, `stki1-ttg-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit at `n=200`: `toggle` 180.2 → 120.6 ms, and 174.4 → 121.7 ms against `master`, once `setGlyph` takes a glyph name instead of minting and swapping an element — the F22.3 census collapses, `ensureStyleRule`, `setRuleStyles`, `deleteStyleRule` and `querySelector` 70.5 → 0 to 0.05, `createElementNS` 141 → 75, `createElement` 70.5 → 37.5, `removeElement` 141 → 75, `release` 211.5 → 112.5, work 12,089 → 8,996 and `sink/u` 4,019.6 → 2,954.6, identically in two sessions. The `svg` and `use` mount counts are equal across all five runs of each cell. The `focused` label differs on exactly one run per session — a fix run in `i1`, a base run in `i2` — so it is timing-sensitive in this panel and does not implicate the change. |
| `tree-nodes` | A fully expanded `Tree` of folders of 5 files each, with the explorer's icon renderer. `n`: folders (default 300, 1,800 nodes). | `key` (default: ArrowLeft and ArrowRight collapse and expand the first folder), `passes`, `resize`, `wheel` | `tree` | M10, slice 18 F18.6: `passes` with `work=1` gives `setStyleState@TreeRow` = 2 × `before.host.treeRows`. F18.4: `key` with `seam=1` gives `ensureStyleRule`, `setRuleStyles` and `deleteStyleRule` 1.50 each. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-tree`) — M10 exactly: `setStyleState@TreeRow` 186 per pass at 93 tree rows, and `ensureStyleRule`, `setRuleStyles` and `deleteStyleRule` 1.50 each per `key` unit, at 96.0 ms per unit over 632 elements. Re-verified on 2026-09-20 against lib `2df6a50d`, current `master`, in a same-session A/B (`tr-*`, old-new-old): census, counters and rectangles identical and the times inside the old arm's own bracket, so these numbers still describe `master` after the tree-reveal merge rewrote `Tree`.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-tree`) — M10 exactly, the same 186 and 1.50s, at 0.3 ms per pass and 91.9 ms per `key` unit (p90 96).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `tn`: `passes` 0.1, `key` 93.2 ms over 632 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g18i1-tnk-*`, `g17i1-tnk-*`, `g24i1-tnq-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `key` 91.3 → 79.9 ms with M10's six `measureText` calls gone, then 80.8 → 74.1 ms with F18.4's `ensureStyleRule`, `setRuleStyles` and `deleteStyleRule` 1.50 → 0 and `createElementNS` 7.03 → 1.03; `passes` work 288 → 2 and 0.10 → 0.01 ms, `setStyleState@TreeRow` 186 and `getPreferredSize@Text` 90 gone with `doLayout@Tree` and `getLaidOutComponents@Tree` held at 1.00. The shell check cells (`g24i1-sdq-*`, `g24i1-ssq-*`) no longer reproduce their absolute figures and cannot: the unchanged-commit opt-ins, four branches below, have already taken a shell pass from 4,938 work units to 169, leaving 1.5 units of row styling to skip. Geometry `=` on every run. |
| `list-items` | A `List` over a store. `n`: items (default 300). | `key` (default: ArrowDown and ArrowUp), `passes`, `resize` | `list` | M11, slice 18 F18.2: `passes` with `seam=1` gives `seam.sink.apply` 908 — the 906 rows of the offline census plus the engine's overlay scroller. F18.1: `key` with `seam=1` gives `apply` 603 and `dispatchCustomEvent` 1. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-list`) — M11 with `passes` at 908 instead of 906 (see the panels plan's M11): `apply` 908 per pass at 3.1 ms, and `apply` 603 with `dispatchCustomEvent` 1 per `key` unit at 16.9 ms, over 932 elements.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-list`) — M11 exactly, the same 908 and 603 with `dispatchCustomEvent` 1, at 3.1 ms per pass and 16.9 ms per `key` unit (p90 17).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `li`: `passes` 3.0, `key` 17.3 ms over 932 elements; `li1` (n=1): `key` 17.4 ms over 35 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g24i1-li3-*`, `g24i1-li30-*`, `g24i1-li1-*`, `stki1-li3-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: an arrow key writes 8 sink calls instead of 604 at `n=300` and instead of 6,004 at `n=3,000` (`apply` 603 → 7 and 6,003 → 7, `dispatchCustomEvent` held at 1.00), and 0 instead of 3 at `n=1`, where a clamped no-op move no longer dispatches at all; the three source reads (`intern`, `getId`, `getScrollMetrics`) stay at 1.00 each. Frame time is vsync-bound and flat, 16.8–17.2 ms in every cell, as the plan predicts. Geometry `=` on every run. |
| `markdown-doc` | A `MarkdownViewer` of a document with bullet lists, TypeScript fences and tables, beside a side header in a `Split`. `n`: sections (default 60). | `drag` (default: the split's gutter), `passes` (the viewer), `resize`, `update` (swaps two versions of the document), `wheel` (the first paragraph), `call` (a ladder of absolute scroll offsets: a triangle over `before.host.maxScrollTop`, period 24 units, so unit `k` is at the same pixel in every run and a lap ends where it started). `work=1` adds `heading@<id>`, one per scroll tick naming the heading the viewer resolved (`heading@none` above the first), and one of `heading.agree` or `heading.disagree` beside it, from re-deriving `findActiveHeading`'s rule over the pane's live rectangles in the same task | `side`, `viewer`, `minimap` (the outline minimap) | M13, slice 25 F25.3: `passes` with `seam=1` gives `seam.source.getElementRect` 2.00. F23.1 (C7): `theme:4` grows the note's rule total by about 51 × 4 × `before.host.editorViews`; fences become editors only near the viewport, so that count depends on the screen. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-markdown`) — M13 exactly: `getElementRect` 2.00 per pass at 1.5 ms, and `theme:4` 494 → 1,514 rules, which is 51 × the 5 editor views the screen upgraded × 4 switches, at 157.0 ms per switch over 1,189 elements.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-markdown`) — M13 exactly, the same `getElementRect` 2.00 per pass and 494 → 1,514 rules, at 1.0 ms per pass and 108.3 ms per switch (p90 111). Its `theme` phase reads a little more than MiniBrowser's (`getElementRect` 2.25 against 1.50, `getScrollMetrics` 7.75 against 7.50, plus `getOffsetSize` 0.25 and `querySelector` 0.50 MiniBrowser never records); no checked figure is among them.<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `md`: `passes` 0.5, `drag` 55.1, `theme` 99.3 ms over 1,181 elements; `mdu`: `update` 182.1 ms over 1,181 elements; `mdu480` (n=480): `update` 4219.2 ms over 6,312 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g26lexi1-mdu-*`, `g26lexi1-mdu480-*`, `stki1-mdu480-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `update` 168.9 → 96.4 ms at 60 sections and 4,436.5 → 283.4 ms at 480, and 4,295.5 → 156.8 ms against `master` — every seam and work count identical between the arms, so the same document is lexed into the same DOM. The lexer's superlinearity is gone: the fix's 480/60 ratio is 2.94, against 26.27 for the arm below it. Geometry `=` on every run. |
| `canvas-idle` | `WebGLCanvas`es whose 2D context is taken before they ask for a WebGL2 one, so the engine refuses it, and 2D `Canvas`es started while shown and then moved under an already-hidden panel; none of them drawing anything. `n`: surfaces per group (default 4). | `idle` (default) | `shown` | M14, slice 26 F26.7 (C35) and F26.8 (C36), both fixed: `before.host.webglContexts` = 0 with `webglAnimating` = 0, and `hiddenStarted` = 4 with `hiddenAnimating` = 0; `idle` with `seam=1` gives `seam.sink.requestAnimationFrame` = `webglAnimating` + `hiddenAnimating` ±0.05, so 0 per idle unit. | pre-fix: `minibrowser`: 2026-09-20, lib `608544c9` (`val-canvas`) — M14 exactly: `webglContexts` 0 with `webglAnimating` 4, `hiddenStarted` and `hiddenAnimating` 4, and `requestAnimationFrame` 8 per idle unit, their sum. The sweep's first run showed neither symptom, and rebuilding the panel for both is what this run confirms; see the panels plan's M14.<br>pre-fix: `tauri`: 2026-09-20, lib `608544c9` (`ta-canvas`) — M14 exactly: 0, 4, 4, 4 and `requestAnimationFrame` 8, at 16.9 ms per idle frame (p90 17). Its `getContext` reads 1.8 per unit against MiniBrowser's 1.9, a read counter no check names.<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `cv`: `idle` 17.3 ms over 22 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record. |
| `diagram-graph` | A `DiagramView` of an ELK-layered three-way tree at zoom 1, under a toolbar of zoom buttons. `n`: nodes (default 400). | `click` (default: the empty node layer behind the nodes), `resize`, `passes` (the view), `pan` (the node layer, along x), `wheel` (the node layer), `hover` (the view). Its `afterMount` waits for the layout. | `view` | M16, slice 27 F27.2: `click` with `seam=1` gives `seam.source.getElementById` + `seam.source.contains` = 802 (2n + 2) — one lookup per node per pass, whatever the screen. The split follows the nodes that have ever rendered an element, which `before.host.nodeElements` (the nodes attached now) only bounds from below, so the total is what this checks. M17, F27.1 (fixed by `motion-transform-inline`): `pan` with `seam=1` gives `seam.sink.setRuleStyles` 0 and `seam.sink.apply` 1.00 — the pan's transform is an inline write; it was `setRuleStyles` 1.00 before the fix. M18, F27.3: `wheel:10` with `work=1` gives `setResidency@DiagramEdgeLayer` 1.00. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-diagram`) — M16's total exactly: `getElementById` 324 + `contains` 478 = 802 per click at `n=400`, 58.4 ms per unit. The split resolves to 238 scanned nodes against the census's 135 attached ones, and the clause deriving it from `nodeElements` was wrong, not the census; see the editors and overlays plan's M16. M17 exactly: `setRuleStyles` 1.00 per `pan` unit at 56.6 ms. M18 exactly: `setResidency@DiagramEdgeLayer` 1.00 per `wheel` unit at 66.1 ms. Over 633 elements.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-diagram`) — M16, M17 and M18 again, the same 324 + 478 = 802, the same `setRuleStyles` 1.00 and the same `setResidency@DiagramEdgeLayer` 1.00, at 55.9 ms per click (p90 84), 56.7 ms per `pan` unit (p90 82) and 63.4 ms per `wheel` unit (p90 91).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `dg`: `click` 54.9, `pan` 50.0, `wheel` 57.1 ms over 633 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g28i1-dgp-*`, `stki1-dgp-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `pan` 53.3 → 17.1 ms, and 54.9 → 17.0 ms against `master` — the one stylesheet rule write per frame becomes one inline write (`setRuleStyles` 1.00 → 0, `apply` 0 → 1.00), and the frame lands on the display's 16.7 ms interval. Geometry `=` on every run. |
| `code-document` | One `CodeEditor` of JavaScript under a toolbar of ten buttons, over a status `Text` updated on every cursor change, beside an outline header in a `Split`. `n`: lines (default 2,000). | `wheel` (default: the editor's scroller), `resize`, `passes` (the editor), `drag` (the split's gutter), `type` and `key` (ArrowDown and ArrowUp) on the editor's content | `editor`, `status` | None recorded: the single-editor partner of the shells. | `minibrowser`: 2026-09-20, lib `608544c9` (`shk-codedoc`) — **baseline**, at `n=2,000` over 644 elements, per unit: `passes` 0.0 ms, `type` 55.0 ms, `drag` 56.0 ms, `resize` 57.1 ms, `wheel` 53.4 ms and `key` 57.5 ms, against 9.3 ms idle frames. No error, and a rectangle for `editor` and `status` in every unit.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-codedoc`) — **baseline**, the same rectangles in every unit, per unit: `passes` 0.0 ms, `type` 56.0 ms (p90 71), `drag` 49.1 ms (p90 54), `resize` 55.2 ms (p90 69), `wheel` 56.7 ms (p90 83) and `key` 60.1 ms (p90 84), against 9.8 ms idle frames.<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `cdoc`: `passes` 0.0, `wheel` 52.9, `resize` 51.7, `key` 54.5, `drag` 51.1, `type` 53.6 ms over 644 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g18i1-cdt-*`, `stki1-cdt-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `type` 49.6 → 18.5 ms, and 52.6 → 19.5 ms against `master` — the status bar's 0.75 `measureText` calls per keystroke become 0.75 canvas `measureTextAdvance` calls, and the forced document layout they used to land after CodeMirror's invalidation goes with them. Work and DOM writes are identical on both arms and geometry `=` on every run. |
| `editor-tabs` | A `Split` of one always-visible `CodeEditor` — the reference — beside a `TabPanel` of `n` `CodeEditor`s of 300 lines each. Its `afterMount` shows every tab once, so each editor mounts a CodeMirror view, then returns to tab 0. `n`: tabs (default 8). | `theme:14,idle:4` (default), `resize`, `passes`. It replaces the page-wide `theme` target with its own, which switches the theme and then shows the next tab on an even unit, and checks the one shown before it on the odd unit after — the only moment a withheld reconfigure is observable, since the driver restores the page's theme when the phase ends. One lap of shows, so every tab is shown exactly once; later units switch the theme without showing anything. `work=1` adds `onThemeChange@CodeEditor`, `theme.hidden` and `theme.shown` (per theme switch, by whether each tab's editor is the selected one). The check itself runs on every unit whatever `work=` says, and files one of `theme.match` or `theme.mismatch` per check, plus `theme.indistinct` when the switch left no trace in the reference's own `.cm-editor` class list; like an ablation's own counters, those three reach the report even where the work family is not installed | `tabbed`, `reference`, `editor0` (tab 0's editor; `[0, 0, 0, 0]` while its tab is hidden, so a real box only in the trailing settled phase, which `restore()` returns to tab 0), `editorLast` (the last tab's editor; a real box only from the unit that shows its tab to the end of the theme phase) | None recorded: it is G25's surface, where a hidden tab's editor still holds a view to withhold a theme reconfigure from — which neither shell gave it. `before.host.editorViews` must be `n + 1`, and plain `theme.hidden` `n - 1` per unit, before the arm is read. | |
| `scroll-panes` | A `Grid` of `n` scrolling panes, six across, each holding 40 `Text` rows — more than a cell of the board can show. `n`: panes (default 24). The panes are a `Panel` **subclass**, and have to be: `Panel.canSkipUnchangedLayout` opts a plain `Panel` into the unchanged-commit layout skip by prototype identity, so a plain pane whose rectangle the board re-commits unchanged never runs `doLayout` and never reaches `remeasureScrollMetrics` — the call this cell measures. | `passes,wheel,idle:4` (default: the board's passes, then a wheel on pane 0), `resize`, `call` (writes an absolute `scrollTop` to every pane but pane 1 once per unit, walking a fixed triangle of 24 units a lap — the phase built to reach the framework's scroll-shadow path deterministically, because `updateScrollShadows` and `resizeScrollShadowOverlay` are reachable from a scroll event alone and no layout pass calls either, while whether the `wheel` phase's eased controller lands a scroll of its own is what `pane.shadowUpdate` is there to say; a wheel's landing offsets follow elapsed time, the ladder's do not). `work=1` adds `pane.remeasure@<class>` — the counter wraps `Panel.prototype`, so the key carries the receiver's class: `pane.remeasure@ScrollPane` once per pane per layout pass, plus `pane.remeasure@Panel` once for the board root, whose own re-measure returns at once because it does not scroll (and which `g16.panel-settled` passes through uncounted for the same reason) — `pane.shadowUpdate@<class>`, one per entry into the scroll-shadow path, and `pane.scrollTick`, one per scroll event pane 0 delivers | `board`, `pane0`, `row0` (pane 0's first row, which the ladder moves by an absolute integer offset every run repeats, so it gates a `call` cell — but labels are the panel's, not a phase's, so a cell that drives the `wheel`, the default drive included, must take it out with `--allow-diff row0@<phase>` and read `pane1` and `row1` instead), `paneLast`, and `pane1` and `row1` (pane 1's first row, both held still by the ladder) above `n = 1` | None recorded: it is G16's surface, carrying both a settled pass and a real scroll on the same scrollable panes in one cell, with a population large enough that the avoided work clears a phase's bracket. `before.host.scrollablePanes` must equal `n` and `before.host.ladderPanes` equal `n − 1`, and `pane.scrollTick` be at least 1 per wheel or `call` unit — with `pane.shadowUpdate@ScrollPane` about `n − 1` per `call` unit — before either arm is read; a zero there says the scroll-shadow path never ran, which is what `g16.scroll-reads`' earlier `unreached` reading could not distinguish from an arm that saved nothing. **Read the number as scoped:** a subclassed pane is the realistic shape — a real application's scrolling pane almost always is one — and the plain `Panel` the panel first used is the case stage 1's own opt-in already covers, so what G16 has left to win exists only for panels that are *not* opted in. | |
| `markdown-editor` | A `MarkdownDocumentPanel` over a status line, beside a `Markdown` preview re-rendered on every change, in a `Split`. `n`: sections (default 60, about 9 KB). | `call` (default: moves the caret in the first paragraph, a commit that changes only the selection), `resize`, `passes`, `update` (swaps two versions of the document), `type` and `wheel` (the editable element), `drag` (the split's gutter) | `editor`, `viewer` | M19, slice 24 F24.1: `call` and `type`, each with `work=1`, give `handleChange@MarkdownEditor` 1.00. A caret move commits on `selectionchange`, a task after its unit, so read the phase's average, not one unit. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-mdedit`) — M19: `handleChange@MarkdownEditor` 1.00 per `type` unit exactly, and 0.97 per `call` unit — 39 commits over 40 caret moves, the fortieth landing after the phase, which is the `selectionchange` lag this row names. 68.0 ms per typed character and 15.3 ms per caret move, over 1,617 elements at `n=60` (10,459 characters).<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-mdedit`) — M19 again, the same `handleChange@MarkdownEditor` 1.00 per `type` unit and 0.97 per `call` unit, at 64.4 ms per typed character (p90 108) and 15.5 ms per caret move (p90 34).<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `me`: `call` 13.7, `type` 66.1 ms over 1,617 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record. |
| `windows` | `n` floating `Window`s of a four-field form, the last ⌊n/2⌋ minimized, a bare `Window`, one with insets (4, 12, 4, 4) and an always-on-top one overlapping the cascade, over a toolbar, with one persistent toast. `n`: windows (default 8). | `viewport` (default), `passes` (the bare window), `hover` (window 0), `drag` (window 0), `click` (the open windows' headers, raising each), `toggle` (opens a six-field `Dialog`, then closes it, every 30 units). `grip=` `header` (a move), `edge` (the east strip, a resize) | `win0`, `bare`, `pinned` (the always-on-top window), `southStrip` (the insets window's south strip), `header0` (window 0's header), `resizeOutline` (the resize outline; `null` unless an outline drag is on screen), `min0` (the first docked window, when one is minimized) | M20, slice 09 F09.11: `drag` counts its own press and release, so run `drive=drag:40,drag:80&grip=header&seam=1`; for `seam.sink.apply` and `seam.source.getViewportSize`, (80 × the second phase's value − 40 × the first's) / 40 = 1.00. M21, F09.4: `passes` with `seam=1` gives `seam.sink.apply` 38.00 and no `setRuleStyles`. M22, F09.6 (fixed): `viewport` with `seam=1` gives `seam.source.getViewportSize` one per open window plus one each for the dock, `Body` and the toast — 10 at the default `n=8` — and `getThemeVar` 0; before the fix it was at least 16 (`before.host.minimized`²). C25 (fixed): with `geom=1`, every `geometry.southStrip` sample is 4 px tall, the bottom inset; it was 12, the right inset, before the fix. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-win`, `shk-windows`, `shk-win-edge`) — M20: the two-phase difference gives `seam.sink.apply` 1.01 and `seam.source.getViewportSize` 1.00 per header move, at 27.7 and 27.1 ms per unit. M21 exactly: `apply` 38.00 per settled pass at 0.4 ms, and no `setRuleStyles`. M22: `getViewportSize` 23 per resize event — the 16 of `minimized`² plus 7 from the page's other listeners — beside `getThemeVar` 16 and `apply` 1,118, at 18.3 ms. C25 holds: `geometry.southStrip` is 12 px tall in every unit of all 11 geometry phases of the three runs. Over 583 elements at `n=8`.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-win`) — M20, M21 and M22 again, every counter where MiniBrowser put it, and C25's `southStrip` 12 px in every unit of all four geometry phases, at 28.9 and 27.7 ms per header-move unit (p90 33 and 32), 0.4 ms per settled pass (p90 1) and 17.3 ms per resize event (p90 17). `shk-windows` and `shk-win-edge` were run under MiniBrowser only.<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `wh` (grip=header): `drag` 28.2, `drag` 27.6 ms over 634 elements; `we` (grip=edge): `drag` 42.8, `passes` 0.2, `viewport` 17.3, `toggle` 29.5 ms over 634 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g08i1-w8v-*`, `g08i1-w4v-*`, `g08i1-wdlg-*`, `g9i1-wq-*`, `roi1-we-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `viewport` holds its frame time and loses more than half its work — `getThemeVar` 16.00 → 0.01 and `getViewportSize` 25.00 → 10.00, work 4,903 → 2,131 and writes 550 → 226 at the default scale, and 4.00 → 0.01, 11.00 → 8.00, 1,763 → 1,301 and 188 → 134 at `n=4`; the dialog `toggle` `getThemeVar` 1.20 → 0.01; `passes` 0.27 → 0.07 ms with work 521 → 89 and `doLayout@WindowHeader` 1 → 0. `grip=edge` is the wave's clearest negative result: `resize=outline` takes its work from 599.6 to 4.2 per unit and `apply` from 51.5 to 1.5, and the frame moves only 43.1 → 40.4 ms, inside the cell's own 2.83 ms bracket — the edge resize is paint-bound, and just 10% of its excess over idle is layout work. Its in-drag geometry gate is unstable (`header0` and `win0` differ between base runs in one unit of 150), so the cell is judged on `rest`, identical on every arm, and on its counters. |
| `form-flat` | `n` fields cycling eight kinds — a `TextField` in a `FieldDecorator` that checks it on every change, a `ComboBox` over 20 options, `DateField`, `TimeField`, `NumberSpinner`, `Checkbox`, `Toggle` and `Slider` — in one two-column `LabeledGrid`, under a one-column `LabeledGrid` of eight `TextField`s, in a scrolling panel. `n`: fields (default 64). | `passes` (default), `resize`, `hover` (the header grid, along y), `wheel` (the scrolling panel), `pan` (the first slider), `click`, `type`, `update` (one programmatic write per unit: flips the first checkbox and moves the first slider one step off its start and back), `call` (C21's sequence on the first two decorated text fields). `passes=` `form`, `header`, `date` (the first `DateField`), `combo` (the first `ComboBox`); `type=` `text` (the first decorated field), `date` (types `2026-09-19` into the first `DateField`); `click=` `toggle` (the first toggle's track and the first checkbox's box — the surface each one toggles from), `root` (the first checkbox's own element, outside its box — where a click on its label lands), `combo` (opens and closes the first combo box on alternate units; give it an even count). A kind the first `n` fields lack gives its target none: a slider, and so `update`, needs `n` ≥ 8, and `call` a second decorated text field, `n` ≥ 9. `work=1` adds `checkbox.action` and `slider.action`, one per `"action"` delivered. | `header`, `form`, `tooltip` (the tooltip on screen; `null` while none is) | M23, slice 28 F28.10: `passes` with `passes=header&work=1` gives `getPreferredSize@…`, `getMinSize@…` and `getMaxSize@…` summing to 160. M24, slice 17 F17.1: `passes` with `passes=date&work=1&seam=1` gives `seam.sink.apply` 12, `doLayout@…` summing to 10 and the three size-hint families to 125. M25, slice 16 F16.3: `passes` with `passes=combo&seam=1` gives `seam.sink.apply` 7. C21 (fixed): `call:120` with `geom=1`, as the run's only phase, gives `geometry.tooltip` `null` until the first decorated field's hover delay runs out, then the same rectangle in every later unit, although a keystroke on the first of them clears the second field's error; before the fix the rectangle went `null` about 100 ms after that keystroke. See *C21's witness hovers the field* below the table. C23 (fixed): `type:10` with `type=date&seam=1` gives `seam.sink.setRuleStyles` 0.2 per unit, 2 over the ten characters of `2026-09-19`: the field turns invalid on the first character and valid on the tenth, one rule write each, in any engine. It stays above 0 because the red border is still a stylesheet-rule write, which C23's fix left alone. C40: `update` with `seam=1` gives `seam.sink.dispatchCustomEvent` 2.00 per unit before the fix — a synthetic `click` from `Checkbox.setSelected` and an `input` from `Slider.setValue` — and 0 after; with `work=1`, `checkbox.action` and `slider.action` read 1.00 each before and 0 after. `click` with `click=root&work=1` gives `checkbox.action` 1.00 per unit before, for a click that changes nothing, and 0 after. C34 and C40's user side: `click` with `click=toggle&seam=1&work=1` over an even count gives `seam.sink.dispatchCustomEvent` and `checkbox.action` 0.50 each per unit on both arms — one announcement per checkbox toggle, a synthetic `click` before and a DOM `change` after — unlike the pooled rebind in `table-rows`; `pan` with `work=1` gives the same `slider.action` on both arms. | `minibrowser`: 2026-09-20, lib `608544c9` (`val-form-flat`, `val-form-date`, `val-form-combo`, `shk-form-date`) — M23 exactly: 48 preferred + 64 min + 48 max = 160 size queries per header pass, at 0.2 ms. M24 with `apply` 12 instead of the 8 re-measured offline (see the editors and overlays plan's M24), `doLayout` 10 and the size hints 125 both exact, at 0.3 ms per `DateField` pass. M25 exactly: `apply` 7 per `ComboBox` pass at 0.1 ms. C23 shows: 4 `setRuleStyles` over the ten characters of `2026-09-19`, where slice 17 recorded six. Over 556 elements at `n=64`.<br>`tauri`: 2026-09-20, lib `608544c9` (`ta-form-flat`, `ta-form-date`, `ta-form-combo`) — M23, M24 and M25 again, the same 160 size queries, the same `apply` 12 with `doLayout` 10 and the size hints 125, and the same `apply` 7, at 0.2, 0.3 and 0.1 ms per pass (p90 1 each). C23's `type=date` run was made under MiniBrowser only.<br>**W3.0 baseline** (MiniBrowser, 2026-09-22, lib `83cfb0d7`, runs `w3s1-b00-*`; [96-w3-0-bounding-sweep.md](../../plans/research/render-review-2026-09-15/96-w3-0-bounding-sweep.md#the-fresh-baseline-b00)), per unit: `ffh` (passes=header, type=date): `passes` 0.1, `type` 19.6 ms over 556 elements; `ffd` (passes=date): `passes` 0.1 ms over 556 elements; `ffc` (passes=combo): `passes` 0.1 ms over 556 elements. The entries above are from `608544c9` and an earlier session; this one supersedes them as the record.<br>**Wave-3 implementation A/B** (MiniBrowser, 2026-09-23, the thirteen-branch wave-3 stack, `master` `37606021` → `9ea84575`, runs `g18i1-ffc-*`, `g28i1-ffk-*`, `g28i1-ffc-*`, `g9i1-ffq-*`, `g9i1-ffy-*`, `lri1-ffq-*`, `lri1-ffh-*`, `g19i1-ffy-*`, `g19i1-c21-*`, `edwi1-ffyh-*`, `stki1-ffy-*`; [97-w3-implementation-measurement.md](../../plans/research/render-review-2026-09-15/97-w3-implementation-measurement.md#results-by-plan)), per unit: `click=toggle` 33.0 → 17.2 ms and `click=combo` 59.2 → 17.9 ms once the per-event transform is an inline write (`setRuleStyles` 0.50 → 0 and 1.07 → 0.07, `apply` up by the same 0.5 and 1.0); `click=combo` also drops the 10 `measureText` calls the shakedown recorded. `passes=form` work 11,747 → 10,881, then size-hint calls 6,866 → 5,916 at 4.98 → 4.46 ms; `passes=header` size-hint calls 160 → 112 and `sizeHintMiss` 78 → 63, M23's 160 per header pass unchanged. Typing in a decorated field writes 1.02 sink calls instead of 2.74 and makes 0.01 `getWindow` calls instead of 1.73 — the error tooltip is no longer detached and re-attached per keystroke — and the hover that follows leaves the event ancestor walk at 3.06 reads instead of 24.09. **C21 is confirmed in a real engine again** (`g19i1-c21-*`): `geometry.tooltip` is `null` until the hover delay runs out, at unit 28 or 29, then the same rectangle in every later unit, on both arms. Geometry `=` on every run of every cell. |
...
`setText` and the check each field's `change` listener runs, so both show
their error. It then sends one `mouseover`, with no button held, to the first
field's own `<input>`, the element a pointer resting on the field hits. On the
first unit that decorator's error tooltip is on screen, it deletes one
character from the second field, which takes it back within its limit, so its
check clears its error and detaches its tooltip — the `detach` C21 fixed. (A
keystroke that left the second field's error as it was would re-attach an
identical tooltip, which `Tooltip` keeps without detaching since
`tooltip-idle-reattach`, so it would not reach C21's code.) If
`geometry.tooltip` is `null` in every unit, the hover never armed, and the run
says nothing about C21. The sequence leaves the first field in error with its
tooltip up, and the second within its limit, so give `call` a run of its own.
...
For wave 3 those maps are superseded by *The Matrix* in
[w3-0-bounding-sweep.md](../../plans/implemented/w3-0-bounding-sweep.md#the-matrix),
which gives every live candidate its cells; there G27's counters are
`querySelector` and `getElementRect`, since C19's fix replaced the
`getElementById` and `contains` the Coverage tables name.
...
| `dragout` | as `drag` | a `mousemove` on `document`, moving `step` px along `axis`, always the same way; a `mousedown` at the element's centre first and a `mouseup` `units` × `step` px away last. Every panel with a `drag` target also has `dragout` |
...
against a run without it bounds what that change could save. An ablation
receives the harness tools and the library objects the page passed in —
`Body`, `DOM`, `Tooltip` and `AbstractWindow` — so it can patch the DOM seam's
sink and source, and the overlay statics the component walk never reaches.
It is applied before the work and seam counters are installed, so those count
whatever reaches the patched code, a memo's hits included.

An ablation's own counters land in `work` or `writes`, and are reported even
when that family's counters are not installed. The W3.0 ablations name theirs
after themselves: `skipped.<ablation>.<what>` for work skipped,
`memo.<ablation>.<what>` for a memo's hits and misses, and
`dose.<ablation>.<what>` for work a dose arm adds, where `<ablation>` is the
`abl=` name verbatim and `<what>` one camelCase word — for example
`skipped.g05.lazy-reads.resolve`. So an arm's own counters tell whether it
engaged, and `qa-ab.py` tells them from the page's and from another arm's. A
`<what>` ending in `Miss` — `memo.g08.env-reads.viewportMiss`,
`memo.g18.canvas-width.fallbackMiss` — records work the ablation did not
remove, a memo's miss or a fall back to the original, so it never counts as
engagement. The older entries count as `skip*` in `writes`, or not at all.

The W3.0 bounding sweep's arms, one per wave-3 candidate group (see
[w3-0-bounding-sweep.md](../../plans/implemented/w3-0-bounding-sweep.md#the-ablations)):
...
| `g16.panel-settled` | G16 | A scrolling `Panel` skips its scroll-metrics re-measure while its size, child count and recorded content extent — the extent `scheduleGutterSettleOnShrink` writes at the end of the same pass — are unchanged. Every term is already cached, so the gate calls no size hint. For `passes` only. |
| `g16.scroll-reads` | G16 | The shadow overlay is re-sized only when the panel's size or gutter changed; scroll metrics read by the scroll path, and the maximum scroll offsets, once per task. |
...
| `g19.tooltip-idle` | G19 | An attachment identical to the component's current one is not rebuilt. A `hide` with nothing showing is left alone: the tooltip then has no element, and `hide` already does no fade. On a library with `tooltip-idle-reattach`, `Tooltip` itself keeps an identical attachment, so the arm removes nothing more there: read it only against an earlier build. |
...
| `g24.list-rows` | G24 | A list row skips a selected or focused write that changes nothing. |
| `g24.tree-window` | G24 | `Tree.renderWindow` does nothing: the ceiling of a render gate, sound only under `passes`. |
| `g25.theme-withhold` | G25 | A hidden editor with a mounted view defers its theme reconfigure until it is shown. |
| `g26.viewer-resize` | G26 | The Markdown viewer's floating panels re-use the text column's rectangle while it is unchanged, and a width change's height measure waits for the width to settle. |
| `g27.heading-cache` | G27 | The active heading is resolved from heading offsets measured once per layout, not by querying every heading each scroll tick. |
| `g28.transform-inline` | G28 | `setTransform` writes the transform inline instead of into the component's stylesheet rule. Inert on a library whose `setTransform` already writes inline (from `motion-transform-inline` on): it notes `no rule-side transform` and patches nothing, since its raw inline write would drop a `setTranslate` offset composed into the same declaration, so it bounds only an earlier build. |
...
phase's work per unit, the ablations' `memo.*`, `skipped.*`, `stubbed.*` and
`dose.*` bookkeeping left out), `sink/u` (seam sink calls per unit) and `geom`, then one
...
`qa-ab.py` scores one A/B cell — one panel, one set of parameters, a plain arm
run three times around each ablated arm run twice (see *Sweeps*) — by the W3.0
decision rule. It reads every `<cell-prefix>*.json`, in the order their
epoch-millisecond suffixes give, which survives a checkout. A report's arm is
its run name with the prefix and the trailing `-<rep>` removed:
`w3s1-b02-sdp-split.noop-drag-2` is arm `split.noop-drag` of cell
`w3s1-b02-sdp-`, and an arm other than `plain` must be its report's `abl=`
value, or the script prints `MISMATCH <run>`.

`--counter` names the counter the arm's work is scored on (default `work`).
Its terms are separated by spaces, with `+` and `-` the only operators:
...
scores the reads a memo left, since the seam counters count its hits too.

Geometry is compared label by label against the first plain report, rectangle
...
`Δms` is the arm's mean average minus the plain mean: `win` below minus the
bracket, `regress` above it, `flat` inside. The counter's change is `win` when
it falls by at least 10% of the plain value and at least 1.0 per unit, `more`
when it rises by as much, and `flat` otherwise. `engaged` is `yes` when the
arm's own counters (`skipped.<arm>.`, `memo.<arm>.`, `dose.<arm>.`), less
those ending in `Miss`, sum above 0 in every repetition: an arm that only
missed removed nothing. The verdict is the first row that applies:
...
| 4 | `Δms` is `regress` | `regress` |
| 5 | the counter's change is `win`, or `Δms` is | `win` |
| 6 | otherwise | `flat` |

A dose arm gets one more line: the plain counter as a share of the phase's
seam source calls, a work `win` at 10% and 1.0 per unit, and its `Δms`, an ms
`win` above the bracket — the fix removes what the dose adds. The candidate
verdict over a candidate's cells is in
[w3-0-bounding-sweep.md](../../plans/implemented/w3-0-bounding-sweep.md#the-decision-rule),
and the counter each arm is scored on under
[*The counter each arm is scored on*](../../plans/implemented/w3-0-bounding-sweep.md#the-counter-each-arm-is-scored-on).
It exits 0 when every report was read; 1 after printing `ERROR <run> <error>`
for an error, unreadable or old-format report, after a `MISMATCH`, with fewer
...
**Every run of a sweep opens a full-screen window on the desktop and holds it
until the run ends. Never start a sweep, and never leave an agent to start
one, without the user's go-ahead.** `--list` and `--dry-run` open nothing.

`sweeps/w3-0.sh` is the W3.0 bounding sweep: the fresh baseline of every
panel, then a same-session A/B of each wave-3 candidate's ablation in the
cells that reach it, then the C40 and C21 in-engine witnesses. Its matrix,
cell shapes and reading are *The Matrix* in
[w3-0-bounding-sweep.md](../../plans/implemented/w3-0-bounding-sweep.md#the-matrix).

```sh
packages/qa/sweeps/w3-0.sh [--dry-run | --list] [<batch> …]
```

With no batch named it runs all twenty, `b00` to `b19`, in order; each run goes
through `runqa.sh`, and the sweep stops on the first failure. Every run
carries `work=1&seam=1&geom=1` and nothing else, and is named
`w3<session>-<batch>-<cell>-<arm>-<rep>`, so
`qa-table.py packages/qa/results w3s1-b02-sdp-` and
`qa-ab.py packages/qa/results w3s1-b02-sdp-` each read exactly one cell. A
scored cell runs `plain-a`, each arm, `plain-b`, each arm again in reverse
order, `plain-c`, so a drift that is linear over the cell cancels in every
arm; a check cell, for counts alone, runs `plain-a`, each arm once, `plain-b`.

| Option or variable | Effect |
|---|---|
| `--list` | prints each selected batch's run count and the total (394 for all); opens nothing |
| `--dry-run` | prints each runner call, `runqa <name> <main\|wt> <params>`; opens nothing |
| `W3_SESSION` | the session tag in every run name (default `s1`) |
| `W3_C40_BEFORE_LIB` | a built `packages/lib` at `3e36ca60`, the `wt` arm of the C40 witnesses in `b19`; running `b19` without it exits 2 before anything starts |
| `W3_RUNQA` | the runner to call (default `runqa.sh` beside `sweeps/`); the tests point it at a stub |
| `QA_MAIN_LIB` | as for `runqa.sh`; a missing build exits 2 before anything starts |

It exits 0 when every selected run passed, 1 at the first that failed, and 2
for an unknown batch or a missing build. Every batch brackets its own cells
with plain runs, so any one can be re-run alone, on another day: give it a new
session tag, so its names do not mix with the first run's, and read it only
against itself:
...
- **Put `toggle`, `theme`, `type`, and `drag` with `grip=tab`, last in a
  ...
  tab strip possibly reordered. A panel whose own `theme` target restores its
  page state as well as the theme may still precede a trailing settled phase:
  `editor-tabs` ships `theme:14,idle:4`, because the check it makes inside
  `cycle` only holds while the page is on the switched theme. Give `theme` few
  units, `theme:4` to `theme:20`: each unit restyles the whole page.
