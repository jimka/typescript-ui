...
**Accepted exception: `Component`'s own `"dirtychange"` event.** `Component.isDirty()`'s change is exposed through dedicated `onDirtyChange(listener)` / `offDirtyChange(listener)` forwarders instead of `on("dirtychange", …)` / `off("dirtychange", …)`, and is dispatched by firing its private `ListenerBag` directly rather than through a `protected emit(...)`. Both departures exist for the same reason: `on`, `off`, and `emit` are each a single shared method name, and a subclass that declares its own overloads for its own custom events (as most emitting classes do) replaces — not extends — the inherited method on that subclass's prototype. A hypothetical base-class `on("dirtychange", …)` / `emit("dirtychange", …)` would be silently unreachable, or silently swallowed, on every subclass with its own event surface. `onDirtyChange` / `offDirtyChange` are unique names no subclass overload set collides with, so they stay reachable everywhere; the same collision rules out routing through `emit` even privately. This is the one base-`Component`-level custom event, so it is the one place this shape applies — a subclass's own custom events still MUST use the full `on` / `off` / `emit` + `ListenerBag` shape as above.
